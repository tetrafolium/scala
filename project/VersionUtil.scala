...
object VersionUtil {
  ...
  lazy val shellBannerString =
    settingKey[String]("Shell welcome banner string.")
  ...
  lazy val buildCharacterPropertiesFile = settingKey[File](
    "The file which gets generated by generateBuildCharacterPropertiesFile")
  lazy val generateVersionPropertiesFile =
    taskKey[File]("Generate version properties file.")
  lazy val generateBuildCharacterPropertiesFile =
    taskKey[File]("Generate buildcharacter.properties file.")
  lazy val extractBuildCharacterPropertiesFile = taskKey[File](
    "Extract buildcharacter.properties file from bootstrap scala-compiler.")
  ...
  private lazy val gitPropertiesImpl: Def.Initialize[GitProperties] =
    Def.setting {
      val log = sLog.value
      val (dateObj, sha) = {
        try {
          ...
          if (head eq null) {
            log.info(
              "No git HEAD commit found -- Using current date and 'unknown' SHA")
            (new Date, "unknown")
          } else {
            val commit = new RevWalk(db).parseCommit(head)
            (new Date(commit.getCommitTime.toLong * 1000L),
  ...
  private lazy val versionPropertiesImpl: Def.Initialize[Versions] =
    Def.setting {
      ...
      val (canonicalV, mavenSuffix, osgiV, release) = suffix match {
        ...
        case "SHA-SNAPSHOT" =>
          (s"$base-$date-$sha",
           s"-$cross-$sha-SNAPSHOT",
           s"$base.v$date-$sha",
           false)
        case "SHA" =>
          (s"$base-$sha", s"-$cross-$sha", s"$base.v$date-$sha", false)
        case "" => (s"$base", "", s"$base.v$date-VFINAL-$sha", true)
        case _ =>
          (s"$base-$suffix", s"-$suffix", s"$base.v$date-$suffix-$sha", true)
      }
  ...
  private lazy val generateVersionPropertiesFileImpl
    ...
    writeProps(
      versionProperties.value.toMap ++ Seq(
  ...
  def copyBootstrapJar(cp: Seq[Attributed[File]],
                       baseDir: File,
                       path: String,
                       libName: String): Unit = {
