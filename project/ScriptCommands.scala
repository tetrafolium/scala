...
object ScriptCommands {
  ...
  def all = Seq(
    ...
    setupValidateTest,
    setupBootstrapStarr,
    setupBootstrapLocker,
    setupBootstrapQuick,
    setupBootstrapPublish,
  ...
  def setupValidateTest = setup("setupValidateTest") { args =>
    Seq(
      testOptions in IntegrationTest in LocalProject("test") ++= Seq(
        Tests.Argument("--show-log"),
        Tests.Argument("--show-diff"))
    ) ++ (args match {
  ...
  def setupBootstrapStarr = setup("setupBootstrapStarr") {
    case Seq(fileOrUrl, ver) =>
  ...
  /** Set up the environment for building locker in `validate/bootstrap`. The arguments are:
    ...
    * - Version number to publish */
  def setupBootstrapLocker = setup("setupBootstrapLocker") {
    case Seq(fileOrUrl, ver) =>
      val url = fileToUrl(fileOrUrl)
      Seq(
        baseVersion in Global := ver,
  ...
  /** Set up the environment for building quick in `validate/bootstrap`. The arguments are:
    ...
    */
  def setupBootstrapQuick = setup("setupBootstrapQuick") {
    case Seq(targetFileOrUrl, ver, resolverFileOrUrl) =>
  ...
  def setupBootstrapPublish = setup("setupBootstrapPublish") {
    case Seq(fileOrUrl, ver) =>
      val url = fileToUrl(fileOrUrl)
      Seq(
        baseVersion in Global := ver,
  ...
  private[this] val enableOptimizer = Seq(
    scalacOptions in Compile in ThisBuild ++= Seq("-opt:l:inline",
  ...
  private[this] def publishTarget(url: String) = {
    ...
    val url2 =
      if (url.startsWith("file:")) url
        ...
        url.replaceAll("/$", "") + ";build.timestamp=" + System.currentTimeMillis

    Seq(
      publishTo in Global := Some("scala-pr-publish" at url2),
      credentials in Global += Credentials("Artifactory Realm",
                                           "scala-ci.typesafe.com",
                                           "scala-ci",
                                           env("PRIVATE_REPO_PASS"))
    )
  }
  ...
  private[this] def fileToUrl(fileOrUrl: String): String =
    if (fileOrUrl.startsWith("file:") || fileOrUrl.startsWith("http:") || fileOrUrl
          .startsWith("https:")) fileOrUrl
    else Paths.get(fileOrUrl).toUri.toString

  /** Like `Def.sequential` but accumulate all results */
  def sequence[B](
      tasks: List[Def.Initialize[Task[B]]]): Def.Initialize[Task[List[B]]] =
    tasks match {
