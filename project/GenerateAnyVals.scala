...
trait GenerateAnyValReps {
  ...
  sealed abstract class AnyValNum(name: String, repr: Option[String], javaEquiv: String) extends AnyValRep(name, repr, javaEquiv) {
    ...
    private def companionCoercions(tos: AnyValRep*) = {
      tos.toList map (to => s"implicit def @javaequiv@2${to.javaEquiv}(x: @name@): ${to.name} = x.to${to.name}")
    }
    def coercionComment =
...
import scala.language.implicitConversions"""
    ...
    def unaryOps = {
      val ops = List(Op("+", "/** Returns this value, unmodified. */"), Op("-", "/** Returns the negation of this value. */"))
