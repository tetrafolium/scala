...
object ScalaOptionParser {
    ...
    def PathSetting(name: String): Parser[String] = {
      concat(
        concat(token(name) ~ Space.string) ~ rep1sep(
          ...
          token(java.io.File.pathSeparator))
          .map(_.mkString(java.io.File.pathSeparator)))
    }
  ...
  private def scalaChoiceSettingNames =
    Map("-howtorun" -> List("object", "script", "jar", "guess"))
