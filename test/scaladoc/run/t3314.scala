...
object Test extends ScaladocModelTest {
  ...
  override def resourceFile = "t3314.scala"
  ...
  def testModel(rootPackage: Package) = {
    ...
    assert(test1Value.refEntity.size == 1, s"${test1Value.refEntity.size} == 1")
    ...
    assert(test1Constants.refEntity.size == 1,
           test1Constants.refEntity.size.toString + " == 1")
    assert(
      ...
      test1Constants
        .refEntity(0)
        ._1
        .toString + " == LinkToMember(test1.Enum.Value)"
    )
    ...
    def testDefinition(doc: DocTemplateEntity) = {
      for (day <- List("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")) {
        ...
        assert(doc._value(day).resultType.refEntity.size == 1,
               doc._value(day).resultType.refEntity.size.toString + " == 1")
        assert(
          ...
          doc
            ...
            .refEntity(0)
            ._1
            .toString + " == LinkToMember(" + doc.qualifiedName + ".Value)"
        )
      }
    }
      ...
      for ((method, name, ref) <- expected) {
        ...
        assert(
          ...
          doc
            ...
            .refEntity(0)
            ._1
            .toString + " == LinkToMember(" + ref.qualifiedName + ") (in " + doc + "." + method + ")"
        )
      }
    }
