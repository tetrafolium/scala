...
object Test extends ScaladocModelTest {
  ...
  override def resourceFile: String = "t5784.scala"
  ...
  def testModel(rootPackage: Package) = {
    ...
    assert(String.companion.isDefined,
           "test.templates.String should have a pseudo-companion object")
    ...
    assert(BaseT.members.map(_.name).sorted == Foo.members.map(_.name).sorted,
           BaseT.members.map(_.name).sorted.toString + " == " + Foo.members
             .map(_.name)
             .sorted)
    ...
    assert(Api.members.filter(_.inDefinitionTemplates.head == Api).length == 2,
           Api.members
             ...
             .length
             .toString + " == 2") // FooApi and override type T
    ...
    assert(ApiT.members.filter(_.inDefinitionTemplates.head == Api).length == 0,
           ApiT.members
             .filter(_.inDefinitionTemplates.head == Api)
             .length
             .toString + " == 0")
    assert(ApiT.members.map(_.name).sorted == FooApi.members.map(_.name).sorted,
           ApiT.members.map(_.name).sorted.toString + " == " + FooApi.members
             .map(_.name)
             .sorted)
