trait LeibnizLiskov {
  ...
  type SASub[+X] = SA <:< X
  (implicitly[B <:< SB]
    .compose(aEqB.substituteCo[SASub](implicitly[SASub[A]]))): SA <:< SB
  (aEqB
    .substituteCo[SASub](implicitly[SASub[A]])
    .andThen(implicitly[B <:< SB])): SA <:< SB

  // checks that inference is working (no explicit types on xs)
  def A(): A
  def B(): B
  locally {
    val xs = aEqB.substituteCo(List(A(), A(), A()));
    implicitly[xs.type <:< List[B]]
  }
  locally {
    val xs = aEqB.substituteContra(List(B(), B(), B()));
    implicitly[xs.type <:< List[A]]
  }
  locally {
    val xs = aEqB.flip.liftCo[List](List(B(), B(), B()));
    implicitly[xs.type <:< List[A]]
  }
  ...
  locally {
    val f = bSubA.substituteCo[From[A]#L](aSubB(_));
    implicitly[f.type <:< (A => A)]
  }
  locally {
    val f = aSubB.substituteContra[To[A]#L](bSubA(_));
    implicitly[f.type <:< (A => A)]
  }
