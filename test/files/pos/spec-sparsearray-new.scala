...
class SparseArray[@specialized(Int) T: ClassTag]
    extends collection.mutable.Map[Int, T]
    with collection.mutable.MapOps[Int,
                                   T,
  ...
  override protected[this] def fromSpecific(
      coll: IterableOnce[(Int, T)]): SparseArray[T] = ???
  override protected[this] def newSpecificBuilder
    : mutable.Builder[(Int, T), SparseArray[T]] = ???
