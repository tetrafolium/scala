...
class PromiseTests extends MinimalScalaTest {
  ...
  "A Promise should not be serializable" should {
    ...
    verifyNonSerializabilityFor(
      Promise.fromTry(Failure(new NullPointerException)))
  }
  ...
  def futureWithResult(f: ((Future[Any], Any) => Unit) => Unit): Unit = {
    ...
    "contain a value" in {
      f((future, result) => future.value mustBe (Some(Success(result))))
    }
  ...
  def futureWithException[E <: Throwable: Manifest](
      f: ((Future[Any], String) => Unit) => Unit): Unit = {
    ...
    "project a failure" in {
      f((future, message) =>
        Await.result(future.failed, defaultTimeout).getMessage mustBe (message))
    }
