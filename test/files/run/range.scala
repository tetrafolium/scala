...
object Test {
  def rangeForeach(range: Range) = {
    ...
    assert(buffer.toList == range.iterator.toList,
           buffer.toList.toString + "/" + range.iterator.toList)
  }
  ...
  def boundaryTests() = {
    ...
    // #4308
    val caught = (
      try { (Long.MinValue to Long.MaxValue).sum; false } catch {
    ...
    // #7432
    val noElemAtMin = (
      try { (10 until 10).min; false } catch {
        case _: NoSuchElementException => true
      }
    )
    assert(noElemAtMin)
    val noElemAtMax = (
      try { (10 until 10).max; false } catch {
        case _: NoSuchElementException => true
      }
    )
    assert(noElemAtMax)
