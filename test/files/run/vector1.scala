...
object Test {
  ...
  def assertVectorIndexed[V](a: Vector[V],
    ...
    assert(res.length == (end - start),
           res.length.toString + "!=" + (end - start) + " (" + res + ")")
...
//    //println(a)
  ...
  def test3() = {
    ...
    val pos = scala.util.Random.shuffle(
      scala.collection.mutable.ArraySeq.make[Int](Array.tabulate[Int](N)(i =>
        i)))
...
//    //println(a)
  ...
  def test4() = {
...

    {
      ...
      var it = a
      while (i < N) {
        assert(it.length == (N - i),
               it.length.toString + " items at iteration " + i)
        val x = it(0)
        val y = it(N - i - 1)
        assert(x == "a" + i, x + "!=a" + i)
