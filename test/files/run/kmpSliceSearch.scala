      ...
      for (i <- 0 to 100) {
        for (j <- 0 to 10) {
          ...
          for (k <- 0 to 5) {
            ...
            val ysv = Vector() ++ ys
            val ysl = ys.toList
            val yss = Vector[Seq[Int]](ys, ysa.toIndexedSeq, ysv, ysl)
            val fwd_slow = slowSearch(xs, ys)
            val bkw_slow = bkwSlowSearch(xs, ys)
            val fwd_fast = xss.flatMap(xs => yss.map(ys => xs.indexOfSlice(ys)))
            val bkw_fast =
              xss.flatMap(xs => yss.map(ys => xs.lastIndexOfSlice(ys)))
