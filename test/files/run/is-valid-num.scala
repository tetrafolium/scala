...
//      assert(!f.isValidLong, f)
  ...
  def checkBigInt(bi: BigInt): Unit = {
    ...
    val isFloat = !bi.toFloat.isInfinity && bd.compare(
      BigDecimal(new java.math.BigDecimal(bi.toFloat))) == 0
    val isDouble = !bi.toDouble.isInfinity && bd.compare(
      BigDecimal(new java.math.BigDecimal(bi.toDouble))) == 0
