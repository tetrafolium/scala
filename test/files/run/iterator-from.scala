...
object Test extends App {
  ...
  def testSet[A](set: SortedSet[A], list: List[A])(
      implicit o: Ordering[A]): Unit = {
    ...
    for (key <- distinctSorted) {
      ...
      check(clazz,
            ...
            iteratorFrom,
            (set rangeFrom key).iterator.toList)
      check(clazz,
            list,
            s"set.iteratorFrom $key",
            s"distinctSorted dropWhile (_ < $key)",
            iteratorFrom,
            distinctSorted dropWhile (o.lt(_, key)))
    }
  }

  def testMap[A, B](map: SortedMap[A, B], list: List[(A, B)])(
      implicit o: Ordering[A]): Unit = {
    ...
    for (keyValue <- distinctSorted) {
      ...
      check(clazz,
            ...
            iteratorFrom,
            (map rangeFrom key).iterator.toList)
      check(clazz,
            list,
            s"map iteratorFrom $key",
            s"distinctSorted dropWhile (_._1 < $key)",
            iteratorFrom,
            distinctSorted dropWhile (x => o.lt(x._1, key)))
      check(clazz,
            list,
  ...
  def distinctByKey[A, B](list: List[(A, B)]): List[(A, B)] =
    list.groupBy(_._1).map(_._2.last).toList
  ...
  0 until maxLength foreach { length =>
    ...
    testMap(treeMap.view.filterKeys(_ % 2 == 0).to(SortedMap),
            keyValues filter (_._1 % 2 == 0))
    testMap(treeMap.view.mapValues(_ + 1).to(SortedMap), keyValues map {
      case (k, v) => (k, v + 1)
    })
  }
}
