...
object Test extends App {
  ...
  def testIterableOps = {
    ...
    implicit def filterMap[Repr](r: Repr)(
        implicit fr: IsIterable[Repr]): FilterMapImpl[fr.A, Repr] =
    ...
    val fmm = m.filterMap {
      case (k, v) => if (k % 2 == 0) Some(v -> k) else None
    }
    typed[Map[String, Int]](fmm)
    println(fmm)

    val tm = TreeMap(1 -> "foo", 2 -> "bar")
    val tmm = tm.filterMap {
      case (k, v) => if (k % 2 == 0) Some(v -> k) else None
    }
