...
object TestNeg extends App {
  ...
  def unwrap[T](x: AbsWrapperCov[T]): Wrapped[T] = x match {
    case Wrapper /*[_ <: T ]*/ (wrapped) =>
      wrapped // : Wrapped[_ <: T], which is a subtype of Wrapped[T] if and only if Wrapped is covariant in its type parameter
  }
  ...
  def unwrapOption[T](x: Option[T]): T = x match {
    case Some(xs) =>
      xs.foo // the error message should not refer to a skolem (testing extrapolation)
  }
