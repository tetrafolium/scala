trait LeibnizLiskov {
  ...
  locally {
    val f = bSubA.substituteCo[To[A]#L](aSubB(_));
    implicitly[f.type <:< (A => A)]
  }
  locally {
    val f = aSubB.substituteContra[From[A]#L](bSubA(_));
    implicitly[f.type <:< (A => A)]
  }
  def convertSub[T, U](l: List[T])(ev: T <:< U): List[U] =
    ev.liftContra[List](l)
  type Consumes[-X] = X => Unit
  def convertConsume1[U, T](c: Consumes[T])(ev: U <:< T): Consumes[U] =
    ev.liftCo[Consumes](c)
