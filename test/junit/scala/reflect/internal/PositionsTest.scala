...
class PositionsTest {
  ...
  @Test def positionValidation(): Unit = {
    ...
    def rangePos(start: Int, end: Int): util.Position =
      util.Position.range(NoSourceFile, start, start, end)
    def offsetPos(point: Int): util.Position =
      util.Position.offset(NoSourceFile, point)
    ...
    // overlapping ranges
    checkInvalid(
      Block(rangePositioned(0, 2), rangePositioned(1, 2), EmptyTree)
        .setPos(rangePos(0, 2)))
    checkInvalid(
      Block(rangePositioned(1, 2), rangePositioned(0, 2), EmptyTree)
        .setPos(rangePos(0, 2)))

    // transparent position not deemed to overlap itself
    checkValid(
      Block(rangePositioned(0, 2),
            tree.setPos(rangePos(1, 2).makeTransparent),
            EmptyTree).setPos(rangePos(0, 2)))

    // children of transparent position overlapping with sibling of transparent position.
    checkInvalid(
      Block(rangePositioned(0, 2),
            Block(Nil, rangePositioned(1, 2)).setPos(
              rangePos(1, 2).makeTransparent),
            EmptyTree).setPos(rangePos(0, 2)))

    // adjacent ranges are allowed to touch
    checkValid(
      Block(rangePositioned(0, 1), rangePositioned(1, 2), EmptyTree)
        .setPos(rangePos(0, 2)))

    // offset position between overlapping ranges
    checkInvalid(
      Block(rangePositioned(0, 2),
            tree.setPos(offsetPos(0)),
            rangePositioned(1, 2),
            EmptyTree).setPos(rangePos(0, 2)))
