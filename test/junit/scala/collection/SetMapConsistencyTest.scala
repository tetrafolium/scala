...
class SetMapConsistencyTest {
  ...
  class BoxMutableMap[A, M <: cm.Map[A, Int]](m0: M, title0: String)
      extends MapBox[A] {
    var m = m0
    def title = title0
    def adders = 5
    ...
    def sub(n: Int, a: A): Unit = {
      n match {
        case 0 => m -= a
        case 1 =>
          m = m.clone().asInstanceOf[M]; m -= a // no `-` any more for mutable Maps
  ...
  def boxMlm[A] =
    new BoxMutableMap[A, cm.ListMap[A, Int]](new cm.ListMap[A, Int],
                                             "mutable.ListMap")

  def boxMhm[A] =
    new BoxMutableMap[A, cm.HashMap[A, Int]](new cm.HashMap[A, Int],
                                             "mutable.HashMap")

  def boxMohm[A] =
    new BoxMutableMap[A, cm.OpenHashMap[A, Int]](new cm.OpenHashMap[A, Int],
                                                 "mutable.OpenHashMap")

  def boxMtm[A: Ordering] =
    new BoxMutableMap[A, cm.TreeMap[A, Int]](new cm.TreeMap[A, Int],
                                             "mutable.TreeMap")
  ...
  def boxMarm[A <: AnyRef] =
    new BoxMutableMap[A, cm.AnyRefMap[A, Int]](
      ...
      override def fiddle(n: Int): Unit = {
        n match {
          case 0 => m = arm.clone
          case 1 => arm.repack
          case _ => oor("fiddle", n)
  ...
  def boxMjm =
    new BoxMutableMap[Long, cm.LongMap[Int]](new cm.LongMap[Int](_ => -1),
                                             "mutable.LongMap") {
  ...
  class BoxImmutableMap[A, M <: ci.Map[A, Int]](m0: M, title0: String)
      extends MapBox[A] {
    var m = m0
    def title = title0
    def adders = 2
  ...
  def boxIhm[A] =
    new BoxImmutableMap[A, ci.HashMap[A, Int]](ci.HashMap.empty[A, Int],
                                               "immutable.HashMap")

  def boxIim =
    new BoxImmutableMap[Int, ci.IntMap[Int]](ci.IntMap.empty[Int],
                                             "immutable.IntMap")

  def boxIjm =
    new BoxImmutableMap[Long, ci.LongMap[Int]](ci.LongMap.empty[Int],
                                               "immutable.LongMap")

  def boxIlm[A] =
    new BoxImmutableMap[A, ci.ListMap[A, Int]](new ci.ListMap[A, Int],
                                               "immutable.ListMap")

  def boxItm[A: Ordering] =
    new BoxImmutableMap[A, ci.TreeMap[A, Int]](new ci.TreeMap[A, Int],
                                               "immutable.TreeMap")

  // Mutable set wrappers placed into the same framework (everything returns 0)

  class BoxMutableSet[A, M <: cm.Set[A]](s0: M, title0: String)
      extends MapBox[A] {
    protected var m = s0
    def title = title0
    def adders = 5
  ...
  def boxMbs =
    new BoxMutableSet[Int, cm.BitSet](new cm.BitSet, "mutable.BitSet")
  ...
  def boxJavaS[A] =
    new BoxMutableSet[A, cm.Set[A]]((new java.util.HashSet[A]).asScala,
                                    "java.util.HashSet") {
      override def adders = 3
      override def subbers = 1
    }
  ...
  class BoxImmutableSet[A, M <: ci.Set[A]](s0: M, title0: String)
      extends MapBox[A] {
    protected var m = s0
    def title = title0
    def adders = 2
  ...
  def boxIhs[A] =
    new BoxImmutableSet[A, ci.HashSet[A]](ci.HashSet.empty[A],
                                          "mutable.HashSet")

  def boxIls[A] =
    new BoxImmutableSet[A, ci.ListSet[A]](ci.ListSet.empty[A],
                                          "mutable.ListSet")

  def boxIts[A: Ordering] =
    new BoxImmutableSet[A, ci.TreeSet[A]](ci.TreeSet.empty[A],
                                          "mutable.TreeSet")
  ...
  def churn[A](map1: MapBox[A],
    ...
    for (i <- 0 until n) {
      if (!check) {
        ...
        throw new Exception(s"Disagreement after ${what
          .result()} between ${map1.title} and ${map2.title} because ${map1.keys
          .map(map2 has _)
  ...
  val longKeys = intKeys.map(_.toLong) ++ Array(Long.MaxValue,
                                                Long.MinValue,
                                                1397198789151L,
                                                -41402148014L)
  ...
  def extraMutableAnyRefMapTests(): Unit = {
    ...
    assert {
      arm.map { case (k, v) => (if (k == null) "" else k + k) -> v.toString }.getClass == arm.getClass
    }
  ...
  def extraFilterTests(): Unit = {
    ...
    def pick(m: M, density: Double) =
      m.keys.filter(_ => rn.nextDouble < density).toSet
    def test: Boolean = {
      for (i <- 0 to 100) {
  ...
  @Test
  def testSI8264(): Unit = {
    val hs = immutable.Set(-2147483648, 1, -45023380, -1, 1971207058, -54312241,
      -234243394) - -1
  ...
  def test_SI8727(): Unit = {
    ...
    val m = map.view
      .filterKeys(i => if (map contains i) true else throw new NSEE)
      .toMap
