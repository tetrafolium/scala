...
class NewBuilderTest {
  ...
  def mapPreservesCollectionType(): Unit = {
    def test[T: ClassTag](mapped: Any): Unit = {
      ...
      assertTrue(
        s"$mapped (of class ${mapped.getClass} is not a in instance of ${expected}",
        isInstance)
    }

    test[sc.GenTraversable[_]](
      (sc.GenTraversable(1): sc.GenTraversable[Int]).map(x => x))
    test[sc.Traversable[_]]((sc.Traversable(1): sc.GenTraversable[Int]).map(x =>
      x))
    test[sc.GenIterable[_]]((sc.GenIterable(1): sc.GenTraversable[Int]).map(x =>
      x))
    ...
    test[sc.IndexedSeq[_]]((sc.IndexedSeq(1): sc.GenTraversable[Int]).map(x =>
      x))
    ...
    test[scm.Traversable[_]](
      (scm.Traversable(1): sc.GenTraversable[Int]).map(x => x))
    ...
    test[scm.IndexedSeq[_]]((scm.IndexedSeq(1): sc.GenTraversable[Int]).map(x =>
      x))
    test[scm.IndexedSeq[_]]((scm.IndexedSeq(1): sc.Seq[Int]).map(x => x))
    test[scm.ArrayBuffer[_]](
      (scm.ArrayBuffer(1): sc.GenTraversable[Int]).map(x => x))
    test[scm.ArrayBuffer[_]]((scm.ArrayBuffer(1): sc.Seq[Int]).map(x => x))
    test[scm.ListBuffer[_]]((scm.ListBuffer(1): sc.GenTraversable[Int]).map(x =>
      x))
    ...
    test[scm.LinkedHashSet[_]](
      (scm.LinkedHashSet(1): sc.GenTraversable[Int]).map(x => x))
    test[scm.LinkedHashSet[_]]((scm.LinkedHashSet(1): sc.Set[Int]).map(x => x))

    test[sci.Traversable[_]](
      (sci.Traversable(1): sc.GenTraversable[Int]).map(x => x))
    test[sci.Iterable[_]]((sci.Iterable(1): sc.GenTraversable[Int]).map(x => x))
    test[sci.LinearSeq[_]]((sci.LinearSeq(1): sc.GenTraversable[Int]).map(x =>
      x))
    ...
    test[sci.IndexedSeq[_]]((sci.IndexedSeq(1): sc.GenTraversable[Int]).map(x =>
      x))
