...
class SerializationTest {
  ...
  def linkedHashMap(): Unit = {
    assertEqualsAfterDeserialization(mutable.LinkedHashMap.empty[Int, String],
                                     classOf[mutable.LinkedHashMap[_, _]])
    assertEqualsAfterDeserialization(
      mutable.LinkedHashMap(1 -> "one", 2 -> "two", 3 -> "three"),
      classOf[mutable.LinkedHashMap[_, _]])
  }
  ...
  def longMap(): Unit = {
    assertEqualsAfterDeserialization(mutable.LongMap.empty[String],
                                     classOf[mutable.LongMap[_]])
    assertEqualsAfterDeserialization(
      mutable.LongMap(1L -> "one", 2L -> "two", 3L -> "three"),
      classOf[mutable.LongMap[_]])
  }
  ...
  def anyRefMap(): Unit = {
    assertEqualsAfterDeserialization(mutable.AnyRefMap.empty[String, String],
                                     classOf[mutable.AnyRefMap[_, _]])
    assertEqualsAfterDeserialization(
      mutable.AnyRefMap("1" -> "one", "2" -> "two", "3" -> "three"),
      classOf[mutable.AnyRefMap[_, _]])
  }
  ...
  def mapWithDefault(): Unit = {
    assertEqualsAfterDeserialization(
      mutable.Map.empty[Int, String].withDefaultValue("none"),
      classOf[mutable.Map[_, _]])
    assertEqualsAfterDeserialization(
      mutable.Map(1 -> "one").withDefaultValue("none"),
      classOf[mutable.Map[_, _]])
  }
  ...
  def sortedMapWithDefault(): Unit = {
    assertEqualsAfterDeserialization(
      mutable.SortedMap.empty[Int, String].withDefaultValue("none"),
      classOf[mutable.SortedMap[_, _]])
    assertEqualsAfterDeserialization(
      mutable.SortedMap(1 -> "one").withDefaultValue("none"),
      classOf[mutable.SortedMap[_, _]])
  }
  ...
  def linkedHashSet(): Unit = {
    assertEqualsAfterDeserialization(mutable.LinkedHashSet.empty[Int],
                                     classOf[mutable.LinkedHashSet[_]])
  ...
  def priorityQueue(): Unit = {
    assertEquals(Seq(),
                 serializeDeserialize(mutable.PriorityQueue.empty[Int]).toSeq)
    assertEquals(Seq(3, 2, 1),
                 serializeDeserialize(mutable.PriorityQueue(1, 2, 3)).toSeq)
  }
  ...
  def queue(): Unit = {
    ...
    assertEquals(Seq(1, 2, 3),
                 serializeDeserialize(mutable.Queue(1, 2, 3)).toSeq)
  }

  private def assertEqualsAfterDeserialization[A](
      original: mutable.Iterable[A],
