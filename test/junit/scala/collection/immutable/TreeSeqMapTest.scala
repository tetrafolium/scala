...
class TreeSeqMapTest {
  ...
  def testBuilder(): Unit = {
    val m =
      TreeSeqMap("d" -> "1", "b" -> "2", "a" -> "3", "e" -> "2.2", "c" -> "4")
    assertEquals(
      List("d" -> "1", "b" -> "2", "a" -> "3", "e" -> "2.2", "c" -> "4"),
      m.toList)
  }
  @Test
  ...
  def testHeadTailLastInitWhenOrderingByModification(): Unit = {
    val m = TreeSeqMap(3 -> 1, 2 -> 2, 1 -> 3)
      .orderingBy(TreeSeqMap.OrderBy.Modification)
      .updated(2, 4)
    assertEquals(3 -> 1, m.head)
  ...
  def testIterator: Unit = {
    ...
    assertEquals(
      List(4 -> 1, 2 -> 2, 1 -> 3, 5 -> 5, 3 -> 4),
      TreeSeqMap(4 -> 1, 2 -> 2, 1 -> 3, 5 -> 5, 3 -> 4).iterator.toList)
  }
  @Test
  ...
  def testSlice: Unit = {
    ...
    assertEquals(List(3 -> 3, 4 -> 4, 5 -> 5),
                 TreeSeqMap(1 -> 1,
                            ...
                            5 -> 5,
                            6 -> 6,
                            7 -> 7).slice(2, 5).iterator.toList)
    assertEquals(List(7 -> 7),
                 TreeSeqMap(1 -> 1,
                            2 -> 2,
                            3 -> 3,
                            4 -> 4,
  ...
  def testSplitAt: Unit = {
    val m = TreeSeqMap(3 -> 1, 2 -> 2, 1 -> 3, 4 -> 5, 5 -> 4)
    var t = m.splitAt(0)
    assertEquals((List(), List(3 -> 1, 2 -> 2, 1 -> 3, 4 -> 5, 5 -> 4)),
                 (t._1.iterator.toList, t._2.iterator.toList))
    t = m.splitAt(1)
    assertEquals((List(3 -> 1), List(2 -> 2, 1 -> 3, 4 -> 5, 5 -> 4)),
                 (t._1.iterator.toList, t._2.iterator.toList))
    t = m.splitAt(2)
    assertEquals((List(3 -> 1, 2 -> 2), List(1 -> 3, 4 -> 5, 5 -> 4)),
                 (t._1.iterator.toList, t._2.iterator.toList))
    t = m.splitAt(3)
    assertEquals((List(3 -> 1, 2 -> 2, 1 -> 3), List(4 -> 5, 5 -> 4)),
                 (t._1.iterator.toList, t._2.iterator.toList))
    t = m.splitAt(4)
    assertEquals((List(3 -> 1, 2 -> 2, 1 -> 3, 4 -> 5), List(5 -> 4)),
                 (t._1.iterator.toList, t._2.iterator.toList))
    t = m.splitAt(5)
    assertEquals((List(3 -> 1, 2 -> 2, 1 -> 3, 4 -> 5, 5 -> 4), List()),
                 (t._1.iterator.toList, t._2.iterator.toList))
  }
  ...
  def testAppend: Unit = {
    ...
    for (i <- 1 until 1027) {
      val o1 = Ordering((1 until i).map(n => n -> toBinaryString(n)): _*)
        .exclude(i / 2)
        .exclude(i / 10)
        .exclude(i - 1)
      val o2 = Ordering((1 until i).map(n => n -> toBinaryString(n)): _*)
        .exclude(i / 2)
        .exclude(i / 10)
        .exclude(i - 1)
...
object TreeSeqMapTest extends App {
  ...
  val o0 = TreeSeqMap.Ordering(1 -> toBinaryString(1),
                               2 -> toBinaryString(2),
                               -1 -> toBinaryString(-1),
                               5 -> toBinaryString(5),
                               100 -> toBinaryString(100))
  println(o0)
  println()
