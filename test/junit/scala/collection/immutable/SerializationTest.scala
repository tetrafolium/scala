...
class SerializationTest {
  ...
  @Test
  def arraySeq(): Unit = {
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array("1", "2", "3")))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Int.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Long.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Double.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Float.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Char.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Byte.MinValue)))
    assertEqualsAfterDeserialization(
      ArraySeq.unsafeWrapArray(Array(Short.MinValue)))
  ...
  def hashMap(): Unit = {
    ...
    assertEquals(
      ...
      serializeDeserialize(
        HashMap(1 -> "1", 2 -> "2", 3 -> "3").view.mapValues(_.toInt)).toMap)
    assertEquals(
      HashMap(2 -> "2", 3 -> "3"),
      serializeDeserialize(
        HashMap(1 -> "1", 2 -> "2", 3 -> "3").view.filterKeys(_ > 1)).toMap)
  }
  ...
  def longMap(): Unit = {
    ...
    assertEqualsAfterDeserialization(
      LongMap(1L -> "one", 2L -> "two", 3L -> "three"),
      classOf[LongMap[_]])
  }
  ...
  def mapWithDefault(): Unit = {
    assertEqualsAfterDeserialization(
      Map.empty[Int, String].withDefaultValue("none"),
      classOf[Map[_, _]])
  ...
  def sortedMapWithDefault(): Unit = {
    assertEqualsAfterDeserialization(
      SortedMap.empty[Int, String].withDefaultValue("none"),
      classOf[SortedMap[_, _]])
    assertEqualsAfterDeserialization(
      SortedMap(1 -> "one").withDefaultValue("none"),
      classOf[SortedMap[_, _]])
  }
  ...
  def lazyList(): Unit = {
    ...
    assertEqualsAfterDeserialization(LazyList.from(1).take(10000).force,
                                     classOf[LazyList[_]])
  }
  ...
  def stream(): Unit = {
    ...
    assertEqualsAfterDeserialization(Stream.from(1).take(10000).force,
                                     classOf[Stream[_]])
  }
  ...
  def numericRange(): Unit = {
    assertEqualsAfterDeserialization(
      NumericRange(start = 0, end = 10, step = 1),
      classOf[NumericRange[_]])
  }
  ...
  private def assertEqualsAfterDeserialization[A](
      original: Iterable[A],
      expectedClass: Class[_] = null): Unit = {
    val after = serializeDeserialize(original)
    assertEquals(original, after)
    if (expectedClass ne null)
      assertTrue(
        "Deserialized class " + after.getClass.getName + " is not assignable to " + expectedClass.getName,
        expectedClass.isInstance(after))
  }
