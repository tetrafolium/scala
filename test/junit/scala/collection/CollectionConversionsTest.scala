...
class CollectionConversionsTest {
  ...
  def testConversion[A: ClassTag](name: String,
                                  col: () => IterableOnce[A]): Unit = {
    out ++= ("-- Testing " + name + " ---\n")
    if (!(
          printResult("[Direct] Vector   ", col().iterator.toVector, testVector) &&
            printResult("[Copy]   Vector   ",
                        col().iterator.to(Vector),
                        testVector) &&
            printResult("[Direct] Buffer   ",
                        col().iterator.toBuffer,
                        testBuffer) &&
            printResult("[Copy]   Buffer   ",
                        col().iterator.to(Buffer),
                        testBuffer) &&
            printResult("[Copy]   Seq      ", col().iterator.to(Seq), testSeq) &&
            printResult("[Direct] Stream   ",
                        col().iterator.toStream,
                        testStream) &&
            printResult("[Copy]   Stream   ",
                        col().iterator.to(Stream),
                        testStream) &&
            printResult("[Direct] Array    ", col().iterator.toArray, testArray) &&
            printResult("[Copy]   Array    ",
                        col().iterator.to(Array),
                        testArray)
        )) {
      print(out)
