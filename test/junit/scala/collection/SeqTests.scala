    ...
    for {
      ...
      sorted.search(needle, from, to)(ord) match {
        case Found(foundIndex: Int) => {
          ...
          Assert.assertTrue(
            s"found value $found not equivalent to searched value $needle in List(0-1000) between indices $from and $to",
            ord.equiv(found, needle))
        }
        case InsertionPoint(insertionPoint: Int) =>
          for ((e, i) <- sorted.zipWithIndex) {
            if (i >= from && i < to) {
              if (i < insertionPoint)
                Assert.assertFalse(
                  s"a value before the insertion point was greater than $needle",
                  ord.gt(e, needle))
              else
                Assert.assertFalse(
                  s"a values past the insertion point was smaller than $needle",
                  ord.lt(e, needle))
            }
