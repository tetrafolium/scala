...
class BuildFromTest {
  ...
  def optionSequence1[CC[X] <: IterableOps[X, CC, _], A](
      xs: CC[Option[A]]): Option[CC[A]] =
    xs.foldLeft[Option[Builder[A, CC[A]]]](
        Some(xs.iterableFactory.newBuilder[A])) {
        case (Some(builder), Some(a)) => Some(builder += a)
        case _                        => None
      }
      .map(_.result())
  def optionSequence1[CC[X] <: SortedSet[X] with SortedSetOps[X, CC, CC[X]],
                      A: Ordering](xs: CC[Option[A]]): Option[CC[A]] =
    xs.foldLeft[Option[Builder[A, CC[A]]]](
        Some(xs.sortedIterableFactory.newBuilder[A])) {
        case (Some(builder), Some(a)) => Some(builder += a)
        case _                        => None
      }
      .map(_.result())
  ...
  def optionSequence1Test: Unit = {
    ...
    val xs2: immutable.TreeSet[Option[String]] =
      immutable.TreeSet(Some("foo"), Some("bar"), None)
    val o2 = optionSequence1(xs2)
    val o2t: Option[immutable.Set[String]] = o2

    val xs4 =
      immutable.List[Option[(Int, String)]](Some((1 -> "a")), Some((2 -> "b")))
    val o4 = optionSequence1(xs4)
    val o4t: Option[immutable.List[(Int, String)]] = o4
    val o5: Option[immutable.TreeMap[Int, String]] =
      o4.map(_.to(immutable.TreeMap))
  }
  ...
  def flatCollect[A, B, To](coll: Iterable[A])(
  ...
  def partitionMapTest: Unit = {
    val xs1 = immutable.List(1, 2, 3)
    val (xs2, xs3) =
      partitionMap(xs1)(x => if (x % 2 == 0) Left(x) else Right(x.toString))
    val xs4: immutable.List[Int] = xs2
    val xs5: immutable.List[String] = xs3

    val xs6 = immutable.TreeMap((1, "1"), (2, "2"))
    val (xs7, xs8) = partitionMap(xs6) {
      case (k, v) => Left[(String, Int), (Int, Boolean)]((v, k))
    }
  ...
  implicitly[
    BuildFrom[immutable.IntMap[_], (Int, String), immutable.IntMap[String]]]
  implicitly[
    BuildFrom[mutable.LongMap[_], (Long, String), mutable.LongMap[String]]]
  implicitly[
    BuildFrom[immutable.LongMap[_], (Long, String), immutable.LongMap[String]]]
  implicitly[BuildFrom[mutable.AnyRefMap[_ <: AnyRef, _],
                       (String, String),
                       mutable.AnyRefMap[String, String]]]
