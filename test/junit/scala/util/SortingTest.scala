...
class SortingTest {
  ...
  def mkA(n: Int, max: Int) =
    Array.tabulate(n)(i => N(util.Random.nextInt(max), i))

  def isStable(a: Array[N]): Boolean = {
    var i = 1;
    while (i < a.length) {
  ...
  def runOneTest(size: Int, variety: Int): Unit = {
    ...
    val rxs = {
      val temp = xs.clone; Sorting.stableSort(temp)(backwardsN); temp
    }
      ...
      i <- 0 until math.min(
        ...
        math
          .max(math.min(math.floor(math.pow(v, size) / 2),
                        math.ceil(workLimit / (math
                          .log(math.max(2, size)) / math.log(2) * size))),
               1)
          .toInt)
    } runOneTest(size, v)
