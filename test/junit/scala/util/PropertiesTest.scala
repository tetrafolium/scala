...
class PropertiesTest {
  ...
  def testPropOrElse(): Unit = {
    ...
    var done = false
    assertEquals(Properties.propOrElse(TestProperty, { done = true; "bar" }),
                 "foo")
  ...
  def testEnvOrElse(): Unit = {
    assertEquals(Properties.envOrElse("_PropertiesTest_NOT_DEFINED", "test"),
                 "test")

    var done = false
    val envName = System.getenv().keySet().iterator().next()
    assertNotEquals(Properties.envOrElse(envName, { done = true; "bar" }),
                    "bar")
    assertFalse("Does not evaluate alt if not needed", done)
  }
}
