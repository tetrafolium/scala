...
class CollectTest {
  ...
  def testing(expectedCalls: Seq[Int], expectedRes: Any = null)(
      body: => Any): Unit = {
    calls.clear()
    val res = body
  ...
  def testListCollect: Unit =
    testing(Seq(1, 2), List(1))(List(1, 2) collect {
      case x if f(x) && x < 2 => x
    })

  @Test
  def testListCollectFirst: Unit =
    testing(Seq(1), Some(1))(List(1, 2) collectFirst {
      case x if f(x) && x < 2 => x
    })
  ...
  def testStreamCollect: Unit =
    testing(Seq(1, 2), List(1))((Stream(1, 2).collect {
      case x if f(x) && x < 2 => x
    }).toList)

  @Test
  def testStreamCollectFirst: Unit =
    testing(Seq(1), Some(1))(Stream.continually(1) collectFirst {
      case x if f(x) && x < 2 => x
    })
