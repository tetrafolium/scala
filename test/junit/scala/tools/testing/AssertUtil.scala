...
object AssertUtil {
  ...
  def assertThrown[T <: Throwable: ClassTag](checker: T => Boolean)(
      body: => Any): Unit =
    try {
      body
      fail("Expression did not throw!")
      ...
      case NonFatal(other) =>
        val ae = new AssertionError(
          s"Exception not a ${implicitly[ClassTag[T]]}: $other")
  ...
  /** JUnit-style assertion for `IterableLike.sameElements`.
    */
  def assertSameElements[A, B >: A](expected: Iterable[A],
                                    actual: Iterable[B],
                                    message: String = ""): Unit =
  ...
  def assertZeroNetThreads(body: => Unit): Unit = {
    ...
    def check() = {
      ...
      assertEquals("Spurious early thread creation.",
                   beforeCount,
                   group.enumerate(beforeThreads))
      ...
      assertEquals("Spurious late thread creation.",
                   afterCount,
                   group.enumerate(afterThreads))
      val staleThreads = afterThreads.toList.diff(beforeThreads)
      //staleThreads.headOption.foreach(_.getStackTrace.foreach(println))
      assertEquals(staleThreads.mkString("There are stale threads: ", ",", ""),
                   beforeCount,
                   afterCount)
      assertTrue(staleThreads.mkString("There are stale threads: ", ",", ""),
                 staleThreads.isEmpty)
    }
    def test() = {
      try {
        check()
  ...
  /** Wait for a condition, with a simple back-off strategy.
    ...
    */
  def waitForIt(terminated: => Boolean,
                progress: Progress = Fast,
                label: => String = "test"): Unit = {
    val limit = 5
    var n = 1
    ...
    assertTrue(
      s"Expired after dormancy period $period waiting for termination condition $label",
      ended)
  }
  ...
  def readyOrNot(awaitable: Awaitable[_]): Boolean =
    Try(Await.ready(awaitable, TestDuration.Standard)).isSuccess
}
