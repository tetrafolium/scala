...
trait BytecodeTesting extends ClearAfterClass {
  ...
  val compiler = cached(
    "compiler",
    () => BytecodeTesting.newCompiler(extraArgs = compilerArgs))
}
...
class Compiler(val global: Global) {
  ...
  /**
    ...
    */
  def compiledClassesFromCache =
    global.genBCode.postProcessor.byteCodeRepository.compilingClasses.valuesIterator
      .map(_._1)
      .toList
      .sortBy(_.name)

  def resetOutput(): Unit = {
    global.settings.outputDirs
  ...
  def compileToBytes(scalaCode: String,
  ...
  def compileClasses(code: String,
                     javaCode: List[(String, String)] = Nil,
                     allowMessage: StoreReporter#Info => Boolean = _ => false)
    : List[ClassNode] = {
  ...
  def compileClass(
      code: String,
      javaCode: List[(String, String)] = Nil,
      allowMessage: StoreReporter#Info => Boolean = _ => false): ClassNode = {
    val List(c) = compileClasses(code, javaCode, allowMessage)
    c
  }

  def compileToBytesTransformed(scalaCode: String,
                                javaCode: List[(String, String)] = Nil,
                                beforeBackend: global.Tree => global.Tree)
    : List[(String, Array[Byte])] = {
    import global._
  ...
  def compileClassesTransformed(
      scalaCode: String,
      javaCode: List[(String, String)] = Nil,
      beforeBackend: global.Tree => global.Tree): List[ClassNode] =
    readAsmClasses(
      compileToBytesTransformed(scalaCode, javaCode, beforeBackend))

  def compileAsmMethods(
      code: String,
      allowMessage: StoreReporter#Info => Boolean = _ => false)
    : List[MethodNode] = {
  ...
  def compileAsmMethod(
      code: String,
      allowMessage: StoreReporter#Info => Boolean = _ => false): MethodNode = {
  ...
  def compileMethods(
      code: String,
      allowMessage: StoreReporter#Info => Boolean = _ => false): List[Method] =
    compileAsmMethods(code, allowMessage).map(convertMethod)

  def compileMethod(
      code: String,
      allowMessage: StoreReporter#Info => Boolean = _ => false): Method = {
    val List(m) = compileMethods(code, allowMessage = allowMessage)
    m
  }

  def compileInstructions(code: String,
...
object BytecodeTesting {
  def genMethod(
    ...
    val node = new MethodNode1(flags,
                               name,
                               descriptor,
                               genericSignature,
                               throwsExceptions)
  ...
  def getGeneratedClassfiles(
      outDir: AbstractFile): List[(String, Array[Byte])] = {
  ...
  def compileClassesSeparately(
      ...
      extraArgs: String = "",
      allowMessage: StoreReporter#Info => Boolean = _ => false,
      afterEach: AbstractFile => Unit = _ => ()): List[ClassNode] = {
    readAsmClasses(
      compileToBytesSeparately(codes, extraArgs, allowMessage, afterEach))
  }
  ...
  def assertSameSummary(actual: List[Instruction],
  ...
  def getAsmMethod(c: ClassNode, name: String): MethodNode = {
    ...
    def fail() = {
      ...
      throw new AssertionFailedError(
        s"Could not find method named $name among ${allNames}")
    }
    methods match {
      case List(m) => m
      case ms @ List(m1, m2) if BytecodeUtils.isInterface(c) =>
        ...
        (statics, nonStatics) match {
          case (List(staticMethod), List(_)) =>
            m1 // prefer the static method of the pair if methods in traits
          case _ => fail()
        }
      case ms => fail()
    }
  ...
  def assertHandlerLabelPostions(h: ExceptionHandler,
  ...
  val ignoreDeprecations = (info: StoreReporter#Info) =>
    info.msg.contains("deprecation")
}
