  ...
  @Test def lookupJavaClasses(): Unit = {
    ...
    val cp: ClassPath =
      if (specVersion == "" || specVersion == "1.8") {
        ...
        val elements = new ClassPathFactory(settings, closeableRegistry)
          .classesInPath(resolver.Calculated.javaBootClassPath)
        AggregateClassPath(elements)
      } else JrtClassPath(None, closeableRegistry).get

    assertEquals(Nil, cp.classes(""))
    assertTrue(cp.packages("java").toString,
    ...
    assertEquals("java/lang/Object",
                 AsmUtils.classFromBytes(jl_Object.file.toByteArray).name)
    assertTrue(
