...
class MultiReleaseJarTest extends BytecodeTesting {
  ...
  def mrJar(): Unit = {
    ...
    } // TODO test that the compiler warns that --release is unsupported.
    ...
    try {
      def code(newApi: String) =
        s"package p1; abstract class Versioned { def oldApi: Int; $newApi }"

      val oldC = compileToBytes(code("")).head._2
      val newC = compileToBytes(code("def newApi: Int")).head._2
      List(temp1, temp2).foreach(
        temp =>
          createZip(temp,
                    List("/p1/Versioned.class" -> oldC,
      ...
      def declsOfC(jarPath: Path, release: String) = {
        ...
        new g.Run
        val decls = g.rootMirror
          .staticClass("p1.Versioned")
          .info
          .decls
    ...
    } finally List(temp1, temp2).foreach(Files.deleteIfExists)
  }
  ...
  private def createManifest = {
    ...
    manifest.getMainAttributes
      .put(new Attributes.Name("Multi-Release"), String.valueOf(true))
    val os = new ByteArrayOutputStream()
    manifest.write(os)
    val manifestBytes = os.toByteArray
    manifestBytes
  }
  private def createZip(zipLocation: Path,
                        content: List[(String, Array[Byte])]): Unit = {
