...
class PicklerTest extends BytecodeTesting {
  ...
  def pickleUnpicklePreserveDeclOrder(): Unit = {
    assertStableDecls("package p1; trait C { def x: T; def y: Int; class T }",
                      "p1.C")
    assertStableDecls(
      "package p1; class D; object D { def x: T = null; def y: Int = 0; class T }",
      "p1.D")
  }
  ...
  def assertStableDecls(source: String, name: String): Unit = {
    ...
    def showDecls(global: Global): Seq[String] = global.exitingPickler {
      ...
      syms.flatMap(sym =>
        sym.name.toString :: sym.info.decls.toList.map(decl =>
          global.definitions.fullyInitializeSymbol(decl).defString))
    }
