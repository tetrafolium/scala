...
class IndySammyTest extends BytecodeTesting {
  ...
  def test(from: String,
           to: String,
           arg: String,
           body: String => String = x => x)(expectedSig: String,
                                            lamBody: List[Instruction],
                                            appArgs: List[Instruction],
                                            ret: Instruction)(
      allowMessage: StoreReporter#Info => Boolean = _ => false) = {
    val List(funClass, vcClass, vcCompanion) = compileClasses(
      s"${classPrologue(from, to)}")
    val c = compileClass(
      s"class C { ${lamDef(from, to, body)}; ${appDef(arg)} }",
    ...
    assert(lamInsn.length == 2 && lamInsn.head.isInstanceOf[InvokeDynamic],
           lamInsn)
    ...
    assertSameCode(
      ...
      List(VarOp(ALOAD, 0),
           Invoke(INVOKEVIRTUAL,
                  "C",
                  "lam",
                  s"()L${funClassName(from, to)};",
                  false)) ++ appArgs ++ List(Invoke(INVOKEINTERFACE,
                                                    funClassName(from, to),
                                                    "apply",
                                                    applySig,
                                                    true),
                                             ret)
...
//  }
  ...
  @Test
  def testVC_Any_VC =
    test("VC", "Any", "new VC(1)", x => s"new VC(1)")(
      "(I)Ljava/lang/Object;",
      List(TypeOp(NEW, "VC"),
           Op(DUP),
           Op(ICONST_1),
           Invoke(INVOKESPECIAL, "VC", "<init>", "(I)V", false),
           Op(ARETURN)),
      List(Op(ICONST_1)),
      Op(ARETURN)
    )()
  ...
  @Test
  def testAny_VC_1 =
    test("Any", "VC", "1", x => s"new VC($x.asInstanceOf[Int])")(
      "(Ljava/lang/Object;)I",
      List(VarOp(ALOAD, 0),
           Invoke(INVOKESTATIC,
...
//    testSpecial("x => x", "Special[Int]", "1")()
  ...
  def testStaticIfNoThisReference: Unit = {
    ...
    methodNodes.forall(m =>
      !m.name.contains("anonfun") || (m.access & ACC_STATIC) == ACC_STATIC)
  }
}
