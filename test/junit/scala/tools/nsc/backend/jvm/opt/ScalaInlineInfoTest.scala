...
class ScalaInlineInfoTest extends BytecodeTesting {
  ...
  def assertSameMethods(c: ClassNode, nameAndSigs: Set[String]): Unit = {
    ...
    for (m <- inClass.diff(nameAndSigs))
      r.append(s"method in classfile found, but no inline info: $m")
    for (m <- nameAndSigs.diff(inClass))
      r.append(s"inline info found, but no method in classfile: $m")
  ...
  def traitMembersInlineInfo(): Unit = {
    ...
    val expectT = InlineInfo(
      ...
      Map(
        ...
        ("T$$super$toString()Ljava/lang/String;",
         MethodInlineInfo(true, false, false)),
        ...
        ("L$2(Lscala/runtime/LazyRef;)LT$L$1$;",
         MethodInlineInfo(true, false, false)),
        ("nest$1()I", MethodInlineInfo(true, false, false)),
        ("$init$(LT;)V", MethodInlineInfo(true, false, false)),
        ("L$lzycompute$1(Lscala/runtime/LazyRef;)LT$L$1$;",
         MethodInlineInfo(true, false, false))
      ),
      None // warning
    )

    assert(infoT == expectT,
           mapDiff(expectT.methodInfos, infoT.methodInfos) + infoT)
    assertSameMethods(t, expectT.methodInfos.keySet)

    val infoC = inlineInfo(c)
    val expectC = InlineInfo(
      false,
      None,
      Map(
        ...
        "x4()I" -> MethodInlineInfo(false, false, false),
        "T$$super$toString()Ljava/lang/String;" -> MethodInlineInfo(true,
                                                                    false,
                                                                    false),
    ...
    assert(infoC == expectC,
           mapDiff(expectC.methodInfos, infoC.methodInfos) + infoC)
  ...
  def looksLikeForwarderTest(): Unit = {
    ...
    def tst(c: ClassNode, m: String, r: Int): Unit =
      assertEquals(
        looksLikeForwarderOrFactoryOrTrivial(getAsmMethod(c, m),
                                             c.name,
