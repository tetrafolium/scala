...
class UnreachableCodeTest extends ClearAfterClass {
  ...
  val methodOptCompiler =
    cached("methodOptCompiler", () => newCompiler(extraArgs = "-opt:l:method"))
  val dceCompiler = cached(
    "dceCompiler",
    () => newCompiler(extraArgs = "-opt:unreachable-code"))
  val noOptCompiler =
    cached("noOptCompiler", () => newCompiler(extraArgs = "-opt:l:none"))

  def assertEliminateDead(code: (Instruction, Boolean)*): Unit = {
    val method = genMethod()(code.map(_._1): _*)
    dceCompiler.global.genBCode.postProcessor.localOpt
      .removeUnreachableCodeImpl(method, "C")
  ...
  def eliminateDeadCatchBlocks(): Unit = {
    ...
    assertSameCode(methodOptCompiler.compileInstructions(code2),
                   wrapInDefault(Op(RETURN)))

    val code3 =
      "def f: Unit = { try { } catch { case _: Exception => try { } catch { case _: Exception => () } }; () }"
    assertSameCode(methodOptCompiler.compileInstructions(code3),
                   wrapInDefault(Op(RETURN)))
    ...
    assertSameCode(methodOptCompiler.compileInstructions(code4),
                   wrapInDefault(Op(RETURN)))
  }
  ...
  def bytecodeEquivalence(): Unit = {
    ...
    assertTrue(
      List(TableSwitch(TABLESWITCH, 1, 3, Label(4), List(Label(5), Label(6))),
           ...
           Label(5),
           Label(6)) ===
        List(TableSwitch(TABLESWITCH, 1, 3, Label(9), List(Label(3), Label(4))),
             Label(9),
             Label(3),
             Label(4)))
  ...
  def loadNullNothingBytecode(): Unit = {
    ...
    assertSameSummary(getMethod(c, "nt"), List(NEW, DUP, LDC, "<init>", ATHROW))

    assertSameSummary(getMethod(c, "t1"),
    ...
    assertSameSummary(getMethod(cDCE, "t3"),
