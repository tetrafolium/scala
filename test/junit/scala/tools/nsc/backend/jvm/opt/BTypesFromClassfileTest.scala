...
class BTypesFromClassfileTest extends BytecodeTesting {
  ...
  def sameBType(fromSym: ClassBType,
    ...
    if (checked(fromSym.internalName)) checked
      ...
      assert(fromSym.internalName == fromClassfile.internalName,
             s"${fromSym.internalName} != ${fromClassfile.internalName}")
  ...
  def sameInfo(fromSym: ClassInfo,
    ...
    assert(
      {
        ...
        if (fromSym.nestedInfo.force.isEmpty)
    ...
    val memberClassesFromClassfile = fromClassfile.nestedClasses.force
      .filter(_.info.get.nestedInfo.force.get.outerName.isDefined)
    ...
    // code generation).
    val chk3 = sameBTypes(fromSym.nestedClasses.force.sortBy(_.internalName),
                          memberClassesFromClassfile.sortBy(_.internalName),
                          chk2)
    sameBTypes(fromSym.nestedInfo.force.map(_.enclosingClass),
               fromClassfile.nestedInfo.force.map(_.enclosingClass),
               chk3)
  }

  // Force all lazy components of a `ClassBType`.
  def forceLazy(classBType: ClassBType,
                done: mutable.Set[ClassBType] = mutable.Set.empty): Unit = {
    if (!done(classBType)) {
      done += classBType
