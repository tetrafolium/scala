...
class ParamAliasTest extends BytecodeTesting {
  ...
  def checkAliasWorksWhenSubclassesAreTypecheckedFirst(): Unit = {
    def test(code: List[String],
      ...
      Predef.assert(!compiler1.global.reporter.hasErrors,
                    compiler1.global.reporter.asInstanceOf[StoreReporter].infos)
      def aliasNames(g: Global) = {
        check.map {
          case (clsName, paramName) =>
            val cls = g.rootMirror.getRequiredClass(clsName)
            val field = g.exitingPickler(
              cls.info
                .decl(g.TermName(paramName))
                .suchThat(_.isParamAccessor)
                .accessed)
      ...
      val out = compiler1.global.settings.outputDirs.getSingleOutput.get
        .asInstanceOf[VirtualDirectory]
      compiler2.global.platform.classPath
      compiler2.global.platform.currentClassPath = Some(AggregateClassPath(new VirtualDirectoryClassPath(
        out) :: compiler2.global.platform.currentClassPath.get :: Nil))
    ...
    {
      val code = List("package p1; class A(val a: Int) extends B(a)",
                      "package p1; class B(b: Int) extends C(b)",
                      "package p1; class C(val c: Int)")
    ...
    {
      val code = List("package p1; class A(val a: Int) extends B(a)",
                      "package p1; class B(val b: Int) extends C(b)",
                      "package p1; class C(val c: Int)")
