...
abstract class RedBlackTreeTest(tname: String)
    extends Properties(tname)
    with RedBlackTreeInvariants[String, Int] {
  def minimumSize = 0
  def maximumSize = 5
  ...
  def genInput: Gen[(Tree[String, Int], ModifyParm, Tree[String, Int])] =
    for {
      tree <- genTree
      parm <- genParm(tree)
    } yield (tree, parm, modify(tree, parm))

  def setup(invariant: Tree[String, Int] => Boolean): Prop = forAll(genInput) {
...
trait RedBlackTreeInvariants[K, V] {
  ...
  def orderIsPreserved[A](t: Tree[K, V]): Boolean =
    iterator(t) zip iterator(t).drop(1) forall {
      case (x, y) => ordering.compare(x._1, y._1) < 0
    }
...
object TestFromOrderedKeys
    extends Properties("RedBlackTree.fromOrderedKeys")
    with RedBlackTreeInvariants[Int, Null] {
  import RB._
...
object TestFromOrderedEntries
    extends Properties("RedBlackTree.fromOrderedEntries")
    with RedBlackTreeInvariants[Int, String] {
  import RB._
  ...
  property("fromOrderedKeys") = forAll { (s: Set[Int]) =>
    s.toIndexedSeq.sorted
      .map(i => (i, i.toString)) == iterator(modify(s)).toIndexedSeq
  }
...
abstract class BulkTest(pName: String) extends RedBlackTreeTest(pName) {
  ...
  override def modify(tree: Tree[String, Int],
                      parm: ModifyParm): Tree[String, Int] = treeOp(tree, parm)
  ...
  override def setup(invariant: Tree[String, Int] => Boolean) = forAll {
...
object TestUnion extends BulkTest("RedBlackTree.union") {
  ...
  def treeOp(t1: Tree[String, Int], t2: Tree[String, Int]): Tree[String, Int] =
    union(t1, t2)
  def setOp(s1: Set[(String, Int)],
            s2: Set[(String, Int)]): Set[(String, Int)] = s1.union(s2)
}
...
object TestIntersect extends BulkTest("RedBlackTree.intersect") {
  ...
  def treeOp(t1: Tree[String, Int], t2: Tree[String, Int]): Tree[String, Int] =
    intersect(t1, t2)
  def setOp(s1: Set[(String, Int)],
            s2: Set[(String, Int)]): Set[(String, Int)] = s1.intersect(s2)
}
...
object TestDifference extends BulkTest("RedBlackTree.difference") {
  ...
  def treeOp(t1: Tree[String, Int], t2: Tree[String, Int]): Tree[String, Int] =
    difference(t1, t2)
  def setOp(s1: Set[(String, Int)],
            s2: Set[(String, Int)]): Set[(String, Int)] = s1.diff(s2)
}
...
object TestFilter extends RedBlackTreeTest("RedBlackTree.filterEntries") {
  ...
  property("filter") = forAll(genInput) {
    case (tree, parm, newTree) =>
      iterator(tree).filter(t => t._1.hashCode % 2 == 0).toList == iterator(
        newTree).toList
  }
}

object TestPartitionLeft
    extends RedBlackTreeTest("RedBlackTree.partitionKeysLeft") {
  import RB._
  ...
  property("partition") = forAll(genInput) {
    case (tree, parm, newTree) =>
      iterator(tree).filter(t => t._1.hashCode % 2 == 0).toList == iterator(
        newTree).toList
  }
...
object TestPartitionRight
    extends RedBlackTreeTest("RedBlackTree.partitionKeysRight") {
  import RB._
...
object TestTransform extends RedBlackTreeTest("RedBlackTree.transform") {
  ...
  property("transform") = forAll(genInput) {
    case (tree, parm, newTree) =>
      iterator(tree).toList.map {
        case (k, v) => if (v < parm) (k, v) else (k, v + 1)
