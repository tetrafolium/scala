...
trait ClassConstruction { self: QuasiquoteProperties =>
  ...
  property("case class bare param") = test {
    assertEqAst(
      q"$CASE class Point(x: Int, y: Int)",
      "case class Point(private[this] val x: Int, private[this] val y: Int)")
  }
  ...
  property("unquote method into class") = forAll {
    (name: TypeName, method: DefDef) =>
      q"class $name { $method }" ≈ classWith(name, body = List(method))
  }

  property("unquote members into class") = forAll {
    (name: TypeName, defs: List[DefDef], extra: DefDef) =>
      q"""class $name {
      ..$defs
      $extra
...
trait TraitConstruction { self: QuasiquoteProperties =>
  ...
  property("unquote defs into early block") = test {
    ...
    assert(
      q"trait T extends { ..$defs } with Bippy" ≈
  ...
  property("fail on splicing of non-valid early tree") = test {
    ...
    assertThrows[IllegalArgumentException] {
      q"trait T extends { $defn } with Bar"
    }
  }
}
...
trait TypeDefConstruction { self: QuasiquoteProperties =>
  ...
  property("unquote trees names into type bounds") = forAll {
    (T: TypeName, t1: Tree, t2: Tree) =>
      q"type $T >: $t1 <: $t2" ≈
        TypeDef(Modifiers(DEFERRED), T, List(), TypeBoundsTree(t1, t2))
  }

  property("unquote tparams into typedef (1)") = forAll {
    (T: TypeName, targs: List[TypeDef], t: Tree) =>
    ...
    (T1: TypeName, T2: TypeName, args: List[Tree]) =>
      q"type $T1 = $T2[..$args]" ≈
        TypeDef(Modifiers(),
                T1,
                List(),
                if (args.nonEmpty) AppliedTypeTree(Ident(T2), args)
                else Ident(T2))
  }
}
...
trait ValDefConstruction { self: QuasiquoteProperties =>
  ...
  property("unquote into var") = forAll {
    (name: TermName, tpt: Tree, rhs: Tree) =>
...
trait PatDefConstruction { self: QuasiquoteProperties =>
  ...
  property("unquote patterns into pat def within other pattern (2)") = test {
    ...
    assertEqAst(
      ...
      "val ((a, b), (c, d)): ((Int, Int), (Int, Int)) = ((1, 2), (3, 4))")
  }
...
trait PackageConstruction { self: QuasiquoteProperties =>
  ...
  property("unquote early def into package object") = test {
    ...
    assertEqAst(
      q"package object foo extends { ..$edefs } with Any",
