...
trait ArbitraryTreesAndNames {
  ...
  def genIf(size: Int) =
    for (cond <- genTree(size - 1); thenp <- genTree(size - 1);
         elsep <- genTree(size - 1))
  ...
  implicit val arbTreeIsTerm: Arbitrary[TreeIsTerm] = arbitrarySized(
