...
object HtmlFactoryTest extends Properties("HtmlFactory") {
  ...
  def createTemplates(basename: String): collection.Map[String, HtmlPage] = {
    ...
    val path: String = SettingsUtil.checkoutRoot
      .resolve(RESOURCES)
      .resolve(basename)
      .toAbsolutePath
      .toString
  ...
  /**
    ...
    */
  def checkText(scalaFile: String, debug: Boolean = true)(
      checks: (Option[String], String, Boolean)*): Boolean = {
    ...
    for ((fileHint, check, expected) <- checks) {
      ...
      val fileTextPretty = textOf(htmlAllFiles(fileName).body)
        .replace('→', ' ')
        .replaceAll("\\s+", " ")
  ...
  private def checkTemplate(base: String, file: String)(
      check: (collection.Map[String, HtmlPage], String) => Boolean): Boolean = {
    val files = createTemplates(base)
    files.get(file).exists { page =>
      check(files, toHtml(page))
    }
  }
  ...
  private def checkTemplate(base: String)(check: String => Boolean): Boolean =
    checkTemplate(base, base.stripSuffix(".scala") + ".html") { (_, s) =>
      check(s)
    }

  property("Trac #3790") = {
    checkTemplate("Trac3790.scala") {
      _.contains("""<p class="shortcomment cmt">A lazy String
        |<p>""".stripMargin)
    }
    checkTemplate("Trac3790.scala") {
      _.contains("""<p class="shortcomment cmt">A non-lazy String
        |</p>""".stripMargin)
    }
  }

  property("Trac #4306") = {
    checkTemplate("Trac4306.scala",
                  "com/example/trac4306/foo/package$$Bar.html") { (_, _) =>
      true
    }
  }
  ...
  property("Trac #4358") = {
    checkTemplate("Trac4358.scala") { html =>
      !html.contains("<em>i.</em>")
    }
  }

  property("Trac #4180") = {
    checkTemplate("Trac4180.scala") { _ =>
      true
    }
  }
  ...
  property("Trac #4372") = {
    checkTemplate("Trac4372.scala") { html =>
      html.contains(
        """<span class="name" title="gt4s: $plus$colon">+:</span>""") &&
      html.contains(
        """<span class="name" title="gt4s: $minus$colon">-:</span>""") &&
      html.contains(
        """<span class="params">(<span name="n">n: <span name="scala.Int" class="extype">Int</span></span>)</span><span class="result">: <span name="scala.Int" class="extype">Int</span></span>""")
    }
  }
  ...
  property("Trac #4409") = {
    checkTemplate("Trac4409.scala") { s =>
      !s.contains("""<div class="block"><ol>since""")
    }
  }

  property("Trac #4452") = {
    checkTemplate("Trac4452.scala") { s =>
      !s.contains(">*")
    }
  }
  ...
  property("scala/bug#4287: Default arguments of synthesized constructor") = {
    checkTemplate("t4287.scala", "ClassWithSugar.html") { (_, s) =>
      s.contains(">123<")
    }
  }
  ...
  property(
    "scala/bug#5054: Use cases should keep their flags - final should not be lost") =
    checkText("t5054_q2.scala")((None, """final def test(): Int""", true))

  property(
    "scala/bug#5054: Use cases should keep their flags - implicit should not be lost") =
    checkText("t5054_q3.scala")((None, """implicit def test(): Int""", true))

  property(
    "scala/bug#5054: Use cases should keep their flags - real abstract should not be lost") =
    checkText("t5054_q4.scala")((None, """abstract def test(): Int""", true))

  property(
    "scala/bug#5054: Use cases should keep their flags - traits should not be affected") =
    ...
    checkText("t5287.scala")(
      (None,
       """def method(): Int
           ...
           [use case] The usecase explanation
           Definition Classes t5287 t5287_B t5287_A""",
       true)
...

  {
    ...
    property("package object") = files.get("com/example/p1/index.html").exists {
      page =>
        val html = toHtml(page)
    ...
    property("scala/bug#8514: No inconsistencies") = checkText("t8514.scala")(
      (Some("a/index"),
  ...
  // scala/bug#8144
  {
    ...
    property("scala/bug#8144: Members' permalink - inner package") = files
      .get("some/pack/index.html")
      .map { page =>
        val html = toHtml(page)
        ("type link" |: html.contains("../../some/pack/index.html")) &&
        ("member: SomeType (object)" |: html.contains(
          """<a href="../../some/pack/index.html#SomeType" title="Permalink">""")) &&
        ("member: SomeType (class)" |: html.contains(
          """<a href="../../some/pack/index.html#SomeTypeextendsAnyRef" title="Permalink">"""))
      }
      .getOrElse(Prop.falsified)

    property("scala/bug#8144: Members' permalink - companion object") = files
      .get("some/pack/SomeType$.html")
      .map { page =>
        val html = toHtml(page)
        ("type link" |: html.contains("../../some/pack/SomeType$.html")) &&
        ("member: someVal" |: html.contains(
          """<a href="../../some/pack/SomeType$.html#someVal:String" title="Permalink">"""))
      }
      .getOrElse(Prop.falsified)

    property("scala/bug#8144: Members' permalink - class") = files
      .get("some/pack/SomeType.html")
      .map { page =>
        val html = toHtml(page)
        (("type link" |: html.contains("../../some/pack/SomeType.html")) &&
        ("constructor " |: html.contains(
          """<span class="permalink"><a href="../../some/pack/SomeType.html#&lt;init&gt;(arg:String):some.pack.SomeType" title="Permalink">""")) &&
        ("member: type TypeAlias" |: html.contains(
          """<span class="permalink"><a href="../../some/pack/SomeType.html#TypeAlias=String" title="Permalink">""")) &&
        ("member: def >#<():Int " |: html.contains(
          """<span class="permalink"><a href="../../some/pack/SomeType.html#&gt;#&lt;():Int" title="Permalink">""")) &&
        ("member: def >@<():TypeAlias " |: html.contains(
          """<span class="permalink"><a href="../../some/pack/SomeType.html#&gt;@&lt;():SomeType.this.TypeAlias" title="Permalink">""")))
      }
      .getOrElse(Prop.falsified)
  ...
  property(
    "scala/bug#9599 Multiple @todo formatted with comma on separate line") = {
    checkTemplate("t9599.scala", "X.html") { (_, s) =>
      s.contains(
        """<span class="cmt"><p>todo1</p></span><span class="cmt"><p>todo2</p></span><span class="cmt"><p>todo3</p></span>""")
    }
  }
}
