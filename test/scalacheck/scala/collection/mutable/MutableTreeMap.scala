...
object MutableTreeMapProperties
  ...
  property("valuesIteratorFrom") = forAll { (entries: Map[K, V], k: K) =>
    ...
    map
      ...
      .toSeq == entries.view.filterKeys(_ >= k).toSeq.sorted.map(_._2)
  }
  ...
  property("same behavior as immutable.TreeMap") = forAll {
    ops: Seq[Either[(K, V), K]] =>
...
object MutableTreeMapProjectionProperties
    extends Properties("mutable.TreeMapProjection")
    with Generators {
  type K = String
  type V = Int
  ...
  def entriesInView[This <: IterableOnce[(K, V)], That](
      entries: This,
      from: Option[K],
      until: Option[K])(implicit bf: BuildFrom[This, (K, V), That]) = {
    (bf.newBuilder(entries) ++= entries.iterator.filter {
  ...
  property("size, isEmpty") = forAll {
    (entries: Map[K, V], from: Option[K], until: Option[K]) =>
      val map = mutable.TreeMap[K, V]()
      map ++= entries
  ...
  property("iteratorFrom") = forAll {
    (entries: Map[K, V], k: K, from: Option[K], until: Option[K]) =>
      val map = mutable.TreeMap[K, V]()
      map ++= entries
      ...
      mapView
        .iteratorFrom(k)
        .toSeq == entriesInView(entries, newLower, until).toSeq.sorted
  }
