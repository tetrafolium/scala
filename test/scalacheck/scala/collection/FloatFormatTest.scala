...
object FloatFormatTest extends Properties("FloatFormat") {
  ...
  val genPossibleFloatString = {
    val parts =
      List(genWS, genDecOrHex, genExpSign, genDecOrHex, genSuffix, genWS)
  ...
  def genBogifiedString(parts: List[Gen[String]]): Gen[String] = {
    ...
    val bogoparts: List[Gen[String]] = parts.map(
      right =>
        Gen.frequency(
          1 -> wrong,
          10 -> right
  ...
  property(
    "for all strings (plausible examples) str.toDoubleOption == Try(str.toDouble).toOption") =
    forAll(genPlausibleFloatString) { str =>
      deq(str.toDoubleOption, Try(str.toDouble).toOption)
    }

  property(
    "for all strings (possible examples) str.toDoubleOption == Try(str.toDouble).toOption") =
    forAll(genPossibleFloatString) { str =>
      deq(str.toDoubleOption, Try(str.toDouble).toOption)
    }

  property(
    "for all strings (random strings) str.toDoubleOption == Try(str.toDouble).toOption") =
    forAll { (str: String) =>
      deq(str.toDoubleOption, Try(str.toDouble).toOption)
    }

  property("double.toString is a valid float format") = forAll { (d: Double) =>
    checkFloatFormat(d.toString)
  }

  property("nan and infinity toDouble == toDoubleOption.get") =
    forAll(genNanInf) { (str: String) =>
      deq(str.toDoubleOption, Try(str.toDouble).toOption)
    }
