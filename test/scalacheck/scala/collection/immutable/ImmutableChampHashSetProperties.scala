...
object ImmutableChampHashSetProperties
    extends Properties("immutable.ChampHashSet") {
  ...
  private def doSubtract(one: HashSet[K], two: HashSet[K]) = {
    one.foldLeft(HashSet.empty[K])((result, elem) =>
      if (two contains elem) result else result + elem)
  }

  private def doIntersect(one: HashSet[K], two: HashSet[K]) = {
    one.foldLeft(HashSet.empty[K])((result, elem) =>
      if (two contains elem) result + elem else result)
  }
  ...
  property("convertToScalaSetAndCheckHashCode") = forAll {
    (input: HashSet[K]) =>
  ...
  property("convertToScalaSetAndCheckEquality") = forAll {
    (input: HashSet[K]) =>
  ...
  property("notContainedAfterInsertRemove") = forAll {
    (input: HashSet[K], item: K) =>
  ...
  property("intersectIdentityStructural") = forAll {
    (inputShared: HashSet[K]) =>
      val builder = HashSet.newBuilder[K]
  ...
  property("intersect") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K], inputShared: HashSet[K]) =>
      val oneWithoutShared =
        doSubtract(doSubtract(inputOne, inputShared), inputTwo)
      val twoWithoutShared =
        doSubtract(doSubtract(inputTwo, inputShared), inputOne)
  ...
  property("intersectMaintainsSizeAndHashCode") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K], inputShared: HashSet[K]) =>
      val oneWithShared = inputOne ++ inputShared
      val twoWithShared = inputTwo ++ inputShared
      val intersectedWithShared = oneWithShared.intersect(twoWithShared)

      convertToScalaSetAndCheckSize(intersectedWithShared) && convertToScalaSetAndCheckHashCode(
        intersectedWithShared)
  }

  property("intersectEqualToDefaultImplementation") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K], inputShared: HashSet[K]) =>
      val oneWithShared = inputOne ++ inputShared
      val twoWithShared = inputTwo ++ inputShared
  ...
  property("intersectIdentityMostlyReference") = forAll {
    (input: HashSet[K], key: K) =>
      val inputCopy =
        if (input.contains(key)) input - key + key else input + key - key
  ...
  property("unionIdentityMostlyReference") = forAll {
    (input: HashSet[K], key: K) =>
      val inputCopy =
        if (input.contains(key)) input - key + key else input + key - key
  ...
  property("unionMaintainsSizeAndHashCode") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K]) =>
      val unioned = inputOne.union(inputTwo)

      convertToScalaSetAndCheckSize(unioned) && convertToScalaSetAndCheckHashCode(
        unioned)
  }

  property("unionEqualToDefaultImplementation") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K]) =>
  ...
  property("subtract") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K]) =>
      ...
      subtracted.forall(inputOne.contains) && !subtracted.exists(
        inputTwo.contains)
  }

  property("subtractMaintainsSizeAndHashCode") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K]) =>
      val subtracted = inputOne.diff(inputTwo)
      convertToScalaSetAndCheckSize(subtracted) && convertToScalaSetAndCheckHashCode(
        subtracted)
  }
  ...
  property("subtractIdentityMostlyReference") = forAll {
    (input: HashSet[K], key: K) =>
      val inputCopy =
        if (input.contains(key)) input - key + key else input + key - key
  ...
  property("subtractMaintainsSizeAndHashCode") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K], inputShared: HashSet[K]) =>
      ...
      convertToScalaSetAndCheckSize(subtractedWithShared) && convertToScalaSetAndCheckHashCode(
        subtractedWithShared)
  }

  property("subtractEqualToDefaultImplementation") = forAll {
    (inputOne: HashSet[K], inputTwo: HashSet[K], inputShared: HashSet[K]) =>
      val oneWithShared = inputOne ++ inputShared
      val twoWithShared = inputTwo ++ inputShared
  ...
  property("iterator equals reverseIterator.reverse()") = forAll {
    input: HashSet[K] =>
      val xs: List[K] =
        input.iterator
          .foldLeft(List.empty[K])((list: List[K], item: K) =>
            list prepended item)
  ...
  property("smaller subsetOf larger") = forAll { inputSet: HashSet[K] =>
    if (inputSet.isEmpty) {
      ...
      val randomIndices =
        ArraySeq.fill(randomSamples)(scala.util.Random.nextInt(inputSet.size))
  ...
  property("(xs ++ ys).toSet == xs.toSet ++ ys.toSet") = forAll {
    (xs: Seq[K], ys: Seq[K]) =>
      (xs ++ ys).toSet =? xs.toSet ++ ys.toSet
  }
  property("HashSetBuilder produces the same Set as SetBuilder") = forAll {
    (xs: Seq[K]) =>
      HashSet.newBuilder[K].addAll(xs).result() =? HashSet
        .newBuilder[K]
        .addAll(xs)
        .result()
  }
  property("HashSetBuilder does not mutate after releasing") = forAll {
    (xs: Seq[K], ys: Seq[K], single: K, addSingleFirst: Boolean) =>
      ...
      Prop(b.result().size >= hashSetA.size) && ((hashSetA: Set[K]) ?= (cloneOfA: Set[
        K]))
  }
  property("Set does not mutate after releasing") = forAll {
    (xs: Seq[K], ys: Seq[K], single: K, addSingleFirst: Boolean) =>
  ...
  property("(xs - elem) == xs.toList.filterNot(_ == elem).toSet") = forAll {
    (xs: Set[K], elem: K) =>
      (xs - elem) == xs.toList.filterNot(_ == elem).toSet
  }
  property("(xs + elem) == (xs.toList :+ elem).toSet") = forAll {
    (xs: Set[K], elem: K) =>
      (xs + elem) == (xs.toList :+ elem).toSet
  }
  property("xs -- range == xs -- range.toSet") = forAll {
    (xs: Set[Int], rangeMax: Int) =>
