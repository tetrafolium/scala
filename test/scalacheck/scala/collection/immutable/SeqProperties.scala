...
object SeqProperties
    extends Properties("immutable.Seq builder implementations") {

  type A = Int

  property("Seq builder stateful testing") =
    new SeqBuilderStateProperties(Seq.newBuilder[A]).property()
  property("List builder stateful testing") =
    new SeqBuilderStateProperties(List.newBuilder[A]).property()
  property("ArraySeq builder stateful testing") =
    new SeqBuilderStateProperties(ArraySeq.newBuilder[A]).property()
  property("LazyList builder stateful testing") =
    new SeqBuilderStateProperties(LazyList.newBuilder[A]).property()
  property("Queue builder stateful testing") =
    new SeqBuilderStateProperties(Queue.newBuilder[A]).property()
  property("IndexedSeq builder stateful testing") =
    new SeqBuilderStateProperties(IndexedSeq.newBuilder[A]).property()
  property("Stream builder stateful testing") =
    new SeqBuilderStateProperties(Stream.newBuilder[A]).property()
  property("Vector builder stateful testing") =
    new SeqBuilderStateProperties(Vector.newBuilder[A]).property()
  property("WrappedString builder stateful testing") =
    new SeqBuilderStateProperties(WrappedString.newBuilder).property()
...
class SeqBuilderStateProperties[A: Arbitrary, To <: Seq[A]](
  ...
  case object Result extends Command {
    ...
    override def postCondition(state: State, result: Try[Result]) =
      result.map(_.toVector) == Success(state.toVector)
