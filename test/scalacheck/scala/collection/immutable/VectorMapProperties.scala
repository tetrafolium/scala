...
object VectorMapProperties extends Properties("immutable.VectorMap") {
  ...
  property("internal underlying and index are consistent after removal") =
    forAll { (m: Map[K, V]) =>
      m.size >= 3 ==> {
        val v = Vector.from(m)
  ...
  property("repeatedly removing to empty is empty at end") = forAll {
    (m: Map[K, V]) =>
      {
  ...
  property("nullable types for keys and values") = forAll {
    m: Map[String, String] =>
      {
        val vm = VectorMap.from(m)
        vm.underlying.size == vm.keys.length
  ...
  property(
    "internal underlying init size of non empty is consistent with original size and keys init size") =
    forAll { m: Map[K, V] =>
      m.size >= 1 ==> {
        val vm = VectorMap.from(m)
        val sz = vm.size - 1
