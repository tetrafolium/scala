...
object MapViewProperties extends Properties("MapView") {
  ...
  property("filter behaves like Map.filter") = forAll {
    (m: Map[K, V], p: ((K, V)) => Boolean, isFlipped: Boolean) =>
      if (isFlipped)
  ...
  property("concat behaves like Map.concat") = forAll {
    (m0: Map[K, V], m1: Map[K, V]) =>
  ...
  property("++ behaves like Map.++") = forAll {
    (m0: Map[K, V], m1: Map[K, V]) =>
  ...
  property("partition behaves like Map.partition") = forAll {
    (m: Map[K, V], p: ((K, V)) => Boolean) =>
