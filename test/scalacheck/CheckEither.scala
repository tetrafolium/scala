...
object CheckEitherTest extends Properties("Either") {
  ...
  object CheckLeftProjection {
    ...
    val prop_getOrElse = forAll((e: Either[Int, Int], or: Int) =>
      e.left.getOrElse(or) == (e match {
        case Left(a)  => a
        case Right(_) => or
      }))

    val prop_forall = forAll((e: Either[Int, Int]) => e.left.forall(_ % 2 == 0) == (e.isRight || e.left.get % 2 == 0))

    val prop_exists = forAll((e: Either[Int, Int]) => e.left.exists(_ % 2 == 0) == (e.isLeft && e.left.get % 2 == 0))
    ...
    val prop_filterToOption = forAll(
      (e: Either[Int, Int], x: Int) =>
        e.left.filterToOption(_ % 2 == 0) ==
          (if (e.isRight || e.left.get % 2 != 0) None else Some(e)))

    val prop_seq = forAll((e: Either[Int, Int]) =>
      e.left.toSeq == (e match {
        case Left(a)  => Seq(a)
        case Right(_) => Seq.empty
      }))

    val prop_option = forAll((e: Either[Int, Int]) =>
      e.left.toOption == (e match {
        case Left(a)  => Some(a)
        case Right(_) => None
      }))
  }
  ...
  object CheckRightProjection {
    ...
    val prop_getOrElse = forAll((e: Either[Int, Int], or: Int) =>
      e.right.getOrElse(or) == (e match {
        case Left(_)  => or
        case Right(b) => b
      }))

    val prop_forall = forAll((e: Either[Int, Int]) => e.right.forall(_ % 2 == 0) == (e.isLeft || e.right.get % 2 == 0))

    val prop_exists = forAll((e: Either[Int, Int]) => e.right.exists(_ % 2 == 0) == (e.isRight && e.right.get % 2 == 0))
    ...
    val prop_filterToOption = forAll(
      (e: Either[Int, Int], x: Int) =>
        e.right.filterToOption(_ % 2 == 0) ==
          (if (e.isLeft || e.right.get % 2 != 0) None else Some(e)))

    val prop_seq = forAll((e: Either[Int, Int]) =>
      e.right.toSeq == (e match {
        case Left(_)  => Seq.empty
        case Right(b) => Seq(b)
      }))

    val prop_option = forAll((e: Either[Int, Int]) =>
      e.right.toOption == (e match {
        case Left(_)  => None
        case Right(b) => Some(b)
      }))
  }
  ...
  val prop_getOrElse = forAll((e: Either[Int, Int], or: Int) =>
    e.getOrElse(or) == (e match {
      case Left(_)  => or
      case Right(b) => b
    }))

  val prop_orElse = forAll((e: Either[Int, Int], or: Either[Int, Int]) =>
    e.orElse(or) == (e match {
      case Left(_)  => or
      case Right(_) => e
    }))

  val prop_contains = forAll((e: Either[Int, Int], n: Int) => e.contains(n) == (e.isRight && e.right.get == n))

  val prop_forall = forAll((e: Either[Int, Int]) => e.forall(_ % 2 == 0) == (e.isLeft || e.right.get % 2 == 0))

  val prop_exists = forAll((e: Either[Int, Int]) => e.exists(_ % 2 == 0) == (e.isRight && e.right.get % 2 == 0))
  ...
  val prop_filterOrElse = forAll(
    (e: Either[Int, Int], x: Int) =>
      e.filterOrElse(_ % 2 == 0, -x) ==
        (if (e.isLeft) e
  ...
  val prop_try = forAll((e: Either[Throwable, Int]) =>
    e.toTry == (e match {
  ...
  val prop_Either_cond = forAll((c: Boolean, a: Int, b: Int) => Either.cond(c, a, b) == (if (c) Right(a) else Left(b)))
