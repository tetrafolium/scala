...
object SeqMapTest extends Properties("SeqMap") {
  ...
  property("ordering append") = forAll(arbitrary[Map[Int, Int]] suchThat (_.size > 1)) { (m: Map[Int, Int]) =>
  ...
  property("ordering on withDefault") = forAll(arbitrary[Map[Int, Int]] suchThat (_.size > 1)) { (m: Map[Int, Int]) =>
    val (m1, m2) = m.splitAt(m.size / 2)
    val first = m1.toList
    val second = m2.toList
