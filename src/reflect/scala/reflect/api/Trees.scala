...
trait Trees { self: Universe =>
  ...
  abstract class ValDefExtractor {
    ...
    def apply(sym: Symbol, rhs: Tree)(implicit token: CompatToken): ValDef =
      internal.valDef(sym, rhs)
  ...
  abstract class DefDefExtractor {
    ...
    /** @see [[InternalApi.defDef]] */
    @deprecated("use `internal.defDef` instead", "2.11.0")
    def apply(sym: Symbol, rhs: Tree)(implicit token: CompatToken): DefDef =
      internal.defDef(sym, rhs)
  ...
  abstract class TypeDefExtractor {
    ...
    def apply(sym: Symbol, rhs: Tree)(implicit token: CompatToken): TypeDef =
      internal.typeDef(sym, rhs)
...
//    protected def currentPackage = currentOwner.enclosingTopLevelClass.owner
  ...
  /** Provides an extension hook for the transformation strategy.
    ...
    *  @group Traversal
    */
  protected def xtransform(transformer: Transformer, tree: Tree): Tree =
    throw new MatchError(tree)
