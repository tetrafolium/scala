...
// ----------- Caching ------------------------------------------------------------------
    ...
    private[this] val constructorCache =
      new TwoWayCache[jConstructor[_], MethodSymbol]
    private[this] val fieldCache = new TwoWayCache[jField, TermSymbol]
    private[this] val tparamCache =
      new TwoWayCache[jTypeVariable[_ <: GenericDeclaration], TypeSymbol]

    private[runtime] def toScala[J: HasJavaClass, S](
        cache: TwoWayCache[J, S],
        key: J)(body: (JavaMirror, J) => S): S =
      cache.toScala(key) {
...
// ----------- Implementations of mirror operations and classes  -------------------
      ...
      def apply(schemaAndValue: (jClass[_], Any)): ClassfileAnnotArg =
        schemaAndValue match {
          ...
          case (clazz @ ArrayClass(), value: Array[_]) =>
            ArrayAnnotArg(value map (x => apply(clazz.getComponentType -> x)))
    ...
    private case class JavaAnnotationProxy(jann: jAnnotation)
      ...
      override lazy val assocs: List[(Name, ClassfileAnnotArg)] = (
        jann.annotationType.getDeclaredMethods
          .sortBy(_.getName)
          .toList map (m =>
    ...
    def moduleSymbol(rtcls: RuntimeClass): ModuleSymbol =
      classToScala(rtcls).companionModule.asModule
    ...
    private def checkConstructorOf(sym: Symbol, owner: ClassSymbol): Unit = {
      ...
      ensuringNotFree(sym) {
        if (!owner.info.decls.toList.contains(sym))
          ErrorNotConstructor(sym, owner)
      }
    }
    ...
    private class JavaFieldMirror(val receiver: Any,
                                  ...
                                  metadata: DerivedValueClassMetadata)
        extends FieldMirror {
      def this(receiver: Any, symbol: TermSymbol) =
        this(receiver, symbol, new DerivedValueClassMetadata(symbol.info))
      def bind(newReceiver: Any) =
        new JavaFieldMirror(newReceiver, symbol, metadata)
      import metadata._
    ...
    lazy val bytecodelessMethodOwners = Set[Symbol](
      ...
      AnyRefClass,
      ObjectClass,
      ArrayClass) ++ ScalaPrimitiveValueClasses
    lazy val bytecodefulObjectMethods = Set[Symbol](
      Object_clone,
      Object_equals,
      Object_finalize,
      Object_hashCode,
    ...
    private def mkMethodMirror[T: ClassTag](
        receiver: T,
        symbol: MethodSymbol): MethodMirror = {
      def existsParam(pred: Type => Boolean) =
        symbol.paramss.flatten.map(_.info).exists(pred)
    ...
    private abstract class JavaMethodMirror(
        ...
        protected val ret: DerivedValueClassMetadata)
      ...
      def jinvokeraw(args: Seq[Any]) =
        if (!symbol.isConstructor)
          ...
          jconstr.newInstance(args.asInstanceOf[Seq[AnyRef]]: _*)
        else
          jconstr.newInstance((receiver +: args).asInstanceOf[Seq[AnyRef]]: _*)
      def jinvoke(args: Seq[Any]): Any = {
        val result = jinvokeraw(args)
        if (!symbol.isConstructor && jmeth.getReturnType == java.lang.Void.TYPE)
          ()
        else if (!symbol.isConstructor && ret.isDerivedValueClass)
          ret.boxer.newInstance(result.asInstanceOf[AnyRef])
        else result
      }

      override def toString = {
        val what =
          if (symbol.isConstructor) "constructor mirror" else "method mirror"
    ...
    private class JavaVanillaMethodMirror(val receiver: Any,
      ...
      def this(receiver: Any, symbol: MethodSymbol) =
        this(receiver, symbol, new DerivedValueClassMetadata(symbol.returnType))
      def bind(newReceiver: Any) =
        new JavaVanillaMethodMirror(newReceiver, symbol, ret)
      def apply(args: Any*): Any = jinvoke(args)
    }

    private class JavaVanillaMethodMirror0(receiver: Any,
                                           symbol: MethodSymbol,
                                           ret: DerivedValueClassMetadata)
    ...
    private class JavaVanillaMethodMirror1(receiver: Any,
                                           symbol: MethodSymbol,
                                           ret: DerivedValueClassMetadata)
        extends JavaVanillaMethodMirror(receiver, symbol, ret) {
      def this(receiver: Any, symbol: MethodSymbol) =
      ...
      override def jinvokeraw(args: Seq[Any]) =
        if (!symbol.isConstructor)
          jmeth.invoke(receiver, args(0).asInstanceOf[AnyRef])
        else if (receiver == null)
    ...
    private class JavaTransformingMethodMirror(val receiver: Any,
                                               symbol: MethodSymbol,
                                               metadata: MethodMetadata)
        extends JavaMethodMirror(symbol, metadata.ret) {
      def this(receiver: Any, symbol: MethodSymbol) =
        this(receiver, symbol, new MethodMetadata(symbol))
      override def bind(newReceiver: Any) =
        new JavaTransformingMethodMirror(newReceiver, symbol, metadata)
      import metadata._
    ...
    private class BytecodelessMethodMirror[T: ClassTag](
        val receiver: T,
        val symbol: MethodSymbol)
        extends MethodMirror {
      def bind(newReceiver: Any) =
        new BytecodelessMethodMirror(newReceiver.asInstanceOf[T], symbol)
      override def toString =
        s"bytecodeless method mirror for ${showDecl(symbol)} (bound to $receiver)"
      ...
      def apply(args: Any*): Any = {
        ...
        if (!perfectMatch && !varargMatch) {
          val n_arguments =
            ...
            else s"${params.length}"
          val s_arguments =
            if (params.lengthIs == 1 && !isVarArgsList(params)) "argument"
            else "arguments"
    ...
    private abstract class JavaTemplateMirror extends TemplateMirror {
      def outer: AnyRef
    ...
    private class JavaClassMirror(val outer: AnyRef, val symbol: ClassSymbol)
      ...
      override def toString =
        s"class mirror for ${symbol.fullName} (bound to $outer)"
    }
...
// -------------------- Java to Scala  -----------------------------------
    ...
    /** how connected????
      ...
      */
    def unpickleClass(clazz: ClassSymbol,
                      module: ModuleSymbol,
                      jclazz: jClass[_]): Unit = {
      ...
      def loadAnnotation(
          name: String): Option[java.lang.annotation.Annotation] =
    ...
    private class TypeParamCompleter(
        jtvar: jTypeVariable[_ <: GenericDeclaration])
        extends LazyType
        with FlagAgnosticCompleter {
      override def load(sym: Symbol) = complete(sym)
      override def complete(sym: Symbol) = {
        sym setInfo TypeBounds.upper(
          glb(jtvar.getBounds.toList map typeToScala map objToAny))
        markAllCompleted(sym)
      }
    }

    private def assignAssociatedFile(clazz: Symbol,
                                     module: Symbol,
                                     jclazz: jClass[_]): Unit = {
    ...
    private def copyAnnotations(sym: Symbol, jann: AnnotatedElement): Unit = {
      ...
      jann match {
        case mem: jMember =>
          mem.javaFlags.toScalaAnnotations(thisUniverse) foreach (ann =>
    ...
    /**
      ...
      */
    private class FromJavaClassCompleter(clazz: Symbol,
                                         module: Symbol,
                                         jclazz: jClass[_])
        extends LazyType
        with JavaClassCompleter
        with FlagAgnosticCompleter {
      ...
      private[this] val relatedSymbols = clazz +: (if (module != NoSymbol)
                                                     List(module,
                                                          module.moduleClass)
                                                   else Nil)
    ...
    /**
      ...
      */
    private def followStatic(clazz: Symbol, mods: JavaAccFlags): Symbol =
      followStatic(clazz, clazz.companionModule, mods)

    private def followStatic(clazz: Symbol,
                             module: Symbol,
                             mods: JavaAccFlags): Symbol =
    ...
    implicit class RichClass(jclazz: jClass[_]) {
      ...
      def isLocalClass0: Boolean =
        jclazz.getEnclosingClass != null && !jclazz.isMemberClass
    }
      ...
      if (fullname == "") EmptyPackage
        ...
        gilSynchronized {
          ...
          val ownerModule: ModuleSymbol =
            if (split > 0) packageNameToScala(fullname take split)
            else this.RootPackage
        ...
        def lookupClass = {
          def coreLookup(name: Name): Symbol =
            owner.info.decl(name) orElse {
              if (name.startsWith(nme.NAME_JOIN_STRING)) coreLookup(name drop 1)
              else NoSymbol
            }
    ...
    /**
      ...
      */
    def typeParamToScala(
        jparam: jTypeVariable[_ <: GenericDeclaration]): TypeSymbol =
      toScala(tparamCache, jparam)(_ typeParamToScala1 _)

    private def typeParamToScala1(
        jparam: jTypeVariable[_ <: GenericDeclaration]): TypeSymbol = {
      val owner = genericDeclarationToScala(jparam.getGenericDeclaration)
      owner.info match {
        case PolyType(tparams, _) =>
          tparams.find(_.name string_== jparam.getName).get.asType
      }
    }
    ...
    private def setMethType(meth: Symbol,
                            tparams: List[Symbol],
                            params: List[Symbol],
                            restpe: Type) = {
    ...
    private def jmethodAsScala1(jmeth: jMethod): MethodSymbol = {
      ...
      if (jmeth.getDefaultValue != null)
        meth.addAnnotation(AnnotationDefaultAttr)
    ...
    /** Transform Java parameters `params` into a list of value parameters
      * for `meth`.
      */
    private def jparamsAsScala(meth: MethodSymbol,
                               params: List[jParameter]): List[Symbol] = {
      params.zipWithIndex.map {
        case (param, ix) =>
          ...
          meth.owner
            .newValueParameter(name, meth.pos)
...
// -------------------- Scala to Java  -----------------------------------
    ...
    private def expandedName(sym: Symbol): String =
      if (sym.isPrivate)
        nme.expandedName(sym.name.toTermName, sym.owner).toString
  ...
  override def missingHook(owner: Symbol, name: Name): Symbol = {
    if (owner.hasPackageFlag) {
      ...
      if (name.isTermName && !owner.isEmptyPackageClass)
        return mirror.makeScalaPackage(
          if (owner.isRootSymbol) name.toString
    ...
    info(
      "*** missing: " + name + "/" + name.isTermName + "/" + owner + "/" + owner.hasPackageFlag + "/" + owner.info.decls.getClass)
