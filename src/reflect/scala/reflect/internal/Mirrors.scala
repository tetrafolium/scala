...
trait Mirrors extends api.Mirrors {
  ...
  abstract class RootsBase(rootOwner: Symbol)
    ...
    protected def universeMissingHook(owner: Symbol, name: Name): Symbol =
      thisUniverse.missingHook(owner, name)
    ...
    def getClassByName(fullname: Name): ClassSymbol =
      ensureClassSymbol(fullname.toString,
    ...
    private def ensureModuleSymbol(fullname: String,
                                   sym: Symbol,
    ...
    def getModuleByName(fullname: Name): ModuleSymbol =
      ensureModuleSymbol(fullname.toString,
    ...
    private def ensurePackageSymbol(fullname: String,
                                    sym: Symbol,
    ...
    def getPackage(fullname: TermName): ModuleSymbol =
      ensurePackageSymbol(fullname.toString,
                          getModuleOrClass(fullname),
                          allowModules = true)
  ...
  abstract class Roots(rootOwner: Symbol) extends RootsBase(rootOwner) {
    thisMirror =>
    ...
    class EmptyPackage
        extends ModuleSymbol(RootClass, NoPosition, nme.EMPTY_PACKAGE_NAME)
        with WellKnownSymbol {
    ...
    class EmptyPackageClass
        extends PackageClassSymbol(RootClass,
                                   NoPosition,
