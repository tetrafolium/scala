...
trait Translations {
  ...
  def isTreeSymbolPickled(tree: Tree): Boolean =
    isTreeSymbolPickled(picklerSubTag(tree))
  ...
  def picklerTag(ref: AnyRef): Int = ref match {
    ...
    case (_: Symbol, _: AnnotationInfo) =>
      SYMANNOT // symbol annotations, i.e. on terms
    case (_: Symbol, _: List[_]) =>
      CHILDREN // the direct subclasses of a sealed symbol
    case _: Modifiers => MODIFIERS
    case _ =>
      throw new IllegalStateException(
        s"unpicklable entry ${shortClassOfInstance(ref)} $ref")
  }
