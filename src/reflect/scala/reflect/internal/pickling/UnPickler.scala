...
abstract class UnPickler {
  ...
  def unpickle(bytes: Array[Byte],
               ...
               moduleRoot: ModuleSymbol,
               filename: String): Unit = {
    try {
      assert(
        classRoot != NoSymbol && moduleRoot != NoSymbol,
        s"The Unpickler expects a class and module symbol: $classRoot - $moduleRoot")
  ...
  class Scan(_bytes: Array[Byte],
    ...
    protected def symScope(sym: Symbol) =
      symScopes.getOrElseUpdate(sym, newScope)
    ...
    protected def readSymbol(): Symbol = {
      ...
      def readExtSymbol(): Symbol = {
          ...
          case _ =>
            val decl = owner match {
              case stub: StubSymbol =>
                NoSymbol // scala/bug#8502 Don't call .info and fail the stub
        ...
        def moduleAdvice(missing: String): String = {
          val module =
            if (missing.startsWith("scala.xml"))
              Some(("org.scala-lang.modules", "scala-xml"))
            else if (missing.startsWith("scala.util.parsing"))
              Some(("org.scala-lang.modules", "scala-parser-combinators"))
            else if (missing.startsWith("scala.swing"))
              Some(("org.scala-lang.modules", "scala-swing"))
            else if (missing.startsWith("scala.collection.parallel"))
              Some(("org.scala-lang.modules", "scala-parallel-collections"))
            else None
      ...
      def isModuleRoot =
        (name == moduleRoot.name) && (owner == moduleRoot.owner)
      def isModuleClassRoot =
        (name == moduleRoot.name.toTypeName) && (owner == moduleRoot.owner)
      ...
      finishSym(tag match {
        ...
        case CLASSsym =>
          val sym = {
            if (isModuleFlag && isModuleClassRoot)
    ...
    protected def readType(forceProperType: Boolean = false): Type = {
      ...
      (tag: @switch) match {
        ...
        case SINGLEtpe =>
          SingleType(readTypeRef(), readSymbolRef().filter(_.isStable)) // scala/bug#7596 account for overloading
    ...
    protected def readConstant(): Constant = {
      ...
      (tag: @switch) match {
        ...
        case LITERALsymbol =>
          Constant(null) // TODO: needed until we have a STARR that does not emit it.
    ...
    protected def readConstantRef(): Constant =
    ...
    def inferMethodAlternative(
        fun: Tree,
        argtpes: List[Type],
        restpe: Type): Unit = {} // can't do it; need a compiler for that.
    ...
    private class LazyTypeRefAndAlias(i: Int, j: Int) extends LazyTypeRef(i) {
      override def completeInternal(sym: Symbol) =
        try {
