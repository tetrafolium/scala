...
trait Definitions extends api.StandardDefinitions {
  ...
  trait ValueClassDefinitions {
    ...
    private[Definitions] def classesMap[T](f: Name => T): Map[Symbol, T] =
      symbolsMap(ScalaValueClassesNoUnit, f)
    private def symbolsMap[T](syms: List[Symbol],
                              f: Name => T): Map[Symbol, T] =
      mapFrom(syms)(x => f(x.name))
    private def symbolsMapFilt[T](syms: List[Symbol],
                                  p: Name => Boolean,
                                  f: Name => T) =
    ...
    lazy val lazyHolders = symbolsMap(
      ScalaValueClasses,
      x => getClassIfDefined("scala.runtime.Lazy" + x))
    ...
    def isNumericValueClass(sym: Symbol) =
      ScalaNumericValueClassesSet contains sym
    ...
    lazy val ScalaNumericValueClassesSet: SymbolSet = new SymbolSet(
  ...
  abstract class DefinitionsClass
      extends DefinitionsApi
      with ValueClassDefinitions {
    ...
    lazy val UninitializedFieldConstructor =
      UninitializedErrorClass.primaryConstructor
    ...
    lazy val UnqualifiedOwners = UnqualifiedModules.toSet ++ UnqualifiedModules
      .map(_.moduleClass)
    ...
    def getWrapVarargsArrayModule =
      if (isNewCollections) ScalaRunTimeModule else PredefModule
    def wrapVarargsArrayMethod(tp: Type) =
      getMemberMethod(getWrapVarargsArrayModule, wrapVarargsArrayMethodName(tp))
    ...
    lazy val ByNameParamClass =
      specialPolyClass(tpnme.BYNAME_PARAM_CLASS_NAME, COVARIANT)(_ => AnyTpe)
    lazy val JavaRepeatedParamClass =
      specialPolyClass(tpnme.JAVA_REPEATED_PARAM_CLASS_NAME, COVARIANT)(
    ...
    def isJavaVarArgsMethod(m: Symbol) =
      m.isMethod && isJavaVarArgs(m.info.params)
    def isJavaVarArgs(params: scala.collection.Seq[Symbol]) =
      !params.isEmpty && isJavaRepeatedParamType(params.last.tpe)
    def isScalaVarArgs(params: scala.collection.Seq[Symbol]) =
      !params.isEmpty && isScalaRepeatedParamType(params.last.tpe)
    def isVarArgsList(params: scala.collection.Seq[Symbol]) =
      !params.isEmpty && isRepeatedParamType(params.last.tpe)
    def isVarArgTypes(formals: scala.collection.Seq[Type]) =
      !formals.isEmpty && isRepeatedParamType(formals.last)
    ...
    def repeatedToSeq(tp: Type): Type =
      elementTransform(RepeatedParamClass, tp)(seqType) orElse tp
    def seqToRepeated(tp: Type): Type =
      elementTransform(SeqClass, tp)(scalaRepeatedType) orElse tp
    def isReferenceArray(tp: Type) =
      elementTest(ArrayClass, tp)(_ <:< AnyRefTpe)
    def isArrayOfSymbol(tp: Type, elem: Symbol) =
      elementTest(ArrayClass, tp)(_.typeSymbol == elem)
    def elementType(container: Symbol, tp: Type): Type =
      elementExtract(container, tp)
    ...
    // collections classes
    private[this] lazy val _isNewCollections = getClassIfDefined(
      "scala.collection.IterableOnce") != NoSymbol
    ...
    @migration("SeqClass now refers to scala.collection.immutable.Seq",
               "2.13.0")
    lazy val SeqClass =
      if (isNewCollections) requiredClass[scala.collection.immutable.Seq[_]]
      else requiredClass[scala.collection.Seq[_]]
    ...
    lazy val TraversableClass =
      if (isNewCollections) IterableClass
      else requiredClass[scala.collection.Traversable[_]]
    ...
    @migration("SeqModule now refers to scala.collection.immutable.Seq",
               "2.13.0")
    lazy val SeqModule =
      if (isNewCollections) requiredModule[scala.collection.immutable.Seq.type]
      else requiredModule[scala.collection.Seq.type]
    ...
    lazy val StructuralCallSite = getClassIfDefined(
    ...
    def StructuralCallSite_add =
      getMemberIfDefined(StructuralCallSite, nme.add_)
    def StructuralCallSite_getParameterTypes =
      getMemberIfDefined(StructuralCallSite, nme.parameterTypes)
    ...
    lazy val BlackboxContextClass = getClassIfDefined(
      "scala.reflect.macros.blackbox.Context") // defined in scala-reflect.jar, so we need to be careful

    lazy val WhiteboxContextClass = getClassIfDefined(
      "scala.reflect.macros.whitebox.Context") // defined in scala-reflect.jar, so we need to be careful
    def MacroContextPrefix =
      BlackboxContextClass.map(sym => getMemberMethod(sym, nme.prefix))
    def MacroContextPrefixType =
      BlackboxContextClass.map(sym => getTypeMember(sym, tpnme.PrefixType))
    def MacroContextUniverse =
      BlackboxContextClass.map(sym => getMemberMethod(sym, nme.universe))
    def MacroContextExprClass =
      BlackboxContextClass.map(sym => getTypeMember(sym, tpnme.Expr))
    def MacroContextWeakTypeTagClass =
    ...
    lazy val MethodHandleClass = getClassIfDefined(
    ...
    // Serialization
    lazy val ModuleSerializationProxyClass: ClassSymbol =
      requiredClass[scala.runtime.ModuleSerializationProxy]

    def compilerTypeFromTag(tt: ApiUniverse#WeakTypeTag[_]): Type =
      tt.in(rootMirror).tpe
    def compilerSymbolFromTag(tt: ApiUniverse#WeakTypeTag[_]): Symbol =
      tt.in(rootMirror).tpe.typeSymbol

    // The given symbol is a method with the right name and signature to be a runnable java program.
    def isJavaMainMethod(sym: Symbol) =
      (sym.name == nme.main) && (sym.info match {
        case MethodType(p :: Nil, restpe) =>
          isArrayOfSymbol(p.tpe, StringClass) && restpe.typeSymbol == UnitClass
        case _ => false
      })
    ...
    class VarArityClass(name: String,
                        maxArity: Int,
                        countFrom: Int = 0,
    ...
    val MaxTupleAritySpecialized, MaxProductAritySpecialized,
    MaxFunctionAritySpecialized = 2
    ...
    lazy val AbstractFunctionClass =
      new VarArityClass("runtime.AbstractFunction", MaxFunctionArity)

    /** Creators for TupleN, ProductN, FunctionN. */
    def tupleType(elems: List[Type]) = TupleClass.specificType(elems)
    def functionType(formals: List[Type], restpe: Type) =
      FunctionClass.specificType(formals, restpe :: Nil)
    def abstractFunctionType(formals: List[Type], restpe: Type) =
      AbstractFunctionClass.specificType(formals, restpe :: Nil)
    ...
    def isTupleSymbol(sym: Symbol) =
      TupleClass contains unspecializedSymbol(sym)
    def isFunctionSymbol(sym: Symbol) =
      FunctionClass contains unspecializedSymbol(sym)
    def isAbstractFunctionSymbol(sym: Symbol) =
      AbstractFunctionClass contains unspecializedSymbol(sym)
    def isProductNSymbol(sym: Symbol) =
      ProductClass contains unspecializedSymbol(sym)
    ...
    def isIterableType(tp: Type) =
      tp.typeSymbol.isNonBottomSubClass(IterableClass)
    ...
    def isFunctionTypeDirect(tp: Type) =
      !tp.isHigherKinded && isFunctionSymbol(tp.typeSymbolDirect)
    def isTupleTypeDirect(tp: Type) =
      !tp.isHigherKinded && isTupleSymbol(tp.typeSymbolDirect)
    ...
    def isFunctionProto(pt: Type): Boolean =
      (isFunctionType(pt)
        || (pt match {
          case pt: ProtoType => pt.expectsFunctionType
          case _             => false
        }) // TODO: this does not work for Function0
        || samOf(pt).exists)
    ...
    def functionOrPfOrSamArgTypes(tp: Type): List[Type] = {
      val dealiased = tp.dealiasWiden
      if (isFunctionTypeDirect(dealiased) || isPartialFunctionType(dealiased))
        dealiased.typeArgs.init
      else
        samOf(tp) match {
    ...
    def methodToExpressionTp(tp: Type): Type = tp match {
      ...
      case mt @ MethodType(_, restpe) if mt.isImplicit =>
        methodToExpressionTp(restpe)
      ...
      case ExistentialType(tparams, qtpe) =>
        newExistentialType(tparams, methodToExpressionTp(qtpe))
    ...
    def samMatchesFunctionBasedOnArity(sam: Symbol,
                                       formals: List[Any]): Boolean =
    ...
    def Product_productArity =
      getMemberMethod(ProductRootClass, nme.productArity)
    def Product_productElement =
      getMemberMethod(ProductRootClass, nme.productElement)
    def Product_productElementName =
      getMemberIfDefined(ProductRootClass, nme.productElementName)
    def Product_iterator =
      getMemberMethod(ProductRootClass, nme.productIterator)
    def Product_productPrefix =
      getMemberMethod(ProductRootClass, nme.productPrefix)
    def Product_canEqual = getMemberMethod(ProductRootClass, nme.canEqual_)

    def productProj(z: Symbol, j: Int): TermSymbol =
    ...
    def isStable(tp: Type): Boolean = tp match {
      ...
      case TypeRef(_, sym, _) if sym.isAbstractType =>
        tp.upperBound.typeSymbol isSubClass SingletonClass
      case TypeRef(pre, sym, _) if sym.isModuleClass => isStable(pre)
      case TypeRef(_, _, _) =>
        val normalize = tp.normalize; (normalize ne tp) && isStable(normalize)
    ...
    def isVolatile(tp: Type): Boolean = {
      ...
      def isVolatileRefinedType: Boolean = {
        ...
        def isVisibleDeferred(m: Symbol) =
          m.isDeferred && ((tp nonPrivateMember m.name).alternatives contains m)
        def contributesAbstractMembers(p: Type) =
          p.deferredMembers exists isVisibleDeferred
      ...
      if (doSam && isNonRefinementClassType(unwrapToClass(tp))) { // TODO: is this really faster than computing tpSym below? how about just `tp.typeSymbol.isClass` (and !tpSym.isRefinementClass)?
        ...
        def compute: Symbol = {
          if (tpSym.exists && tpSym.isClass && !(tpSym hasFlag (FINAL | SEALED))
            ...
            val deferredMembers =
              tpSym.info
                .membersBasedOnFlags(excludedFlags = BridgeAndPrivateFlags,
                                     requiredFlags = METHOD)
                .toList
    ...
    def samOfProto(pt: Type): Symbol =
      pt match {
        case proto: ProtoType =>
          samOf(proto.underlying) // TODO: add more semantic accessor to ProtoType?
        case pt => samOf(pt)
      }
    ...
    def javaRepeatedType(arg: Type) =
      appliedType(JavaRepeatedParamClass, arg :: Nil)
    def optionType(tp: Type) = appliedType(OptionClass, tp :: Nil)
    def scalaRepeatedType(arg: Type) =
      appliedType(RepeatedParamClass, arg :: Nil)
    ...
    def elementTypeFromGet(tp: Type) =
      typeArgOfBaseTypeOr(tp, OptionClass)(
        resultOfMatchingMethod(tp, nme.get)())
    def elementTypeFromApply(tp: Type) =
      typeArgOfBaseTypeOr(tp, SeqClass)(
        resultOfMatchingMethod(tp, nme.apply)(IntTpe))
    ...
    /** If `tp` has a term member `name`, the first parameter list of which
      ...
      */
    private def resultOfMatchingMethod(tp: Type, name: TermName)(
    ...
    lazy val Any_getClass = enterNewMethod(
      AnyClass,
      nme.getClass_,
      Nil,
      getMemberMethod(ObjectClass, nme.getClass_).tpe.resultType,
      DEFERRED)
    lazy val Any_isInstanceOf =
      newT1NullaryMethod(AnyClass, nme.isInstanceOf_, FINAL)(_ => BooleanTpe)
    lazy val Any_asInstanceOf =
      newT1NullaryMethod(AnyClass, nme.asInstanceOf_, FINAL)(_.typeConstructor)

    lazy val primitiveGetClassMethods = Set[Symbol](Any_getClass,
                                                    AnyVal_getClass) ++ (
    ...
    lazy val ConstantAnnotationClass = getClassIfDefined(
    ...
    lazy val AnnotationRepeatableAttr =
      requiredClass[java.lang.annotation.Repeatable]
    ...
    lazy val SerialVersionUIDAnnotation = AnnotationInfo(
      SerialVersionUIDAttr.tpe,
      List(),
      List(nme.value -> LiteralAnnotArg(Constant(0))))
    ...
    lazy val UncheckedBoundsClass = getClassIfDefined(
    ...
    lazy val FunctionalInterfaceClass =
      requiredClass[java.lang.FunctionalInterface]
    ...
    def getLanguageFeature(name: String,
                           owner: Symbol = languageFeatureModule): Symbol =
    ...
    def getMemberMethod(owner: Symbol, name: Name): TermSymbol = {
      ...
      getMember(owner, name.toTermName) match {
        case x: TermSymbol =>
          x.filter(_.isMethod).orElse(miss).asInstanceOf[TermSymbol]
        case _ => miss
      }
    }
    ...
    private def newAlias(owner: Symbol,
                         name: TypeName,
                         alias: Type): AliasTypeSymbol =
    ...
    def newT1NoParamsMethod(owner: Symbol, name: TermName, flags: Long)(
        createFn: Symbol => Type): MethodSymbol = {
      newPolyMethod(1, owner, name, flags)(tparams =>
        (util.SomeOfNil, createFn(tparams.head)))
    }
    ...
    lazy val isPossibleSyntheticParent = ProductClass.seq
      .toSet[Symbol] + ProductRootClass + SerializableClass

    private lazy val boxedValueClassesSet = boxedClass.values
    ...
    lazy val ShowAsInfixAnnotationClass =
      rootMirror.getClassIfDefined("scala.annotation.showAsInfix")
    ...
    final class RunDefinitions {
      ...
      lazy val StringContext_apply =
        getMemberMethod(StringContextModule, nme.apply)

      lazy val ArrowAssocClass =
        getMemberClass(PredefModule, TypeName("ArrowAssoc")) // scala/bug#5731
      ...
      lazy val Predef_any2stringaddMethod =
        getMemberMethod(PredefModule, nme.any2stringadd).suchThat(_.isMethod)
      ...
      lazy val wrapVarargsRefArrayMethod =
        getMemberMethod(getWrapVarargsArrayModule, nme.wrapRefArray)

      lazy val RuntimeStatics_ioobe =
        getMemberMethod(RuntimeStaticsModule, nme.ioobe)
      ...
      lazy val ImplicitConversionsFeature = getLanguageFeature(
        "implicitConversions")
      ...
      private lazy val PolymorphicSignatureClass =
        MethodHandleClass.companionModule.info
          .decl(TypeName("PolymorphicSignature"))
      private val PolymorphicSignatureName = TypeName(
        "java.lang.invoke.MethodHandle$PolymorphicSignature")

      def isPolymorphicSignature(sym: Symbol) =
        sym != null && sym.isJavaDefined && {
          ...
          (owner == MethodHandleClass || owner == VarHandleClass) && {
            if (PolymorphicSignatureClass eq NoSymbol) {
              ...
              sym.annotations.exists(
                _.atp.typeSymbolDirect.name == PolymorphicSignatureName)
            } else sym.hasAnnotation(PolymorphicSignatureClass)
          }
        }

      lazy val Scala_Java8_CompatPackage =
        rootMirror.getPackageIfDefined(TermName("scala.runtime.java8"))
    }
  }
}
