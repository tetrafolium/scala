...
trait TypeComparers {
  ...
  private def isUnifiable(pre1: Type, pre2: Type) = (
    (isEligibleForPrefixUnification(pre1) || isEligibleForPrefixUnification(
      pre2))
      && (pre1 =:= pre2)
  )
  ...
  /** Returns true iff we are past phase specialize,
    ...
    */
  private def isSameSpecializedSkolem(sym1: Symbol,
                                      sym2: Symbol,
                                      pre1: Type,
                                      pre2: Type) = {
  ...
  private def equalSymsAndPrefixes(sym1: Symbol,
                                   pre1: Type,
                                   sym2: Symbol,
                                   pre2: Type): Boolean = (
    if (sym1 eq sym2)
  ...
  def isDifferentTypeConstructor(tp1: Type, tp2: Type) =
    !isSameTypeConstructor(tp1, tp2)
  ...
  private def isSameType1(tp1: Type, tp2: Type): Boolean =
    typeRelationPreCheck(tp1, tp2) match {
      ...
      case _
          if tp1.isInstanceOf[AnnotatedType] || tp2
            .isInstanceOf[AnnotatedType] =>
  ...
  private def methodHigherOrderTypeParamsSameVariance(sym1: Symbol,
                                                      sym2: Symbol) = {
  ...
  private def methodHigherOrderTypeParamsSubVariance(low: Symbol,
                                                     high: Symbol) =
  ...
  def isSameType2(tp1: Type, tp2: Type): Boolean = {
    def retry() = {
      ...
      val skip = isNoArgStaticClassTypeRef(tp1) && isNoArgStaticClassTypeRef(
        tp2)
      !skip && {
    ...
    def sameTypeAndSameCaseClass = tp1 match {
      ...
      case FoldableConstantType(c1) =>
        tp2 match { case FoldableConstantType(c2) => c1 == c2; case _ => false }
      case LiteralType(c1) =>
        tp2 match { case LiteralType(c2) => c1 == c2; case _ => false }
  ...
  def isHKSubType(tp1: Type, tp2: Type, depth: Depth): Boolean = {
    ...
    def isSubHKTypeVar(tp1: Type, tp2: Type) = (tp1, tp2) match {
      case (tv1: TypeVar, tv2: TypeVar) =>
        devWarning(
          sm"Unexpected code path: testing two type variables for subtype relation: $tv1 <:< $tv2")
        tv1 eq tv2
      case (_, tv2: TypeVar) =>
        val ntp1 = tp1.normalize
        val kindsMatch = (ntp1.typeSymbol eq AnyClass) || hkSubVariance(
          tv2.params,
          ntp1.typeParams)
      ...
      case (tv1: TypeVar, _) =>
        ...
        val kindsMatch = (ntp2.typeSymbol eq NothingClass) || hkSubVariance(
          ntp2.typeParams,
          tv1.params)
    ...
    def isSub2(ntp1: Type, ntp2: Type) = (ntp1, ntp2) match {
      ...
      case (WildcardType, _) | (_, WildcardType) =>
        true // treat `?` as kind-polymorphic
      case (TypeRef(_, AnyClass, _), _) | (_, TypeRef(_, NothingClass, _)) =>
    ...
    || (if (isNoArgStaticClassTypeRef(tp1) && isNoArgStaticClassTypeRef(tp2))
          ...
          isSub(tp1.withoutAnnotations, tp2.withoutAnnotations) && annotationsConform(
            tp1,
            tp2)))
  }
  ...
  private def isSubType2(tp1: Type, tp2: Type, depth: Depth): Boolean = {
    ...
    def fourthTry = {
      def retry(lhs: Type, rhs: Type) =
        ((tp1 ne lhs) || (tp2 ne rhs)) && isSubType(lhs, rhs, depth)
      def abstractTypeOnLeft(hi: Type) =
        isDifferentTypeConstructor(tp1, hi) && retry(hi, tp2)
      ...
      tp1 match {
        case tr1 @ TypeRef(pre1, sym1, _) =>
          def nullOnLeft = tp2 match {
            ...
            case _ =>
              isSingleType(tp2) && tp2.widen <:< AnyRefTpe && retry(tp1,
                                                                    tp2.widen)
          }
          ...
          sym1 match {
            ...
            case _: ClassSymbol if sym1.isModuleClass =>
              retry(normalizePlus(tp1), normalizePlus(tp2))
            case _: ClassSymbol if sym1.isRefinementClass =>
              retry(sym1.info, tp2)
            case _: TypeSymbol if sym1.isDeferred =>
              abstractTypeOnLeft(tp1.upperBound)
            case _: TypeSymbol => retry(normalizePlus(tp1), normalizePlus(tp2))
            case _             => false
          }
