...
trait Trees extends api.Trees {
  ...
  abstract class Tree extends TreeContextApiImpl with Attachable with Product {
    ...
    def symbol: Symbol =
      null //!!!OPT!!! symbol is about 3% of hot compile times -- megamorphic dispatch?
  ...
  abstract class TreeContextApiImpl extends TreeApi { this: Tree =>
    ...
    override def foreach(f: Tree => Unit): Unit = {
    ...
    override def equalsStructure(that: Tree) =
    ...
    def freeTerms: List[FreeTermSymbol] =
      freeSyms[FreeTermSymbol](_.isFreeTerm, _.termSymbol)
  ...
  object RefTree extends RefTreeExtractor {
    ...
    def unapply(refTree: RefTree): Option[(Tree, Name)] =
      Some((refTree.qualifier, refTree.name))
  }
  ...
  case class PackageDef(pid: RefTree, stats: List[Tree])
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.PackageDef(
        this,
        transformer.transform(pid).asInstanceOf[RefTree],
  ...
  case class ClassDef(mods: Modifiers,
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(this.symbol) {
        transformer.treeCopy.ClassDef(this,
                                      transformer.transformModifiers(mods),
                                      name,
                                      transformer.transformTypeDefs(tparams),
                                      transformer.transformTemplate(impl))
      }
    override def traverse(traverser: Traverser): Unit =
  ...
  case class ModuleDef(mods: Modifiers, name: TermName, impl: Template)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(mclass(this.symbol)) {
        transformer.treeCopy.ModuleDef(this,
                                       transformer.transformModifiers(mods),
                                       name,
                                       transformer.transformTemplate(impl))
      }
    override def traverse(traverser: Traverser): Unit =
      traverser.atOwner(mclass(symbol)) {
        traverser.traverseModifiers(mods)
        traverser.traverseName(name)
        traverser.traverse(impl)
      }
  ...
  case class ValDef(mods: Modifiers, name: TermName, tpt: Tree, rhs: Tree)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(this.symbol) {
        transformer.treeCopy.ValDef(this,
                                    transformer.transformModifiers(mods),
                                    name,
  ...
  case class DefDef(mods: Modifiers,
                    name: TermName,
                    tparams: List[TypeDef],
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(this.symbol) {
        transformer.treeCopy.DefDef(
          this,
          transformer.transformModifiers(mods),
          name,
          transformer.transformTypeDefs(tparams),
          transformer.transformValDefss(vparamss),
          transformer.transform(tpt),
          transformer.transform(rhs)
        )
      }
  ...
  object DefDef extends DefDefExtractor {
    ...
    def apply(sym: Symbol, vparamss: List[List[ValDef]], rhs: Tree): DefDef =
      newDefDef(sym, rhs)(vparamss = vparamss)
    def apply(sym: Symbol, mods: Modifiers, rhs: Tree): DefDef =
      newDefDef(sym, rhs)(mods = mods)
    def apply(sym: Symbol,
              mods: Modifiers,
              vparamss: List[List[ValDef]],
              rhs: Tree): DefDef =
      newDefDef(sym, rhs)(mods = mods, vparamss = vparamss)
    def apply(sym: Symbol, rhs: List[List[Symbol]] => Tree): DefDef =
  ...
  case class TypeDef(mods: Modifiers,
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(this.symbol) {
        transformer.treeCopy.TypeDef(this,
                                     transformer.transformModifiers(mods),
                                     name,
                                     transformer.transformTypeDefs(tparams),
                                     transformer.transform(rhs))
  ...
  case class LabelDef(name: TermName, params: List[Ident], rhs: Tree)
      ...
      with LabelDefApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.LabelDef(
        this,
        name,
        transformer.transformIdents(params),
        transformer
          .transform(rhs)) //bq: Martin, once, atOwner(...) works, also change `LambdaLifter.proxy`
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverseName(name)
  ...
  case class ImportSelector(name: Name,
    ...
    private def isLiteralWildcard =
      name == nme.WILDCARD && rename == nme.WILDCARD
  ...
  case class Template(parents: List[Tree], self: ValDef, body: List[Tree])
      ...
      with TemplateApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Template(
        this,
        transformer.transformTrees(parents),
        transformer.transformValDef(self),
        transformer.transformStats(body, this.symbol))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class CaseDef(pat: Tree, guard: Tree, body: Tree)
      ...
      with CaseDefApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.CaseDef(this,
                                   transformer.transform(pat),
                                   transformer.transform(guard),
  ...
  case class UnApply(fun: Tree, args: List[Tree])
      ...
      with UnApplyApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.UnApply(
        this,
        transformer.transform(fun),
        transformer
          .transformTrees(args)) // bq: see test/.../unapplyContexts2.scala
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class ArrayValue(elemtpt: Tree, elems: List[Tree]) extends TermTree {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.ArrayValue(this,
                                      transformer.transform(elemtpt),
                                      transformer.transformTrees(elems))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class Function(vparams: List[ValDef], body: Tree)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.atOwner(this.symbol) {
        transformer.treeCopy.Function(this,
                                      transformer.transformValDefs(vparams),
                                      transformer.transform(body))
      }
    override def traverse(traverser: Traverser): Unit =
      traverser.atOwner(this.symbol) {
  ...
  case class Assign(lhs: Tree, rhs: Tree) extends TermTree with AssignApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Assign(this,
                                  transformer.transform(lhs),
                                  transformer.transform(rhs))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class NamedArg(lhs: Tree, rhs: Tree) extends TermTree with NamedArgApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.NamedArg(this,
                                    transformer.transform(lhs),
                                    transformer.transform(rhs))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(lhs)
  ...
  case class If(cond: Tree, thenp: Tree, elsep: Tree)
      ...
      with IfApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.If(this,
                              transformer.transform(cond),
                              transformer.transform(thenp),
                              transformer.transform(elsep))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class Match(selector: Tree, cases: List[CaseDef])
      ...
      with MatchApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Match(this,
                                 transformer.transform(selector),
                                 transformer.transformCaseDefs(cases))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class Try(block: Tree, catches: List[CaseDef], finalizer: Tree)
      ...
      with TryApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Try(this,
                               transformer.transform(block),
                               transformer.transformCaseDefs(catches),
                               transformer.transform(finalizer))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class Typed(expr: Tree, tpt: Tree) extends TermTree with TypedApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Typed(this,
                                 transformer.transform(expr),
                                 transformer.transform(tpt))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(expr)
  ...
  case class TypeApply(fun: Tree, args: List[Tree])
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.TypeApply(this,
                                     transformer.transform(fun),
                                     transformer.transformTrees(args))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(fun)
  ...
  case class Apply(fun: Tree, args: List[Tree])
    ...
    override def symbol_=(sym: Symbol): Unit = { fun.symbol = sym }
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Apply(this,
                                 transformer.transform(fun),
                                 transformer.transformTrees(args))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(fun)
  ...
  def ApplyConstructor(tpt: Tree, args: List[Tree]) =
  ...
  case class ApplyDynamic(qual: Tree, args: List[Tree])
      ...
      with TermTree {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.ApplyDynamic(this,
                                        transformer.transform(qual),
                                        transformer.transformTrees(args))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class ReferenceToBoxed(ident: Ident)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy
        .ReferenceToBoxed(this, transformer.transform(ident) match {
...
//  def Literal(x: Any) = new Literal(Constant(x))
  ...
  case class Annotated(annot: Tree, arg: Tree) extends Tree with AnnotatedApi {
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.Annotated(this,
                                     transformer.transform(annot),
  ...
  case class SelectFromTypeTree(qualifier: Tree, name: TypeName)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.SelectFromTypeTree(this,
  ...
  case class CompoundTypeTree(templ: Template)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy
        .CompoundTypeTree(this, transformer.transformTemplate(templ))
    override def traverse(traverser: Traverser): Unit = {
  ...
  case class AppliedTypeTree(tpt: Tree, args: List[Tree])
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.AppliedTypeTree(this,
                                           transformer.transform(tpt),
                                           transformer.transformTrees(args))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(tpt)
  ...
  case class TypeBoundsTree(lo: Tree, hi: Tree)
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.TypeBoundsTree(this,
                                          transformer.transform(lo),
  ...
  case class ExistentialTypeTree(tpt: Tree, whereClauses: List[MemberDef])
    ...
    override def transform(transformer: Transformer): Tree =
      transformer.treeCopy.ExistentialTypeTree(
        this,
        transformer.transform(tpt),
        transformer.transformMemberDefs(whereClauses))
    override def traverse(traverser: Traverser): Unit = {
      traverser.traverse(tpt)
  ...
  case class TypeTree() extends TypTree with TypeTreeApi {
    ...
    override def symbol =
      typeTreeSymbol(this) // if (tpe == null) null else tpe.typeSymbol
  ...
  class LazyTreeCopier extends InternalTreeCopierOps {
    ...
    def PackageDef(tree: Tree, pid: RefTree, stats: List[Tree]) = tree match {
      case t @ PackageDef(pid0, stats0) if (pid0 == pid) && (stats0 == stats) =>
        t
    ...
    def LabelDef(tree: Tree, name: Name, params: List[Ident], rhs: Tree) =
      tree match {
        case t @ LabelDef(name0, params0, rhs0)
            if (name0 == name) && (params0 == params) && (rhs0 == rhs) =>
          t
    ...
    def Import(tree: Tree, expr: Tree, selectors: List[ImportSelector]) =
      tree match {
        case t @ Import(expr0, selectors0)
            if (expr0 == expr) && (selectors0 == selectors) =>
          t
    ...
    def ArrayValue(tree: Tree, elemtpt: Tree, trees: List[Tree]) = tree match {
      case t @ ArrayValue(elemtpt0, trees0)
          if (elemtpt0 == elemtpt) && (trees0 == trees) =>
        t
    ...
    def Function(tree: Tree, vparams: List[ValDef], body: Tree) = tree match {
      case t @ Function(vparams0, body0)
          if (vparams0 == vparams) && (body0 == body) =>
        t
    ...
    def NamedArg(tree: Tree, lhs: Tree, rhs: Tree) = tree match {
      case t @ NamedArg(lhs0, rhs0) if (lhs0 == lhs) && (rhs0 == rhs) => t
    ...
    def Match(tree: Tree, selector: Tree, cases: List[CaseDef]) = tree match {
      case t @ Match(selector0, cases0)
          if (selector0 == selector) && (cases0 == cases) =>
        t
    ...
    def ReferenceToBoxed(tree: Tree, idt: Ident) = tree match {
      case t @ ReferenceToBoxed(idt0) if (idt0 == idt) => t
    ...
    def Annotated(tree: Tree, annot: Tree, arg: Tree) = tree match {
      case t @ Annotated(annot0, arg0) if (annot0 == annot && arg0 == arg) => t
  ...
  def New(tpt: Tree, argss: List[List[Tree]]): Tree = argss match {
    ...
    case xs :: rest =>
      rest.foldLeft(ApplyConstructor(tpt, xs): Tree)(Apply.apply)
  }
  ...
  def SuperSelect(clazz: Symbol, sym: Symbol): Tree = {
    Select(Super(clazz, tpnme.EMPTY), sym)
      .updateAttachment(new QualTypeSymAttachment(sym.owner))
  }
  ...
  private def substituterString(fromStr: String,
                                toStr: String,
                                from: List[Any],
                                to: List[Any]): String = {
    val toAndFro =
      from.lazyZip(to).map((f, t) => s"$f -> $t").mkString("(", ", ", ")")
  ...
  /** Substitute clazz.this with `to`. `to` must be an attributed tree.
    */
  class ThisSubstituter(clazz: Symbol, to: => Tree)
      extends InternalTransformer {
    val newtpe = to.tpe
    override def transform(tree: Tree) = {
  ...
  class TreeTypeSubstituter(val from: List[Symbol], val to: List[Type])
      extends TypeMapTreeSubstituter(new SubstTypeMap(from, to)) {
  ...
  class TreeSymSubstTraverser(val from: List[Symbol], val to: List[Symbol])
    ...
    override def toString() =
      "TreeSymSubstTraverser/" + substituterString("Symbol", "Symbol", from, to)
  }
  ...
  /** Substitute symbols in `from` with symbols in `to`. Returns a new
    ...
    */
  class TreeSymSubstituter(from: List[Symbol], to: List[Symbol])
      extends InternalTransformer {
    ...
    override def toString() =
      "TreeSymSubstituter/" + substituterString("Symbol", "Symbol", from, to)
  }
  ...
  class CollectTreeTraverser[T](pf: PartialFunction[Tree, T])
      extends InternalTraverser {
  ...
  private class Duplicator(focusPositions: Boolean)
      extends InternalTransformer {
    override val treeCopy = newStrictTreeCopier
  ...
  def duplicateAndKeepPositions(tree: Tree) =
    new Duplicator(focusPositions = false) transform tree
  ...
  def deriveTemplate(templ: Tree)(
      applyToBody: List[Tree] => List[Tree]): Template = templ match {
  ...
  def deriveClassDef(cdef: Tree)(applyToImpl: Template => Template): ClassDef =
    cdef match {
      ...
      case t =>
        throw new IllegalStateException(
          "Not a ClassDef: " + t + "/" + t.getClass)
    }
  def deriveModuleDef(mdef: Tree)(
      applyToImpl: Template => Template): ModuleDef = mdef match {
    case ModuleDef(mods0, name0, impl0) =>
...
trait TreesStats {
  ...
  val retainedByType =
    newByClass("#retained tree nodes by type")(newCounter(""))
}
