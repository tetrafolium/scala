...
class Flags extends ModifierFlags {
  ...
  assert(
    PhaseIndependentFlags == (-1L & ~LateFlags & ~AntiFlags & ~(LateFlags >>> LateShift) & ~(AntiFlags >>> AntiShift)))
...
//  final val notOVERRIDE   = (0L + OVERRIDE) << AntiShift // unused
  ...
  final val SetterFlags =
    ~(PRESUPER | MUTABLE | STABLE | CASEACCESSOR | IMPLICIT)
