...
trait Printers extends api.Printers { self: SymbolTable =>
  ...
  def quotedName(name: String): String =
    quotedName(newTermName(name), decode = false)

  private def symNameInternal(tree: Tree,
                              name: Name,
                              decoded: Boolean): String = {
    val sym = tree.symbol
  ...
  def decodedSymName(tree: Tree, name: Name) =
    symNameInternal(tree, name, decoded = true)
  def symName(tree: Tree, name: Name) =
    symNameInternal(tree, name, decoded = false)
  ...
  class TreePrinter(out: PrintWriter) extends super.TreePrinter {
    ...
    protected def parenthesize(condition: Boolean = true,
                               open: String = "(",
    ...
    protected def printDefDef(tree: DefDef, resultName: => String)(
        printTypeSignature: => Unit)(printRhs: => Unit) = {
    ...
    protected def printImport(tree: Import, resSelect: => String) = {
      ...
      def selectorToString(s: ImportSelector): String = {
        def selectorName(n: Name): String =
          if (s.isWildcard) nme.WILDCARD.decoded else quotedName(n)
    ...
    protected def printSuper(tree: Super,
                             resultName: => String,
    ...
    def printTree(tree: Tree) = {
      tree match {
        ...
        case dd @ DefDef(mods, name, tparams, vparamss, tp, rhs) =>
          printDefDef(dd, symName(tree, name))(printOpt(": ", tp))(
            printOpt(" = ", rhs))
  ...
  class CodePrinter(out: PrintWriter, printRootPkg: Boolean)
      extends TreePrinter(out) {
    ...
    protected def removeDefaultTypesFromList(trees: List[Tree])(
        ...
        traitsToRemove: List[Name]) = {
      def removeDefaultTraitsFromList(trees: List[Tree],
                                      traitsToRemove: List[Name]): List[Tree] =
        trees match {
          case Nil => trees
          case init :+ last =>
            last match {
              case Select(Select(Ident(nme.scala_), nme.PACKAGE), name)
                  if traitsToRemove.contains(name) =>
                removeDefaultTraitsFromList(init, traitsToRemove)
              case Select(Ident(nme.scala_), name)
                  if traitsToRemove.contains(name) =>
                removeDefaultTraitsFromList(init, traitsToRemove)
              case _ => trees
            }
    ...
    protected def removeDefaultClassesFromList(trees: List[Tree],
                                               classesToRemove: List[Name] =
                                                 defaultClasses) =
    ...
    override def printColumn(ts: List[Tree],
                             start: String,
                             sep: String,
                             end: String) = {
      ...
      tree match {
        ...
        case Literal(k @ Constant(s: String)) if s.contains(Chars.LF) =>
          ...
          if (lines.lengthCompare(1) <= 0) print(k.escapedStringValue)
            ...
            print(tq)
            printSeq(lines.map(x => tqp.replaceAllIn(x, tqq)))(print(_))(
              print(Chars.LF))
            print(tq)
          }
  ...
  def newCodePrinter(writer: PrintWriter,
                     tree: Tree,
  ...
  private class Footnotes {
    ...
    private def classIndex[T: ClassTag] =
      index.getOrElseUpdate(classTag[T].runtimeClass, WeakHashMap[Any, Int]())
    ...
    private def classFootnotes[T: ClassTag] =
      footnotes.getOrElseUpdate(classTag[T].runtimeClass, SortedSet[Int]())
  ...
  class RawTreePrinter(out: PrintWriter) extends super.TreePrinter {
      ...
      args foreach {
        ...
        case sym: Symbol =>
          ...
          if (printMirrors)
            print(
              "%M",
              footnotes.put[scala.reflect.api.Mirror[_]](mirrorThatLoaded(sym)))
        case tag: TypeTag[_] =>
    ...
    def printIterable(iterable: List[_],
                      preamble: => Unit = print(""),
                      body: Any => Unit = print(_),
