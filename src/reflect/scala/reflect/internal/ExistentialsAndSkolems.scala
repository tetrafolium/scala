...
trait ExistentialsAndSkolems {
  ...
  private def existentialBoundsExcludingHidden(
    ...
    def safeBound(t: Type): Type =
      if (hidden contains t.typeSymbol)
        safeBound(t.typeSymbol.existentialBound.upperBound)
      else t

    def hiBound(s: Symbol): Type =
      safeBound(s.existentialBound.upperBound).resultType match {
    ...
    mapFrom(hidden)(s =>
      s.existentialBound match {
        case GenPolyType(tparams, TypeBounds(lo, _)) =>
          genPolyType(tparams, TypeBounds(lo, hiBound(s)))
        case _ => hiBound(s)
    })
  }
  ...
  final def packSymbols(hidden: List[Symbol],
    ...
    if (hidden.isEmpty) tp
      ...
      existentialTransform(hidden, tp, rawOwner)(existentialAbstraction(_, _))
}
