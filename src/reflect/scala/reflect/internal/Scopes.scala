...
trait Scopes extends api.Scopes { self: SymbolTable =>
  ...
  class Scope protected[Scopes] ()
      extends AbstractIterable[Symbol]
      with ScopeApi
      with MemberScopeApi {
    ...
    def lookupAllEntries(name: Name): Iterator[ScopeEntry] =
      new AbstractIterator[ScopeEntry] {
        private[this] var e = lookupEntry(name)
        def hasNext: Boolean = e ne null
        def next(): ScopeEntry =
          try e
    ...
    def lookupEntry(name: Name): ScopeEntry = {
      val startTime =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startTimer(statistics.scopeLookupTime)
        else null
      var e: ScopeEntry = null
      if (hashtable ne null) {
      ...
      if (StatisticsStatics.areSomeColdStatsEnabled)
        statistics.stopTimer(statistics.scopeLookupTime, startTime)
      e
    }
    ...
    final def lookupNameInSameScopeAs(original: Symbol,
                                      companionName: Name): Symbol = {
  ...
  final def newNestedScope(outer: Scope): Scope = {
    val startTime =
      if (StatisticsStatics.areSomeColdStatsEnabled)
        statistics.startTimer(statistics.scopePopulationTime)
      else null
    ...
    if (StatisticsStatics.areSomeColdStatsEnabled)
      statistics.stopTimer(statistics.scopePopulationTime, startTime)
    nested
  }
