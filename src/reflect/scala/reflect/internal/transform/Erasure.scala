  ...
  def unboundedGenericArrayLevel(tp: Type): Int = tp match {
    ...
    case RefinedType(ps, _) if ps.nonEmpty =>
      logResult(s"Unbounded generic level for $tp is")(
        unboundedGenericArrayLevel(intersectionDominator(ps)))
    case _ => 0
  }
  ...
  abstract class ErasureMap extends TypeMap {
    ...
    def apply(tp: Type): Type = tp match {
      case FoldableConstantType(ct) =>
        ...
        if (ct.tag == ClazzTag && ct.typeValue.typeSymbol != UnitClass)
          ConstantType(Constant(apply(ct.typeValue)))
        else tp
      ...
      case tref @ TypeRef(pre, sym, args) =>
        ...
        else if ((sym eq AnyClass) || (sym eq AnyValClass) || (sym eq SingletonClass))
          ObjectTpe
      ...
      case ClassInfoType(parents, decls, clazz) =>
        val newParents =
          if (parents.isEmpty || (clazz eq ObjectClass) || isPrimitiveValueClass(
                clazz)) Nil
            ...
            if (clazz.hasFlag(Flags.TRAIT) && !clazz.hasFlag(Flags.JAVA))
              ObjectTpe :: erasedParents.tail.filter(
                _.typeSymbol ne ObjectClass)
            else erasedParents
          }
