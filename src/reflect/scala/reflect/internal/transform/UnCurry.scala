...
trait UnCurry {
      ...
      tp match {
        case ClassInfoType(parents, decls, clazz) if !clazz.isJavaDefined =>
          ...
          for (decl <- decls
            ...
            if (mexists(decl.paramss)(sym =>
                  definitions.isRepeatedParamType(sym.tpe))) {
              varargOverloads += varargForwarderSym(
                clazz,
                decl,
                exitingPhase(phase)(decl.info))
            }
          }
  ...
  private def varargForwarderSym(currentClass: Symbol,
                                 origSym: Symbol,
                                 newInfo: Type): Symbol = {
    val forwSym = origSym
      .cloneSymbol(currentClass,
                   VARARGS | SYNTHETIC | origSym.flags & ~DEFERRED,
                   origSym.name.toTermName)
      .withoutAnnotations
    ...
    val isRepeated = origSym.info.paramss.flatten.map(sym =>
      definitions.isRepeatedParamType(sym.tpe))
    def toArrayType(tp: Type, newParam: Symbol): Type = {
      val arg = elementType(SeqClass, tp)
      val elem =
        if (arg.typeSymbol.isTypeParameterOrSkolem && !(arg <:< AnyRefTpe)) {
