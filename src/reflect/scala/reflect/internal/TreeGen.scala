...
abstract class TreeGen {
  ...
  def scalaFunctionConstr(argtpes: List[Tree],
                          restpe: Tree,
  ...
  def mkAttributedSelect(qual: Tree, sym: Symbol): RefTree = {
    ...
    if (qual.symbol != null && (qual.symbol.isEffectiveRoot || qual.symbol.isEmptyPackage))
      ...
      if (pkgQualifier.tpe == null) tree
        ...
        tree setType {
          if (sym.rawowner == ObjectClass || sym.rawowner == AnyClass)
            sym.tpeHK.normalize // opt for asInstanceOf
  ...
  def mkAttributedTypeApply(target: Tree,
                            method: Symbol,
                            targs: List[Type]): Tree =
    mkTypeApply(mkAttributedSelect(target, method), targs map TypeTree)

  private def mkSingleTypeApply(value: Tree,
                                tpe: Type,
                                what: Symbol,
                                wrapInApply: Boolean) = {
  ...
  private def typeTestSymbol(any: Boolean) =
    if (any) Any_isInstanceOf else Object_isInstanceOf
  private def typeCastSymbol(any: Boolean) =
    if (any) Any_asInstanceOf else Object_asInstanceOf

  /** Builds an instance test with given value and type. */
  def mkIsInstanceOf(value: Tree,
                     tpe: Type,
                     any: Boolean = true,
  ...
  def maybeMkAsInstanceOf(tree: Tree,
                          pt: Type,
                          tpe: Type,
                          beforeRefChecks: Boolean = false): Tree =
    if ((pt == UnitTpe) || (tpe <:< pt)) tree
    else
      atPos(tree.pos)(
        mkAsInstanceOf(tree, pt, any = true, wrapInApply = !beforeRefChecks))
  ...
  def mkNamedArg(lhs: Tree, rhs: Tree): Tree =
    atPos(rhs.pos)(NamedArg(lhs, rhs))
  ...
  /** Generates a template with constructor corresponding to
    ...
    *  }
    */
  def mkTemplate(parents: List[Tree],
                 self: ValDef,
                 constrMods: Modifiers,
                 vparamss: List[List[ValDef]],
                 body: List[Tree],
                 superPos: Position = NoPosition): Template = {
      ...
      if (constrMods.isTrait) {
        ...
        Some(atPos(
          ...
          DefDef(constrMods,
                 ...
                 vparamss1,
                 TypeTree(),
                 Block(lvdefs ::: List(superCall), mkLiteralUnit))))
      }
    }
  ...
  def mkParents(ownerMods: Modifiers,
                parents: List[Tree],
                parentPos: Position = NoPosition) =
    if (ownerMods.isCase)
      parents ::: List(scalaDot(tpnme.Product), scalaDot(tpnme.Serializable))
  ...
  /** Create a literal unit tree that is inserted by the compiler but not
    ...
    */
  def mkSyntheticUnit() =
    mkLiteralUnit.updateAttachment(SyntheticUnitAttachment)
  ...
  def mkPackageObject(defn: ModuleDef,
                      pidPos: Position = NoPosition,
                      pkgPos: Position = NoPosition) = {
  ...
  def mkFor(enums: List[Tree], sugarBody: Tree)(
     ...
     */
    def makeCombination(pos: Position,
                        meth: TermName,
                        qual: Tree,
                        pat: Tree,
                        body: Tree): Tree =
  ...
  private def propagateNoWarnAttachment(from: Tree, to: ValDef): to.type =
    if (isPatVarWarnable && from.hasAttachment[NoWarnAttachment.type])
      ...
      case None =>
        ...
        vars match {
          ...
          case _ =>
            ...
            var cnt = 0
            val restDefs = for ((vname, tpt, pos, original) <- vars)
              yield
                atPos(pos) {
                  cnt += 1
                  propagatePatVarDefAttachments(
                    ...
                    ValDef(mods,
                           vname.toTermName,
                           tpt,
                           Select(Ident(tmp), TermName("_" + cnt))))
                }
            firstDef :: restDefs
        }
    }
  ...
  private def matchVarPattern(tree: Tree): Option[(Name, Tree)] = {
    ...
    tree match {
      case id @ Ident(name) if name.toTermName != WC || id.isBackquoted =>
        Some((name, TypeTree()))
      case Bind(name, Ident(x)) if x.toTermName == WC =>
        Some((name, TypeTree()))
      case Bind(name, Typed(Ident(x), tpt)) if x.toTermName == WC =>
        Some((name, tpt))
      case Typed(id @ Ident(name), tpt)
          if name.toTermName != WC || id.isBackquoted =>
        Some((name, tpt))
      case _ => None
    }
  }
  ...
  def mkVisitor(
      cases: List[CaseDef],
      checkExhaustive: Boolean,
      prefix: String = "x$")(implicit fresh: FreshNameCreator): Tree = {
  ...
  class GetVarTraverser extends Traverser {
    ...
    override def traverse(tree: Tree): Unit = {
      ...
      def add(name: Name, t: Tree) =
        if (!seenName(name)) buf += ((name, t, namePos(tree, name), tree))
      val bl = buf.length
      ...
      tree match {
        ...
        case Bind(name, Typed(tree1, tpt)) =>
          val newTree =
            if (treeInfo.mayBeTypePat(tpt)) TypeTree() else tpt.duplicate
          add(name, newTree)
          traverse(tree1)
  ...
  /** Returns list of all pattern variables, possibly with their types,
    ...
    */
  private def getVariables(tree: Tree): List[(Name, Tree, Position, Tree)] =
    (new GetVarTraverser)(tree)
