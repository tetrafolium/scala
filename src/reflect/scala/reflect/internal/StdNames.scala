...
trait StdNames {
  ...
  private final object compactify extends (String => String) {
    ...
    def toMD5(s: String, edge: Int): String = {
      ...
      val cs = s.toArray
      val bytes =
        Codec.toUTF8(new scala.runtime.ArrayCharSequence(cs, 0, cs.length))
      md5 update bytes
  ...
  abstract class CommonNames extends NamesApi {
    ...
    val NESTED_IN_LAMBDA
      : String = NESTED_IN + DELAMBDAFY_LAMBDA_CLASS_NAME.toString.replace("$",
                                                                           "")
  ...
  abstract class TermNames extends Keywords with TermNamesApi {
    ...
    private class NameContext(s: String) {
      def n(args: Any*): TermName = {
        require(args.isEmpty); createNameType("$" + s)
      }
    }
    private def StringContext(parts: String*) = {
      require(parts.size == 1); new NameContext(parts.head)
    }
    ...
    val REIFY_FREE_VALUE_SUFFIX
      : NameType = n"value" // looks like missing interpolator due to `value` in scopre
    ...
    /** The expanded name of `name` relative to this class `base`
      */
    def expandedName(name: TermName, base: Symbol) =
      expandedNameInternal(name, base, EXPAND_SEPARATOR_STRING)

    /** The expanded setter name of `name` relative to this class `base`
      */
    def expandedSetterName(name: TermName, base: Symbol) =
      expandedNameInternal(name, base, TRAIT_SETTER_SEPARATOR_STRING)
    ...
    def protSetterName(name: Name): TermName =
      newTermName(PROTECTED_SET_PREFIX + name)
    ...
    val copyArrayToImmutableIndexedSeq: NameType =
    ...
    val asInstanceOf_Ob
      : NameType = n"asInstanceOf" // looks like missing interpolator due to Any member in scope
    ...
    val conforms
      : NameType = n"conforms" // $ prefix to avoid shadowing Predef.conforms
    ...
    val isInstanceOf_Ob
      : NameType = n"isInstanceOf" // looks like missing interpolator due to Any member in scope
    ...
    def isTransientBitmap(name: Name) =
      name == nme.BITMAP_TRANSIENT || name == nme.BITMAP_CHECKINIT_TRANSIENT
  ...
  sealed abstract class SymbolNames {
    ...
    final val InvTargetException: TypeName =
