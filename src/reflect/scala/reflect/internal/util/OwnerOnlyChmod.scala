...
object OwnerOnlyChmod {
  ...
  def chmodFileOrCreateEmpty(path: Path): Unit = {
    Files
      .newByteChannel(path, EnumSet.of(WRITE, CREATE))
      .close() // make sure it exists
    chmod(path)
  }

  def chmodFileAndWrite(path: Path, contents: Array[Byte]): Unit = {
    val sbc =
      Files.newByteChannel(path, EnumSet.of(WRITE, CREATE, TRUNCATE_EXISTING))
    try sbc.write(ByteBuffer.wrap(contents))
    finally sbc.close()
    chmod(path)
  }
}
