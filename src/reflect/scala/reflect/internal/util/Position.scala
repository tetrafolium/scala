...
private[util] trait InternalPositionImpl {
  ...
  final def makeTransparentIf(cond: Boolean): Position =
    if (cond && isOpaqueRange) Position.transparent(source, start, point, end)
    else this
  ...
  def withShift(shift: Int): Position =
    Position.range(source, start + shift, point + shift, end + shift)
  ...
  def properlyPrecedes(pos: Position): Boolean =
    bothDefined(pos) && end < pos.start
  def sameRange(pos: Position): Boolean =
    bothRanges(pos) && start == pos.start && end == pos.end
