...
trait Collections {
  ...
  final def mforeach[A](xss: Iterable[Iterable[A]])(f: A => Unit) =
    xss foreach (_ foreach f)
  ...
  final def sameElementsEquals(thiss: List[AnyRef],
                               that: List[AnyRef]): Boolean = {
  ...
  final def flatCollect[A, B](elems: List[A])(
      pf: PartialFunction[A, Iterable[B]]): List[B] = {
  ...
  // @inline
  final def findOrElse[A](xs: IterableOnce[A])(p: A => Boolean)(
      orElse: => A): A = {
  ...
  final def partitionInto[A](xs: List[A],
                             pred: A => Boolean,
                             ayes: ListBuffer[A],
                             nays: ListBuffer[A]): Unit = {
    var ys = xs
    while (!ys.isEmpty) {
      val y = ys.head
  ...
  final def bitSetByPredicate[A](xs: List[A])(
      pred: A => Boolean): mutable.BitSet = {
