...
final class WeakHashSet[A <: AnyRef](val initialCapacity: Int,
  ...
  /**
    ...
    */
  private[this] def remove(bucket: Int,
                           prevEntry: Entry[A],
                           entry: Entry[A]): Unit = {
    prevEntry match {
  ...
  private[util] class Diagnostics {
    ...
    /**
      ...
      */
    def dump = {
      def deep[T](a: Array[T]): IndexedSeqView[Any] =
        new AbstractIndexedSeqView[Any] {
          def length = a.length
          def apply(idx: Int): Any = a(idx) match {
            case x: AnyRef if x.getClass.isArray =>
              deep(x.asInstanceOf[Array[_]])
            case x => x
          }
...
object WeakHashSet {
  ...
  /**
    ...
    */
  private class Entry[A](element: A,
                         val hash: Int,
                         var tail: Entry[A],
