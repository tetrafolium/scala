...
abstract class TreeInfo {
  ...
  def isConstructorWithDefault(t: Tree) = t match {
    case DefDef(_, nme.CONSTRUCTOR, _, vparamss, _, _) =>
      mexists(vparamss)(_.mods.hasDefault)
    case _ => false
  }
  ...
  def isPureDef(tree: Tree): Boolean = tree match {
    case EmptyTree | ClassDef(_, _, _, _) | TypeDef(_, _, _, _) | Import(_, _) |
        DefDef(_, _, _, _, _, _) =>
      true
  ...
  def isStableIdentifier(tree: Tree, allowVolatile: Boolean): Boolean =
    tree match {
      ...
      case Select(qual, _) =>
        isStableMemberOf(tree.symbol, qual, allowVolatile) && isPath(
          qual,
          allowVolatile)
  ...
  private def symOk(sym: Symbol) =
    sym != null && !sym.isError && sym != NoSymbol
  ...
  /** Is `tree` either a non-volatile type,
    ...
    */
  def admitsTypeSelection(tree: Tree): Boolean =
    isPath(tree, allowVolatile = false)
  ...
  /** Is `tree` admissible as a stable identifier pattern (8.1.5 Stable Identifier Patterns)?
    ...
    */
  def isStableIdentifierPattern(tree: Tree): Boolean =
    isStableIdentifier(tree, allowVolatile = true)
  ...
  def mapMethodParamsAndArgs[R](params: List[Symbol], args: List[Tree])(
      f: (Symbol, Tree) => R): List[R] = {
    val b = List.newBuilder[R]
    foreachMethodParamAndArg(params, args)((param, arg) => b += f(param, arg))
    b.result()
  }
  def foreachMethodParamAndArg(params: List[Symbol], args: List[Tree])(
      f: (Symbol, Tree) => Unit): Boolean = {
  ...
  def mayBeVarGetter(sym: Symbol): Boolean = sym.info match {
    ...
    case PolyType(_, mt @ MethodType(_, _)) =>
      mt.isImplicit && sym.owner.isClass && !sym.isStable
  ...
  def isVariableOrGetter(tree: Tree) = {
    ...
    tree match {
      case Ident(_) => isVar
      case Select(qual, _) =>
        isVar || mayBeVarGetter(sym) && qual.tpe.member(sym.setterName) != NoSymbol
      case Applied(Select(qual, nme.apply), _, _) =>
  ...
  def noFieldFor(vd: ValDef, owner: Symbol) =
    vd.mods.isDeferred || vd.mods.isLazy || (owner.isTrait && !vd.mods.hasFlag(
      PRESUPER))
  ...
  private[internal] def untypecheckedTreeBody(tree: Tree, tbody: List[Tree]) = {
    def filterBody(body: List[Tree]) = body filter {
      ...
      case dd: DefDef if dd.mods.hasAccessorFlag =>
        !nme.isSetterName(dd.name) && !tbody.exists {
  ...
  /**
    ...
    * }}}
    */
  def effectivePatternArity(args: List[Tree]): Int =
  ...
  object Applied {
    ...
    def unapply(
        applied: Applied): Option[(Tree, List[Tree], List[List[Tree]])] =
  ...
  /** Does list of trees start with a definition of
    ...
    */
  def firstDefinesClassOrObject(trees: List[Tree], name: Name): Boolean =
    trees match {
      case Import(_, _) :: xs => firstDefinesClassOrObject(xs, name)
      case Annotated(_, tree1) :: _ =>
        firstDefinesClassOrObject(List(tree1), name)
  ...
  class DynamicApplicationExtractor(nameTest: Name => Boolean) {
    def unapply(tree: Tree) = tree match {
      case Apply(TypeApply(Select(qual, oper), _),
                 List(LiteralNameOrAdapted(name))) if nameTest(oper) =>
        Some((qual, name))
      case Apply(Select(qual, oper), List(LiteralNameOrAdapted(name)))
          if nameTest(oper) =>
        Some((qual, name))
      case Apply(Ident(oper), List(LiteralNameOrAdapted(name)))
          if nameTest(oper) =>
        Some((EmptyTree, name))
      case _ => None
    }
  ...
  object DynamicUpdate
      extends DynamicApplicationExtractor(_ == nme.updateDynamic)
  object DynamicApplication
      extends DynamicApplicationExtractor(isApplyDynamicName)
  object DynamicApplicationNamed
      extends DynamicApplicationExtractor(_ == nme.applyDynamicNamed)
...
trait MacroAnnotionTreeInfo { self: TreeInfo =>
  ...
  def anyConstructorHasDefault(tree: ClassDef): Boolean =
    tree.impl.body exists {
      case DefDef(_, nme.CONSTRUCTOR, _, paramss, _, _) =>
        mexists(paramss)(_.mods.hasDefault)
      case _ => false
    }
  ...
  def isMacroAnnotation(tree: ClassDef): Boolean = {
    ...
    def hasMacroTransformMethod =
      clazz.info.member(nme.macroTransform) != NoSymbol
  ...
  def getAnnotationZippers(tree: Tree): List[AnnotationZipper] = {
    def loop[T <: Tree](tree: T, deep: Boolean): List[AnnotationZipper] =
      tree match {
        case SyntacticClassDef(mods,
          ...
          val czippers = mods.annotations.map(ann => {
            ...
            val annottee = PatchedSyntacticClassDef(mods1,
                                                    name,
                                                    tparams,
                                                    constrMods,
                                                    vparamss,
          ...
          if (!deep) czippers
            ...
            val tzippers = for {
              tparam <- tparams
              AnnotationZipper(ann, tparam1: TypeDef, _) <- loop(tparam,
                                                                 deep = false)
            ...
            val vzippers = for {
              vparams <- vparamss
              vparam <- vparams
              AnnotationZipper(ann, vparam1: ValDef, _) <- loop(vparam,
                                                                deep = false)
        ...
        case SyntacticTraitDef(mods,
          ...
          val czippers = mods.annotations.map(ann => {
            val annottee =
              tdef.copy(mods = mods.mapAnnotations(_ diff List(ann)))
            AnnotationZipper(ann, annottee, annottee)
          })
          if (!deep) czippers
          else {
            val tzippers = for {
              ...
              AnnotationZipper(ann, tparam1, tdef.copy(tparams = tparams1))
            czippers ++ tzippers
          }
        case mdef @ ModuleDef(mods, _, _) =>
          mods.annotations.map(ann => {
            val annottee =
              mdef.copy(mods = mods.mapAnnotations(_ diff List(ann)))
            AnnotationZipper(ann, annottee, annottee)
          })
        case ddef @ DefDef(mods, _, tparams, vparamss, _, _) =>
          val dzippers = mods.annotations.map(ann => {
            val annottee =
              ddef.copy(mods = mods.mapAnnotations(_ diff List(ann)))
            AnnotationZipper(ann, annottee, annottee)
          })
          if (!deep) dzippers
          else {
            val tzippers = for {
              ...
              AnnotationZipper(ann, tparam1, ddef.copy(tparams = tparams1))
            val vzippers = for {
              vparams <- vparamss
              vparam <- vparams
              AnnotationZipper(ann, vparam1: ValDef, _) <- loop(vparam,
              ...
              AnnotationZipper(ann, vparam1, ddef.copy(vparamss = vparamss1))
            dzippers ++ tzippers ++ vzippers
          }
        case vdef @ ValDef(mods, _, _, _) =>
          mods.annotations.map(ann => {
            val annottee =
              vdef.copy(mods = mods.mapAnnotations(_ diff List(ann)))
            AnnotationZipper(ann, annottee, annottee)
          })
  ...
  final def splitAtSuper(stats: List[Tree],
                         classOnly: Boolean): (List[Tree], List[Tree]) = {
    def isConstr(tree: Tree): Boolean = tree match {
      case Block(_, expr) =>
        isConstr(expr) // scala/bug#6481 account for named argument blocks
      case _ =>
        (tree.symbol ne null) && (if (classOnly) tree.symbol.isClassConstructor
                                  else tree.symbol.isConstructor)
    }
