...
trait Importers { to: SymbolTable =>
  ...
  abstract class StandardImporter extends Importer {
    ...
    protected class Cache[K <: AnyRef, V <: AnyRef]
        extends WeakHashMap[K, WeakReference[V]] {
      def weakGet(key: K): Option[V] =
        this get key flatMap WeakReference.unapply
    ...
    object reverse extends from.StandardImporter {
      ...
      for ((theirsym, WeakReference(mysym)) <- StandardImporter.this.symMap)
        symMap += ((mysym, WeakReference(theirsym)))
      for ((theirtpe, WeakReference(mytpe)) <- StandardImporter.this.tpeMap)
        tpeMap += ((mytpe, WeakReference(theirtpe)))
    }

    // ============== SYMBOLS ==============

    protected def recreatedSymbolCompleter(my: to.Symbol,
                                           their: from.Symbol) = {
          ...
          myexisting.orElse {
            ...
            if (myscope != NoType) {
              assert(myscope.decls.lookup(myname) == NoSymbol,
                     s"$myname ${myscope.decl(myname)} $myexisting")
    ...
    def recreateTree(their: from.Tree): to.Tree = their match {
      ...
      case from.Template(parents, self, body) =>
        new Template(parents map importTree,
                     importValDef(self),
                     body map importTree)
      ...
      case from.ExistentialTypeTree(tpt, whereClauses) =>
        new ExistentialTypeTree(importTree(tpt),
    ...
    def importAnnotArg(arg: from.ClassfileAnnotArg): ClassfileAnnotArg =
      arg match {
    ...
    def importTypeDef(tree: from.TypeDef): TypeDef =
      importTree(tree).asInstanceOf[TypeDef]
    def importMemberDef(tree: from.MemberDef): MemberDef =
      importTree(tree).asInstanceOf[MemberDef]
    def importTemplate(tree: from.Template): Template =
      importTree(tree).asInstanceOf[Template]
    def importRefTree(tree: from.RefTree): RefTree =
      importTree(tree).asInstanceOf[RefTree]
    def importIdent(tree: from.Ident): Ident =
      importTree(tree).asInstanceOf[Ident]
