...
object ScalaCompilerOptionsExporter {
  ...
  case class Section(_category: Category,
                     description: Option[String],
                     options: List[ScalacOption])
      extends JacksonWorkaround {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sortedInOrderOfAppearance =
      runtimeMirror.classSymbol(settings.getClass).toType.members.sorted
    val accessors = sortedInOrderOfAppearance.collect {
      case m: MethodSymbol if m.isGetter && m.isPublic => m
    }

    def mergeChoice(labels: Seq[String],
                    descriptions: Seq[String]): Seq[Choice] = {
      for {
        ...
        Choice(
          ...
          description = Option(d)
            .map(markdownifyBackquote)
            .map(dehtmlfy)
            .filter(_.nonEmpty),
          deprecated = Some("EXPLAIN_ALTERNATIVE").filter(_ =>
            d.toLowerCase.contains("deprecated"))
        )
      }
    }

    val extractedSettings: List[ScalacOption] =
      accessors.map(acc => instanceMirror.reflectMethod(acc).apply()).collect {
        case s: settings.Setting =>
          val schema = s match {
            ...
            case mc: settings.MultiChoiceSetting[_] =>
              ...
              Schema(_type = "Choice",
                     multiple = Some(true),
                     arg = Some(mc.helpArg).map(dehtmlfy),
                     choices = choices)
            ...
            case sv: settings.ScalaVersionSetting =>
              Schema(_type = "ScalaVersion",
                     arg = Some(sv.arg).map(dehtmlfy),
                     default = Some(sv.initial.unparse))
            case pathStr: settings.PathSetting =>
              Schema(_type = "Path",
                     arg = Some(pathStr.arg),
                     default = Some(pathStr.default))
            case str: settings.StringSetting =>
              Schema(_type = "String",
                     arg = Some(str.arg).map(dehtmlfy),
                     default = Some(str.default))
            case ms: settings.MultiStringSetting =>
              Schema(_type = "String",
                     multiple = Some(true),
                     arg = Some(ms.arg).map(dehtmlfy))
          }
          ...
          ScalacOption(
            ...
            deprecated = Some("EXPLAIN_ALTERNATIVE").filter(_ =>
              s.helpDescription.toLowerCase.contains("deprecated"))
          )
      }
    ...
    val categorized = extractedSettings.groupBy { option =>
      ...
      if (name.startsWith("-Xfatal-warnings") || name == "-Xlint" || name
        ...
        PluginSettings
      } else if (name.startsWith("-J") || name.startsWith("-D") || name
                   .startsWith("-nobootcp")) {
        JVMSettings
      } else {
        StandardSettings
    ...
    val source = categoriezed.toSeq.sortBy(_._1).map {
      case (key, options) =>
