...
abstract class Pickler[T] {
  ...
  /** A pickler obtained from the current pickler by a pair of transformer functions
    ...
    */
  def wrapped[U](in: T => U)(out: U => T): Pickler[U] =
    wrappedPickler(this)(in)(out)
...
object Pickler {
  ...
  /** A class representing unpickle failures
    ...
    */
  class UnpickleFailure(msg: => String, val rd: Lexer)
      extends Unpickled[Nothing] {
    def errMsg = msg
  ...
  /** Same as `p.cond(condition)`
    */
  def conditionalPickler[T](p: Pickler[T], condition: Any => Boolean) =
    new CondPickler[T](condition) {
  ...
  def javaInstancePickler[T <: AnyRef]: Pickler[T] =
    (stringPickler labelled "$new")
      .wrapped { name =>
        Class.forName(name).getConstructor().newInstance().asInstanceOf[T]
      } { _.getClass.getName }
...
/** A subclass of Pickler can indicate whether a particular value can be pickled by instances
  ...
  */
abstract class CondPickler[T](val canPickle: Any => Boolean)
    extends Pickler[T] {
  import Pickler._
