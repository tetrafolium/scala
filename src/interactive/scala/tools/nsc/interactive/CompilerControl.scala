...
trait CompilerControl { self: Global =>
  ...
  def removeUnitOf(s: SourceFile): Option[RichCompilationUnit] = {
    toBeRemoved.synchronized { toBeRemoved += s.file }; unitOfFile get s.file
  }
  ...
  /** If source is not yet loaded, loads it, and starts a new run, otherwise
    ...
    */
  def askLoadedTyped(source: SourceFile,
                     keepLoaded: Boolean,
  ...
  /** If source if not yet loaded, get an outline view with askParseEntered.
    ...
    */
  def askStructure(keepSrcLoaded: Boolean)(source: SourceFile,
                                           response: Response[Tree]) = {
  ...
  /** Set sync var `response` to the parse tree of `source` with all top-level symbols entered.
    ...
    */
  def askParsedEntered(source: SourceFile,
                       keepLoaded: Boolean,
                       response: Response[Tree]) =
  ...
  case class ScopeMember(sym: Symbol,
                         tpe: Type,
                         accessible: Boolean,
                         viaImport: Tree)
      extends Member {
  ...
  case class AskLinkPosItem(sym: Symbol,
                            source: SourceFile,
                            response: Response[Position])
      extends WorkItem {
  ...
  case class AskParsedEnteredItem(source: SourceFile,
      ...
      extends WorkItem {
    def apply() =
      self.getParsedEntered(source, keepLoaded, response, this.onCompilerThread)
    override def toString =
...
class NoSuchUnitError(file: AbstractFile)
    extends Exception("no unit found for file " + file)
