    ...
    override def enterExistingSym(sym: Symbol, tree: Tree): Context = {
      if (sym != null && sym.owner.isTerm) {
        ...
        val defTree = tree match {
          case dd: DocDef =>
            dd.definition // See scala/bug#9011, Scala IDE's presentation compiler incorporates ScaladocGlobal with InteractiveGlobal, so we have to unwrap DocDefs.
          case _ => tree
        }
...
} with scala.tools.nsc.Global(settings, _reporter) with CompilerControl
with ContextTrees with RichCompilationUnits with Picklers {
  ...
  /** A set containing all those files that need to be removed after a full background compiler run
    */
  protected val toBeRemovedAfterRun: HashSet[AbstractFile] =
    new HashSet[AbstractFile]
  ...
  /** Work through toBeRemoved list to remove any units.
    ...
    */
  protected[interactive] def getUnit(
      s: SourceFile): Option[RichCompilationUnit] = {
    ...
    if (canObserveTree) {
      if (context.unit.exists &&
        ...
        if (located == EmptyTree) {
          println(
            "something's wrong: no " + context.unit + " in " + result + result.pos)
          located = result
        }
      ...
      breakable {
        ...
        def nodeWithWork(): Option[WorkEvent] =
          if (scheduler.moreWork || pendingResponse.isCancelled)
            Some(new WorkEvent(nodesSeen, System.currentTimeMillis))
          else None
  ...
  private[interactive] def filesDeleted(sources: List[SourceFile],
    ...
    val deletedSyms = currentTopLevelSyms filter { sym =>
...
//          println("tree not found at "+pos)
  ...
  private[interactive] def getLinkPos(sym: Symbol,
                                      source: SourceFile,
                                      response: Response[Position]): Unit = {
  ...
  private[interactive] def getDocComment(
      sym: Symbol,
      source: SourceFile,
      site: Symbol,
  ...
  private[interactive] def getScopeCompletion(
      pos: Position,
      response: Response[List[Member]]): Unit = {
...
//    if (debugIDE) for (m <- result) println(m)
  ...
  object CompletionResult {
    ...
    final case class TypeMembers(positionDelta: Int,
                                 qualifier: Tree,
                                 tree: Tree,
                                 results: List[TypeMember],
                                 name: Name)
        extends CompletionResult {
      type M = TypeMember
    }
    ...
    private def camelComponents(s: String,
                                allowSnake: Boolean): List[String] = {
      if (allowSnake && s.forall(c => c.isUpper || c == '_'))
        s.split('_').toList.filterNot(_.isEmpty)
      else
    ...
    def camelMatch(entered: Name): Name => Boolean = {
      ...
      { candidate: Name =>
        def candidateChunks =
          camelComponents(candidate.dropLocal.toString, allowSnake)
        ...
        def lenientMatch(entered: String,
          ...
          candidate match {
            ...
            case head :: tail =>
              ...
              val n = head.toIterable.lazyZip(entered).count {
                case (c, e) => c == e || (c.isUpper && c == e.toUpper)
              }
              head
                .take(n)
                .inits
  ...
  final def completionsAt(pos: Position): CompletionResult = {
    ...
    def typeCompletions(tree: Tree,
                        qual: Tree,
                        nameStart: Int,
                        name: Name): CompletionResult = {
  ...
  private[interactive] def waitLoadedTyped(
      source: SourceFile,
      response: Response[Tree],
      keepLoaded: Boolean = false,
      onSameThread: Boolean = true): Unit = {
  ...
  private[interactive] def getParsedEntered(
      source: SourceFile,
      keepLoaded: Boolean,
      response: Response[Tree],
      onSameThread: Boolean = true): Unit = {
