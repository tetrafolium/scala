...
abstract class ScaladocSyntaxAnalyzer[G <: Global](val global: G)
  ...
  class ScaladocUnitScanner(unit0: CompilationUnit, patches0: List[BracePatch])
    ...
    protected def docPosition: Position =
      Position.range(unit.source, offset, offset, charOffset - 2)
  }
  class ScaladocUnitParser(unit: CompilationUnit, patches: List[BracePatch])
    ...
    override def withPatches(patches: List[BracePatch]) =
      new ScaladocUnitParser(unit, patches)
  ...
  class ScaladocJavaUnitScanner(unit: CompilationUnit)
      extends JavaUnitScanner(unit)
      with ScaladocScanner {
    ...
    override protected def docPosition =
      Position.range(unit.source, docStart, docStart, in.cpos)
  }
