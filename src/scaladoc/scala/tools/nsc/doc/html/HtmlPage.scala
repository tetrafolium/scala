...
abstract class HtmlPage extends Page { thisPage =>
      ...
      Html(
        Head(
          Meta(`http-equiv` = "X-UA-Compatible", content = "IE=edge") ::
            ...
            content = s"text/html; charset=${encoding}") :: headers) :: body)
  ...
  def blockToHtml(block: Block): Elems =
    (block match {
      ...
      case comment.Code(data) =>
        Pre(SyntaxHigh(data)) //<pre>{ Txt(data) }</pre>
      case UnorderedList(items) => Ul(listItemsToHtml(items))
      case OrderedList(items, listStyle) =>
        Ol(`class` = listStyle, elems = listItemsToHtml(items))
      case DefinitionList(items) =>
        Dl(items.toList flatMap {
          case (t, d) => Dt(inlineToHtml(t)) :: Dd(blockToHtml(d)) :: NoElems
        })
  ...
  def listItemsToHtml(items: Seq[Block]) =
    items.foldLeft(NoElems) { (xmlList, item) =>
      item match {
        ...
        case OrderedList(_, _) | UnorderedList(_) =>
          xmlList.init ++ Li(xmlList.last.elems ++ blockToHtml(item))
        case Paragraph(inline) =>
          xmlList :+ Li(inlineToHtml(inline)) // LIs are blocks, no need to use Ps
  ...
  def inlineToHtml(inl: Inline): Elems = inl match {
    ...
    case comment.Link(raw, title) =>
      A(href = raw, target = "_blank", elems = inlineToHtml(title)) :: NoElems
  ...
  def linkToHtml(text: Inline, link: LinkTo, hasLinks: Boolean): Elems =
    link match {
      case LinkToTpl(dtpl: TemplateEntity) =>
        (if (hasLinks)
           A(href = relativeLinkTo(dtpl),
             `class` = "extype",
             id = dtpl.qualifiedName,
             name = dtpl.qualifiedName,
             elems = inlineToHtml(text))
         else
           Span(`class` = "extype",
                name = dtpl.qualifiedName,
                elems = inlineToHtml(text))) :: NoElems
      case LinkToMember(mbr: MemberEntity, inTpl: TemplateEntity) =>
        (if (hasLinks)
           A(href = relativeLinkTo(inTpl) + "#" + mbr.signature,
             `class` = "extmbr",
             id = mbr.qualifiedName,
             name = mbr.qualifiedName,
             elems = inlineToHtml(text))
         else
           Span(`class` = "extmbr",
                name = mbr.qualifiedName,
      ...
      case LinkToExternalTpl(name, baseUrlString, dtpl: TemplateEntity) =>
        val baseUrl = new URI(
          Page.makeUrl(baseUrlString, Page.templateToPath(dtpl)))
        val url =
  ...
  private def tableToHtml(table: comment.Table): Elem = {
    ...
    Table(
      thead = THead(
        Tr(
          (header.cells zip cc).toList.map {
            case (cell, cls) =>
              Th(cell.blocks.flatMap(blockToHtml).toList, `class` = cls)
          }
        ) :: immutable.Nil
      ),
      tbody = if (rows.nonEmpty) {
        TBody(
          rows.toList.map { row =>
            Tr((row.cells zip cc).toList.map {
              case (cell, cls) =>
                Td(elems = cell.blocks.flatMap(blockToHtml).toList,
                   `class` = cls)
            })
          }
        )
  ...
  def typeToHtml(tpes: List[model.TypeEntity], hasLinks: Boolean): Elems =
    tpes match {
      case Nil       => NoElems
      case List(tpe) => typeToHtml(tpe, hasLinks)
      case tpe :: rest =>
        typeToHtml(tpe, hasLinks) ++ (Txt(" with ") :: typeToHtml(rest,
                                                                  hasLinks))
    }
  ...
  def typesToHtml(tpess: List[model.TypeEntity],
                  hasLinks: Boolean,
                  sep: Elems): Elems = tpess match {
    case Nil        => NoElems
  ...
  def permalink(template: Entity, isSelf: Boolean = true): Elem =
    Span(`class` = "permalink",
         elems =
           A(href = memberToUrl(template, isSelf),
             title = "Permalink",
             elems = I(`class` = "material-icons", elems = Txt("\uE157"))))

  def docEntityImageClass(tpl: DocTemplateEntity): String =
    tpl.kind + tpl.companion.fold("")("-companion-" + _.kind)

  def docEntityKindToCompanionTitle(ety: DocTemplateEntity,
    ...
    Span(
      ...
      elems = tpl.companion match {
        case Some(companionTpl) =>
          ...
          Div(
            elems = Txt("Companion ") :: A(
              href = relativeLinkTo(companionTpl),
              title = docEntityKindToCompanionTitle(tpl),
              elems = Txt(objClassTrait)) :: NoElems)
        case None => NoElems
      }
    )

  private def memberToUrl(template: Entity,
                          isSelf: Boolean /*= true*/ ): String = {
