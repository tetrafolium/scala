...
trait EntityPage extends HtmlPage {
  ...
  def headers: Elems = {
    ...
    List(
      HtmlTags.Link(href = relativeLinkTo(List("index.css", "lib")),
                    media = "screen",
                    `type` = "text/css",
                    rel = "stylesheet"),
      HtmlTags.Link(href = relativeLinkTo(List("template.css", "lib")),
                    media = "screen",
                    `type` = "text/css",
                    rel = "stylesheet"),
      HtmlTags.Link(href = relativeLinkTo(List("diagrams.css", "lib")),
                    media = "screen",
                    `type` = "text/css",
                    rel = "stylesheet",
                    id = "diagrams-css"),
  ...
  def body =
    HtmlTags.Body(
      search ::
        ...
        id = "search-results",
        elems =
          Div(id = "search-progress", elems = Div(id = "progress-fill")) ::
        ...
        elems = Div(
          ...
          elems =
            Div(
              id = "subpackage-spacer",
              elems = Div(
                id = "packages",
                elems =
                  H(1, Txt("Packages")) ::
                    Ul(elems = {
                    def entityToUl(mbr: TemplateEntity with MemberEntity,
                                   indentation: Int): Elems =
                      if (mbr.isObject && hasCompanion(mbr))
                        NoElems
                      else
                        Li(
                          `class` = s"current-entities indented$indentation",
                          elems =
                            (mbr match {
                              case dtpl: DocTemplateEntity =>
                                dtpl.companion.fold(
                                  Span(`class` = "separator"): Elem) {
                                  c: DocTemplateEntity =>
                                    A(`class` = "object",
                                      href = relativeLinkTo(c),
                                      title = memberToShortCommentTitleTag(c))
                                }
                              case _ => Span(`class` = "separator")
                            }) :: Txt(" ") ::
                              A(`class` = mbr.kind,
                                href = relativeLinkTo(mbr),
                                title = memberToShortCommentTitleTag(mbr)) ::
                              A(href = relativeLinkTo(mbr),
                                title = memberToShortCommentTitleTag(mbr),
                                elems = Txt(mbr.name)) :: NoElems
                        )

                    // Get path from root
                    val rootToParentLis: Elems =
                      tpl.toRoot.tail.reverse.zipWithIndex
                        .flatMap {
                          case (pack, ind) =>
                            memberToHtml(
                              pack,
                    ...
                    val parent = tpl.toRoot match {
                      case _ :: parent :: _ if !parent.isRootPackage =>
                        Some(parent)
                      case _ => None
                    }

                    val parentSub =
                      parent.fold(Seq[TemplateEntity with MemberEntity](tpl)) {
                        p =>
                    ...
                    val currentPackageTpls =
                      if (tpl.isPackage) tpl.templates
                        ...
                        parent.fold(
                          Seq.empty[TemplateEntity with MemberEntity])(p =>
                          p.templates)

                    val (subsToTpl, subsAfterTpl) =
                      parentSub.partition(_.name <= tpl.name)

                    val subsToTplLis = subsToTpl.toList.flatMap(
                      memberToHtml(_,
                                   tpl,
                                   indentation = rootToParentLis.length))
                    val subsAfterTplLis = subsAfterTpl.toList.flatMap(
                      memberToHtml(_,
                                   tpl,
                                   indentation = rootToParentLis.length))
                    val currEntityLis = currentPackageTpls
                      .filter(x =>
                        !x.isPackage && (x.isTrait || x.isClass || x.isAbstractType || x.isObject))
                      .sortBy(_.name)
                      .toList
                      .flatMap(entityToUl(
                        _,
                    ...
                    val currSubLis = tpl.templates
                      ...
                      .flatMap(
                        memberToHtml(_,
                                     tpl,
                                     indentation = rootToParentLis.length + 1))
              ...
              Div(id = "content", elems = content) :: NoElems
        )
      )
    )
    ...
    Div(
      ...
      elems =
        Span(id = "doc-title",
          ...
          Span(
          `class` = "close-results",
          elems = Span(`class` = "left", elems = Txt("<")) :: Txt(" Back")) ::
          Div(
          id = "textfilter",
          elems = Span(
            `class` = "input",
  ...
  val packageMembers = tpl.templates.filter(x => x.isPackage) sorted (implicitly[
  ...
  val content = {
    ...
    val displayName: Elems = tpl.companion match {
      case Some(companion)
        ...
        A(href = relativeLinkTo(companion),
          title = docEntityKindToCompanionTitle(tpl),
          elems = templateName)
      case _ =>
        templateName
    }
    val owner: Elems = {
      if (tpl.isRootPackage || tpl.inTemplate.isRootPackage)
        ...
        P(id = "owner",
          elems = templatesToHtml(tpl.inTemplate.toRoot.reverse.tail, Txt(".")))
    }
          ...
          elems = {
            ...
            tpl.companion match {
              case Some(companion)
                  if (companion.visibility.isPublic && companion.inSource != None) =>
                A(
                  href = relativeLinkTo(companion),
                  title = docEntityKindToCompanionTitle(tpl),
                  elems = Div(`class` = s"big-circle $imageClass",
                              elems = Txt(imageClass.substring(0, 1)))
                )
              case _ =>
                Div(`class` = s"big-circle $imageClass",
                    elems = Txt(imageClass.substring(0, 1)))
            }
          } ::
            owner ++
            H(1, displayName ++ permalink(tpl)) ++ {
            if (tpl.isPackage) NoElems
            else H(3, companionAndPackage(tpl)) :: NoElems
          }
        ))
                    ...
                    elems =
                      Span(`class` = "filtertype", elems = Txt("Ordering")) ::
                        ...
                        if (!universe.settings.docGroups.value || tpl.members
                              .map(_.group)
                              .distinct
                              .forall(_ == ModelFactory.defaultGroup))
                          NoElems
                        else
                          Li(`class` = "group out",
                             elems = Span(elems = Txt("Grouped"))) :: NoElems
                      } ++
                        (Li(`class` = "alpha in",
                            elems = Span(elems = Txt("Alphabetic"))) :: {
                          if (tpl.linearizationTemplates.isEmpty && tpl.conversions.isEmpty)
                            NoElems
                          else
                            Li(`class` = "inherit out",
                               elems = Span(elems = Txt("By Inheritance"))) :: NoElems
                        }))
                  ) ++ (if (tpl.linearizationTemplates.isEmpty && tpl.conversions.isEmpty)
                          NoElems
                        else {
                            ...
                            (if (tpl.conversions.isEmpty) NoElems
                               ...
                               Div(
                                 ...
                                 elems =
                                   Span(
                                     `class` = "filtertype",
                                     elems = Txt("Implicitly") :: Br :: NoElems) ::
                                     Ol(
                                     id = "implicits",
                                     elems = {
                              ...
                              Span(`class` = "filtertype") ::
                                ...
                                elems =
                                  Li(`class` = "hideall out",
                                     elems = Span(elems = Txt("Hide All"))) ::
                                    Li(`class` = "showall in",
                                       elems = Span(elems = Txt("Show All"))))
                          ))
                        }) ++ List(Div(
                    id = "visbl",
                    elems =
                      Span(`class` = "filtertype", elems = Txt("Visibility")) ::
                        ...
                        elems =
                          Li(`class` = "public in",
                             elems = Span(elems = Txt("Public"))) :: Li(
                            `class` = "all out",
                            elems = Span(elems = Txt("All"))))
                  ))
              )
          ))
      ...
      Div(
        ...
        elems = List(
          ...
          Div(
            ...
            elems =
              // linearization
              (for ((superTpl, superType) <- tpl.linearizationTemplates zip tpl.linearizationTypes)
                yield
                  Div(`class` = "parent",
                      name = superTpl.qualifiedName,
                      elems = H(3,
                                elems = Txt("Inherited from ") ::
                ...
                (for (conversion <- tpl.conversions)
                  yield
                    Div(
                      `class` = "conversion",
                      name = conversion.conversionQualifiedName,
                      elems = H(
                        3,
                        elems = Txt(
                          s"Inherited by implicit conversion ${conversion.conversionShortName} from") ::
                          typeToHtml(tpl.resultType, hasLinks = true) ++ (Txt(
                          " to ") :: typeToHtml(conversion.targetType,
                                                hasLinks = true))
                      )
                    ))
          ),
              ...
              for (group <- orderedGroups)
                yield
                  Div(
                    ...
                    name = group,
                    elems =
                      H(3, Txt(tpl.groupName(group))) :: (tpl.groupDescription(
                        group) match {
                        case Some(body) =>
                          Div(`class` = "comment cmt", elems = bodyToHtml(body)) :: NoElems
                        case _ => NoElems
                      })
                  )
            }
      ...
      List(
        ...
        if (Set("epfl", "EPFL").contains(tpl.universe.settings.docfooter.value))
          Div(
            id = "footer",
            elems = Txt(
              "Scala programming documentation. Copyright (c) 2002-2019 ") :: A(
              href = "http://www.epfl.ch",
              target = "_top",
              elems = Txt("EPFL")) :: Txt(", with contributions from ") :: A(
              href = "http://www.lightbend.com",
              target = "_top",
              elems = Txt("Lightbend")) :: Txt(".")
          )
        else
    ...
    HtmlTags.Body(
      `class` = tpl.kind + (if (tpl.isType) " type" else " value"),
      elems =
        definition ++ signature(tpl, isSelf = true) ++ memberToCommentHtml(
          tpl,
          tpl.inTemplate,
          isSelf = true) ++ memberSel ++ template ++ postamble
    )
  }

  def memsDiv(cls: String,
              header: String,
              mems: List[MemberEntity],
              name: String = null) =
    if (mems.isEmpty) NoElems
      ...
      List(
        Div(id = name,
            `class` = cls,
            elems = List(H(3, Txt(header)),
                         Ol(elems = mems flatMap (memberToHtml(_, tpl))))))
  ...
  def memberToHtml(
    ...
    Li(
      name = mbr.definitionName,
      visbl = if (mbr.visibility.isProtected) "prt" else "pub",
      `class` = s"indented$indentation " + (if (mbr eq inTpl) "current"
                                            else ""),
      `data-isabs` = mbr.isAbstract.toString,
      fullComment =
        if (memberComment.filter(_.tagName == "div").isEmpty) "no" else "yes",
      group = mbr.group,
      elems = { sig } ++
        (Txt(" ") :: { signature(mbr, isSelf = false) }) ++ { memberComment }
    )
  }

  def memberToCommentHtml(mbr: MemberEntity,
                          inTpl: DocTemplateEntity,
                          isSelf: Boolean): Elems = {
    mbr match {
      // comment of class itself
      case dte: DocTemplateEntity if isSelf =>
        Div(id = "comment",
            `class` = "fullcommenttop",
            elems = memberToCommentBodyHtml(mbr, inTpl, isSelf = true))
      case _ =>
  ...
  def memberToShortCommentHtml(mbr: MemberEntity, isSelf: Boolean): Elems =
    mbr.comment.toList.flatMap { comment =>
      P(`class` = "shortcomment cmt",
        elems = memberToUseCaseCommentHtml(mbr, isSelf) ++ inlineToHtml(
          comment.short))
    }
  ...
  def memberToCommentBodyHtml(mbr: MemberEntity,
                              inTpl: DocTemplateEntity,
                              isSelf: Boolean,
                              isReduced: Boolean = false): Elems = {
    val s = universe.settings

    val memberComment =
      if (mbr.comment.isEmpty) NoElems
      else
        Div(`class` = "comment cmt", elems = commentToHtml(mbr.comment)) :: NoElems
        ...
        Div(`class` = "comment cmt",
            elems =
              H(6,
                ...
                mbr.comment.get.authors.flatMap(bodyToHtml)) :: NoElems
    ...
    val paramComments = {
      ...
      def paramCommentToHtml(prs: List[ParameterEntity],
                             comment: Comment): Elems = prs match {

        case (tp: TypeParam) :: rest =>
          val paramEntry: Elems = {
            Dt(`class` = "tparam", elems = Txt(tp.name)) :: Dd(
              `class` = "cmt",
              elems = bodyToHtml(comment.typeParams(tp.name)))
          }
          paramEntry ++ paramCommentToHtml(rest, comment)

        case (vp: ValueParam) :: rest =>
          val paramEntry: Elems = {
            Dt(`class` = "param", elems = Txt(vp.name)) :: Dd(
              `class` = "cmt",
              elems = bodyToHtml(comment.valueParams(vp.name)))
          }
          paramEntry ++ paramCommentToHtml(rest, comment)
      ...
      mbr.comment.fold(NoElems) { comment =>
        ...
        if (cmtedPrs.isEmpty && comment.result.isEmpty) NoElems
          ...
          Dl(
            `class` = "paramcmts block",
            elems =
              paramCommentToHtml(cmtedPrs, comment) ++ (comment.result match {
                case None => NoElems
                case Some(cmt) =>
                  Dt(elems = Txt("returns")) :: Dd(`class` = "cmt",
                                                   elems = bodyToHtml(cmt))
              })
          )
        }
      }
    ...
    val implicitInformation = mbr.byConversion match {
      case Some(conv) =>
        Dt(`class` = "implicit", elems = Txt("Implicit")) ++ {
          ...
          val conversionOwnerQualifiedName =
            conv.convertorOwner.qualifiedName.stripSuffix(".package")
          val conversionOwner =
            templateToHtml(conv.convertorOwner, conversionOwnerQualifiedName)

          val constraintText = conv.constraints match {
            case Nil =>
              NoElems
            case List(constraint) =>
              Txt("This conversion will take place only if ") ++ constraintToHtml(
                constraint) ++ Txt(".")
            ...
            case constraints =>
              Br :: Txt(
                "This conversion will take place only if all of the following constraints are met:") :: Br :: {
                var index = 0
                constraints flatMap { constraint =>
                  Txt({ index += 1; index } + ". ") :: (constraintToHtml(
                    constraint) :+ Br)
                }
              }
          }
          ...
          Dd(
            elems =
              (Txt("This member is added by an implicit conversion from ") :: (typeToHtml(
                inTpl.resultType,
                hasLinks = true) :+ Txt(" to"))) ++
                targetType ++ (Txt(" performed by method ") :: (Txt(
                conversionMethod + " in ") :: (conversionOwner :+ Txt(".")))) ++
                constraintText)
        } ++ {
            ...
            val shadowingSuggestion = {
              ...
              Br ++ Txt("To access this member you can use a ") ++
                A(href =
                  ...
                  target = "_blank",
                  elems = Txt("type ascription")) ++ Txt(":") ++
                Br ++ Div(`class` = "cmt", elems = Pre(Txt(s"(${EntityPage.lowerFirstLetter(
    ...
    val attributes: Elems = {
      ...
      if (fvs.isEmpty || isReduced) NoElems
        ...
        dt("Attributes") ::
          Dd(elems = fvs flatMap { fv =>
          { inlineToHtml(fv.text) :+ Txt(" ") }
        }) :: NoElems
      }
    }
    ...
    val fullSignature: Elems = {
      mbr match {
        case nte: NonTemplateMemberEntity if nte.isUseCase =>
          Div(
            `class` = "full-signature-block toggleContainer",
            elems =
              Span(`class` = "toggle",
                   elems =
                ...
                Div(`class` = "hiddenContent full-signature-usecase",
                    elems = signature(nte.useCaseOf.get, isSelf = true))
          )
        case _ => NoElems
      }
    }
    ...
    val selfType: Elems = mbr match {
      case dtpl: DocTemplateEntity
          if (isSelf && dtpl.selfType.isDefined && !isReduced) =>
        dt("Self Type") ::
    ...
    val annotations: Elems = {
      ...
      if (mbr.annotations.nonEmpty) {
        dt("Annotations") ::
          Dd(elems = mbr.annotations.flatMap { annot =>
          Span(
            `class` = "name",
            elems = Txt("@") :: templateToHtml(annot.annotationClass)) :: ((if (showArguments(
                                                                                  annot))
                                                                              argumentsToHtml(
                                                                                annot.arguments)
                                                                            else
    ...
    val sourceLink: Elems = mbr match {
      case dtpl: DocTemplateEntity
        ...
        dt("Source") ::
          Dd(
          elems = A(href = dtpl.sourceUrl.get.toString,
                    target = "_blank",
                    elems = Txt(absFile.file.getName))) :: NoElems
      case _ => NoElems
    }
      ...
      case Some(comment) if (!isReduced) =>
        ...
        val example =
          orEmpty(comment.example) {
            Div(
              `class` = "block",
              elems = Txt(
                s"Example${if (comment.example.lengthIs > 1) "s" else ""}:") ::
                Ol(elems = {
                val exampleXml: List[Elems] = for (ex <- comment.example)
                  yield Li(`class` = "cmt", elems = bodyToHtml(ex)) :: NoElems
        ...
        val note: Elems =
          orEmpty(comment.note) {
            dt("Note") ::
              Dd(elems = {
              val noteXml: List[Elems] = for (note <- comment.note)
                yield Span(`class` = "cmt", elems = bodyToHtml(note)) :: NoElems
        ...
        val seeAlso: Elems =
          orEmpty(comment.see) {
            dt("See also") ::
              Dd(elems = {
              val seeXml: List[Elems] = for (see <- comment.see)
                yield Span(`class` = "cmt", elems = bodyToHtml(see)) :: NoElems
        ...
        val todo: Elems =
          orEmpty(comment.todo) {
            dt("To do") ::
              Dd(elems = {
              val todoXml: List[Elems] = for (todo <- comment.todo)
    ...
    val linearization = mbr match {
      case dtpl: DocTemplateEntity
        ...
        Div(
          ...
          elems =
            Span(`class` = "toggle", elems = Txt("Linear Supertypes")) ::
              Div(`class` = "superTypes hiddenContent",
                  elems = typesToHtml(dtpl.linearizationTypes,
                                      hasLinks = true,
                                      sep = Txt(", ")))
        ) :: NoElems
      case _ => NoElems
      ...
      case dtpl: DocTemplateEntity if isSelf && !isReduced =>
        ...
        if (subs.nonEmpty)
          Div(
            `class` = "toggleContainer block",
            elems =
              Span(`class` = "toggle", elems = Txt("Known Subclasses")) ::
                Div(`class` = "subClasses hiddenContent",
                    elems =
                      templatesToHtml(subs.toList.sorted(Entity.EntityOrdering),
                                      Txt(", ")))
          ) :: NoElems
        else NoElems
      case _ => NoElems
    ...
    def diagramDiv(description: String, diagId: String)(
        diagramSvg: Elems): Elems =
      Div(
        `class` = "toggleContainer block diagram-container",
        id = diagId + "-container",
        elems =
          List(Span(`class` = "toggle diagram-link", elems = Txt(description)),
               Div(`class` = "diagram hiddenContent",
                   id = diagId,
                   elems = diagramSvg))
      ) :: NoElems

    def ifDiags(genDiag: DocTemplateEntity => Option[Diagram])(
        embedDiagSvg: Elems => Elems): Elems =
      mbr match {
        case dtpl: DocTemplateEntity
            if s.docDiagrams.value && isSelf && !isReduced =>
          genDiag(dtpl)
            .map(diag => embedDiagSvg(generator.generate(diag, tpl, this)))
            .getOrElse(NoElems)
        case _ => NoElems
      }

    val typeHierarchy = ifDiags(_.inheritanceDiagram)(
      diagramDiv("Type Hierarchy", "inheritance-diagram"))
    val contentHierarchy = ifDiags(_.contentDiagram)(
      diagramDiv("Content Hierarchy", "content-diagram"))

    memberComment ++ authorComment ++ paramComments ++ attributesBlock ++ linearization ++ subclasses ++ typeHierarchy ++ contentHierarchy
  }

  def boundsToHtml(hi: Option[TypeEntity],
                   lo: Option[TypeEntity],
                   hasLinks: Boolean): Elems = {
      ...
      case PrivateInTemplate(owner) =>
        Some(
          Paragraph(
            Chain(
              List(CText("private["),
                   EntityLink(comment.Text(owner.qualifiedName),
                              LinkToTpl(owner)),
                   CText("]")))))
  ...
  def signature(mbr: MemberEntity,
                isSelf: Boolean,
                isReduced: Boolean = false): Elems = {
      ...
      Span(
        ...
        elems =
          Span(
            `class` = "modifier",
            elems =
              mbr.flags.flatMap(flag => inlineToHtml(flag.text) :+ Txt(" "))) ::
        ...
        elems = {
          ...
          val nameHtml: Elem = {
            ...
            val title = if (encoded != value) {
              "gt4s: " + encoded + (if (titleDepr == null) ""
                                    else ". " + titleDepr)
            } else {
              titleDepr
            }
          ...
          if (!nameLink.isEmpty)
            A(title = memberToShortCommentTitleTag(mbr),
              href = nameLink,
              elems = nameHtml)
          else nameHtml
        } :: {
          def tparamsToHtml(mbr: Any): Elems = mbr match {
            case hk: HigherKinded =>
              ...
              if (tpss.isEmpty) NoElems
              else {
                def tparam0(tp: TypeParam): Elems =
                  Span(
                    name = tp.name,
                    elems = Txt(tp.variance + tp.name) :: tparamsToHtml(tp) ++ boundsToHtml(
                      tp.hi,
                      tp.lo,
                      hasLinks))
                def tparams0(tpss: List[TypeParam]): Elems =
                  (tpss: @unchecked) match {
                    case tp :: Nil => tparam0(tp)
                    case tp :: tps => tparam0(tp) ++ Txt(", ") ++ tparams0(tps)
                  }
                Span(`class` = "tparams",
                     elems = Txt("[") :: (tparams0(tpss) :+ Txt("]")))
              }
            case _ => NoElems
          }
          tparamsToHtml(mbr)
        } ++ {
          if (isReduced) NoElems
          else {
            def paramsToHtml(vlsss: List[List[ValueParam]]): Elems = {
              ...
              def implicitCheck(vlss: List[ValueParam]): Elems = vlss match {
                case vl :: vls =>
                  if (vl.isImplicit) {
                    Span(`class` = "implicit", elems = Txt("implicit "))
                  } else Txt("")
                case _ => Txt("")
              }
              vlsss map { vlss =>
                Span(`class` = "params",
                     elems = Txt("(") :: implicitCheck(vlss) ++ params0(vlss) :+ Txt(
                       ")"))
              }
            }
            mbr match {
          ...
          if (isReduced) NoElems
            ...
            mbr match {
              case tme: MemberEntity
                ...
                Span(`class` = "result",
                     elems = Txt(": ") :: typeToHtml(tme.resultType, hasLinks))
              ...
              case alt: MemberEntity with AliasType =>
                Span(`class` = "result alias",
                     elems = Txt(" = ") :: typeToHtml(alt.alias, hasLinks))

              case tpl: MemberTemplateEntity if tpl.parentTypes.nonEmpty =>
                Span(`class` = "result",
                     elems = Txt(" extends ") :: typeToHtml(
                       tpl.parentTypes.map(_._2),
                       hasLinks))
      ...
      if (index == from) {
        member match {
          case mbr: DocTemplateEntity =>
            ...
            myXml ++= Span(
              `class` = "name",
              elems = A(href = link, elems = Txt(str.substring(from, to))))
          case mbr: MemberEntity =>
            val anchor = "#" + mbr.signature
            val link = relativeLinkTo(mbr.inTemplate)
            myXml ++= Span(`class` = "name",
                           elems = A(href = link + anchor,
                                     elems = Txt(str.substring(from, to))))
        }
        index = to
      }
    }
  ...
  private def argumentsToHtml(argss: List[ValueArgument]): Elems = {
    def argumentsToHtml0(argss: List[ValueArgument]): Elems = argss match {
      ...
      case arg :: args =>
        argumentToHtml(arg) ++ Txt(", ") ++ argumentsToHtml0(args)
    }
    Span(`class` = "args",
         elems = (Txt("(") :: argumentsToHtml0(argss)) :+ Txt(")"))
  }

  private def argumentToHtml(arg: ValueArgument): Elems = {
    Span(elems = arg.parameter match {
  ...
  private def constraintToHtml(constraint: Constraint): Elems =
    constraint match {
      ...
      case impl: ImplicitInScopeConstraint =>
        Txt("an implicit value of type ") ++ typeToHtml(impl.implicitType,
                                                        hasLinks = true) ++ Txt(
          " is in scope")
      ...
      case lb: LowerBoundedTypeParamConstraint =>
        Txt(
          lb.typeParamName + " is a superclass of " + lb.lowerBound.name + " (" + lb.typeParamName + " >: ") ++
          typeToHtml(lb.lowerBound, hasLinks = true) ++ Txt(")")
      case ub: UpperBoundedTypeParamConstraint =>
        Txt(
          ub.typeParamName + " is a subclass of " + ub.upperBound.name + " (" + ub.typeParamName + " <: ") ++
