...
class IndexScript(universe: doc.Universe) extends Page {
    ...
    val pairs = allPackagesWithTemplates.toIterable
      .map(_ match {
        case (pack, templates) => {
          ...
          val ary = merged.keys.toVector.sortBy(_.toLowerCase).map {
            key =>
              ...
              val pairs = merged(key).flatMap { t: DocTemplateEntity =>
                ...
                Seq(
                  ...
                  "kind" -> kind,
                  s"members_$kind" -> membersToJSON(
                    t.members.toVector.filter(!_.isShadowedOrAmbiguousImplicit),
                    t),
  ...
  private def mergeByQualifiedName(source: List[DocTemplateEntity])
    : collection.mutable.Map[String, List[DocTemplateEntity]] = {
  ...
  def membersToJSON(entities: Vector[MemberEntity],
                    parent: DocTemplateEntity): JSONArray =
    JSONArray(entities.map(memberToJSON(_, parent)))

  private def memberToJSON(mbr: MemberEntity,
                           parent: DocTemplateEntity): JSONObject = {
    ...
    def jsonObject(m: MemberEntity): JSONObject =
      JSONObject(
        Map(
          "label" -> "[^\\.]*\\.([^#]+#)?".r
            .replaceAllIn(m.definitionName, ""), // member name
    ...
    mbr match {
      ...
      case e @ (_: Class | _: Trait)
          if parent.isRootPackage || !parent.isPackage =>
        jsonObject(e)
      case m: MemberEntity =>
        JSONObject(
