...
trait ModelFactoryImplicitSupport {
  ...
  /** This is a flag that indicates whether to eliminate implicits that cannot be satisfied within the current scope.
    ...
    */
  class ImplicitNotFound(tpe: Type)
      extends Exception("No implicit of type " + tpe + " found in scope.")
  ...
  def makeImplicitConversions(
    ...
    if (!(sym.isClass || sym.isTrait || sym == AnyRefClass) || sym == NothingClass || sym == NullClass)
      ...
      val results = global.analyzer.allViewsFrom(sym.tpe_*,
                                                 context,
                                                 sym.typeParams) ++
        global.analyzer.allViewsFrom(byNameType(sym.tpe_*),
                                     context,
                                     sym.typeParams)
        ...
        uniteConstraints(constr) match {
          case (loBounds, upBounds) =>
            (loBounds filter (_ != NothingTpe),
             upBounds filter (up => up != AnyTpe && up != SingletonClass.tpe)) match {
              case (Nil, Nil) =>
                Nil
              ...
              case other =>
                ...
                error(
                  "Scaladoc implicits: Error computing lub/glb for: " + ((tparam,
                                                                          constr)) + ":\n" + other)
                Nil
            }
        }
  ...
  class ImplicitConversionImpl(val sym: Symbol,
    ...
    def targetTypeComponents: List[(TemplateEntity, TypeEntity)] =
      makeParentTypes(toType, None, inTpl)
  ...
  /**
    ...
    */
  def makeShadowingTable(
      members: List[MemberImpl],
    ...
    val convsByMember =
      convs.foldLeft(Map.empty[MemberImpl, ImplicitConversionImpl]) {
      ...
      for (member <- conv.memberImpls) {
        ...
        val ambiguous = otherConvMembers.get(sym1.name).toList.flatten filter {
          other =>
            ...
            !isDistinguishableFrom(tpe1, tpe2) || !isDistinguishableFrom(tpe2,
                                                                         tpe1)
        }
