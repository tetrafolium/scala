...
class ModelFactory(val global: Global, val settings: doc.Settings) {
  ...
  def templatesCount =
    docTemplatesCache.count(_._2.isDocTemplate) - droppedPackages.size
  ...
  protected val docTemplatesCache =
  ...
  abstract class EntityImpl(val sym: Symbol, val inTpl: TemplateImpl)
      extends Entity {
    ...
    def annotations =
      sym.annotations.filterNot(_.atp =:= typeOf[macroImpl]).map(makeAnnotation)
    def inPackageObject: Boolean =
      sym.owner.isModuleClass && sym.owner.sourceModule.isPackageObject
  ...
  abstract class MemberImpl(sym: Symbol, inTpl: DocTemplateImpl)
    ...
    lazy val comment =
      thisFactory.comment(commentCarryingSymbol(sym), linkTarget, inTpl)
    ...
    def flags = {
      ...
      if (!sym.isTrait && (sym hasFlag Flags.DEFERRED) && (!isImplicitlyInherited))
        fgs += Paragraph(Text("abstract"))
    ...
    lazy val signatureCompat = {
      ...
      def tParams(mbr: Any): String = mbr match {
        case hk: HigherKinded if hk.typeParams.nonEmpty =>
          def boundsToString(hi: Option[TypeEntity],
          ...
          "[" + hk.typeParams
            .map(tp =>
              tp.variance + tp.name + tParams(tp) + boundsToString(tp.hi,
                                                                   tp.lo))
            .mkString(", ") + "]"
        case _ => ""
      }
  ...
  abstract class DocTemplateImpl(sym: Symbol, inTpl: DocTemplateImpl)
    ...
    def sourceUrl = {
      ...
      if (!settings.docsourceurl.isDefault)
        inSource map {
          case (file, line) =>
            val filePathExt =
              fixPath(file.path).replaceFirst("^" + assumedSourceRoot, "")
            val (filePath, fileExt) = filePathExt.splitAt(
              filePathExt.indexOf(".", filePathExt.lastIndexOf("/")))
            val tplOwner = this.inTemplate.qualifiedName
            val tplName = this.name
            val patches = new Regex(
              """€\{(FILE_PATH|FILE_EXT|FILE_PATH_EXT|FILE_LINE|TPL_OWNER|TPL_NAME)\}""")
    ...
    def completeModel(): Unit = {
      ...
      members :::= memberSymsLazy.map(
        modelCreation.createLazyTemplateMember(_, this))
  ...
  abstract class NonTemplateMemberImpl(
    ...
    override lazy val comment = {
      ...
      inRealTpl flatMap (tpl =>
        thisFactory.comment(commentCarryingSymbol(sym), tpl, tpl))
    }

    override def inDefinitionTemplates =
      useCaseOf.fold(super.inDefinitionTemplates)(_.inDefinitionTemplates)
  ...
  abstract class NonTemplateParamMemberImpl(
      sym: Symbol,
      conversion: Option[ImplicitConversionImpl],
      useCaseOf: Option[MemberImpl],
      inTpl: DocTemplateImpl)
  ...
  abstract class ParameterImpl(val sym: Symbol, val inTpl: TemplateImpl)
      extends ParameterEntity {
    val name = optimize(sym.nameString)
  }
  ...
  object modelCreation {
          ...
          modelCreation.createTemplate(RootPackage, null) match {
            ...
            case _ =>
              throw new IllegalStateException(
                "Scaladoc: Unable to create root package!")
          }
      }
    ...
    def createTemplate(aSym: Symbol,
      ...
      def createDocTemplate(bSym: Symbol,
        ...
        if (bSym.isAliasType && bSym != AnyRefClass)
          ...
          new DocTemplateImpl(bSym, inTpl) with TypeBoundsImpl
          with AbstractType {
          ...
          with Trait {} else if (bSym.isClass && bSym.asClass.baseClasses
                                   .contains(AnnotationClass))
          new DocTemplateImpl(bSym, inTpl)
          with model.AnnotationClass {} else if (bSym.isClass || bSym == AnyRefClass)
          new DocTemplateImpl(bSym, inTpl) with Class {} else
          throw new IllegalArgumentException(
            s"'$bSym' isn't a class, trait or object thus cannot be built as a documentable template.")
      }
    ...
    /**
      ...
      */
    def createLazyTemplateMember(aSym: Symbol,
      ...
      def createNoDocMemberTemplate(
        ...
        if (bSym.isModule || (bSym.isAliasType && bSym.tpe.typeSymbol.isModule))
          ...
          with Object {} else if (bSym.isTrait || (bSym.isAliasType && bSym.tpe.typeSymbol.isTrait))
          new MemberTemplateImpl(bSym, inTpl)
          with Trait {} else if (bSym.isClass || (bSym.isAliasType && bSym.tpe.typeSymbol.isClass))
          new MemberTemplateImpl(bSym, inTpl) with Class {} else
          throw new IllegalArgumentException(
            s"'$bSym' isn't a class, trait or object thus cannot be built as a member template.")
      }
  ...
  def makeAnnotation(
      annot: AnnotationInfo): scala.tools.nsc.doc.model.Annotation = {
  ...
  /** */
  def makeValueParam(aSym: Symbol,
                     inTpl: DocTemplateImpl,
  ...
  /** */
  def makeTypeInTemplateContext(aType: Type,
                                inTpl: TemplateImpl,
  ...
  def makeParentTypes(aType: Type,
  ...
  def classExcluded(clazz: TemplateEntity): Boolean =
    settings.hardcoded.isExcluded(clazz.qualifiedName)
  ...
  def implicitExcluded(convertorMethod: String): Boolean =
    settings.hiddenImplicits(convertorMethod)
