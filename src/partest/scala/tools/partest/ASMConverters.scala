...
object ASMConverters {
  ...
  implicit class RichInstructionLists(val self: List[Instruction])
    ...
    def dropStaleLabels = {
      ...
      val usedLabels: Set[Instruction] =
        self.iterator.flatMap(referencedLabels).toSet
  ...
  sealed abstract class Instruction extends Product {
    ...
    final override def toString() = {
      ...
      productPrefix + (
        if (printOpcode)
          Iterator(opcodeToString(opcode)) ++ productIterator.drop(1).map(q)
        else productIterator.map(q)
      ).mkString("(", ", ", ")")
    }
  }

  case class Method(instructions: List[Instruction],
  ...
  case class LookupSwitch(opcode: Int,
                          dflt: Label,
                          keys: List[Int],
                          labels: List[Label])
      extends Instruction
  case class TableSwitch(opcode: Int,
                         min: Int,
                         max: Int,
                         dflt: Label,
                         labels: List[Label])
      extends Instruction
  case class Invoke(opcode: Int,
                    owner: String,
                    name: String,
                    desc: String,
                    itf: Boolean)
      extends Instruction
  case class InvokeDynamic(opcode: Int,
                           name: String,
                           desc: String,
                           bsm: MethodHandle,
                           bsmArgs: List[AnyRef])
  ...
  case class MethodHandle(tag: Int,
                          owner: String,
                          name: String,
                          desc: String,
                          itf: Boolean)

  case class ExceptionHandler(start: Label,
                              end: Label,
                              handler: Label,
                              desc: Option[String])
  ...
  class AsmToScala(asmMethod: t.MethodNode) {
    ...
    private def convertHandlers(
      ...
      method.tryCatchBlocks.iterator.asScala
        .map(
          h =>
            ExceptionHandler(applyLabel(h.start),
                             applyLabel(h.end),
    ...
    private def convertLocalVars(method: t.MethodNode): List[LocalVariable] = {
      method.localVariables.iterator.asScala
        .map(
          v =>
  ...
  /**
    ...
    */
  def equivalentBytecode(as: List[Instruction],
                         bs: List[Instruction],
    ...
    if (as.isEmpty) bs.isEmpty
      ...
      ((as.head, bs.head) match {
        ...
        case (LookupSwitch(op1, l1, keys1, ls1),
              LookupSwitch(op2, l2, keys2, ls2)) =>
          op1 == op2 && sameLabel(l1, l2) && keys1 == keys2 && sameLabels(ls1,
                                                                          ls2)
        case (TableSwitch(op1, min1, max1, l1, ls1),
  ...
  def applyToMethod(method: t.MethodNode,
  ...
  def applyToMethod(asmMethod: t.MethodNode, method: Method): Unit = {
    ...
    method.handlers.foreach(
      h =>
        asmMethod.visitTryCatchBlock(asmLabel(h.start),
                                     asmLabel(h.end),
                                     asmLabel(h.handler),
                                     h.desc.orNull))
    method.localVars.foreach(
      v =>
        asmMethod.visitLocalVariable(v.name,
                                     v.desc,
                                     v.signature.orNull,
                                     asmLabel(v.start),
                                     asmLabel(v.end),
  ...
  private def visitMethod(method: t.MethodNode,
    ...
    instruction match {
      ...
      case InvokeDynamic(op, name, desc, bsm, bsmArgs) =>
        method.visitInvokeDynamicInsn(name,
                                      desc,
