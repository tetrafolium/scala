...
object PartestDefaults {
  ...
  def javacCmd =
    prop("partest.javac_cmd") orElse jdkexec("javac") getOrElse "javac"
  def javaOpts =
    prop("partest.java_opts") getOrElse "" // opts when running java during tests
  ...
  def numThreads =
    math.max(
      1,
      prop("partest.threads") map (_.toInt) getOrElse runtime.availableProcessors)
  ...
  def printDurationThreshold =
    java.lang.Integer.getInteger("partest.print.duration.threshold.ms", 5000)
  ...
  private def jdkexec(name: String): Option[String] = {
    ...
    (Path(jdkHome) / "bin").walk
      .find(e =>
        e.name == name || isWin && e.name.equalsIgnoreCase(s"$name.exe"))
      .map(_.path)
  }
}
