...
class SBTRunner(config: RunnerSpec.Config,
  ...
  override val log = new ConsoleLog({
    val ptOverride =
      defs.collect { case ("partest.colors", v) => v.toBoolean }.lastOption
    ptOverride.getOrElse {
      val sbtOverride1 = sys.props.get("sbt.log.format").map(_.toBoolean)
      val sbtOverride2 =
        sys.props.get("sbt.log.noformat").map(s => !s.toBoolean)
    ...
    synchronized {
      eventHandler.handle(new Event {
        def fullyQualifiedName: String =
          pathSettings.testRoot.name + "/" + testSourcePath
  ...
  def makeStatus(t: TestState): (Status, OptionalThrowable) = t match {
    ...
    case Fail(_, reason, transcript) =>
      (Status.Failure,
       new OptionalThrowable(
         new TestFailedThrowable(reason, transcript.mkString("\n"))))
    case Crash(_, e, _) => (Status.Error, new OptionalThrowable(e))
  }
}
class TestFailedThrowable(reason: String, transcript: String)
    extends Throwable(reason + "\n\n" + transcript)
