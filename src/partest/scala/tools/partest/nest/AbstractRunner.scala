...
class AbstractRunner(val config: RunnerSpec.Config,
                     protected final val testSourcePath: String,
  ...
  def statusLine(state: TestState, durationMs: Long) = {
    ...
    def durationString =
      if (durationMs > PartestDefaults.printDurationThreshold)
        f"[duration ${(1.0 * durationMs) / 1000}%.2fs]"
      else ""
  ...
  def reportTest(state: TestState,
                 ...
                 durationMs: Long,
                 diffOnFail: Boolean,
                 logOnFail: Boolean): List[String] = {
    def errInfo: List[String] = {
      def showLog() =
        if (info.logFile.canRead)
          List(
            bold(
              cyan(
                s"##### Log file '${info.logFile}' from failed test #####\n")),
    ...
    if (terse) {
      if (state.isOk) { printDot(); Nil } else {
        printEx(); statusLine(state, durationMs) :: errInfo
      }
    } else {
  ...
  private[this] def passFailString(passed: Int,
                                   failed: Int,
                                   skipped: Int): String = {
  ...
  def issueSummaryReport(): Unit = {
    ...
    if (!summarizing) {
      ...
      val elapsed =
        if (elapsedMillis > 0)
          " (elapsed time: " + elapsedString(elapsedMillis) + ")"
        else ""
  ...
  def run(): Boolean = {
    ...
    if (config.optVersion) echo(versionMsg)
      ...
      val (individualTests, invalid) = config.parsed.residualArgs map (p =>
        Path(p)) partition denotesTestPath
      if (invalid.nonEmpty) {
        if (verbose)
          invalid foreach (p =>
            echoWarning(s"Discarding invalid test path " + p))
        else if (!terse)
      ...
      val kinds =
        (
          if (givenKinds.nonEmpty) givenKinds
            ...
            standardKinds // If no kinds, --grep, or individual tests were given, assume --all, unless there were invalid files specified
          else Nil
        )
      ...
      def testContributors = {
        List(
          ...
          if (kindsTests.isEmpty) ""
          else s"${kinds.size} named test categories",
          if (greppedTests.isEmpty) ""
          else s"${greppedTests.size} tests matching '$grepExpr'",
          if (individualTests.isEmpty) "" else "specified tests"
        ) filterNot (_ == "") mkString ", "
      }

      val allTests: Array[Path] = distinctBy(miscTests ++ kindsTests)(
        _.toCanonical).sortBy(_.toString).toArray
      val grouped = (allTests groupBy kindOf).toArray sortBy (x =>
        standardKinds indexOf x._1)
      ...
      val expectedFailureMessage =
        ...
        else s" (expecting $expectedFailures to fail)"
      echo(
        s"Selected $totalTests tests drawn from $testContributors$expectedFailureMessage\n")
      if (config.optNoExec)
        echoMixed(
          "Under --no-exec, tests will be compiled but not run! Runnable tests will be marked skipped!")
      ...
      val (_, millis) = timed {
        for ((kind, paths) <- grouped) {
          ...
          val results =
            runTestsForFiles(paths map (_.jfile.getAbsoluteFile), kind)
  ...
  def banner = {
    ...
    def relativize(path: String) =
      path
        .replace(baseDir, s"$$baseDir")
        .replace(pathSettings.srcDir.toString, "$sourceDir")
    val vmBin = javaHome + fileSeparator + "bin"
    val vmName =
      "%s (build %s, %s)".format(javaVmName, javaVmVersion, javaVmInfo)

    s"""|Partest version:     ${Properties.versionNumberString}
        |Compiler under test: ${relativize(
         fileManager.compilerUnderTest.getAbsolutePath)}
        |Scala version is:    $versionMsg
        |Scalac options are:  ${(scalacExtraArgs ++ scalacOpts.split(' '))
         .mkString(" ")}
        |Compilation Path:    ${relativize(
         FileManager.joinPaths(fileManager.testClassPath))}
  ...
  def onFinishTest(testFile: File,
                   result: TestState,
                   durationMs: Long): TestState = {
    result
  }
  ...
  def runTestsForFiles(kindFiles: Array[File],
    ...
    val futures = kindFiles map (f =>
      pool submit callable(runTest(f.getAbsoluteFile)))
