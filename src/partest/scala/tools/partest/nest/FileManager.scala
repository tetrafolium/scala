...
object FileManager {
  ...
  def joinPaths(paths: List[Path]) =
    ClassPath.join(paths.map(_.getAbsolutePath).distinct: _*)
  ...
  def compareFiles(original: File, revised: File): String = {
    compareContents(io.Source.fromFile(original).getLines.toSeq,
                    io.Source.fromFile(revised).getLines.toSeq,
                    original.getName,
                    revised.getName)
  }
  ...
  def compareContents(original: Seq[String],
    ...
    if (diff.getDeltas.isEmpty) ""
      ...
      difflib.DiffUtils
        .generateUnifiedDiff(originalName,
                             revisedName,
                             original.asJava,
                             diff,
                             1)
  ...
  def withTempFile[A](outFile: File, fileBase: String, lines: Seq[String])(
      body: File => A): A = {
...
class FileManager(val testClassLoader: URLClassLoader) {
  ...
  lazy val testClassPath =
    testClassLoader.getURLs().map(url => Path(new File(url.toURI))).toList
  ...
  private def findArtifact(name: String): Path = {
    val canaryClass =
      name match {
        ...
        case "reflect" =>
          Class.forName("scala.reflect.api.Symbols", false, testClassLoader)
        case "compiler" =>
          Class.forName("scala.tools.nsc.Main", false, testClassLoader)
        case "javaagent" =>
          Class.forName("scala.tools.partest.javaagent.ProfilingAgent",
                        false,
                        testClassLoader)
      }

    val path = Path(
      canaryClass.getProtectionDomain.getCodeSource.getLocation.getPath)
