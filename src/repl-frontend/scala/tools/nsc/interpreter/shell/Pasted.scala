...
abstract class Pasted(prompt: String,
  ...
  private def matchesString(line: String, target: String): Boolean =
    line.startsWith(target) || (line.nonEmpty && spacey(line.head) && matchesString(
      line.tail,
      target))
  ...
  private class PasteAnalyzer(val lines: List[String]) {
    val referenced = lines
      .flatMap(s => resReference.findAllIn(s.trim.stripPrefix("res")))
      .toSet
    val ActualPromptString = lines
      .find(matchesPrompt)
      .map(s =>
        if (matchesString(s, PromptString)) PromptString else AltPromptString)
      .getOrElse(PromptString)
    val cmds = lines
      .reduceLeft(append)
      .split(ActualPromptString)
      .filterNot(_.trim == "")
      .toList
