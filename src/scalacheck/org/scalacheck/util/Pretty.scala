...
object Pretty {
  ...
  def apply(f: Params => String): Pretty = new Pretty {
    def apply(p: Params) = f(p)
  }
  ...
  def break(s: String, lead: String, length: Int): String =
    if (s.length <= length || length <= 0) s
      ...
      s.substring(0, length) / break(lead + s.substring(length), lead, length)

  def format(s: String, lead: String, trail: String, width: Int) =
    s.linesIterator
      .map(l => break(lead + l + trail, "  ", width))
      .mkString("\n")
  ...
  implicit def prettyAny(t: Any) = Pretty { p =>
    toStrOrNull(t)
  }

  implicit def prettyString(t: String) = Pretty { p =>
    "\"" ++ escapeControlChars(t) ++ "\""
  }
  ...
  implicit def prettyTestRes(res: Test.Result) = Pretty { prms =>
    ...
    val s = res.status match {
      ...
      case Test.Failed(args, l) =>
        "Falsified after " + res.succeeded + " passed tests." / labels(l) / prettyArgs(
          args)(prms)
      case Test.Exhausted =>
        "Gave up after only " + res.succeeded + " passed tests. " +
          res.discarded + " tests were discarded."
      case Test.PropException(args, e, l) =>
        "Exception raised on property evaluation." / labels(l) / prettyArgs(
          args)(prms) /
          "> Exception: " + pretty(e, prms)
    }
    val t =
      if (prms.verbosity <= 1) "" else "Elapsed time: " + prettyTime(res.time)
