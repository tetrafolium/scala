...
sealed abstract class Gen[+T] extends Serializable { self =>
  ...
  def doPureApply(p: Gen.Parameters,
                  seed: Seed,
                  retries: Int = 100): Gen.R[T] = {
  ...
  def map[U](f: T => U): Gen[U] = gen { (p, seed) =>
    doApply(p, seed).map(f)
  }
  ...
  case class RetryUntilException(n: Int)
      extends RuntimeException(s"retryUntil failed after $n attempts")
  ...
  def retryUntil(p: T => Boolean, maxTries: Int): Gen[T] = {
    ...
    def loop(params: P, seed: Seed, tries: Int): R[T] =
      if (tries > maxTries) throw RetryUntilException(tries)
      else {
...
object Gen extends GenArities {
  ...
  sealed abstract class Parameters extends Serializable {
    ...
    private case class cp(size: Int = size, initialSeed: Option[Seed] = None)
        extends Parameters
  }
  ...
  object Choose {
    ...
    class IllegalBoundsError[A](low: A, high: A)
        extends IllegalArgumentException(
          s"invalid bounds: low=$low, high=$high")
  ...
  def sequence[C, T](gs: Iterable[Gen[T]])(
      implicit b: Buildable[T, C]): Gen[C] = {
    val g = gen { (p, seed) =>
  ...
  def sized[T](f: Int => Gen[T]): Gen[T] =
    gen { (p, seed) =>
      f(p.size).doApply(p, seed)
    }

  /** A generator that returns the current generation size */
  lazy val size: Gen[Int] = sized { sz =>
    sz
  }

  /** Creates a resized version of a generator */
  def resize[T](s: Int, g: Gen[T]) =
    gen((p, seed) => g.doApply(p.withSize(s), seed))
  ...
  def oneOf[T](g0: Gen[T], g1: Gen[T], gn: Gen[T]*): Gen[T] = {
    ...
    choose(0, gs.size - 1)
      .flatMap(gs(_))
      .suchThat(x => gs.exists(_.sieveCopy(x)))
  }
  ...
  def buildableOf[C, T](g: Gen[T])(implicit
                                   evb: Buildable[T, C],
                                   evt: C => Iterable[T]): Gen[C] =
    sized(s => choose(0, s max 0).flatMap(buildableOfN[C, T](_, g))) suchThat {
      c =>
  ...
  def containerOfN[C[_], T](n: Int, g: Gen[T])(
      implicit
      evb: Buildable[T, C[T]],
      evt: C[T] => Iterable[T]): Gen[C[T]] = buildableOfN[C[T], T](n, g)

  /** A convenience method for calling `buildableOf[C[T],T](g)`. */
  def containerOf[C[_], T](g: Gen[T])(implicit
                                      evb: Buildable[T, C[T]],
                                      evt: C[T] => Iterable[T]): Gen[C[T]] =
    buildableOf[C[T], T](g)

  /** A convenience method for calling `nonEmptyBuildableOf[C[T],T](g)`. */
  def nonEmptyContainerOf[C[_], T](g: Gen[T])(
      implicit
      evb: Buildable[T, C[T]],
      evt: C[T] => Iterable[T]): Gen[C[T]] = nonEmptyBuildableOf[C[T], T](g)
  ...
  def nonEmptyMap[T, U](g: => Gen[(T, U)]) =
    nonEmptyBuildableOf[Map[T, U], (T, U)](g)
  ...
  def infiniteStream[T](g: => Gen[T]): Gen[Stream[T]] = {
    ...
    gen { (p, seed0) =>
      new R[Stream[T]] {
        val result: Option[Stream[T]] =
          Some(unfold(seed0)(s => Some(g.pureApply(p, s) -> s.next)))
  ...
  def pick[T](n: Int, l: Iterable[T]): Gen[collection.Seq[T]] = {
    if (n > l.size || n < 0)
      throw new IllegalArgumentException(s"invalid choice: $n")
  ...
  def pick[T](n: Int, g1: Gen[T], g2: Gen[T], gn: Gen[T]*): Gen[Seq[T]] = {
    val gs = g1 +: g2 +: gn
    pick(n, 0 until gs.size)
      .flatMap(idxs => sequence[List[T], T](idxs.toList.map(gs(_))))
  ...
  lazy val calendar: Gen[Calendar] = {
    ...
    val firstDayOfYearGen: Gen[Calendar] =
      for { c <- calendar; y <- yearGen(c) } yield
        adjust(c)(_.set(y, JANUARY, 1))

    val lastDayOfYearGen: Gen[Calendar] =
      for { c <- calendar; y <- yearGen(c) } yield
        adjust(c)(_.set(y, DECEMBER, 31))
    ...
    val lastDayOfMonthGen: Gen[Calendar] =
      calendar.map(c =>
        adjust(c)(_.set(DAY_OF_MONTH, c.getActualMaximum(DAY_OF_MONTH))))
  ...
  /**
    ...
    */
  val duration: Gen[Duration] = frequency(1 -> const(Duration.Inf),
