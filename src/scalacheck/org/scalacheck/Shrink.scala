...
object Shrink extends ShrinkLowPriority {
  ...
  implicit def shrinkContainer[C[_], T](implicit v: C[T] => Iterable[T],
                                        s: Shrink[T],
                                        b: Buildable[T, C[T]]): Shrink[C[T]] =
    Shrink { xs: C[T] =>
      val ys = v(xs)
      val zs = ys.toStream
      removeChunks(ys.size, zs)
        .lazyAppendedAll(shrinkOne(zs))
        .map(b.fromIterable)
    }

  /** Shrink instance of container2 */
  implicit def shrinkContainer2[C[_, _], T, U](
      implicit v: C[T, U] => Iterable[(T, U)],
      s: Shrink[(T, U)],
      b: Buildable[(T, U), C[T, U]]): Shrink[C[T, U]] = Shrink { xs: C[T, U] =>
    val ys = v(xs)
    val zs = ys.toStream
  ...
  private def removeChunks[T](n: Int, xs: Stream[T]): Stream[Stream[T]] =
    if (xs.isEmpty) empty
      ...
      lazy val xs3 =
        for (ys1 <- removeChunks(n1, xs1) if !ys1.isEmpty)
          yield ys1 lazyAppendedAll xs2
      lazy val xs4 =
        for (ys2 <- removeChunks(n2, xs2) if !ys2.isEmpty)
          yield xs1 lazyAppendedAll ys2
