...
object Test {
  ...
  sealed abstract class Parameters {
    ...
    def withCustomClassLoader(
        customClassLoader: Option[ClassLoader]): Parameters = cp(
  ...
  object Parameters {
    ...
    val defaultVerbose: Parameters =
      default.withTestCallback(ConsoleReporter(2))
  }
  ...
  sealed case class Failed(args: List[Arg[Any]], labels: Set[String])
      extends Status
  ...
  sealed case class PropException(args: List[Arg[Any]],
                                  e: Throwable,
                                  labels: Set[String])
      extends Status
  ...
  trait TestCallback { self =>
    ...
    def chain(testCallback: TestCallback): TestCallback = new TestCallback {
      override def onPropEval(name: String,
                              threadIdx: Int,
                              succeeded: Int,
                              discarded: Int): Unit = {
  ...
  private[scalacheck] lazy val cmdLineParser = new CmdLineParser {
    ...
    def parseParams(
        args: Array[String]): (Parameters => Parameters, List[String]) = {
      val (optMap, us) = parseArgs(args)
      val params = (p: Parameters) =>
        p.withMinSuccessfulTests(optMap(OptMinSuccess): Int)
  ...
  def checkProperties(prms: Parameters,
                      ps: Properties): collection.Seq[(String, Result)] = {
