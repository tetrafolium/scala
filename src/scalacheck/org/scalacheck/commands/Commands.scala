...
trait Commands {
  ...
  /** A command that runs a sequence of other commands */
  private final class CommandSequence(val head: Command,
                                      snd: Command,
                                      rest: Command*)
    ...
    val tail: Command =
      if (rest.isEmpty) snd
      else new CommandSequence(snd, rest.head, rest.tail: _*)
  ...
  private def runSeqCmds(sut: Sut,
                         s0: State,
                         cs: Commands): (Prop, State, List[Try[String]]) =
    cs.foldLeft((Prop.proved, s0, List[Try[String]]())) {
      case ((p, s, rs), c) =>
  ...
  private def runParCmds(
    ...
    def run(endStates: List[State],
            cs: Commands): Future[(Prop, List[(Command, Try[String])])] =
      Future {
  ...
  private def runActions(sut: Sut, as: Actions, finalize: => Unit): Prop = {
    try {
      ...
      if (as.parCmds.isEmpty) p1 :| l1
        ...
        propAnd(
          p1.flatMap { r =>
            if (!r.success) finalize; Prop(prms => r)
          } :| l1, {
            try {
  ...
  private def actions(threadCount: Int, maxParComb: Int): Gen[Actions] = {
    ...
    def sizedCmds(s: State)(sz: Int): Gen[(State, Commands)] = {
      ...
      l.foldLeft(const((s, Nil: Commands))) {
        case (g, ()) =>
          for {
            (s0, cs) <- g
    ...
    val parSz = {
      ...
      def seqs(n: Long, m: Long): Long =
        if (n == 1) 1
        else math.round(math.pow(n.toDouble, m.toDouble)) * seqs(n - 1, m)
  ...
  private def scan[T, U](xs: List[T])(f: (T, List[T]) => U): List[U] =
    xs match {
      case Nil     => Nil
