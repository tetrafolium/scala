...
object PartialFunction {
  ...
  /** Composite function produced by `PartialFunction#andThen` method
    */
  private class Combined[-A, B, +C](pf: PartialFunction[A, B],
                                    k: PartialFunction[B, C])
      extends PartialFunction[A, C]
      with Serializable {
    def isDefinedAt(x: A): Boolean = {
    ...
    override def applyOrElse[A1 <: A, C1 >: C](x: A1, default: A1 => C1): C1 = {
      ...
      if (!fallbackOccurred(pfv)) k.applyOrElse(pfv, (_: B) => default(x))
      else default(x)
    }
  }
  ...
  private def fallbackOccurred[B](x: B) =
