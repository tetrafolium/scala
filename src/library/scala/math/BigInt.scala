...
final class BigInt(val bigInteger: BigInteger)
  ...
  def isProbablePrime(certainty: Int) =
    this.bigInteger.isProbablePrime(certainty)
  ...
  /** Create a `NumericRange[BigInt]` in range `[start;end)`
    ...
    *  @return       the range
    */
  def until(end: BigInt, step: BigInt = BigInt(1)) =
    Range.BigInt(this, end, step)

  /** Like until, but inclusive of the end value.
    */
  def to(end: BigInt, step: BigInt = BigInt(1)) =
    Range.BigInt.inclusive(this, end, step)
