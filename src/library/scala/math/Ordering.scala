...
trait LowPriorityOrderingImplicits {
  ...
  implicit def comparatorToOrdering[A](
      implicit cmp: Comparator[A]): Ordering[A] = new Ordering[A] {
...
object Ordering extends LowPriorityOrderingImplicits {
  ...
  sealed trait CachedReverse[T] extends Ordering[T] {
    ...
    override final def isReverseOf(other: Ordering[_]): Boolean =
      other eq _reverse
  }

  /** A reverse ordering */
  private final class Reverse[T](private[Ordering] val outer: Ordering[T])
      extends Ordering[T] {
  ...
  private final class IterableOrdering[CC[X] <: Iterable[X], T](
      private val ord: Ordering[T])
      extends Ordering[CC[T]] {
  ...
  trait ExtraImplicits {
    ...
    /** Not in the standard scope due to the potential for divergence:
      ...
      */
    implicit def seqOrdering[CC[X] <: scala.collection.Seq[X], T](
        implicit ord: Ordering[T]): Ordering[CC[T]] =
      new IterableOrdering[CC, T](ord)

    implicit def sortedSetOrdering[CC[X] <: scala.collection.SortedSet[X], T](
        implicit ord: Ordering[T]): Ordering[CC[T]] =
      new IterableOrdering[CC, T](ord)
    ...
    implicit def infixOrderingOps[T](x: T)(
        implicit ord: Ordering[T]): Ordering[T]#OrderingOps =
  ...
  object Float {
    ...
    trait IeeeOrdering extends Ordering[Float] {
      ...
      override def max[U <: Float](x: U, y: U): U =
        math.max(x, y).asInstanceOf[U]
      override def min[U <: Float](x: U, y: U): U =
        math.min(x, y).asInstanceOf[U]
    }
    implicit object IeeeOrdering extends IeeeOrdering
  }
  @deprecated(
    "There are multiple ways to order Floats (Ordering.Float.TotalOrdering, " +
  ...
  @deprecated(
    "There are multiple ways to order Doubles (Ordering.Double.TotalOrdering, " +
  ...
  /** @deprecated Iterables are not guaranteed to have a consistent order, so the `Ordering`
    ...
    */
  @deprecated(
    "Iterables are not guaranteed to have a consistent order; if using a type with a " +
      "consistent order (e.g. Seq), use its Ordering (found in the Ordering.Implicits object)",
    since = "2.13.0"
  )
  ...
  private[this] final class Tuple2Ordering[T1, T2](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2])
      extends Ordering[(T1, T2)] {
  ...
  implicit def Tuple3[T1, T2, T3](implicit ord1: Ordering[T1],
                                  ord2: Ordering[T2],
                                  ord3: Ordering[T3]): Ordering[(T1, T2, T3)] =
    new Tuple3Ordering(ord1, ord2, ord3)

  private[this] final class Tuple3Ordering[T1, T2, T3](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3])
      extends Ordering[(T1, T2, T3)] {
  ...
  private[this] final class Tuple4Ordering[T1, T2, T3, T4](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
  ...
  private[this] final class Tuple5Ordering[T1, T2, T3, T4, T5](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
      private val ord4: Ordering[T4],
  ...
  private[this] final class Tuple6Ordering[T1, T2, T3, T4, T5, T6](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
      private val ord4: Ordering[T4],
    ...
    override def hashCode(): Int =
      (ord1, ord2, ord3, ord4, ord5, ord6).hashCode()
  }
  ...
  private[this] final class Tuple7Ordering[T1, T2, T3, T4, T5, T6, T7](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
      private val ord4: Ordering[T4],
    ...
    def compare(x: (T1, T2, T3, T4, T5, T6, T7),
    ...
    override def hashCode(): Int =
      (ord1, ord2, ord3, ord4, ord5, ord6, ord7).hashCode()
  }
  ...
  private[this] final class Tuple8Ordering[T1, T2, T3, T4, T5, T6, T7, T8](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
      private val ord4: Ordering[T4],
    ...
    def compare(x: (T1, T2, T3, T4, T5, T6, T7, T8),
    ...
    override def hashCode(): Int =
      (ord1, ord2, ord3, ord4, ord5, ord6, ord7, ord8).hashCode()
  }
  ...
  private[this] final class Tuple9Ordering[T1, T2, T3, T4, T5, T6, T7, T8, T9](
      private val ord1: Ordering[T1],
      private val ord2: Ordering[T2],
      private val ord3: Ordering[T3],
      private val ord4: Ordering[T4],
    ...
    def compare(x: (T1, T2, T3, T4, T5, T6, T7, T8, T9),
    ...
    override def hashCode(): Int =
      (ord1, ord2, ord3, ord4, ord5, ord6, ord7, ord8, ord9).hashCode()
  }
}
