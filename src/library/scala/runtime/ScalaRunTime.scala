...
object ScalaRunTime {
  ...
  private def isArrayClass(clazz: jClass[_], atLevel: Int): Boolean =
    clazz.isArray && (atLevel == 1 || isArrayClass(clazz.getComponentType,
                                                   atLevel - 1))

  // A helper method to make my life in the pattern matcher a lot easier.
  def drop[Repr](coll: Repr, num: Int)(implicit iterable: IsIterable[Repr] {
    type C <: Repr
  }): Repr =
  ...
  @inline def array_length(xs: AnyRef): Int =
    java.lang.reflect.Array.getLength(xs)
  ...
  def stringOf(arg: Any, maxElements: Int): String = {
    ...
    def isTuple(x: Any) =
      x != null && x.getClass.getName.startsWith("scala.Tuple")
    ...
    def isXmlMetaData(potentialSubClass: Class[_]) =
      isSubClassOf(potentialSubClass, "scala.xml.MetaData")
    ...
    def useOwnToString(x: Any) = x match {
      ...
      case x: Iterable[_] =>
        (!x.isInstanceOf[StrictOptimizedIterableOps[_, AnyConstr, _]]) || !isScalaClass(
          x) || isScalaCompilerClass(x) || isXmlNode(x.getClass) || isXmlMetaData(
          x.getClass)
    ...
    def inner(arg: Any): String = arg match {
      ...
      case x: Iterable[_] =>
        x.iterator take maxElements map inner mkString (x.collectionClassName + "(", ", ", ")")
