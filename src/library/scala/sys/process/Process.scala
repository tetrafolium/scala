...
trait ProcessCreation {
  ...
  /** Creates a [[scala.sys.process.ProcessBuilder]] from a sequence of `String`,
    ...
    */
  def apply(command: scala.collection.Seq[String]): ProcessBuilder =
    apply(command, None)
  ...
  def apply(command: String,
            arguments: scala.collection.Seq[String]): ProcessBuilder =
  ...
  /** Creates a [[scala.sys.process.ProcessBuilder]] with working dir set to `File` and extra
    ...
    */
  def apply(command: scala.collection.Seq[String],
            cwd: File,
            extraEnv: (String, String)*): ProcessBuilder =
    apply(command, Some(cwd), extraEnv: _*)
  ...
  /** Creates a sequence of [[scala.sys.process.ProcessBuilder.Source]] from a sequence of
    ...
    */
  def applySeq[T](builders: scala.collection.Seq[T])(
      implicit convert: T => Source): scala.collection.Seq[Source] =
...
trait ProcessImplicits {
  ...
  /** Return a sequence of [[scala.sys.process.ProcessBuilder.Source]] from a sequence
    ...
    */
  implicit def buildersToProcess[T](builders: scala.collection.Seq[T])(
      implicit convert: T => Source): scala.collection.Seq[Source] =
    applySeq(builders)
  ...
  implicit def stringSeqToProcess(
      command: scala.collection.Seq[String]): ProcessBuilder = apply(command)
}
