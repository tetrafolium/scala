...
abstract class Enumeration(initial: Int) extends Serializable {
  ...
  protected def readResolve(): AnyRef =
    thisenum.getClass.getField(MODULE_INSTANCE_NAME).get(null)
  ...
  class ValueSet private[ValueSet] (private[this] var nnIds: immutable.BitSet)
    ...
    override def iteratorFrom(start: Value) =
      nnIds iteratorFrom start.id map (id => thisenum.apply(bottomId + id))
    ...
    override protected def fromSpecific(coll: IterableOnce[Value]) =
      ValueSet.fromSpecific(coll)
    override protected def newSpecificBuilder = ValueSet.newBuilder

    def map(f: Value => Value): ValueSet =
      fromSpecific(new View.Map(toIterable, f))
    def flatMap(f: Value => IterableOnce[Value]): ValueSet =
      fromSpecific(new View.FlatMap(toIterable, f))

    // necessary for disambiguation:
    override def map[B](f: Value => B)(implicit @implicitNotFound(
                                         ValueSet.ordMsg) ev: Ordering[B])
      : SortedIterableCC[B] =
      super[SortedSet].map[B](f)
    override def flatMap[B](f: Value => IterableOnce[B])(
        implicit @implicitNotFound(ValueSet.ordMsg) ev: Ordering[B])
      : SortedIterableCC[B] =
      super[SortedSet].flatMap[B](f)
    override def zip[B](that: IterableOnce[B])(
        implicit @implicitNotFound(ValueSet.zipOrdMsg) ev: Ordering[(Value, B)])
      : SortedIterableCC[(Value, B)] =
      super[SortedSet].zip[B](that)
    override def collect[B](pf: PartialFunction[Value, B])(
        implicit @implicitNotFound(ValueSet.ordMsg) ev: Ordering[B])
      : SortedIterableCC[B] =
  ...
  object ValueSet extends SpecificIterableFactory[Value, ValueSet] {
    private final val ordMsg =
      "No implicit Ordering[${B}] found to build a SortedSet[${B}]. You may want to upcast to a Set[Value] first by calling `unsorted`."
    private final val zipOrdMsg =
      "No implicit Ordering[${B}] found to build a SortedSet[(Value, ${B})]. You may want to upcast to a Set[Value] first by calling `unsorted`."
