...
object Duration {
  ...
  def apply(length: Double, unit: TimeUnit): Duration =
    fromNanos(unit.toNanos(1) * length)
  ...
  def apply(length: Long, unit: String): FiniteDuration =
    new FiniteDuration(length, Duration.timeUnit(unit))
...
final class FiniteDuration(val length: Long, val unit: TimeUnit)
  ...
  private[this] def unitString =
    timeUnitName(unit) + (if (length == 1) "" else "s")
  ...
  def *(factor: Long): FiniteDuration =
    new FiniteDuration(safeMul(length, factor), unit)
