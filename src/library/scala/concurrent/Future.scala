...
trait Future[+T] extends Awaitable[T] {
  ...
  def transform[S](s: T => S, f: Throwable => Throwable)(
    ...
    transform { t =>
      if (t.isInstanceOf[Success[T]]) t map s
        ...
        throw f(t.asInstanceOf[Failure[T]].exception) // will throw fatal errors!
    }
  ...
  def flatMap[S](f: T => Future[S])(
      implicit executor: ExecutionContext): Future[S] = transformWith { t =>
  ...
  final def withFilter(p: T => Boolean)(
      implicit executor: ExecutionContext): Future[T] = filter(p)(executor)
  ...
  def collect[S](pf: PartialFunction[T, S])(
    ...
    transform { t =>
      if (t.isInstanceOf[Success[T]])
        Success(
          pf.applyOrElse(t.asInstanceOf[Success[T]].value,
                         Future.collectFailed))
  ...
  def recoverWith[U >: T](pf: PartialFunction[Throwable, Future[U]])(
    ...
    transformWith { t =>
      if (t.isInstanceOf[Failure[T]]) {
        val result = pf.applyOrElse(t.asInstanceOf[Failure[T]].exception,
  ...
  def zipWith[U, R](that: Future[U])(f: (T, U) => R)(
    ...
    flatMap(r1 => that.map(r2 => f(r1, r2)))(
      if (executor.isInstanceOf[BatchingExecutor]) executor
      else internalExecutor)
    ...
    if (this eq that) this
      ...
      transformWith { t =>
        if (t.isInstanceOf[Success[T]]) this
          ...
          that transform { tt =>
            if (tt.isInstanceOf[Success[U]]) tt else t
          }
      }
    }
...
object Future {
  ...
  private[concurrent] final val filterFailure =
    Failure[Nothing](
      new NoSuchElementException("Future.filter predicate is not satisfied")
      with NoStackTrace)

  private[this] final val failedFailure =
    Failure[Nothing](
      new NoSuchElementException(
        "Future.failed not completed with a throwable.") with NoStackTrace)
  ...
  private[this] final val _failedFun: Try[Any] => Try[Throwable] =
    v =>
      if (v.isInstanceOf[Failure[Any]])
        Success(v.asInstanceOf[Failure[Any]].exception)
      else failedFailure
  ...
  private[concurrent] final val recoverWithFailed = (t: Throwable) =>
    recoverWithFailedMarker

  private[this] final val _zipWithTuple2: (Any, Any) => (Any, Any) =
    Tuple2.apply _
  private[concurrent] final def zipWithTuple2Fun[T, U] =
    _zipWithTuple2.asInstanceOf[(T, U) => (T, U)]

  private[this] final val _addToBuilderFun
    : (Builder[Any, Nothing], Any) => Builder[Any, Nothing] =
  ...
  object never extends Future[Nothing] {
    ...
    override final def ready(atMost: Duration)(
      ...
      atMost match {
        ...
        case f: FiniteDuration =>
          if (f > Duration.Zero)
            notGoingToHappen.await(f.toNanos, TimeUnit.NANOSECONDS)
      }
      throw new TimeoutException(s"Future timed out after [$atMost]")
    }

    @throws[Exception]
    override final def result(atMost: Duration)(
        implicit permit: CanAwait): Nothing = {
      ready(atMost)
      throw new TimeoutException(s"Future timed out after [$atMost]")
    }
    ...
    override final def flatMap[S](f: Nothing => Future[S])(
        implicit executor: ExecutionContext): Future[S] = this
    override final def flatten[S](
        implicit ev: Nothing <:< Future[S]): Future[S] = this
    override final def filter(p: Nothing => Boolean)(
        implicit executor: ExecutionContext): Future[Nothing] = this
    override final def collect[S](pf: PartialFunction[Nothing, S])(
        implicit executor: ExecutionContext): Future[S] = this
    override final def recover[U >: Nothing](pf: PartialFunction[Throwable, U])(
        implicit executor: ExecutionContext): Future[U] = this
    override final def recoverWith[U >: Nothing](
    ...
    override final def fallbackTo[U >: Nothing](that: Future[U]): Future[U] =
      this
  ...
  final def failed[T](exception: Throwable): Future[T] =
    Promise.failed(exception).future
  ...
  /** Starts an asynchronous computation and returns a `Future` instance with the result of that computation once it completes.
    ...
    *  @return          the `Future` holding the result of the computation
    */
  final def delegate[T](body: => Future[T])(
      implicit executor: ExecutionContext): Future[T] =
    unit.flatMap(_ => body)
  ...
  final def sequence[A, CC[X] <: IterableOnce[X], To](in: CC[Future[A]])(
      ...
      executor: ExecutionContext): Future[To] =
    in.iterator
      .foldLeft(successful(bf.newBuilder(in))) { (fr, fa) =>
        fr.zipWith(fa)(Future.addToBuilderFun)
      }
      .map(_.result())(if (executor.isInstanceOf[BatchingExecutor]) executor
      else InternalCallbackExecutor)
  ...
  final def firstCompletedOf[T](futures: IterableOnce[Future[T]])(
    ...
    if (!i.hasNext) Future.never
      ...
      val p = Promise[T]()
      val firstCompleteHandler = new AtomicReference[Promise[T]](p)
      with (Try[T] => Unit) {
  ...
  /** Asynchronously and non-blockingly returns a `Future` that will hold the optional result
    ...
    */
  final def find[T](futures: scala.collection.immutable.Iterable[Future[T]])(
      p: T => Boolean)(
  ...
  private[this] final def foldNext[T, R](
      i: Iterator[Future[T]],
      prevValue: R,
      op: (R, T) => R)(implicit executor: ExecutionContext): Future[R] =
  ...
  def fold[T, R](futures: IterableOnce[Future[T]])(zero: R)(
  ...
  // not removed in 2.13, to facilitate 2.11/2.12/2.13 cross-building; remove in 2.14 (see scala/scala#6319)
  final def reduce[T, R >: T](futures: IterableOnce[Future[T]])(
      op: (R, T) => R)(implicit executor: ExecutionContext): Future[R] =
    if (futures.isEmpty)
  ...
  final def reduceLeft[T, R >: T](
      futures: scala.collection.immutable.Iterable[Future[T]])(op: (R, T) => R)(
      implicit executor: ExecutionContext): Future[R] = {
    val i = futures.iterator
    if (!i.hasNext)
      failed(
  ...
  final def traverse[A, B, M[X] <: IterableOnce[X]](in: M[A])(
      fn: A => Future[B])(implicit bf: BuildFrom[M[A], B, M[B]],
                          executor: ExecutionContext): Future[M[B]] =
    in.iterator
      .foldLeft(successful(bf.newBuilder(in))) { (fr, a) =>
        fr.zipWith(fn(a))(Future.addToBuilderFun)
      }
      .map(_.result())(if (executor.isInstanceOf[BatchingExecutor]) executor
      else InternalCallbackExecutor)
  ...
  private[concurrent] object InternalCallbackExecutor
    ...
    override final def submitForExecution(runnable: Runnable): Unit =
      runnable.run()
