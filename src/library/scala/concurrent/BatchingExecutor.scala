  ...
  object MissingParentBlockContext extends BlockContext {
    override def blockOn[T](thunk: => T)(implicit permission: CanAwait): T =
      try thunk
        ...
        "BUG in BatchingExecutor.Batch: parentBlockContext is null")
  }
}
...
private[concurrent] trait BatchingExecutor extends Executor {
  ...
  private[this] sealed abstract class AbstractBatch protected (
      protected final var first: Runnable,
      protected final var other: Array[Runnable],
      ...
      if (curSize <= curLen) curOther
        ...
        if (newLen <= curLen)
          throw new StackOverflowError(
            "Space limit of asynchronous stack reached: " + curLen)
  ...
  private[this] final class AsyncBatch private (_first: Runnable,
                                                _other: Array[Runnable],
                                                _size: Int)
      extends AbstractBatch(_first, _other, _size)
      with Runnable
      with BlockContext
      with (BlockContext => Throwable) {
    private[this] final var parentBlockContext: BlockContext =
      BatchingExecutorStatics.MissingParentBlockContext

    final def this(runnable: Runnable) =
      this(runnable, BatchingExecutorStatics.emptyBatchArray, 1)
    ...
    override final def apply(prevBlockContext: BlockContext): Throwable =
      try {
        ...
        case t: Throwable =>
          t // We are handling exceptions on the outside of this method
      } finally {
      ...
      if (this.size > 0) {
        try { submitForExecution(this); cause } catch {
          case inner: Throwable =>
            if (NonFatal(inner)) {
              val e = new ExecutionException(
                "Non-fatal error occurred and resubmission failed, see suppressed exception.",
                cause)
        ...
        cause // TODO: consider if NonFatals should simply be `reportFailure`:ed rather than rethrown

    private[this] final def cloneAndClear(): AsyncBatch = {
      val newBatch = new AsyncBatch(this.first, this.other, this.size)
      this.first = null
      this.parentBlockContext =
        BatchingExecutorStatics.MissingParentBlockContext
    ...
    override final def blockOn[T](thunk: => T)(
  ...
  private[this] final class SyncBatch(runnable: Runnable)
      extends AbstractBatch(runnable,
                            BatchingExecutorStatics.emptyBatchArray,
                            1)
      with Runnable {
    @tailrec override final def run(): Unit = {
      try runN(BatchingExecutorStatics.runLimit)
      catch {
