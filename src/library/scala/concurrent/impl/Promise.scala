...
/**
  ...
  */
private[impl] final class CompletionLatch[T]
...
private[concurrent] object Promise {
  ...
  /**
    ...
    * the `owner`'s value is completed with that value, and the Link chain is discarded.
     **/
  private[concurrent] final class Link[T](to: DefaultPromise[T])
      extends AtomicReference[DefaultPromise[T]](to) {
    ...
    /**
      * The combination of traversing and possibly unlinking of a given `target` DefaultPromise.
       **/
    @inline @tailrec private[this] final def compressed(
        current: DefaultPromise[T],
        target: DefaultPromise[T],
        owner: DefaultPromise[T]): DefaultPromise[T] = {
      val value = target.get()
      if (value.isInstanceOf[Callbacks[T]]) {
        if (compareAndSet(current, target)) target // Link
        else
          compressed(current = get(), target = target, owner = owner) // Retry
      } else if (value.isInstanceOf[Link[T]])
        compressed(current = current,
                   target = value.asInstanceOf[Link[T]].get(),
  ...
  private[this] final def resolve[T](value: Try[T]): Try[T] =
    if (requireNonNull(value).isInstanceOf[Success[T]]) value
      ...
      if (t.isInstanceOf[ControlThrowable] || t
            .isInstanceOf[InterruptedException] || t.isInstanceOf[Error]) {
  ...
  class DefaultPromise[T] private[this] (initial: AnyRef)
    ...
    override final def transform[S](f: Try[T] => Try[S])(
        implicit executor: ExecutionContext): Future[S] =
      dispatchOrAddCallbacks(
        get(),
        new Transformation[T, S](Xform_transform, f, executor))

    override final def transformWith[S](f: Try[T] => Future[S])(
        implicit executor: ExecutionContext): Future[S] =
      dispatchOrAddCallbacks(
        get(),
        new Transformation[T, S](Xform_transformWith, f, executor))

    override final def foreach[U](f: T => U)(
        implicit executor: ExecutionContext): Unit = {
      val state = get()
      if (!state.isInstanceOf[Failure[T]])
        dispatchOrAddCallbacks(
          state,
          new Transformation[T, Unit](Xform_foreach, f, executor))
    }

    override final def flatMap[S](f: T => Future[S])(
        implicit executor: ExecutionContext): Future[S] = {
      val state = get()
      if (!state.isInstanceOf[Failure[T]])
        dispatchOrAddCallbacks(
          state,
          new Transformation[T, S](Xform_flatMap, f, executor))
      else this.asInstanceOf[Future[S]]
    }

    override final def map[S](f: T => S)(
        implicit executor: ExecutionContext): Future[S] = {
      val state = get()
      if (!state.isInstanceOf[Failure[T]])
        dispatchOrAddCallbacks(state,
                               new Transformation[T, S](Xform_map, f, executor))
      else this.asInstanceOf[Future[S]]
    }

    override final def filter(p: T => Boolean)(
        implicit executor: ExecutionContext): Future[T] = {
      val state = get()
      if (!state.isInstanceOf[Failure[T]])
        dispatchOrAddCallbacks(state,
                               new Transformation[T, T](
                                 Xform_filter,
                                 p,
                                 executor)) // Short-circuit if we get a Success
      else this
    }

    override final def collect[S](pf: PartialFunction[T, S])(
        implicit executor: ExecutionContext): Future[S] = {
      val state = get()
      if (!state.isInstanceOf[Failure[T]])
        dispatchOrAddCallbacks(state,
                               new Transformation[T, S](
                                 Xform_collect,
                                 pf,
                                 executor)) // Short-circuit if we get a Success
      else this.asInstanceOf[Future[S]]
    ...
    override final def recoverWith[U >: T](
        pf: PartialFunction[Throwable, Future[U]])(
        implicit executor: ExecutionContext): Future[U] = {
      val state = get()
      if (!state.isInstanceOf[Success[T]])
        dispatchOrAddCallbacks(state,
                               new Transformation[T, U](
                                 Xform_recoverWith,
                                 pf,
                                 executor)) // Short-circuit if we get a Failure
      else this.asInstanceOf[Future[U]]
    }

    override final def recover[U >: T](pf: PartialFunction[Throwable, U])(
        implicit executor: ExecutionContext): Future[U] = {
      val state = get()
      if (!state.isInstanceOf[Success[T]])
        dispatchOrAddCallbacks(state,
    ...
    override final def mapTo[S](
        implicit tag: scala.reflect.ClassTag[S]): Future[S] =
      if (!get().isInstanceOf[Failure[T]])
        super[Future].mapTo[S](tag) // Short-circuit if we get a Success
      else this.asInstanceOf[Future[S]]

    override final def onComplete[U](func: Try[T] => U)(
        implicit executor: ExecutionContext): Unit =
      dispatchOrAddCallbacks(
        get(),
        new Transformation[T, Unit](Xform_onComplete, func, executor))

    override final def failed: Future[Throwable] =
      if (!get().isInstanceOf[Success[T]]) super.failed
      else
        Future.failedFailureFuture // Cached instance in case of already known success

    @tailrec override final def toString: String = {
      val state = get()
      if (state.isInstanceOf[Try[T]]) "Future(" + state + ")"
      else if (state.isInstanceOf[Link[T]])
        state.asInstanceOf[Link[T]].promise(this).toString
        ...
        if (v ne null) v
          ...
          if (r ne null) r
            ...
            throw new TimeoutException(
              "Future timed out after [" + atMost + "]")
        }
      } else
        throw new IllegalArgumentException(
          "Cannot wait for Undefined duration of time")
    ...
    private final def value0: Try[T] = {
      ...
      if (state.isInstanceOf[Try[T]]) state.asInstanceOf[Try[T]]
        ...
        state.asInstanceOf[Link[T]].promise(this).value0
    ...
    private[Promise] final def tryComplete0(state: AnyRef,
                                            resolved: Try[T]): Boolean =
      if (state.isInstanceOf[Callbacks[T]]) {
        if (compareAndSet(state, resolved)) {
          if (state ne Noop)
            submitWithValue(state.asInstanceOf[Callbacks[T]], resolved)
          true
        ...
        if (!state.isInstanceOf[Try[T]]) {
          val resolved =
            if (other.isInstanceOf[DefaultPromise[T]])
              other.asInstanceOf[DefaultPromise[T]].value0
            else other.value.orNull
    ...
    /** Tries to add the callback, if already completed, it dispatches the callback to be executed.
      ...
      */
    @tailrec private final def dispatchOrAddCallbacks[C <: Callbacks[T]](
        state: AnyRef,
        callbacks: C): C =
      if (state.isInstanceOf[Try[T]]) {
        ...
        if (compareAndSet(
              state,
              if (state ne Noop)
                concatCallbacks(callbacks, state.asInstanceOf[Callbacks[T]])
    ...
    @tailrec private[this] final def concatCallbacks(
        left: Callbacks[T],
        right: Callbacks[T]): Callbacks[T] =
      if (left.isInstanceOf[Transformation[T, _]])
        new ManyCallbacks[T](left.asInstanceOf[Transformation[T, _]], right)
      else
    ...
    /** Link this promise to the root of another promise.
      */
    @tailrec private[concurrent] final def linkRootOf(target: DefaultPromise[T],
                                                      link: Link[T]): Unit =
      if (this ne target) {
        val state = get()
        if (state.isInstanceOf[Try[T]]) {
          ...
          if ((this ne p) && compareAndSet(state, l)) {
            if (state ne Noop)
              p.dispatchOrAddCallbacks(
                p.get(),
                state
                  .asInstanceOf[Callbacks[T]]) // Noop-check is important here
          } else linkRootOf(p, l)
        } else
          /* if (state.isInstanceOf[Link[T]]) */
    ...
    @tailrec private[concurrent] final def unlink(resolved: Try[T]): Unit = {
      ...
      if (state.isInstanceOf[Link[T]]) {
        val next =
          if (compareAndSet(state, resolved)) state.asInstanceOf[Link[T]].get()
          else this
    ...
    private def writeObject(out: ObjectOutputStream): Unit =
      throw new NotSerializableException(
        "Promises and Futures cannot be serialized")

    @throws[IOException]
    @throws[ClassNotFoundException]
    private def readObject(in: ObjectInputStream): Unit =
      throw new NotSerializableException(
        "Promises and Futures cannot be deserialized")
  }
  ...
  final class ManyCallbacks[-T](final val first: Transformation[T, _],
                                final val rest: Callbacks[T])
      extends Callbacks[T] {
    override final def toString: String = "ManyCallbacks"
  }

  private[this] final val Noop = new Transformation[Nothing, Nothing](
    Xform_noop,
    null,
  ...
  final class Transformation[-F, T] private[this] (
    ...
    final def this(xform: Int, f: _ => _, ec: ExecutionContext) =
      this(f.asInstanceOf[Any => Any], ec.prepare(), null, xform)

    final def benefitsFromBatching: Boolean =
      _xform != Xform_onComplete && _xform != Xform_foreach
    ...
    private[this] final def handleFailure(t: Throwable,
                                          e: ExecutionContext): Unit = {
        ...
        val resolvedResult: Try[_] =
          (_xform: @switch) match {
            ...
            case Xform_onComplete =>
              ...
              null
            case Xform_recover =>
              resolve(v.recover(fun
                .asInstanceOf[PartialFunction[Throwable, F]])) //recover F=:=T
            case Xform_recoverWith =>
              if (v.isInstanceOf[Failure[F]]) {
                val f = fun
                  .asInstanceOf[PartialFunction[Throwable, Future[T]]]
                  .applyOrElse(v.asInstanceOf[Failure[F]].exception,
                               Future.recoverWithFailed)
            ...
            case Xform_filter =>
              if (v.isInstanceOf[Failure[F]] || fun.asInstanceOf[F => Boolean](
                    v.get)) v
            ...
            case Xform_collect =>
              if (v.isInstanceOf[Success[F]])
                Success(
                  fun
                    .asInstanceOf[PartialFunction[F, T]]
                    .applyOrElse(v.get, Future.collectFailed))
              else v
            case _ =>
              Failure(new IllegalStateException(
                "BUG: encountered transformation promise with illegal type: " + _xform)) // Safe not to `resolve`
          }
