...
private[collection] object RedBlackTree {
  ...
  def countInRange[A](tree: Tree[A, _], from: Option[A], to: Option[A])(
    ...
    if (tree eq null) 0
      ...
      (from, to) match {
        ...
        case (_, Some(ub)) if ordering.gteq(tree.key, ub) =>
          countInRange(tree.left, from, to)
  ...
  def slice[A: Ordering, B](tree: Tree[A, B],
                            from: Int,
                            until: Int): Tree[A, B] =
  ...
  def tail[A, B](tree: Tree[A, B]): Tree[A, B] = {
    def _tail[A, B](tree: Tree[A, B]): Tree[A, B] =
      ...
      else if (isBlackTree(tree.left))
        balLeft(tree.key, tree.value, _tail(tree.left), tree.right)
  ...
  def init[A, B](tree: Tree[A, B]): Tree[A, B] = {
    def _init[A, B](tree: Tree[A, B]): Tree[A, B] =
      ...
      else if (isBlackTree(tree.right))
        balRight(tree.key, tree.value, tree.left, _init(tree.right))
  ...
  /**
    ...
    */
  def minAfter[A, B](tree: Tree[A, B], x: A)(
      implicit ordering: Ordering[A]): Tree[A, B] =
    if (tree eq null) null
    else {
      val cmp = ordering.compare(x, tree.key)
  ...
  /**
    ...
    */
  def maxBefore[A, B](tree: Tree[A, B], x: A)(
      implicit ordering: Ordering[A]): Tree[A, B] =
    if (tree eq null) null
    else {
      val cmp = ordering.compare(x, tree.key)
  ...
  def foreach[A, B, U](tree: Tree[A, B], f: ((A, B)) => U): Unit =
    if (tree ne null) _foreach(tree, f)

  private[this] def _foreach[A, B, U](tree: Tree[A, B],
                                      f: ((A, B)) => U): Unit = {
  ...
  def foreachKey[A, U](tree: Tree[A, _], f: A => U): Unit =
    if (tree ne null) _foreachKey(tree, f)
  ...
  @`inline` private[this] def isRedTree(tree: Tree[_, _]) =
    tree.isInstanceOf[RedTree[_, _]]
  @`inline` private[this] def isBlackTree(tree: Tree[_, _]) =
    tree.isInstanceOf[BlackTree[_, _]]

  private[this] def blacken[A, B](t: Tree[A, B]): Tree[A, B] =
    if (t eq null) null else t.black

  private[this] def mkTree[A, B](isBlack: Boolean,
                                 k: A,
                                 v: B,
                                 l: Tree[A, B],
                                 r: Tree[A, B]) =
  ...
  private[this] def balanceLeft[A, B, B1 >: B](isBlack: Boolean,
                                               z: A,
                                               zv: B,
                                               l: Tree[A, B1],
  ...
  private[this] def balanceRight[A, B, B1 >: B](isBlack: Boolean,
                                                x: A,
                                                xv: B,
                                                a: Tree[A, B1],
  ...
  private[this] def upd[A, B, B1 >: B](
      ...
      k: A,
      v: B1,
      overwrite: Boolean)(implicit ordering: Ordering[A]): Tree[A, B1] =
    if (tree eq null) {
  ...
  private[this] def updNth[A, B, B1 >: B](tree: Tree[A, B],
                                          idx: Int,
                                          k: A,
                                          v: B1,
                                          overwrite: Boolean): Tree[A, B1] =
  ...
  private[this] def doFrom[A, B](tree: Tree[A, B], from: A)(
    ...
    if (newLeft eq tree.left) tree
      ...
      upd(tree.right, tree.key, tree.value, overwrite = false)
  ...
  private[this] def doTo[A, B](tree: Tree[A, B], to: A)(
    ...
    if (newRight eq tree.right) tree
      ...
      upd(tree.left, tree.key, tree.value, overwrite = false)
  ...
  private[this] def doRange[A, B](tree: Tree[A, B], from: A, until: A)(
    ...
    if ((newLeft eq tree.left) && (newRight eq tree.right)) tree
      ...
      upd(newRight, tree.key, tree.value, overwrite = false)
    else if (newRight eq null)
      upd(newLeft, tree.key, tree.value, overwrite = false)
  ...
  private[this] def doTake[A, B](tree: Tree[A, B], n: Int): Tree[A, B] =
    if ((tree eq null) || (n <= 0)) null
      ...
      if (n <= l) doTake(tree.left, n)
        ...
        updNth(tree.left, n, tree.key, tree.value, overwrite = false)
      else join(tree.left, tree.key, tree.value, doTake(tree.right, n - l - 1))
    }

  private[this] def doSlice[A, B](tree: Tree[A, B],
                                  from: Int,
                                  until: Int): Tree[A, B] =
    if ((tree eq null) || (from >= until) || (from >= tree.count) || (until <= 0))
      null
      ...
      if (until <= l) doSlice(tree.left, from, until)
        ...
        join(doDrop(tree.left, from),
             tree.key,
             tree.value,
             doTake(tree.right, until - l - 1))
    }
  ...
  sealed abstract class Tree[A, +B](
    ...
    @(`inline` @getter)
    final val count: Int = 1 + RedBlackTree.count(left) + RedBlackTree.count(
      right)
    def black: Tree[A, B]
    def red: Tree[A, B]
  }
  ...
  private[this] abstract class TreeIterator[A, B, R](
      root: Tree[A, B],
      start: Option[A])(implicit ordering: Ordering[A])
    ...
    private[this] def popNext(): Tree[A, B] =
      if (index == 0) null
      else {
        index -= 1
        stackOfNexts(index)
    ...
    private[this] var stackOfNexts =
      if (root eq null) null
         ...
         */
        val maximumHeight = 2 * (32 - Integer.numberOfLeadingZeros(
          root.count + 2 - 1)) - 2
        new Array[Tree[A, B]](maximumHeight)
      }
    private[this] var index = 0
    private[this] var lookahead
      : Tree[A, B] = start map startFrom getOrElse findLeftMostOrPopOnEmpty(
      root)
  ...
  private[this] class EntriesIterator[A: Ordering, B](tree: Tree[A, B],
                                                      focus: Option[A])
      extends TreeIterator[A, B, (A, B)](tree, focus) {
    override def nextResult(tree: Tree[A, B]) = (tree.key, tree.value)
  }

  private[this] class KeysIterator[A: Ordering, B](tree: Tree[A, B],
                                                   focus: Option[A])
      extends TreeIterator[A, B, A](tree, focus) {
    override def nextResult(tree: Tree[A, B]) = tree.key
  }

  private[this] class ValuesIterator[A: Ordering, B](tree: Tree[A, B],
                                                     focus: Option[A])
  ...
  def fromOrderedKeys[A](xs: Iterator[A], size: Int): Tree[A, Null] = {
    ...
    def f(level: Int, size: Int): Tree[A, Null] = size match {
      case 0 => null
      case 1 =>
        mkTree(level != maxUsedDepth || level == 1, xs.next(), null, null, null)
      case n =>
  ...
  def filterEntries[A, B](t: Tree[A, B], f: (A, B) => Boolean): Tree[A, B] =
    if (t eq null) null
    else {
  ...
  def filterKeys[A, B](t: Tree[A, B], f: A => Boolean): Tree[A, B] =
    if (t eq null) null
    else {
      def fk(t: Tree[A, B]): Tree[A, B] = {
        val k = t.key
  ...
  def partitionEntries[A, B](t: Tree[A, B],
                             p: (A, B) => Boolean): (Tree[A, B], Tree[A, B]) =
    if (t eq null) (null, null)
    else {
      var tmpk,
      tmpd = null: Tree[A, B] // shared vars to avoid returning tuples from fk
  ...
  def partitionKeys[A, B](t: Tree[A, B],
                          p: A => Boolean): (Tree[A, B], Tree[A, B]) =
    if (t eq null) (null, null)
    else {
      var tmpk,
      tmpd = null: Tree[A, B] // shared vars to avoid returning tuples from fk
  ...
  private[this] def balLeft[A, B](x: A, xv: B, tl: Tree[A, B], tr: Tree[A, B]) =
    if (isRedTree(tl)) RedTree(x, xv, tl.black, tr)
      ...
      RedTree(tr.left.key,
              tr.left.value,
              BlackTree(x, xv, tl, tr.left.left),
              balance(tr.key, tr.value, tr.left.right, tr.right.red))
    else sys.error("Defect: invariance violation")

  private[this] def balRight[A, B](x: A,
                                   xv: B,
                                   tl: Tree[A, B],
                                   tr: Tree[A, B]) =
    if (isRedTree(tr)) RedTree(x, xv, tl, tr.black)
      ...
      RedTree(tl.right.key,
              tl.right.value,
              balance(tl.key, tl.value, tl.left.red, tl.right.left),
              BlackTree(x, xv, tl.right.right, tr))
    else sys.error("Defect: invariance violation")
  ...
  def union[A, B](t1: Tree[A, B], t2: Tree[A, B])(
      implicit ordering: Ordering[A]): Tree[A, B] = blacken(_union(t1, t2))

  def intersect[A, B](t1: Tree[A, B], t2: Tree[A, B])(
      implicit ordering: Ordering[A]): Tree[A, B] = blacken(_intersect(t1, t2))

  def difference[A, B](t1: Tree[A, B], t2: Tree[A, _])(
      implicit ordering: Ordering[A]): Tree[A, B] =
  ...
  private[this] def joinRight[A, B](tl: Tree[A, B],
                                    k: A,
                                    v: B,
                                    tr: Tree[A, B],
                                    bhtl: Int,
  ...
  private[this] def joinLeft[A, B](tl: Tree[A, B],
                                   k: A,
                                   v: B,
                                   tr: Tree[A, B],
                                   rtl: Int,
  ...
  private[this] def join[A, B](tl: Tree[A, B],
                               k: A,
                               v: B,
                               tr: Tree[A, B]): Tree[A, B] = {
  ...
  private[this] def split[A, B](t: Tree[A, B], k: A)(
      implicit ordering: Ordering[A]): (Tree[A, B], Tree[A, B], Tree[A, B]) =
  ...
  private[this] def _union[A, B](t1: Tree[A, B], t2: Tree[A, B])(
      implicit ordering: Ordering[A]): Tree[A, B] =
    if (t1 eq null) t2
  ...
  private[this] def _intersect[A, B](t1: Tree[A, B], t2: Tree[A, B])(
      implicit ordering: Ordering[A]): Tree[A, B] =
  ...
  private[this] def _difference[A, B](t1: Tree[A, B], t2: Tree[A, B])(
      implicit ordering: Ordering[A]): Tree[A, B] =
