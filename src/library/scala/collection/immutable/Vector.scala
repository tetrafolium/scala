...
final class Vector[+A] private[immutable] (
  ...
  private def checkRangeConvert(index: Int) = {
    ...
    if (index >= 0 && idx < endIndex)
      ...
      throw new IndexOutOfBoundsException(
        s"$index is out of bounds (min 0, max ${endIndex - 1})")
  }
  ...
  private final def getElem(index: Int, xor: Int): A = {
    if (xor < (1 << 5)) { // level = 0
      ...
      (display1((index >>> 5) & 31)(index & 31).asInstanceOf[A])
    } else if (xor < (1 << 15)) { // level = 2
      (display2((index >>> 10) & 31)((index >>> 5) & 31)(index & 31)
        .asInstanceOf[A])
    } else if (xor < (1 << 20)) { // level = 3
      (display3((index >>> 15) & 31)((index >>> 10) & 31)((index >>> 5) & 31)(
        index & 31).asInstanceOf[A])
    } else if (xor < (1 << 25)) { // level = 4
      (display4((index >>> 20) & 31)((index >>> 15) & 31)((index >>> 10) & 31)(
        (index >>> 5) & 31)(index & 31).asInstanceOf[A])
    } else if (xor < (1 << 30)) { // level = 5
      (display5((index >>> 25) & 31)((index >>> 20) & 31)((index >>> 15) & 31)(
        (index >>> 10) & 31)((index >>> 5) & 31)(index & 31).asInstanceOf[A])
    } else { // level = 6
  ...
  override def appendedAll[B >: A](
      suffix: collection.IterableOnce[B]): Vector[B] = {
        ...
        case suffix: collection.Iterable[B] =>
          suffix.size match {
            ...
            case n
              ...
              v
            case n
                if this.size < (n >>> Log2ConcatFaster) && suffix
  ...
  override def prependedAll[B >: A](
      prefix: collection.IterableOnce[B]): Vector[B] = {
        ...
        case prefix: collection.Iterable[B] =>
          prefix.size match {
            case n
              ...
              v
            case n
                if this.size < (n >>> Log2ConcatFaster) && prefix
                  .isInstanceOf[Vector[_]] =>
  ...
  private def copyLeft[T <: AnyRef](array: Array[T], right: Int): Array[T] = {
    ...
    java.util.Arrays.fill(copy.asInstanceOf[Array[AnyRef]],
                          right,
                          array.length,
                          null)
    copy
  ...
  override protected def applyPreferredMaxLength: Int =
    Vector.defaultApplyPreferredMaxLength
...
@deprecated(
  "This class is not intended for public consumption and will be made private in the future.",
  "2.13.0")
class VectorIterator[+A](_startIndex: Int, private[this] var endIndex: Int)
  ...
  override def copyToArray[B >: A](xs: Array[B], start: Int, len: Int): Int = {
    val xsLen = xs.length
    val totalToBeCopied =
      IterableOnce.elemsToCopyToArray(remainingElementCount, xsLen, start, len)
    var totalCopied = 0
    while (hasNext && totalCopied < totalToBeCopied) {
      val _start = start + totalCopied
      val toBeCopied = IterableOnce.elemsToCopyToArray(endLo - lo,
                                                       xsLen,
                                                       _start,
                                                       len - totalCopied)
...
final class VectorBuilder[A]()
    extends ReusableBuilder[A, Vector[A]]
    with VectorPointer[A @uncheckedVariance] {
  ...
  override def addAll(xs: IterableOnce[A]): this.type = {
    ...
    while (it.hasNext) {
      advanceToNextBlockIfNecessary()
      lo += it.copyToArray(xs = display0,
                           start = lo,
                           len = display0.length - lo)
    }
    this
  }
...
private[immutable] trait VectorPointer[T] {
  ...
  private[immutable] var display5
    : Array[Array[Array[Array[Array[Array[AnyRef]]]]]] = _
  ...
  private[immutable] final def gotoNewBlockStart(index: Int,
                                                 depth: Int): Unit = {
  ...
  private[immutable] final def nullSlotAndCopy[T <: AnyRef](
      array: Array[Array[T]],
      index: Int): Array[T] = {
    val x = array(index)
    array(index) = null
  ...
  private[immutable] final def copyRange[T <: AnyRef](array: Array[T],
                                                      oldLeft: Int,
                                                      newLeft: Int) = {
    val elems = java.lang.reflect.Array
      .newInstance(array.getClass.getComponentType, 32)
      .asInstanceOf[Array[T]]
    java.lang.System.arraycopy(array,
                               oldLeft,
                               elems,
                               newLeft,
