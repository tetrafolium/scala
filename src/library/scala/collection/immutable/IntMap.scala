...
private[immutable] object IntMapUtils extends BitOperations.Int {
  ...
  def bin[T](prefix: Int,
             mask: Int,
             left: IntMap[T],
             right: IntMap[T]): IntMap[T] = (left, right) match {
...
object IntMap {
  ...
  private[this] object ToFactory
      extends Factory[(Int, AnyRef), IntMap[AnyRef]]
      with Serializable {
  ...
  implicit def toBuildFrom[V](
      factory: IntMap.type): BuildFrom[Any, (Int, V), IntMap[V]] =
...
private[immutable] abstract class IntMapIterator[V, T](it: IntMap[V])
...
private[immutable] class IntMapEntryIterator[V](it: IntMap[V])
    extends IntMapIterator[V, (Int, V)](it) {
  def valueOf(tip: IntMap.Tip[V]) = (tip.key, tip.value)
}

private[immutable] class IntMapValueIterator[V](it: IntMap[V])
    extends IntMapIterator[V, V](it) {
  def valueOf(tip: IntMap.Tip[V]) = tip.value
}

private[immutable] class IntMapKeyIterator[V](it: IntMap[V])
    extends IntMapIterator[V, Int](it) {
...
sealed abstract class IntMap[+T]
  ...
  override protected def fromSpecific(
      coll: scala.collection.IterableOnce[(Int, T) @uncheckedVariance])
    : IntMap[T] =
    intMapFrom[T](coll)
  protected def intMapFrom[V2](
      coll: scala.collection.IterableOnce[(Int, V2)]): IntMap[V2] = {
  ...
  override protected def newSpecificBuilder
    : Builder[(Int, T), IntMap[T]] @uncheckedVariance =
  ...
  def flatMap[V2](f: ((Int, T)) => IterableOnce[(Int, V2)]): IntMap[V2] =
    intMapFrom(new View.FlatMap(toIterable, f))

  override def concat[V1 >: T](
      that: collection.IterableOnce[(Int, V1)]): IntMap[V1] =
    super
      .concat(that)
      .asInstanceOf[IntMap[V1]] // Already has corect type but not declared as such

  override def ++[V1 >: T](
      that: collection.IterableOnce[(Int, V1)]): IntMap[V1] = concat(that)
  ...
  protected[this] def writeReplace(): AnyRef =
    new DefaultSerializationProxy(IntMap.toFactory[T](IntMap), this)
}
