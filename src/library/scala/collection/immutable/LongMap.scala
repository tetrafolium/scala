...
private[immutable] object LongMapUtils extends BitOperations.Long {
  ...
  def bin[T](prefix: Long,
             mask: Long,
             left: LongMap[T],
             right: LongMap[T]): LongMap[T] = (left, right) match {
...
object LongMap {
  ...
  private[immutable] case class Tip[+T](key: Long, value: T)
      extends LongMap[T] {
    def withValue[S](s: S) =
  ...
  private[immutable] case class Bin[+T](prefix: Long,
                                        mask: Long,
                                        left: LongMap[T],
                                        right: LongMap[T])
      extends LongMap[T] {
  ...
  implicit def toFactory[V](
      dummy: LongMap.type): Factory[(Long, V), LongMap[V]] =
    ToFactory.asInstanceOf[Factory[(Long, V), LongMap[V]]]

  @SerialVersionUID(3L)
  private[this] object ToFactory
      extends Factory[(Long, AnyRef), LongMap[AnyRef]]
      with Serializable {
    def fromSpecific(it: IterableOnce[(Long, AnyRef)]): LongMap[AnyRef] =
      LongMap.from[AnyRef](it)
    def newBuilder: Builder[(Long, AnyRef), LongMap[AnyRef]] =
      LongMap.newBuilder[AnyRef]
  }

  implicit def toBuildFrom[V](
      factory: LongMap.type): BuildFrom[Any, (Long, V), LongMap[V]] =
    ToBuildFrom.asInstanceOf[BuildFrom[Any, (Long, V), LongMap[V]]]
  private[this] object ToBuildFrom
      extends BuildFrom[Any, (Long, AnyRef), LongMap[AnyRef]] {
    def fromSpecific(from: Any)(it: IterableOnce[(Long, AnyRef)]) =
      LongMap.from(it)
  ...
  implicit def iterableFactory[V]: Factory[(Long, V), LongMap[V]] =
    toFactory(this)
  implicit def buildFromLongMap[V]
    : BuildFrom[LongMap[_], (Long, V), LongMap[V]] = toBuildFrom(this)
}

// Iterator over a non-empty LongMap.
private[immutable] abstract class LongMapIterator[V, T](it: LongMap[V])
...
private[immutable] class LongMapEntryIterator[V](it: LongMap[V])
    extends LongMapIterator[V, (Long, V)](it) {
  def valueOf(tip: LongMap.Tip[V]) = (tip.key, tip.value)
}

private[immutable] class LongMapValueIterator[V](it: LongMap[V])
    extends LongMapIterator[V, V](it) {
  def valueOf(tip: LongMap.Tip[V]) = tip.value
}

private[immutable] class LongMapKeyIterator[V](it: LongMap[V])
    extends LongMapIterator[V, Long](it) {
...
sealed abstract class LongMap[+T]
  ...
  override protected def fromSpecific(
      coll: scala.collection.IterableOnce[(Long, T)] @uncheckedVariance)
    : LongMap[T] = {
  ...
  override protected def newSpecificBuilder
    : Builder[(Long, T), LongMap[T]] @uncheckedVariance =
  ...
  def unionWith[S >: T](that: LongMap[S], f: (Long, S, S) => S): LongMap[S] =
    (this, that) match {
      ...
      case (LongMap.Tip(key, value), x) =>
        x.updateWith(key, value, (x, y) => f(key, y, x))
  ...
  /**
    ...
    */
  def intersectionWith[S, R](that: LongMap[S],
  ...
  def map[V2](f: ((Long, T)) => (Long, V2)): LongMap[V2] =
    LongMap.from(new View.Map(coll, f))

  def flatMap[V2](f: ((Long, T)) => IterableOnce[(Long, V2)]): LongMap[V2] =
    LongMap.from(new View.FlatMap(coll, f))

  override def concat[V1 >: T](
      that: scala.collection.IterableOnce[(Long, V1)]): LongMap[V1] =
    super
      .concat(that)
      .asInstanceOf[LongMap[V1]] // Already has corect type but not declared as such
  ...
  protected[this] def writeReplace(): AnyRef =
    new DefaultSerializationProxy(LongMap.toFactory[T](LongMap), this)
}
