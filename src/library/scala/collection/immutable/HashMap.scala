...
/** This class implements immutable maps using a Compressed Hash-Array Mapped Prefix-tree.
  ...
  */
final class HashMap[K, +V] private[immutable] (
    private[immutable] val rootNode: MapNode[K, V])
  ...
  def updated[V1 >: V](key: K, value: V1): HashMap[K, V1] = {
    ...
    val keyHash = improve(keyUnimprovedHash)
    val newRootNode =
      rootNode.updated(key, value, keyUnimprovedHash, keyHash, 0)
  ...
  override def concat[V1 >: V](
      that: scala.IterableOnce[(K, V1)]): HashMap[K, V1] = that match {
    case hm: HashMap[K, V1] =>
      if (hm.rootNode eq rootNode) this
      else {
  ...
  def merged[V1 >: V](that: HashMap[K, V1])(
      mergef: MergeFunction[K, V1]): HashMap[K, V1] = {
    val thisKeys = this.keySet
    if (mergef eq null)
      that.removedAll(thisKeys) ++ this
    else {
      val thatKeys = that.keySet
      that.removedAll(thisKeys) ++ this.removedAll(thatKeys) ++ thisKeys
        .intersect(thatKeys)
        .map { case k => mergef((k, this(k)), (k, that(k))) }
    }
  }
  ...
  override protected[collection] def filterImpl(
      pred: ((K, V)) => Boolean,
      isFlipped: Boolean): HashMap[K, V] = {
      ...
      keys match {
        case hashSet: HashSet[K] =>
          if (hashSet.isEmpty) {
            ...
            val newRootNode = new MapNodeRemoveAllSetNodeIterator(
              hashSet.rootNode).removeAll(rootNode)
            if (newRootNode eq rootNode) this
            else if (newRootNode.size <= 0) HashMap.empty
            else new HashMap(newRootNode)
          }
  ...
  override def partition(
      p: ((K, V)) => Boolean): (HashMap[K, V], HashMap[K, V]) = {
...
private[immutable] object MapNode {
  ...
  private final val EmptyMapNode =
    new BitmapIndexedMapNode(0, 0, Array.empty, Array.empty, 0, 0)
...
private[immutable] sealed abstract class MapNode[K, +V]
  ...
  def updated[V1 >: V](key: K,
                       value: V1,
                       originalHash: Int,
                       hash: Int,
                       shift: Int): MapNode[K, V1]

  def removed[V1 >: V](key: K,
                       originalHash: Int,
                       hash: Int,
                       shift: Int): MapNode[K, V1]
...
private final class BitmapIndexedMapNode[K, +V](
    ...
    var size: Int,
    var cachedJavaKeySetHashCode: Int)
    extends MapNode[K, V] {
  ...
  def getPayload(index: Int) =
    Tuple2(content(TupleLength * index).asInstanceOf[K],
  ...
  def apply(key: K, originalHash: Int, keyHash: Int, shift: Int): V = {
    ...
    if ((dataMap & bitpos) != 0) {
      ...
      return if (key == key0) this.getValue(index)
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      return this
        .getNode(index)
        .apply(key, originalHash, keyHash, shift + BitPartitionSize)
    }
  ...
  def get(key: K, originalHash: Int, keyHash: Int, shift: Int): Option[V] = {
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      return this
        .getNode(index)
        .get(key, originalHash, keyHash, shift + BitPartitionSize)
    }
  ...
  def getOrElse[V1 >: V](key: K,
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      return this
        .getNode(index)
        .getOrElse(key, originalHash, keyHash, shift + BitPartitionSize, f)
    }
    f
  }

  override def containsKey(key: K,
                           originalHash: Int,
                           keyHash: Int,
                           shift: Int): Boolean = {
    val mask = maskFrom(keyHash, shift)
    ...
    if ((dataMap & bitpos) != 0) {
      ...
      return (originalHashes(index) == originalHash) && key == this.getKey(
        index)
    }
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      return this
        .getNode(index)
        .containsKey(key, originalHash, keyHash, shift + BitPartitionSize)
    }
  ...
  def updated[V1 >: V](key: K,
    ...
    if ((dataMap & bitpos) != 0) {
      ...
      if (key0UnimprovedHash == originalHash && key0 == key) {
        ...
        return (
          if ((key0.asInstanceOf[AnyRef] eq key.asInstanceOf[AnyRef]) && (value0
                .asInstanceOf[AnyRef] eq value.asInstanceOf[AnyRef]))
            this
        ...
        val subNodeNew = mergeTwoKeyValPairs(key0,
                                             value0,
                                             key0UnimprovedHash,
                                             key0Hash,
                                             key,
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      val subNodeNew = subNode.updated(key,
                                       value,
                                       originalHash,
                                       keyHash,
                                       shift + BitPartitionSize)
  ...
  def removed[V1 >: V](key: K,
    ...
    if ((dataMap & bitpos) != 0) {
      ...
      if (key0 == key) {
        if (this.payloadArity == 2 && this.nodeArity == 0) {
           ...
           */
          val newDataMap =
            if (shift == 0) (dataMap ^ bitpos)
            else bitposFrom(maskFrom(keyHash, 0))
          if (index == 0)
            return new BitmapIndexedMapNode[K, V1](
              newDataMap,
              0,
              Array(getKey(1), getValue(1)),
              Array(originalHashes(1)),
              1,
              improve(getHash(1)))
          else
            return new BitmapIndexedMapNode[K, V1](
              newDataMap,
              0,
              Array(getKey(0), getValue(0)),
              Array(originalHashes(0)),
              1,
              improve(getHash(0)))
    ...
    if ((nodeMap & bitpos) != 0) {
      ...
      val subNodeNew =
        subNode.removed(key, originalHash, keyHash, shift + BitPartitionSize)
  ...
  def mergeTwoKeyValPairs[V1 >: V](key0: K,
                                   value0: V1,
                                   originalHash0: Int,
                                   keyHash0: Int,
                                   key1: K,
    ...
    if (shift >= HashCodeLength) {
      new HashCollisionMapNode[K, V1](originalHash0,
                                      keyHash0,
                                      Vector((key0, value0), (key1, value1)))
    } else {
      ...
      if (mask0 != mask1) {
        ...
        if (mask0 < mask1) {
          new BitmapIndexedMapNode[K, V1](dataMap,
                                          0,
                                          Array(key0, value0, key1, value1),
                                          Array(originalHash0, originalHash1),
                                          2,
                                          newCachedHash)
        } else {
          new BitmapIndexedMapNode[K, V1](dataMap,
                                          0,
                                          Array(key1, value1, key0, value0),
                                          Array(originalHash1, originalHash0),
                                          2,
                                          newCachedHash)
        }
      } else {
        ...
        val node = mergeTwoKeyValPairs(key0,
                                       value0,
                                       originalHash0,
                                       keyHash0,
                                       key1,
                                       ...
                                       keyHash1,
                                       shift + BitPartitionSize)
        new BitmapIndexedMapNode[K, V1](0,
                                        nodeMap,
                                        Array(node),
                                        Array(),
                                        node.size,
  ...
  def copyAndSetValue[V1 >: V](bitpos: Int,
                               newKey: K,
                               newValue: V1): MapNode[K, V1] = {
  ...
  def copyAndSetNode[V1 >: V](bitpos: Int,
                              oldNode: MapNode[K, V1],
                              newNode: MapNode[K, V1]): MapNode[K, V1] = {
  ...
  def copyAndInsertValue[V1 >: V](bitpos: Int,
                                  key: K,
                                  originalHash: Int,
                                  keyHash: Int,
                                  value: V1): BitmapIndexedMapNode[K, V1] = {
    val dataIx = dataIndex(bitpos)
    val idx = TupleLength * dataIx
    ...
    new BitmapIndexedMapNode[K, V1](dataMap | bitpos,
                                    nodeMap,
                                    dst,
                                    dstHashes,
                                    size + 1,
  ...
  def copyAndRemoveValue(bitpos: Int,
                         keyHash: Int): BitmapIndexedMapNode[K, V] = {
    val dataIx = dataIndex(bitpos)
    val idx = TupleLength * dataIx
    ...
    new BitmapIndexedMapNode[K, V](dataMap ^ bitpos,
                                   nodeMap,
                                   dst,
                                   dstHashes,
                                   size - 1,
  ...
  def copyAndMigrateFromInlineToNode[V1 >: V](
      bitpos: Int,
      keyHash: Int,
      node: MapNode[K, V1]): BitmapIndexedMapNode[K, V1] = {
    val dataIx = dataIndex(bitpos)
  ...
  def copyAndMigrateFromNodeToInline[V1 >: V](
      bitpos: Int,
      originalHash: Int,
      keyHash: Int,
      oldNode: MapNode[K, V1],
  ...
  override def transform[W](f: (K, V) => W): BitmapIndexedMapNode[K, W] = {
    ...
    if (newContent eq null) this.asInstanceOf[BitmapIndexedMapNode[K, W]]
      ...
      new BitmapIndexedMapNode[K, W](dataMap,
                                     nodeMap,
                                     newContent,
                                     originalHashes,
                                     size,
  ...
  @`inline` private def deepContentEquality(a1: Array[Any],
                                            a2: Array[Any],
                                            length: Int): Boolean = {
    if (a1 eq a2)
      true
  ...
  override def hashCode(): Int =
    throw new UnsupportedOperationException(
      "Trie nodes do not support hashing.")

  override def concat[V1 >: V](that: MapNode[K, V1],
                               shift: Int): MapNode[K, V1] = that match {
    case bm: BitmapIndexedMapNode[K, V] =>
      if (size == 0) return that
        ...
        return this.updated(bm.getKey(0),
                            bm.getValue(0),
                            originalHash,
      ...
      val maximumBitPos: Int = Node.bitposFrom(
      ...
      val newContentSize = (MapNode.TupleLength * newDataSize) + bitCount(
        newNodeMap)
...

      {
        ...
        while (!finished) {
          ...
          if ((bitpos & leftNodeRightNode) != 0) {
            ...
            newContent(MapNode.TupleLength * compressedDataIdx) = getKey(
              leftDataIdx).asInstanceOf[AnyRef]
            newContent(MapNode.TupleLength * compressedDataIdx + 1) = getValue(
              leftDataIdx).asInstanceOf[AnyRef]
            ...
            newContent(MapNode.TupleLength * compressedDataIdx) =
              bm.getKey(rightDataIdx).asInstanceOf[AnyRef]
            newContent(MapNode.TupleLength * compressedDataIdx + 1) =
              bm.getValue(rightDataIdx).asInstanceOf[AnyRef]
            newOriginalHashes(compressedDataIdx) = originalHash
  ...
  override def copy(): BitmapIndexedMapNode[K, V] = {
    ...
    new BitmapIndexedMapNode[K, V](dataMap,
                                   nodeMap,
                                   contentClone,
  ...
  override def filterImpl(pred: ((K, V)) => Boolean,
                          flipped: Boolean): MapNode[K, V] = {
    if (size == 0) this
    else if (size == 1) {
      ...
      val maximumIndex: Int = Node.BranchingFactor - Integer
        .numberOfLeadingZeros(allMap)
      ...
      if (newSize == 0) {
        ...
        val maxIndex = Node.BranchingFactor - Integer.numberOfLeadingZeros(
          newAllMap)
        ...
        while (i < maxIndex) {
          ...
          if ((bitpos & oldDataPassThrough) != 0) {
            ...
            newContent(newContentSize - newNodeIndex - 1) = getNode(
              oldNodeIndex)
            newNodeIndex += 1
            oldNodeIndex += 1
        ...
        new BitmapIndexedMapNode[K, V](newDataMap,
                                       newNodeMap,
                                       newContent,
                                       newOriginalHashes,
                                       newSize,
...
private final class HashCollisionMapNode[K, +V](
  ...
  def apply(key: K, originalHash: Int, hash: Int, shift: Int): V =
    get(key, originalHash, hash, shift).getOrElse(
      throw new NoSuchElementException)
  ...
  def contains[V1 >: V](key: K, value: V1, hash: Int, shift: Int): Boolean =
    this.hash == hash && {
      val index = indexOf(key)
      index >= 0 && (content(index)._2.asInstanceOf[AnyRef] eq value
        .asInstanceOf[AnyRef])
    }
  ...
  def updated[V1 >: V](key: K,
    ...
    if (index >= 0) {
      ...
      if (content(index)._2.asInstanceOf[AnyRef] eq value
            .asInstanceOf[AnyRef]) {
        this
      } else {
        new HashCollisionMapNode[K, V1](
          originalHash,
          hash,
          content.updated[(K, V1)](index, (key, value)))
      }
    } else {
      new HashCollisionMapNode[K, V1](originalHash,
                                      hash,
                                      content.appended[(K, V1)]((key, value)))
    }
  }
  ...
  def removed[V1 >: V](key: K,
    ...
    if (!this.containsKey(key, originalHash, hash, shift)) {
      ...
      val updatedContent =
        content.filterNot(keyValuePair => keyValuePair._1 == key)
      // assert(updatedContent.size == content.size - 1)

      updatedContent.size match {
        case 1 =>
          val (k, v) = updatedContent(0)
          new BitmapIndexedMapNode[K, V1](bitposFrom(maskFrom(hash, 0)),
                                          0,
                                          Array(k, v),
                                          Array(originalHash),
                                          1,
  ...
  def getNode(index: Int): MapNode[K, V] =
    throw new IndexOutOfBoundsException(
      "No sub-nodes present in hash-collision leaf node.")
  ...
  override def concat[V1 >: V](that: MapNode[K, V1],
                               shift: Int): HashCollisionMapNode[K, V1] =
    that match {
      case hc: HashCollisionMapNode[K, V1] =>
        if (hc eq this) {
          ...
          if (newContent eq null) hc
          else new HashCollisionMapNode(originalHash, hash, newContent.result())
        }
  ...
  override def filterImpl(pred: ((K, V)) => Boolean, flipped: Boolean) = {
    ...
    if (newContentLength == 0) {
      ...
      new BitmapIndexedMapNode[K, V](bitposFrom(maskFrom(hash, 0)),
                                     0,
                                     Array(k, v),
                                     Array(originalHash),
                                     1,
  ...
  override def copy(): HashCollisionMapNode[K, V] =
    new HashCollisionMapNode[K, V](originalHash, hash, content)
...
private final class MapKeyIterator[K, V](rootNode: MapNode[K, V])
    extends ChampBaseIterator[MapNode[K, V]](rootNode)
    with Iterator[K] {
...
private final class MapKeyValueTupleReverseIterator[K, V](
    rootNode: MapNode[K, V])
    extends ChampBaseReverseIterator[MapNode[K, V]](rootNode)
    with Iterator[(K, V)] {
...
private final class MapKeyValueTupleHashIterator[K, V](rootNode: MapNode[K, V])
    extends ChampBaseReverseIterator[MapNode[K, V]](rootNode)
    with Iterator[Product2[AnyRef, V]]
    with Product2[AnyRef, V] {
...
private final class MapNodeRemoveAllSetNodeIterator[K](rootSetNode: SetNode[K])
    extends ChampBaseIterator(rootSetNode) {
...
object HashMap extends MapFactory[HashMap] {
  ...
  /** Create a new Builder which can be reused after calling `result()` without an
    ...
    */
  def newBuilder[K, V]: ReusableBuilder[(K, V), HashMap[K, V]] =
    new HashMapBuilder[K, V]
}

/** A Builder for a HashMap.
  * $multipleResults
  */
private[immutable] final class HashMapBuilder[K, V]
    extends ReusableBuilder[(K, V), HashMap[K, V]] {
  import Node._
  import MapNode._

  private def newEmptyRootNode =
    new BitmapIndexedMapNode[K, V](0, 0, Array(), Array(), 0, 0)
  ...
  private def insertValue[V1 >: V](bm: BitmapIndexedMapNode[K, V],
                                   bitpos: Int,
                                   key: K,
                                   originalHash: Int,
                                   keyHash: Int,
  ...
  private def migrateFromInlineToNode(bm: BitmapIndexedMapNode[K, V],
                                      keyHash: Int,
                                      bitpos: Int,
                                      node: MapNode[K, V]): Unit = {
    val dataIx = bm.dataIndex(bitpos)
    val idxOld = TupleLength * dataIx
  ...
  private def setValue[V1 >: V](bm: BitmapIndexedMapNode[K, V],
                                bitpos: Int,
                                newKey: K,
                                newValue: V1): Unit = {
    val dataIx = bm.dataIndex(bitpos)
  ...
  private[immutable] def update(mapNode: MapNode[K, V],
                                key: K,
                                value: V,
                                originalHash: Int,
                                keyHash: Int,
