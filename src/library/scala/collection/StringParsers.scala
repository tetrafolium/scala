...
private[scala] object StringParsers {
  ...
  @inline
  private[this] final def decValue(ch: Char): Int =
  ...
  final def parseByte(from: String): Option[Byte] = {
    ...
    if (len == 0) None
      ...
      if (len == 1) {
        ...
        stepToOverflow(from, len, -v, true, Byte.MinValue).map(_.toByte)
      else if (first == '+')
        stepToOverflow(from, len, 0, true, Byte.MinValue).map(_.toByte)
      else if (first == '-')
        stepToOverflow(from, len, 0, false, Byte.MinValue).map(_.toByte)
      else None
    }
  }
  ...
  final def parseShort(from: String): Option[Short] = {
    ...
    if (len == 0) None
      ...
      if (len == 1) {
        ...
        stepToOverflow(from, len, -v, true, Short.MinValue).map(_.toShort)
      else if (first == '+')
        stepToOverflow(from, len, 0, true, Short.MinValue).map(_.toShort)
      else if (first == '-')
        stepToOverflow(from, len, 0, false, Short.MinValue).map(_.toShort)
      else None
    }
  }
  ...
  final def parseInt(from: String): Option[Int] = {
    ...
    def step(i: Int, agg: Int, isPositive: Boolean): Option[Int] = {
      if (i == len) {
        ...
        val digit = decValue(from.charAt(i))
        if (digit == -1 || (agg == intOverflowBoundary && digit == intOverflowDigit))
          None
  ...
  final def parseLong(from: String): Option[Long] = {
    ...
    def step(i: Int, agg: Long, isPositive: Boolean): Option[Long] = {
      if (i == len) {
        ...
        val digit = decValue(from.charAt(i))
        if (digit == -1 || (agg == longOverflowBoundary && digit == longOverflowDigit))
          None
        else step(i + 1, agg * 10 - digit, isPositive)
      }
    }
  ...
  final def checkFloatFormat(format: String): Boolean = {
    ...
    def forAllBetween(start: Int, end: Int, pred: Char => Boolean): Boolean = {
      def rec(i: Int): Boolean =
        i >= end || pred(format.charAt(i)) && rec(i + 1)
      rec(start)
    }
    ...
    @inline
    def skipIndexWhile(predicate: Char => Boolean,
                       from: Int,
                       until: Int): Int = {
      @tailrec @inline
      def rec(i: Int): Int =
    ...
    def isHexFloatLiteral(startIndex: Int, endIndex: Int): Boolean = {
      ...
      (pIndex <= endIndex) && prefixOK(startIndex, pIndex) && postfixOK(
        pIndex + 1,
        endIndex)
    }
    ...
    def isDecFloatLiteral(startIndex: Int, endIndex: Int): Boolean = {
      ...
      if (startChar == '.') {
        ...
        isExp(format.charAt(noSignificant)) && expOK(noSignificant + 1,
                                                     endIndex))
    ...
    if (unspacedStart == -1 || unspacedStart >= unspacedEnd || unspacedEnd <= 0)
      ...
      val unsigned = {
        ...
        if (startchar == '-' || startchar == '+') unspacedStart + 1
        else unspacedStart
      }
      if (unsigned >= unspacedEnd) false
      //that's it for NaN and Infinity
      else if (format.charAt(unsigned) == 'N')
        format.substring(unsigned, unspacedEnd) == "NaN"
      else if (format.charAt(unsigned) == 'I')
        format.substring(unsigned, unspacedEnd) == "Infinity"
      else {
        //all other formats can have a format suffix
        val desuffixed = {
          val endchar = format.charAt(unspacedEnd - 1)
          if (endchar == 'f' || endchar == 'F' || endchar == 'd' || endchar == 'D')
            unspacedEnd - 1
          else unspacedEnd
        }
        val len = desuffixed - unsigned
        if (len <= 0) false
        else if (len >= 2 && (format.charAt(unsigned + 1) == 'x' || format
                   .charAt(unsigned + 1) == 'X'))
          format.charAt(unsigned) == '0' && isHexFloatLiteral(unsigned + 2,
                                                              desuffixed)
