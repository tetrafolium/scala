...
/** Decorator representing lazily zipped pairs.
  ...
  */
final class LazyZip2[+El1, +El2, C1] private[collection] (
    src: C1,
    coll1: Iterable[El1],
    coll2: Iterable[El2]) {
  ...
  def lazyZip[B](that: Iterable[B]): LazyZip3[El1, El2, B, C1] =
    new LazyZip3(src, coll1, coll2, that)
  ...
  def flatMap[B, C](f: (El1, El2) => Iterable[B])(
      implicit bf: BuildFrom[C1, B, C]): C = {
    bf.fromSpecific(src)(new AbstractView[B] {
      def iterator = new AbstractIterator[B] {
        private[this] val elems1 = coll1.iterator
        private[this] val elems2 = coll2.iterator
        ...
        private def current = {
          while (!_current.hasNext && elems1.hasNext && elems2.hasNext) _current =
            f(elems1.next(), elems2.next()).iterator
          _current
        }
      ...
      override def knownSize: Int =
        if (coll1.knownSize == 0 || coll2.knownSize == 0) 0 else super.knownSize
      override def isEmpty: Boolean = coll1.isEmpty || coll2.isEmpty
    })
  }

  def filter[C](p: (El1, El2) => Boolean)(
      implicit bf: BuildFrom[C1, (El1, El2), C]): C = {
  ...
  def exists(p: (El1, El2) => Boolean): Boolean = {
    ...
    while (!res && elems1.hasNext && elems2.hasNext) res =
      p(elems1.next(), elems2.next())

    res
  }

  def forall(p: (El1, El2) => Boolean): Boolean =
    !exists((el1, el2) => !p(el1, el2))
...
object LazyZip2 {
  implicit def lazyZip2ToIterable[El1, El2](
      zipped2: LazyZip2[El1, El2, _]): View[(El1, El2)] = zipped2.toIterable
}
...
/** Decorator representing lazily zipped triples.
  ...
  *              Note: will not terminate for infinite-sized collections.
  */
final class LazyZip3[+El1, +El2, +El3, C1] private[collection] (
    src: C1,
    coll1: Iterable[El1],
    coll2: Iterable[El2],
    coll3: Iterable[El3]) {
  ...
  /** Zips `that` iterable collection with an existing `LazyZip3`. The elements in each collection are
    ...
    */
  def lazyZip[B](that: Iterable[B]): LazyZip4[El1, El2, El3, B, C1] =
    new LazyZip4(src, coll1, coll2, coll3, that)

  def map[B, C](f: (El1, El2, El3) => B)(
      implicit bf: BuildFrom[C1, B, C]): C = {
    bf.fromSpecific(src)(new AbstractView[B] {
      def iterator = new AbstractIterator[B] {
        private[this] val elems1 = coll1.iterator
      ...
      override def isEmpty: Boolean =
        coll1.isEmpty || coll2.isEmpty || coll3.isEmpty
    })
  }

  def flatMap[B, C](f: (El1, El2, El3) => Iterable[B])(
      implicit bf: BuildFrom[C1, B, C]): C = {
    bf.fromSpecific(src)(new AbstractView[B] {
      def iterator = new AbstractIterator[B] {
        private[this] val elems1 = coll1.iterator
        ...
        private[this] var _current: Iterator[B] = Iterator.empty
        private def current = {
          while (!_current.hasNext && elems1.hasNext && elems2.hasNext && elems3.hasNext) _current =
            f(elems1.next(), elems2.next(), elems3.next()).iterator
          _current
        }
        def hasNext = current.hasNext
        def next() = current.next()
      ...
      override def knownSize: Int =
        if (coll1.knownSize == 0 || coll2.knownSize == 0 || coll3.knownSize == 0)
          0
        else super.knownSize
      override def isEmpty: Boolean = iterator.isEmpty
    })
  }

  def filter[C](p: (El1, El2, El3) => Boolean)(
      implicit bf: BuildFrom[C1, (El1, El2, El3), C]): C = {
  ...
  def exists(p: (El1, El2, El3) => Boolean): Boolean = {
    ...
    while (!res && elems1.hasNext && elems2.hasNext && elems3.hasNext) res =
      p(elems1.next(), elems2.next(), elems3.next())

    res
  }

  def forall(p: (El1, El2, El3) => Boolean): Boolean =
    !exists((el1, el2, el3) => !p(el1, el2, el3))
  ...
  def foreach[U](f: (El1, El2, El3) => U): Unit = {
    ...
    while (elems1.hasNext && elems2.hasNext && elems3.hasNext) f(elems1.next(),
                                                                 elems2.next(),
                                                                 elems3.next())
  }

  private def toIterable: View[(El1, El2, El3)] =
    new AbstractView[(El1, El2, El3)] {
      def iterator = new AbstractIterator[(El1, El2, El3)] {
        private[this] val elems1 = coll1.iterator
        private[this] val elems2 = coll2.iterator
        private[this] val elems3 = coll3.iterator
...
object LazyZip3 {
  implicit def lazyZip3ToIterable[El1, El2, El3](
      zipped3: LazyZip3[El1, El2, El3, _]): View[(El1, El2, El3)] =
    zipped3.toIterable
}
...
/** Decorator representing lazily zipped 4-tuples.
  ...
  *              Note: will not terminate for infinite-sized collections.
  */
final class LazyZip4[+El1, +El2, +El3, +El4, C1] private[collection] (
    src: C1,
    coll1: Iterable[El1],
    coll2: Iterable[El2],
    coll3: Iterable[El3],
  ...
  def map[B, C](f: (El1, El2, El3, El4) => B)(
      implicit bf: BuildFrom[C1, B, C]): C = {
    bf.fromSpecific(src)(new AbstractView[B] {
      def iterator = new AbstractIterator[B] {
        private[this] val elems1 = coll1.iterator
        ...
        def hasNext =
          elems1.hasNext && elems2.hasNext && elems3.hasNext && elems4.hasNext
        def next() =
          f(elems1.next(), elems2.next(), elems3.next(), elems4.next())
      }
      override def knownSize: Int = zipKnownSize
      override def isEmpty: Boolean =
        coll1.isEmpty || coll2.isEmpty || coll3.isEmpty || coll4.isEmpty
    })
  }

  def flatMap[B, C](f: (El1, El2, El3, El4) => Iterable[B])(
      implicit bf: BuildFrom[C1, B, C]): C = {
    bf.fromSpecific(src)(new AbstractView[B] {
      def iterator = new AbstractIterator[B] {
        private[this] val elems1 = coll1.iterator
        ...
        private[this] var _current: Iterator[B] = Iterator.empty
        private def current = {
          while (!_current.hasNext && elems1.hasNext && elems2.hasNext && elems3.hasNext && elems4.hasNext) _current =
            f(elems1.next(), elems2.next(), elems3.next(), elems4.next()).iterator
          _current
        }
        def hasNext = current.hasNext
        def next() = current.next()
      ...
      override def knownSize: Int =
        if (coll1.knownSize == 0 || coll2.knownSize == 0 || coll3.knownSize == 0 || coll4.knownSize == 0)
          0
        else super.knownSize
      override def isEmpty: Boolean = iterator.isEmpty
    })
  ...
  def filter[C](p: (El1, El2, El3, El4) => Boolean)(
      implicit bf: BuildFrom[C1, (El1, El2, El3, El4), C]): C = {
  ...
  def exists(p: (El1, El2, El3, El4) => Boolean): Boolean = {
    ...
    while (!res && elems1.hasNext && elems2.hasNext && elems3.hasNext && elems4.hasNext) res =
      p(elems1.next(), elems2.next(), elems3.next(), elems4.next())

    res
  }

  def forall(p: (El1, El2, El3, El4) => Boolean): Boolean =
    !exists((el1, el2, el3, el4) => !p(el1, el2, el3, el4))
  ...
  def foreach[U](f: (El1, El2, El3, El4) => U): Unit = {
    ...
    while (elems1.hasNext && elems2.hasNext && elems3.hasNext && elems4.hasNext) f(
      elems1.next(),
      elems2.next(),
      elems3.next(),
      elems4.next())
  ...
  private def toIterable: View[(El1, El2, El3, El4)] =
    new AbstractView[(El1, El2, El3, El4)] {
      def iterator = new AbstractIterator[(El1, El2, El3, El4)] {
        private[this] val elems1 = coll1.iterator
        private[this] val elems2 = coll2.iterator
        private[this] val elems3 = coll3.iterator
        ...
        def hasNext =
          elems1.hasNext && elems2.hasNext && elems3.hasNext && elems4.hasNext
        def next() =
          (elems1.next(), elems2.next(), elems3.next(), elems4.next())
      }
      override def knownSize: Int = zipKnownSize
      override def isEmpty: Boolean =
        coll1.isEmpty || coll2.isEmpty || coll3.isEmpty || coll4.isEmpty
  ...
  override def toString =
    s"$coll1.lazyZip($coll2).lazyZip($coll3).lazyZip($coll4)"
}

object LazyZip4 {
  implicit def lazyZip4ToIterable[El1, El2, El3, El4](
      zipped4: LazyZip4[El1, El2, El3, El4, _]): View[(El1, El2, El3, El4)] =
    zipped4.toIterable
}
