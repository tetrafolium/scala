...
trait StrictOptimizedSortedMapOps[
  ...
  override def flatMap[K2, V2](f: ((K, V)) => IterableOnce[(K2, V2)])(
      implicit @implicitNotFound(SortedMapOps.ordMsg) ordering: Ordering[K2])
    : CC[K2, V2] =
  ...
  override def collect[K2, V2](pf: PartialFunction[(K, V), (K2, V2)])(
      implicit @implicitNotFound(SortedMapOps.ordMsg) ordering: Ordering[K2])
    : CC[K2, V2] =
  ...
  override def +[V1 >: V](elem1: (K, V1),
                          ...
                          elems: (K, V1)*): CC[K, V1] = {
    val m =
      ((this + elem1).asInstanceOf[Map[K, V]] + elem2).asInstanceOf[CC[K, V1]]
