...
trait MapView[K, +V]
  ...
  def concat[V1 >: V](that: SomeMapOps[K, V1]): MapView[K, V1] =
    new MapView.Concat(this, that)

  def ++[V1 >: V](that: SomeMapOps[K, V1]): MapView[K, V1] = concat(that)

  override def +[V1 >: V](kv: (K, V1)): MapView[K, V1] =
    concat(new Map1(kv._1, kv._2))
  ...
  override def mapValues[W](f: V => W): MapView[K, W] =
    new MapView.MapValues(this, f)

  override def filter(pred: ((K, V)) => Boolean): MapView[K, V] =
    new MapView.Filter(this, false, pred)

  override def filterNot(pred: ((K, V)) => Boolean): MapView[K, V] =
    new MapView.Filter(this, true, pred)

  override def partition(
      p: ((K, V)) => Boolean): (MapView[K, V], MapView[K, V]) =
...
object MapView extends MapViewFactory {
  ...
  @SerialVersionUID(3L)
  private val EmptyMapView: MapView[Any, Nothing] =
    new AbstractMapView[Any, Nothing] {
      override def get(key: Any): Option[Nothing] = None
      override def iterator: Iterator[Nothing] = Iterator.empty[Nothing]
      override def knownSize: Int = 0
      override def isEmpty: Boolean = true
      override def concat[V1 >: Nothing](
          that: SomeMapOps[Any, V1]): MapView[Any, V1] = mapFactory.from(that)
      ...
      override def filter(
          pred: ((Any, Nothing)) => Boolean): MapView[Any, Nothing] = this
      override def filterNot(
          pred: ((Any, Nothing)) => Boolean): MapView[Any, Nothing] = this
      override def partition(p: ((Any, Nothing)) => Boolean)
        : (MapView[Any, Nothing], MapView[Any, Nothing]) = (this, this)
    }
  ...
  @SerialVersionUID(3L)
  class Concat[K, +V](left: SomeMapOps[K, V], right: SomeMapOps[K, V])
  ...
  @SerialVersionUID(3L)
  class MapValues[K, +V, +W](underlying: SomeMapOps[K, V], f: V => W)
      extends AbstractMapView[K, W] {
    def iterator: Iterator[(K, W)] =
      underlying.iterator.map(kv => (kv._1, f(kv._2)))
  ...
  @SerialVersionUID(3L)
  class FilterKeys[K, +V](underlying: SomeMapOps[K, V], p: K => Boolean)
      extends AbstractMapView[K, V] {
    def iterator: Iterator[(K, V)] = underlying.iterator.filter {
      case (k, _) => p(k)
    }
  ...
  @SerialVersionUID(3L)
  class Filter[K, +V](underlying: SomeMapOps[K, V],
                      isFlipped: Boolean,
                      p: ((K, V)) => Boolean)
      extends AbstractMapView[K, V] {
    def iterator: Iterator[(K, V)] =
      underlying.iterator.filterImpl(p, isFlipped)
  ...
  override def newBuilder[X, Y]: Builder[(X, Y), MapView[X, Y]] =
    mutable.HashMap.newBuilder[X, Y].mapResult(_.view)

  override def empty[K, V]: MapView[K, V] =
    EmptyMapView.asInstanceOf[MapView[K, V]]
...
trait MapViewFactory
    extends collection.MapFactory[({ type l[X, Y] = View[(X, Y)] })#l] {
...
@SerialVersionUID(3L)
abstract class AbstractMapView[K, +V]
    extends AbstractView[(K, V)]
    with MapView[K, V]
