...
trait SortedMap[K, +V]
  ...
  override protected def fromSpecific(
      coll: IterableOnce[(K, V)] @uncheckedVariance)
    : SortedMapCC[K, V] @uncheckedVariance = sortedMapFactory.from(coll)
  override protected def newSpecificBuilder
    : mutable.Builder[(K, V), SortedMapCC[K, V]] @uncheckedVariance =
    sortedMapFactory.newBuilder[K, V]
  ...
  override def empty: SortedMapCC[K, V] @uncheckedVariance =
...
trait SortedMapOps[K,
                   +V,
                   +CC[X, Y] <: Map[X, Y] with SortedMapOps[X, Y, CC, _],
                   +C <: SortedMapOps[K, V, CC, C]]
  ...
  /** Similar to `mapFromIterable`, but returns a SortedMap collection type.
    ...
    */
  @`inline` protected final def sortedMapFromIterable[K2, V2](
      it: Iterable[(K2, V2)])(implicit ordering: Ordering[K2]): CC[K2, V2] =
  ...
  protected trait GenKeySortedSet extends GenKeySet { this: SortedSet[K] =>
    ...
    def iteratorFrom(start: K): Iterator[K] =
      SortedMapOps.this.keysIteratorFrom(start)
  }

  override def withFilter(p: ((K, V)) => Boolean)
    : SortedMapOps.WithFilter[K, V, IterableCC, MapCC, CC] =
    new SortedMapOps.WithFilter(this, p)
  ...
  def map[K2, V2](f: ((K, V)) => (K2, V2))(
      implicit @implicitNotFound(SortedMapOps.ordMsg) ordering: Ordering[K2])
    : CC[K2, V2] =
  ...
  override def concat[V2 >: V](suffix: IterableOnce[(K, V2)]): CC[K, V2] =
    sortedMapFactory.from(suffix match {
      case it: Iterable[(K, V2)] => new View.Concat(toIterable, it)
      case _                     => iterator.concat(suffix.iterator)
    })

  /** Alias for `concat` */
  @`inline` override final def ++[V2 >: V](
      xs: IterableOnce[(K, V2)]): CC[K, V2] = concat(xs)
...
object SortedMapOps {
  private[collection] final val ordMsg =
    "No implicit Ordering[${K2}] found to build a SortedMap[${K2}, ${V2}]. You may want to upcast to a Map[${K}, ${V}] first by calling `unsorted`."
  ...
  class WithFilter[K,
                   ...
                   +MapCC[X, Y] <: Map[X, Y],
                   +CC[X, Y] <: Map[X, Y] with SortedMapOps[X, Y, CC, _]](
      self: SortedMapOps[K, V, CC, _] with MapOps[K, V, MapCC, _] with IterableOps[
        (K, V),
        IterableCC,
        _],
      p: ((K, V)) => Boolean
    ...
    def flatMap[K2: Ordering, V2](
        f: ((K, V)) => IterableOnce[(K2, V2)]): CC[K2, V2] =
      self.sortedMapFactory.from(new View.FlatMap(filtered, f))

    override def withFilter(
        q: ((K, V)) => Boolean): WithFilter[K, V, IterableCC, MapCC, CC] =
      new WithFilter[K, V, IterableCC, MapCC, CC](
        self,
        (kv: (K, V)) => p(kv) && q(kv))
...
@SerialVersionUID(3L)
object SortedMap
    extends SortedMapFactory.Delegate[SortedMap](immutable.SortedMap)
