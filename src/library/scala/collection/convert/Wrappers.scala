...
private[collection] trait Wrappers {
  ...
  @SerialVersionUID(3L)
  case class JIteratorWrapper[A](underlying: ju.Iterator[A])
  ...
  @SerialVersionUID(3L)
  case class JEnumerationWrapper[A](underlying: ju.Enumeration[A])
      extends AbstractIterator[A]
      with Iterator[A]
      with Serializable {
  ...
  @SerialVersionUID(3L)
  case class SeqWrapper[A](underlying: Seq[A])
      extends ju.AbstractList[A]
      with IterableWrapperTrait[A]
      with Serializable {
  ...
  case class JListWrapper[A](underlying: ju.List[A])
    ...
    override def clone(): JListWrapper[A] =
      JListWrapper(new ju.ArrayList[A](underlying))
    ...
    def filterInPlace(p: A => Boolean): this.type = {
      underlying.removeIf((x => p(x)): Predicate[A]); this
    }
    def remove(from: Int, n: Int): Unit =
      underlying.subList(from, from + n).clear()
    ...
    override def subtractOne(elem: A): this.type = {
      underlying.remove(elem.asInstanceOf[AnyRef]); this
    }
  }
  ...
  case class MutableSetWrapper[A](underlying: mutable.Set[A])
      extends SetWrapper[A](underlying)
      with Serializable {
  ...
  @SerialVersionUID(3L)
  class MapWrapper[K, V](underlying: Map[K, V])
  ...
  @SerialVersionUID(3L)
  case class MutableMapWrapper[K, V](underlying: mutable.Map[K, V])
      extends MapWrapper[K, V](underlying) {
    override def put(k: K, v: V) = underlying.put(k, v) match {
      case Some(v1) => v1
      case None     => null.asInstanceOf[V]
    }

    override def remove(k: AnyRef): V =
      try {
  ...
  trait JMapWrapperLike[K,
                        V,
                        +CC[X, Y] <: mutable.MapOps[X, Y, CC, _],
                        +C <: mutable.MapOps[K, V, CC, C]]
  ...
  class ConcurrentMapWrapper[K, V](
      override val underlying: concurrent.Map[K, V])
      extends MutableMapWrapper[K, V](underlying)
      with juc.ConcurrentMap[K, V] {
    ...
    override def replace(k: K, oldval: V, newval: V) =
  ...
  case class JConcurrentMapWrapper[K, V](underlying: juc.ConcurrentMap[K, V])
    ...
    override def knownSize: Int = if (underlying.isEmpty) 0 else super.knownSize
    override def empty =
      new JConcurrentMapWrapper(new juc.ConcurrentHashMap[K, V])

    def putIfAbsent(k: K, v: V): Option[V] =
      Option(underlying.putIfAbsent(k, v))
  ...
  @SerialVersionUID(3L)
  case class DictionaryWrapper[K, V](underlying: mutable.Map[K, V])
  ...
  @SerialVersionUID(3L)
  case class JDictionaryWrapper[K, V](underlying: ju.Dictionary[K, V])
      extends mutable.AbstractMap[K, V]
      with Serializable {
    override def size: Int = underlying.size
    override def isEmpty: Boolean = underlying.isEmpty
    ...
    override def clear() =
      iterator.foreach(entry => underlying.remove(entry._1))
  ...
  case class JPropertiesWrapper(underlying: ju.Properties)
    ...
    def addOne(kv: (String, String)): this.type = {
