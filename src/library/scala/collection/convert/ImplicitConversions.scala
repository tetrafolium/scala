...
trait ToScalaImplicits {
  ...
  /** Implicitly converts a Java `Enumeration` to a Scala `Iterator`.
    ...
    */
  implicit def `enumeration AsScalaIterator`[A](
      i: ju.Enumeration[A]): Iterator[A] = enumerationAsScalaIterator(i)

  /** Implicitly converts a Java `Iterable` to a Scala `Iterable`.
    *  @see [[AsScalaConverters.iterableAsScalaIterable]]
    */
  implicit def `iterable AsScalaIterable`[A](i: jl.Iterable[A]): Iterable[A] =
    iterableAsScalaIterable(i)

  /** Implicitly converts a Java `Collection` to an Scala `Iterable`.
    *  @see [[AsScalaConverters.collectionAsScalaIterable]]
    */
  implicit def `collection AsScalaIterable`[A](
      i: ju.Collection[A]): Iterable[A] = collectionAsScalaIterable(i)

  /** Implicitly converts a Java `List` to a Scala mutable `Buffer`.
    *  @see [[AsScalaConverters.asScalaBuffer]]
    */
  implicit def `list asScalaBuffer`[A](l: ju.List[A]): mutable.Buffer[A] =
    asScalaBuffer(l)
  ...
  /** Implicitly converts a Java `Map` to a Scala mutable `Map`.
    ...
    */
  implicit def `map AsScala`[K, V](m: ju.Map[K, V]): mutable.Map[K, V] =
    mapAsScalaMap(m)

  /** Implicitly converts a Java `ConcurrentMap` to a Scala mutable `ConcurrentMap`.
    *  @see [[AsScalaConverters.mapAsScalaConcurrentMap]]
    */
  implicit def `map AsScalaConcurrentMap`[K, V](
      m: juc.ConcurrentMap[K, V]): concurrent.Map[K, V] =
    mapAsScalaConcurrentMap(m)

  /** Implicitly converts a Java `Dictionary` to a Scala mutable `Map`.
    *  @see [[AsScalaConverters.dictionaryAsScalaMap]]
    */
  implicit def `dictionary AsScalaMap`[K, V](
      p: ju.Dictionary[K, V]): mutable.Map[K, V] = dictionaryAsScalaMap(p)

  /** Implicitly converts a Java `Properties` to a Scala `mutable Map[String, String]`.
    *  @see [[AsScalaConverters.propertiesAsScalaMap]]
    */
  implicit def `properties AsScalaMap`(
      p: ju.Properties): mutable.Map[String, String] = propertiesAsScalaMap(p)
}
...
trait ToJavaImplicits {
  ...
  /** Implicitly converts a Scala mutable `Set` to a Java `Set`.
    ...
    */
  implicit def `mutableSet AsJavaSet`[A](s: mutable.Set[A]): ju.Set[A] =
    mutableSetAsJavaSet(s)
  ...
  /** Implicitly converts a Scala mutable `Map` to a Java `Map`.
    ...
    */
  implicit def `mutableMap AsJavaMap`[K, V](
      m: mutable.Map[K, V]): ju.Map[K, V] = mutableMapAsJavaMap(m)

  /** Implicitly converts a Scala mutable `Map` to a Java `Dictionary`.
    *  @see [[AsJavaConverters.asJavaDictionary]]
    */
  implicit def `dictionary asJava`[K, V](
      m: mutable.Map[K, V]): ju.Dictionary[K, V] = asJavaDictionary(m)

  /** Implicitly converts a Scala `Map` to a Java `Map`.
    *  @see [[AsJavaConverters.mapAsJavaMap]]
    */
  implicit def `map AsJavaMap`[K, V](m: Map[K, V]): ju.Map[K, V] =
    mapAsJavaMap(m)

  /** Implicitly converts a Scala mutable `concurrent.Map` to a Java `ConcurrentMap`.
    *  @see [[AsJavaConverters.mapAsJavaConcurrentMap]]
    */
  implicit def `map AsJavaConcurrentMap`[K, V](
      m: concurrent.Map[K, V]): juc.ConcurrentMap[K, V] =
    mapAsJavaConcurrentMap(m)
}
