...
trait DecorateAsJava extends AsJavaConverters {
  ...
  /**
    ...
    */
  implicit def mutableMapAsJavaMapConverter[K, V](
      m: mutable.Map[K, V]): AsJava[ju.Map[K, V]] =
  ...
  /**
    ...
    */
  implicit def asJavaDictionaryConverter[K, V](
      m: mutable.Map[K, V]): AsJavaDictionary[K, V] =
  ...
  /**
    ...
    */
  implicit def mapAsJavaConcurrentMapConverter[K, V](
      m: concurrent.Map[K, V]): AsJava[juc.ConcurrentMap[K, V]] =
