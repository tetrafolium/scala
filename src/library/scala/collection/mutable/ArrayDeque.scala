...
class ArrayDeque[A] protected (
  ...
  private[this] def reset(array: Array[AnyRef], start: Int, end: Int) = {
    assert((array.length & (array.length - 1)) == 0,
           s"Array.length must be power of 2")
  ...
  @inline private[ArrayDeque] def prependAssumingCapacity(
      elem: A): this.type = {
    start = start_-(1)
    ...
    if (it.nonEmpty) {
      ...
      elems.knownSize match {
        ...
        case srcLength if srcLength < 0 =>
          prependAll(it.to(
            IndexedSeq: Factory[A, IndexedSeq[A]] /* type ascription needed by Dotty */ ))
    ...
    if (count > 0) {
      ...
      if (isResizeNecessary(finalLength)) {
        ...
        copySliceToArray(srcStart = 0,
                         ...
                         destStart = 0,
                         maxItems = idx)
        copySliceToArray(srcStart = suffixStart,
                         dest = array2,
                         destStart = idx,
                         maxItems = n)
        reset(array = array2, start = 0, end = finalLength)
        ...
        while (i >= 0) {
          i -= 1
          _set(i,
               if (i >= removals) _get(i - removals) else null.asInstanceOf[A])
        }
        ...
        while (i < n) {
          _set(
            i,
            if (i < finalLength) _get(i + removals) else null.asInstanceOf[A])
          i += 1
        }
        end = end_-(removals)
      }
  ...
  def removeHead(resizeInternalRepr: Boolean = false): A =
    ...
    else removeHeadAssumingNonEmpty(resizeInternalRepr)

  @inline private[this] def removeHeadAssumingNonEmpty(
      resizeInternalRepr: Boolean = false): A = {
  ...
  def removeLast(resizeInternalRepr: Boolean = false): A =
    if (isEmpty) throw new NoSuchElementException(s"empty collection")
    else removeLastAssumingNonEmpty(resizeInternalRepr)

  @`inline` private[this] def removeLastAssumingNonEmpty(
      resizeInternalRepr: Boolean = false): A = {
    end = end_-(1)
    val elem = array(end)
    array(end) = null
  ...
  @inline def ensureSize(hint: Int) =
    if (hint > length && isResizeNecessary(hint)) resize(hint + 1)

  def length = end_-(start)

  override def isEmpty = start == end

  override def clone(): ArrayDeque[A] =
    new ArrayDeque(array.clone(), start = start, end = end)
  ...
  override def slice(from: Int, until: Int): IterableCC[A] = {
    ...
    if (len <= 0) {
      ...
      val array2 = copySliceToArray(srcStart = left,
                                    dest = ArrayDeque.alloc(len),
                                    destStart = 0,
                                    maxItems = len)
      ofArray(array2, len)
    }
  ...
  override def sliding(window: Int, step: Int): Iterator[IterableCC[A]] = {
    require(window > 0 && step > 0,
            s"window=$window and step=$step, but both must be positive")
    val lag = if (window > step) window - step else 0
    if (length <= window) Iterator.single(slice(0, length))
    else
      Iterator
        .range(start = 0, end = length - lag, step = step)
        .map(i => slice(i, i + window))
  }

  override def grouped(n: Int): Iterator[IterableCC[A]] = sliding(n, n)

  override def copyToArray[B >: A](dest: Array[B],
                                   destStart: Int,
                                   len: Int): Int = {
    val copied =
      IterableOnce.elemsToCopyToArray(length, dest.length, destStart, len)
    if (copied > 0) {
      copySliceToArray(srcStart = 0,
                       dest = dest,
                       destStart = destStart,
  ...
  /**
    ...
    * @param maxItems
    */
  def copySliceToArray(srcStart: Int,
                       dest: Array[_],
                       destStart: Int,
                       maxItems: Int): dest.type = {
    requireBounds(destStart, dest.length + 1)
    val toCopy =
      Math.min(maxItems, Math.min(length - srcStart, dest.length - destStart))
    if (toCopy > 0) {
  ...
  @inline private[this] def start_+(idx: Int) =
    (start + idx) & (array.length - 1)
  @inline private[this] def start_-(idx: Int) =
    (start - idx) & (array.length - 1)
  ...
  @inline private[this] def _get(idx: Int): A =
    array(start_+(idx)).asInstanceOf[A]

  @inline private[this] def _set(idx: Int, elem: A) =
    array(start_+(idx)) = elem.asInstanceOf[AnyRef]

  private[this] def resize(len: Int) = if (isResizeNecessary(len)) {
    val n = length
    val array2 = copySliceToArray(srcStart = 0,
                                  dest = ArrayDeque.alloc(len),
                                  destStart = 0,
                                  maxItems = n)
  ...
  @inline private[this] def requireBounds(idx: Int, until: Int = length) =
    if (idx < 0 || idx >= until)
      throw new IndexOutOfBoundsException(
        s"$idx is out of bounds (min 0, max ${until - 1})")
...
object ArrayDeque extends StrictOptimizedSeqFactory[ArrayDeque] {
  ...
  private[mutable] def alloc(len: Int) = {
    ...
    require(size >= 0,
            s"ArrayDeque too big - cannot allocate ArrayDeque of length $len")
