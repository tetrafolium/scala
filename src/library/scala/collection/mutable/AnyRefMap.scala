...
/** This class implements mutable maps with `AnyRef` keys based on a hash table with open addressing.
  ...
  *
  */
class AnyRefMap[K <: AnyRef, V] private[collection] (defaultEntry: K => V,
  ...
  def this(initialBufferSize: Int) =
    this(AnyRefMap.exceptionDefault, initialBufferSize, true)
  ...
  override protected def fromSpecific(
      coll: scala.collection.IterableOnce[(K, V)]): AnyRefMap[K, V] = {
  ...
  override protected def newSpecificBuilder: Builder[(K, V), AnyRefMap[K, V]] =
  ...
  /** Adds a new key/value pair to this map and returns the map. */
  @inline final def addOne(key: K, value: V): this.type = {
    update(key, value); this
  }

  @inline override final def addOne(kv: (K, V)): this.type = {
  ...
  private abstract class AnyRefMapIterator[A] extends AbstractIterator[A] {
    ...
    def next(): A = {
      if (hasNext) {
        val ans =
          nextResult(kz(index).asInstanceOf[K], vz(index).asInstanceOf[V])
        index += 1
        ans
  ...
  @deprecated("Consider requiring an immutable Map or fall back to Map.concat",
              "2.13.0")
  override def +[V1 >: V](kv: (K, V1)): AnyRefMap[K, V1] =
    AnyRefMap.from(new View.Appended(toIterable, kv))

  @deprecated(
    "Use ++ with an explicit collection argument instead of + with varargs",
    "2.13.0")
  override def +[V1 >: V](elem1: (K, V1),
                          elem2: (K, V1),
                          elems: (K, V1)*): AnyRefMap[K, V1] = {
    val m = this + elem1 + elem2
    if (elems.isEmpty) m else m.concat(elems)
  }

  override def concat[V2 >: V](
      xs: scala.collection.IterableOnce[(K, V2)]): AnyRefMap[K, V2] = {
  ...
  override def ++[V2 >: V](
      xs: scala.collection.IterableOnce[(K, V2)]): AnyRefMap[K, V2] = concat(xs)
  ...
  private[this] def foreachElement[A, B](elems: Array[AnyRef],
                                         f: A => B): Unit = {
    var i, j = 0
  ...
  @`inline` final def transformValues(f: V => V): this.type =
    transformValuesInPlace(f)
  ...
  def map[K2 <: AnyRef, V2](f: ((K, V)) => (K2, V2))(
      implicit dummy: DummyImplicit): AnyRefMap[K2, V2] =
    AnyRefMap.from(new View.Map(toIterable, f))
  def flatMap[K2 <: AnyRef, V2](f: ((K, V)) => IterableOnce[(K2, V2)])(
      implicit dummy: DummyImplicit): AnyRefMap[K2, V2] =
    AnyRefMap.from(new View.FlatMap(toIterable, f))
  def collect[K2 <: AnyRef, V2](pf: PartialFunction[(K, V), (K2, V2)])(
      implicit dummy: DummyImplicit): AnyRefMap[K2, V2] =
    strictOptimizedCollect(AnyRefMap.newBuilder[K2, V2], pf)

  protected[this] def writeReplace(): AnyRef =
    new DefaultSerializationProxy(AnyRefMap.toFactory[K, V](AnyRefMap), this)
...
object AnyRefMap {
  ...
  /** A builder for instances of `AnyRefMap`.
    ...
    */
  final class AnyRefMapBuilder[K <: AnyRef, V]
      extends ReusableBuilder[(K, V), AnyRefMap[K, V]] {
  ...
  def apply[K <: AnyRef, V](elems: (K, V)*): AnyRefMap[K, V] =
    buildFromIterableOnce(elems)

  def newBuilder[K <: AnyRef, V]: ReusableBuilder[(K, V), AnyRefMap[K, V]] =
    new AnyRefMapBuilder[K, V]

  private def buildFromIterableOnce[K <: AnyRef, V](
      elems: IterableOnce[(K, V)]): AnyRefMap[K, V] = {
  ...
  def from[K <: AnyRef, V](source: IterableOnce[(K, V)]): AnyRefMap[K, V] =
    source match {
      case source: AnyRefMap[_, _] =>
        source.clone().asInstanceOf[AnyRefMap[K, V]]
  ...
  implicit def toFactory[K <: AnyRef, V](
      dummy: AnyRefMap.type): Factory[(K, V), AnyRefMap[K, V]] =
    ToFactory.asInstanceOf[Factory[(K, V), AnyRefMap[K, V]]]

  @SerialVersionUID(3L)
  private[this] object ToFactory
      extends Factory[(AnyRef, AnyRef), AnyRefMap[AnyRef, AnyRef]]
      with Serializable {
    def fromSpecific(
        it: IterableOnce[(AnyRef, AnyRef)]): AnyRefMap[AnyRef, AnyRef] =
      AnyRefMap.from[AnyRef, AnyRef](it)
    def newBuilder: Builder[(AnyRef, AnyRef), AnyRefMap[AnyRef, AnyRef]] =
      AnyRefMap.newBuilder[AnyRef, AnyRef]
  }

  implicit def toBuildFrom[K <: AnyRef, V](
      factory: AnyRefMap.type): BuildFrom[Any, (K, V), AnyRefMap[K, V]] =
    ToBuildFrom.asInstanceOf[BuildFrom[Any, (K, V), AnyRefMap[K, V]]]
  private[this] object ToBuildFrom
      extends BuildFrom[Any, (AnyRef, AnyRef), AnyRefMap[AnyRef, AnyRef]] {
    def fromSpecific(from: Any)(it: IterableOnce[(AnyRef, AnyRef)]) =
      AnyRefMap.from(it)
    def newBuilder(from: Any) = AnyRefMap.newBuilder[AnyRef, AnyRef]
  }

  implicit def iterableFactory[K <: AnyRef, V]
    : Factory[(K, V), AnyRefMap[K, V]] = toFactory[K, V](this)
  implicit def buildFromAnyRefMap[K <: AnyRef, V]
    : BuildFrom[AnyRefMap[_, _], (K, V), AnyRefMap[K, V]] = toBuildFrom(this)
}
