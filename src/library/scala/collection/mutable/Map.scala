...
trait Map[K, V]
  ...
  def withDefaultValue(d: V): Map[K, V] =
    new Map.WithDefault[K, V](this, x => d)
}
...
object Map extends MapFactory.Delegate[Map](HashMap) {
  ...
  class WithDefault[K, V](val underlying: Map[K, V], val defaultValue: K => V)
    ...
    def subtractOne(elem: K): WithDefault.this.type = {
      underlying.subtractOne(elem); this
    }

    def addOne(elem: (K, V)): WithDefault.this.type = {
      underlying.addOne(elem); this
    }

    override def concat[V2 >: V](
        suffix: collection.IterableOnce[(K, V2)]): Map[K, V2] =
