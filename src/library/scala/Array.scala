...
object Array {
  ...
  implicit def toFactory[A: ClassTag](dummy: Array.type): Factory[A, Array[A]] =
    new ArrayFactory(dummy)
  @SerialVersionUID(3L)
  private class ArrayFactory[A: ClassTag](dummy: Array.type)
  ...
  def from[A: ClassTag](it: IterableOnce[A]): Array[A] = {
    ...
    if (n > -1) {
      ...
      val iterator = it.iterator
      while (iterator.hasNext) b += iterator.next()
      b.result()
    }
  }
  ...
  /** Copy one array to another, truncating or padding with default values (if
    ...
    * @see `java.util.Arrays#copyOf`
    */
  def copyAs[A](original: Array[_], newLength: Int)(
      implicit ct: ClassTag[A]): Array[A] = {
    val runtimeClass = ct.runtimeClass
    if (runtimeClass == Void.TYPE)
      newUnitArray(newLength).asInstanceOf[Array[A]]
    else {
      val destClass = runtimeClass.asInstanceOf[Class[A]]
      if (destClass.isAssignableFrom(original.getClass.getComponentType)) {
        if (destClass.isPrimitive)
          copyOf[A](original.asInstanceOf[Array[A]], newLength)
        else {
          val destArrayClass = java.lang.reflect.Array
            .newInstance(destClass, 0)
            .getClass
            .asInstanceOf[Class[Array[AnyRef]]]
          java.util.Arrays
            .copyOf(original.asInstanceOf[Array[AnyRef]],
                    newLength,
                    destArrayClass)
            .asInstanceOf[Array[A]]
        }
  ...
  def range(start: Int, end: Int, step: Int): Array[Int] = {
    ...
    val array = new Array[Int](
      immutable.Range.count(start, end, step, isInclusive = false))
