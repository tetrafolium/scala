...
private[scala] trait PropertiesTrait {
  ...
  def propOrElse(name: String, alt: => String) =
    Option(System.getProperty(name)).getOrElse(alt)
  ...
  def scalaPropOrElse(name: String, alt: => String): String =
    scalaPropOrNone(name).getOrElse(alt)
  def scalaPropOrEmpty(name: String): String = scalaPropOrElse(name, "")
  def scalaPropOrNone(name: String): Option[String] =
    Option(scalaProps.getProperty(name)).orElse(propOrNone("scala." + name))
  ...
  /** The version number of the jar this was loaded from plus "version " prefix,
    ...
    */
  val versionString = "version " + scalaPropOrElse("version.number",
                                                   "(unknown)")
  val copyrightString = scalaPropOrElse(
    "copyright.string",
    "Copyright 2002-2019, LAMP/EPFL and Lightbend, Inc.")
  ...
  private[scala] def coloredOutputEnabled: Boolean =
    propOrElse("scala.color", "auto") match {
  ...
  private[this] def versionFor(command: String) =
  ...
  def isJavaAtLeast(version: Int): Boolean =
    isJavaAtLeast(math.max(version, 0).toString)
