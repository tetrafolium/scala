...
class Regex private[matching] (val pattern: Pattern, groupNames: String*)
  ...
  def unapplySeq(s: CharSequence): Option[List[String]] = {
    ...
    if (runMatcher(m)) Some(List.tabulate(m.groupCount) { i =>
      m.group(i + 1)
    })
    else None
  }
  ...
  /** Return all non-overlapping matches of this `Regex` in the given character
    ...
    */
  def findAllIn(source: CharSequence) =
  ...
  def unanchored: UnanchoredRegex =
    new Regex(pattern, groupNames: _*) with UnanchoredRegex {
...
object Regex {
  ...
  trait MatchData {
    ...
    private[this] lazy val nameToIndex
      : Map[String, Int] = Map[String, Int]() ++ ("" :: groupNames.toList).zipWithIndex
  ...
  @inline private def extractGroupsFromMatch(m: Match): Option[List[String]] =
    Some(List.tabulate(m.groupCount) { i =>
      m.group(i + 1)
    })
