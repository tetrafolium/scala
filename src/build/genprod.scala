...
object genprod extends App {
  ...
  trait Arity extends Group {
    ...
    def typeArgsToTupleSyntacticSugarString(xs: Seq[String]) =
      xs.mkString("(", ", ", ")")
...
""".format(i, i, commaXs, i, commaXs, commaXs)
    ...
    comment + "\n  @annotation.unspecialized def tupled: (%s) => R = {\n    %s\n  }"
      .format(typeArgsToTupleSyntacticSugarString(targs), body)
  }
...
class AbstractFunction(val i: Int)
    extends Group("AbstractFunction")
    with Arity {
