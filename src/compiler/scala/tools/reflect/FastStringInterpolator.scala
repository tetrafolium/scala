...
trait FastStringInterpolator extends FormatInterpolator {
  ...
  private def interpolated(macroApp: Tree, isRaw: Boolean): Tree =
    macroApp match {
      case Apply(Select(Apply(stringCtx @ Select(qualSC, _), parts), _interpol),
                 args)
          ...
          if (isRaw) parts
            ...
            try parts.mapConserve {
              ...
              case e: StringContext.InvalidEscapeException =>
                c.abort(parts.head.pos.withShift(e.index), e.getMessage)
            }

        var result: Tree = treated.head

        def concat(t: Tree): Unit =
          result = atPos(t.pos)((result, t) match {
            case (_, Literal(Constant(""))) => result
            case (Literal(Constant(a: String)), Literal(Constant(b: String))) =>
              Literal(Constant(a + b))
            case _ =>
              gen.mkMethodCall(
                gen.mkAttributedSelect(result, definitions.String_+),
                t :: Nil)
      ...
      case t @ Apply(Select(someStringContext, _interpol), args) =>
        ...
        _root_.scala.StringContext.standardInterpolator(
          ${if (isRaw) q"_root_.scala.Predef.identity"
          else q"_root_.scala.StringContext.processEscapes"},
          $args,
          sc.parts)
      }"""
    }
