...
abstract class FormatInterpolator {
          ...
          def alt = {
            ...
            val suggest = {
              ...
              (s0 drop e.index + 1) match {
                case r(n) =>
                  altOf { n.foldLeft(0) { case (a, o) => (8 * a) + (o - '0') } }
                case _ => ""
              }
            }
            val txt =
  ...
  sealed trait Conversion {
    ...
    def groupPosAt(g: SpecGroup, i: Int) =
      pos withPoint (pos.point + m.start(g.id) + i)
  ...
  class CharacterXn(val m: Match, val pos: Position, val argc: Int)
      extends Conversion {
    override def verify = super.verify && noPrecision && only_-("c conversion")
    def accepts(arg: Tree) =
      pickAcceptable(arg, CharTpe, ByteTpe, ShortTpe, IntTpe)
  }
  class IntegralXn(val m: Match, val pos: Position, val argc: Int)
      extends Conversion {
    override def verify = {
      def d_# =
        (cc == 'd' && hasFlag('#') &&
  ...
  class DateTimeXn(val m: Match, val pos: Position, val argc: Int)
      extends Conversion {
