...
case class VirtualDirectoryClassPath(dir: VirtualDirectory)
  ...
  protected def getSubDir(packageDirName: String): Option[AbstractFile] =
    Option(
      AbstractFileClassLoader.lookupPath(dir)(
        packageDirName.split('/').toIndexedSeq,
        directory = true))
  protected def listChildren(
      dir: AbstractFile,
      filter: Option[AbstractFile => Boolean] = None): Array[F] = filter match {
  ...
  def findClassFile(className: String): Option[AbstractFile] = {
    ...
    Option(AbstractFileClassLoader
      .lookupPath(dir)(relativePath.split('/').toIndexedSeq, directory = false))
  }
