...
sealed trait ZipAndJarFileLookupFactory {
  ...
  def create(zipFile: AbstractFile,
             settings: Settings,
             closeableRegistry: CloseableRegistry): ClassPath = {
    cache.checkCacheability(
      zipFile.toURL :: Nil,
      checkStamps = true,
      disableCache = settings.YdisableFlatCpCaching.value || zipFile.file == null) match {
      case Left(_) =>
        val result: ClassPath with Closeable =
  ...
  protected def createForZipFile(
      ...
      release: Option[String]): ClassPath with Closeable
}
...
object ZipAndJarClassPathFactory extends ZipAndJarFileLookupFactory {
  ...
  private object ManifestResourcesClassPath {
    case class PackageFileInfo(packageFile: AbstractFile,
...
final class FileBasedCache[T] {
  ...
  def checkCacheability(
      urls: Seq[URL],
      checkStamps: Boolean,
      disableCache: Boolean): Either[String, Seq[java.nio.file.Path]] = {
    import scala.reflect.io.{AbstractFile, Path}
    lazy val urlsAndFiles = urls
      .filterNot(_.getProtocol == "jrt")
      .map(u => u -> AbstractFile.getURL(u))
    ...
    if (!checkStamps) Right(paths)
      ...
      val nonJarZips = urlsAndFiles.filter {
        case (url, file) => file == null || !Jar.isJarOrZip(file.file)
      }
      if (nonJarZips.nonEmpty)
        Left(
          s"caching is disabled because of the following classpath elements: ${nonJarZips.map(_._1).mkString(", ")}.")
      else Right(paths)
    }
  }

  def getOrCreate(paths: Seq[Path],
                  create: () => T,
                  closeableRegistry: CloseableRegistry,
                  checkStamps: Boolean): T = cache.synchronized {
    val stamps =
      if (!checkStamps) Nil
      else
        paths.map { path =>
    ...
    cache.get(paths) match {
      case Some(e @ Entry(cachedStamps, cached)) =>
        if (!checkStamps || cachedStamps == stamps) {
          ...
          closeableRegistry.registerClosable(
            referenceCountDecrementer(e, paths))
          cached
        } else {
          ...
          closeableRegistry.registerClosable(
            referenceCountDecrementer(entry, paths))
          value
        }
      case _ =>
        // Cache miss
...
object FileBasedCache {
  ...
  private val deferCloseMs =
    Integer.getInteger("scalac.filebasedcache.defer.close.ms", 1000)
