...
object ClassPathEntries {
  ...
  implicit def entry2Tuple(
      entry: ClassPathEntries): (scala.collection.Seq[PackageEntry],
...
private[nsc] trait NoSourcePaths {
  ...
  final private[nsc] def sources(inPackage: String): Seq[SourceFileEntry] =
    Seq.empty
}

private[nsc] trait NoClassPaths {
  final def findClassFile(className: String): Option[AbstractFile] = None
  private[nsc] final def classes(inPackage: String): Seq[ClassFileEntry] =
    Seq.empty
}
