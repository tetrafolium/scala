...
trait PositionFiltering extends InternalReporter with Filtering {
  ...
  private val positions = mutable
    .Map[Position, Severity]() withDefaultValue INFO
  private val messages = mutable
    .Map[Position, List[String]]() withDefaultValue Nil
  ...
  override protected def filter(pos: Position,
    ...
    severity match {
      ...
      case _ =>
        !testAndLog(pos, severity, msg) || {
          suppressed(pos, msg, severity); false
        }
    }
  ...
  private def testAndLog(pos: Position,
    ...
    pos != null && pos.isDefined && {
      ...
      val suppress = positions(fpos) match {
        ...
        case `severity` =>
          matchAt(fpos, msg) // already issued this (in)exact message
  ...
  private def matchAt(pos: Position, msg: String): Boolean =
    messages(pos).exists(msg.startsWith(_))
}

/** This reporter implements filtering by severity and position.
  */
class PositionFilter(settings: Settings,
                     protected val delegate: InternalReporter)
    extends InternalReporter
    with FilteringReporter
    with PositionFiltering {
  protected def noWarnings = settings.nowarnings
  protected def suppressed(pos: Position,
                           msg: String,
