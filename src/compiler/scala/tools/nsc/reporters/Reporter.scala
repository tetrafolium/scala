...
abstract class Reporter extends InternalReporter {
  ...
  @deprecated("Use echo, as internal.Reporter does not support unforced info",
              since = "2.13.0")
  final def info(pos: Position, msg: String, force: Boolean): Unit =
    info0(pos, msg, INFO, force)
...
object Reporter {
  ...
  class AdaptedReporter(val delegate: InternalReporter)
      extends Reporter
      with ForwardingReporter {
    override protected def info0(pos: Position,
                                 msg: String,
                                 severity: Severity,
                                 force: Boolean): Unit = delegate.echo(pos, msg)
    ...
    override def resetCount(severity: Severity) =
      delegate.resetCount(other(severity))
  ...
  class LimitingReporter(settings: Settings, delegate0: Reporter)
      extends AdaptedReporter(delegate0)
      with FilteringReporter
      with LimitedReporter {
    override protected def filter(pos: Position,
                                  msg: String,
                                  severity: Severity) =
      severity match {
    ...
    override protected def info0(pos: Position,
      ...
      severity match {
        case ERROR =>
          delegate.error(pos, msg) // for symmetry, but error and warn are already forwarded
  ...
  implicit def `adapt reporter to legacy API`(
      reporter: InternalReporter): Reporter =
    reporter match {
  ...
  def limitedReporter(settings: Settings, reporter: Reporter): Reporter =
    reporter match {
      ...
      case _
          if settings.maxerrs.isSetByUser && settings.maxerrs.value < settings.maxerrs.default =>
  ...
  def apply(settings: Settings): Reporter = {
    ...
    val res = loader.create[InternalReporter](settings.reporter.value,
                                              settings.errorFn)(settings)
