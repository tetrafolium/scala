...
trait Warnings {
  ...
  val warnOctalLiteral =
    BooleanSetting("-Ywarn-octal-literal", "Warn on obsolete octal syntax.")

  object UnusedWarnings extends MultiChoiceEnumeration {
    val Imports =
      Choice("imports", "Warn if an import selector is not referenced.")
    val PatVars =
      Choice("patvars", "Warn if a variable bound in a pattern is unused.")
    ...
    val Explicits =
      Choice("explicits", "Warn if an explicit parameter is unused.")
    val Implicits =
      Choice("implicits", "Warn if an implicit parameter is unused.")
    val Params = Choice("params",
                        "Enable -Ywarn-unused:explicits,implicits.",
                        expandsTo = List(Explicits, Implicits))
    val Linted = Choice("linted",
                        "-Xlint:unused.",
                        expandsTo = List(Imports, Privates, Locals, Implicits))
  }
  ...
  object LintWarnings extends MultiChoiceEnumeration {
    ...
    val NullaryUnit =
      LintWarning("nullary-unit", "Warn when nullary methods return Unit.")
