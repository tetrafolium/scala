...
class MutableSettings(val errorFn: String => Unit)
  ...
  def copyInto(settings: MutableSettings): Unit = {
    allSettings foreach { thisSetting =>
      val otherSetting = settings.allSettings find {
  ...
  /** Iterates over the arguments applying them to settings where applicable.
    ...
    */
  def processArguments(arguments: List[String],
                       processAll: Boolean): (Boolean, List[String]) = {
    def loop(args: List[String],
             residualArgs: List[String]): (Boolean, List[String]) = args match {
      case Nil =>
  ...
  def processArgumentString(params: String) =
    processArguments(splitParams(params), processAll = true)
  ...
  def BooleanSetting(name: String, descr: String) =
    add(new BooleanSetting(name, descr))
  def ChoiceSetting(name: String,
                    helpArg: String,
                    descr: String,
                    choices: List[String],
                    default: String,
                    choicesHelp: List[String]) =
  ...
  def ChoiceSettingForcedDefault(name: String,
                                 ...
                                 default: String,
                                 choicesHelp: List[String]) =
    ChoiceSetting(name, helpArg, descr, choices, default, choicesHelp)
  ...
  def MultiStringSetting(name: String,
    ...
    add(new MultiStringSetting(name, arg, descr, helpText))
  ...
  def OutputSetting(outputDirs: OutputDirs, default: String) =
    add(new OutputSetting(outputDirs, default))
  def PhasesSetting(name: String, descr: String, default: String = "") =
    add(new PhasesSetting(name, descr, default))
  def StringSetting(name: String,
                    arg: String,
                    descr: String,
                    default: String,
                    helpText: Option[String]) =
    add(new StringSetting(name, arg, descr, default, helpText))
  def ScalaVersionSetting(name: String,
                          arg: String,
                          descr: String,
                          initial: ScalaVersion,
                          default: Option[ScalaVersion] = None) =
  ...
  def PrefixSetting(name: String,
                    prefix: String,
                    descr: String): PrefixSetting =
    add(new PrefixSetting(name, prefix, descr))
  ...
  class OutputDirs {
    ...
    private def checkDir(dir: AbstractFile,
      ...
      if (dir != null && dir.isDirectory)
        dir
      else if (allowJar && dir == null && Jar.isJarOrZip(name,
                                                         examineFile = false))
    ...
    /** Return the source file path(s) which correspond to the given
      ...
      *  paths.
      */
    def srcFilesFor(classFile: AbstractFile,
  ...
  class IntSetting private[nsc] (name: String,
                                 descr: String,
                                 val default: Int,
  ...
  class BooleanSetting private[nsc] (name: String, descr: String)
      extends Setting(name, descr) {
    type T = Boolean
  ...
  class PrefixSetting private[nsc] (name: String, prefix: String, descr: String)
      extends Setting(name, descr) {
    type T = List[String]
  ...
  class StringSetting private[nsc] (name: String,
                                    val arg: String,
                                    descr: String,
                                    val default: String,
                                    helpText: Option[String])
  ...
  /** A setting represented by a Scala version.
    ...
    */
  class ScalaVersionSetting private[nsc] (name: String,
                                          val arg: String,
                                          descr: String,
  ...
  class PathSetting private[nsc] (name: String,
                                  descr: String,
                                  default: String,
  ...
  class MultiChoiceSetting[E <: MultiChoiceEnumeration] private[nsc] (
      ...
      val default: Option[List[String]]
                      ...
                      s"$descr: `_` for all, `$name:help` to list choices.")
      with Clearable {
    ...
    object ChoiceOrVal {
      def unapply(
          a: domain.Value): Option[(String, String, List[domain.Choice])] =
        a match {
    ...
    private def badChoice(s: String) =
      errorFn(s"'$s' is not a valid choice for '$name'")
    ...
    def descriptions: List[String] = domain.values.toList map {
      case ChoiceOrVal(_, "", x :: xs) =>
        "Enables the options " + (x :: xs).map(_.name).mkString(", ")
    ...
    override def tryToSetColon(args: List[String]) =
      tryToSetArgs(args, halting = false)
    override def tryToSetFromPropertyValue(s: String) =
      tryToSet(s.trim.split(',').toList) // used from ide
    ...
    // programmatically.
    def enable(choice: domain.Value): Unit = {
      nays -= choice; yeas += choice; compute()
    }

    // programmatically. Disabling expanding option is otherwise disallowed.
    def disable(choice: domain.Value): Unit = {
      yeas -= choice; nays += choice; compute()
    }
    ...
    override def help: String = {
      ...
      val verboseDefault = default match {
        case Some("_" :: Nil) =>
          Some("All choices are enabled by default." :: Nil)
        case _ => default
      }
      val orelse = verboseDefault
        .map(_.mkString(f"%nDefault: ", ", ", f"%n"))
        .getOrElse("")
      choices
        .zipAll(descriptions, "", "")
        .map(describe)
        .mkString(f"${descr}%n", f"%n", orelse)
    }
    ...
    def contains(s: String) =
      domain.values.find(_.toString == s).exists(value.contains)
  }

  /** A setting that accumulates all strings supplied to it,
    *  until it encounters one starting with a '-'.
    */
  class MultiStringSetting private[nsc] (name: String,
                                         val arg: String,
                                         descr: String,
                                         helpText: Option[String])
      extends Setting(name, descr)
  ...
  /** A setting represented by a string in a given set of `choices`,
    *  (`default` unless set).
    */
  class ChoiceSetting private[nsc] (name: String,
                                    val helpArg: String,
                                    descr: String,
      ...
      extends Setting(
        name,
        if (choicesHelp.isEmpty)
          s"$descr Choices: ${choices.mkString("(", ",", ")")}, default: $default."
    ...
    private def choicesHelpMessage =
      if (choicesHelp.isEmpty) ""
      else {
    ...
    private def usageErrorMessage =
      f"Usage: $name:<$helpArg> where <$helpArg> choices are ${choices mkString ", "} (default: $default).%n$choicesHelpMessage"
  ...
  protected class EnableSettings[T <: BooleanSetting](val s: T) {
    ...
    def disabling(toDisable: List[BooleanSetting]): s.type =
      s withPostSetHook (_ => toDisable foreach (_.value = !s.value))
    def andThen(f: s.T => Unit): s.type =
      s withPostSetHook (setting => f(setting.value))
  }
  import scala.language.implicitConversions
  protected implicit def installEnableSettings[T <: BooleanSetting](
      s: T): EnableSettings[T] = new EnableSettings(s)
}
