...
abstract class TreeInfo
    extends scala.reflect.internal.TreeInfo
    with MacroAnnotionTreeInfo {
  val global: Global
  import global._
  ...
  object ValueClass {
    def isValueClass(tpe: Type) =
      enteringErasure(tpe.typeSymbol.isDerivedValueClass)
    def valueUnbox(tpe: Type) =
      enteringErasure(tpe.typeSymbol.derivedValueClassUnbox)
  ...
  object ArrayInstantiation {
    def unapply(tree: Apply) = tree match {
      case Apply(Select(New(tpt), name), arg :: Nil)
          if tpt.tpe != null && tpt.tpe.typeSymbol == definitions.ArrayClass =>
        tpt.tpe match {
          case erasure.GenericArray(level, componentType) =>
            Some((level, componentType, arg))
          case _ => None
        }
      case _ => None
    }
