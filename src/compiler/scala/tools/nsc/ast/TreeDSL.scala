  ...
  object CODE {
    ...
    class TreeMethods(target: Tree) {
      ...
      def GEN_|(other: Tree, kind: ClassSymbol) =
        fn(target, getMember(kind, nme.OR), other)
      def GEN_&(other: Tree, kind: ClassSymbol) =
        fn(target, getMember(kind, nme.AND), other)
      def GEN_==(other: Tree, kind: ClassSymbol) =
        fn(target, getMember(kind, nme.EQ), other)
      def GEN_!=(other: Tree, kind: ClassSymbol) =
      ...
      /** Casting & type tests -- working our way toward understanding exactly
        ...
        */
      def AS(tpe: Type) =
        gen.mkAsInstanceOf(target, tpe, any = true, wrapInApply = false)
