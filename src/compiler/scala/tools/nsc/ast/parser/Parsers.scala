...
trait Parsers extends Scanners with MarkupParsers with ParsersCommon {
  ...
  class SourceFileParser(val source: SourceFile) extends Parser {
    ...
    def incompleteInputError(msg: String): Unit =
      throw new MalformedInput(source.content.length - 1, msg)
  ...
  class OutlineParser(source: SourceFile) extends SourceFileParser(source) {
    ...
    override def templateBody(isPre: Boolean) =
      skipBraces((noSelfType, EmptyTree.asList))
  }

  class UnitParser(override val unit: global.CompilationUnit,
    ...
    override def deprecationWarning(offset: Offset,
                                    msg: String,
                                    since: String): Unit =
  ...
  abstract class Parser extends ParserCommon { parser =>
    ...
    def r2p(start: Offset, mid: Offset, end: Offset): Position =
      rangePos(source, start, mid, end)
    def r2p(start: Offset, mid: Offset): Position =
      r2p(start, mid, in.lastOffset max start)
    ...
    def scriptBody(): Tree = {
      ...
      def searchForMain(): Tree = {
        ...
        def isApp(t: Tree) = t match {
          case Template(parents, _, _) =>
            parents.exists(cond(_) { case Ident(tpnme.App) => true })
          case _ => false
        }
        ...
        val newStmts = stmts.map {
          case md @ ModuleDef(mods, name, template)
              if !seenModule && (isApp(template) || md.exists(isMainMethod)) =>
            seenModule = true
        ...
        if (disallowed.isEmpty) makeEmptyPackage(0, newStmts)
          ...
          if (seenModule)
            warning(disallowed.pos.point,
                    "Script has a main object but statement is disallowed")
          EmptyTree
        }
      }
      ...
      def repackaged: Tree = {
        ...
        // def main
        def mainParamType =
          AppliedTypeTree(Ident(tpnme.Array), List(Ident(tpnme.String)))
        def mainParameter =
          List(
            ValDef(Modifiers(Flags.PARAM), nme.args, mainParamType, EmptyTree))
        def mainDef =
          DefDef(NoMods,
                 nme.main,
                 Nil,
                 List(mainParameter),
                 scalaDot(tpnme.Unit),
                 gen.mkAnonymousNew(stmts))

        // object Main
        def moduleName = newTermName(ScriptRunner scriptMain settings)
        def moduleBody =
          Template(atInPos(scalaAnyRefConstr) :: Nil,
                   noSelfType,
                   List(emptyInit, mainDef))
    ...
    def isWildcard(t: Tree): Boolean = t match {
      case Ident(name1) =>
        !placeholderParams.isEmpty && name1 == placeholderParams.head.name
    ...
    val assumedClosingParens =
      mutable.Map(RPAREN -> 0, RBRACKET -> 0, RBRACE -> 0)
    ...
    def syntaxErrorOrIncompleteAnd[T](msg: String, skipIt: Boolean)(
        and: T): T = {
      syntaxErrorOrIncomplete(msg, skipIt)
      and
    }

    def expectedMsgTemplate(exp: String, fnd: String) =
      s"$exp expected but $fnd found."
    def expectedMsg(token: Token): String =
      in.token match {
        case NEWLINE | NEWLINES => s"${token2string(token)} expected."
        case actual =>
          expectedMsgTemplate(token2string(token), token2string(actual))
      }
    ...
    def isTypeIntroToken(token: Token): Boolean =
      (isLiteralToken(token) && token != NULL) || (token match {
    ...
    def atPos[T <: Tree](start: Offset, point: Offset)(t: T): T =
      atPos(r2p(start, point))(t)
    def atPos[T <: Tree](start: Offset, point: Offset, end: Offset)(t: T): T =
      atPos(r2p(start, point, end))(t)
    ...
    def convertToParam(tree: Tree): ValDef = atPos(tree.pos) {
      ...
      def propagateNoWarnAttachment(from: Tree, to: ValDef): to.type =
        if (from.hasAttachment[NoWarnAttachment.type])
          to.updateAttachment(NoWarnAttachment)
        else to
      tree match {
        case id @ Ident(name) =>
          removeAsPlaceholder(name)
          propagateNoWarnAttachment(
            id,
    ...
    final def tokenSeparated[T](separator: Token,
                                sepFirst: Boolean,
                                part: => T): List[T] = {
    ...
    private[this] def checkTupleSize(elems: List[Tree]): Boolean =
      elems.lengthCompare(definitions.MaxTupleArity) <= 0 || {
        ...
        val msg =
          s"tuples may not have more than ${definitions.MaxFunctionArity} elements, but ${elems.length} given"
    ...
    private[this] def checkFunctionArity(argtpes: List[Tree]): Boolean =
      argtpes.lengthCompare(definitions.MaxFunctionArity) <= 0 || {
        ...
        val msg =
          s"function values may not have more than ${definitions.MaxFunctionArity} parameters, but ${argtpes.length} given"
    ...
    def makeBinop(isExpr: Boolean,
                  left: Tree,
                  op: TermName,
                  right: Tree,
                  opPos: Position,
                  targs: List[Tree] = Nil): Tree = {
      ...
      def mkNamed(args: List[Tree]) =
        if (isExpr) args map treeInfo.assignmentToMaybeNamedArg else args
      ...
      if (isExpr) {
        if (rightAssoc) {
          ...
          val liftedArg = atPos(left.pos) {
            ValDef(Modifiers(FINAL | SYNTHETIC | ARTIFACT),
                   x,
                   TypeTree(),
                   stripParens(left))
          }
    ...
    def checkHeadAssoc(leftAssoc: Boolean) =
    ...
    def finishPostfixOp(start: Int,
                        base: List[OpInfo],
                        opinfo: OpInfo): Tree = {
      if (opinfo.targs.nonEmpty)
        syntaxError(opinfo.offset,
    ...
    def reduceExprStack(base: List[OpInfo], top: Tree): Tree =
      reduceStack(isExpr = true, base, top)
    def reducePatternStack(base: List[OpInfo], top: Tree): Tree =
      reduceStack(isExpr = false, base, top)

    def reduceStack(isExpr: Boolean, base: List[OpInfo], top: Tree): Tree = {
      val opPrecedence =
        if (isIdent) Precedence(in.name.toString) else Precedence(0)
      val leftAssoc = !isIdent || (nme isLeftAssoc in.name)

      reduceStack(isExpr, base, top, opPrecedence, leftAssoc)
    }

    def reduceStack(isExpr: Boolean,
                    base: List[OpInfo],
                    top: Tree,
                    opPrecedence: Precedence,
                    leftAssoc: Boolean): Tree = {
      ...
      def loop(top: Tree): Tree =
        if (canReduce) {
          ...
          if (!isExpr && info.targs.nonEmpty) {
            syntaxError(info.offset,
                        "type application is not allowed in pattern")
    ...
    sealed trait PatternContextSensitive {
      ...
      private def tupleInfixType(start: Offset) = {
        ...
        if (in.token == RPAREN) {
          ...
          accept(RPAREN)
          if (in.token == ARROW)
            atPos(start, in.skipToken()) { makeSafeFunctionType(ts, typ()) } else {
      ...
      /** {{{
        ...
        *  }}}
        */
      def annotType(): Tree = placeholderTypeBoundary {
        annotTypeRest(simpleType())
      }
      ...
      def simpleType(): Tree = {
        if (isLiteralToken(in.token) && in.token != NULL)
          atPos(in.offset) { SingletonTypeTree(literal()) } else if (in.name == raw.MINUS && lookingAhead(
                                                                       isNumericLit)) {
          val start = in.offset
          in.nextToken()
          atPos(start) {
            SingletonTypeTree(literal(isNegated = true, start = start))
          }
        } else {
          val start = in.offset
      ...
      def compoundTypeRest(t: Tree): Tree = {
        ...
        if (hasRefinement) types match {
          case Ident(name) :: Nil if name endsWith "Unit" =>
            warning(
              braceOffset,
              "Detected apparent refinement of Unit; are you missing an '=' sign?")
          case _ =>
        }
        // The second case includes an empty refinement - refinements is empty, but
        // it still gets a CompoundTypeTree.
        ts.toList match {
          case tp :: Nil if !hasRefinement =>
            tp // single type, no refinement, already positioned
          case tps =>
            atPos(t.pos.start)(
              CompoundTypeTree(Template(tps, noSelfType, refinements)))
        }
      }
      ...
      def infixTypeRest(t: Tree, mode: InfixMode.Value): Tree = {
        ...
        def asInfix = {
          ...
          def mkOp(t1: Tree) = atPos(t.pos.start, opOffset) {
            AppliedTypeTree(tycon, List(t, t1))
          }
          if (leftAssoc)
    ...
    def selector(t: Tree): Tree = {
      ...
      if (t != EmptyTree)
        Select(t, ident(skipIt = false)) setPos r2p(t.pos.start,
                                                    point,
                                                    in.lastOffset)
      else
      ...
      if (in.token == THIS) {
        ...
        if (in.token == DOT) {
          ...
          if (in.token == THIS) {
            ...
            in.nextToken()
            t = atPos(start) {
              Super(This(name.toTypeName), mixinQualifierOpt())
            }
            accept(DOT)
            t = selector(t)
            if (in.token == DOT) t = selectors(t, typeOK, in.skipToken())
    ...
    def selectors(t: Tree, typeOK: Boolean, dotOffset: Offset): Tree =
      if (typeOK && in.token == TYPE) {
        ...
        val t1 = selector(t)
        if (in.token == DOT) { selectors(t1, typeOK, in.skipToken()) } else t1
      }
    ...
    def qualId(): Tree = {
      ...
      if (in.token == DOT) { selectors(id, typeOK = false, in.skipToken()) } else
        id
    }
    ...
    /** {{{
      ...
      *  }}}
      */
    def literal(isNegated: Boolean = false,
      ...
      if (in.token == INTERPOLATIONID)
        ...
        finish(in.token match {
          ...
          case NULL                   => null
          case _ =>
            syntaxErrorOrIncompleteAnd("illegal literal", skipIt = true)(null)
        })
    }
    ...
    /** {{{
      ...
      */
    def typedOpt(): Tree =
      if (in.token == COLON) { in.nextToken(); typ() } else TypeTree()
    ...
    def wildcardType(start: Offset) = {
      ...
      val param = atPos(t.pos union bounds.pos) {
        makeSyntheticTypeParam(pname, bounds)
      }
     ...
     */
    def statement(location: Location): Tree =
      expr(location) // !!! still needed?
    ...
    def expr(location: Location): Tree =
      withPlaceholders(expr0(location), isAny = false)

    def expr0(location: Location): Tree =
      (in.token: @scala.annotation.switch) match {
        case IF =>
          def parseIf = atPos(in.skipToken()) {
            ...
            val thenp = expr()
            val elsep =
              if (in.token == ELSE) { in.nextToken(); expr() } else literalUnit
            If(cond, thenp, elsep)
          }
          parseIf
        case TRY =>
                  ...
                  if (isWildcard(t))
                    (placeholderParams: @unchecked) match {
                      case (vd @ ValDef(mods, name, _, _)) :: rest =>
                        placeholderParams = treeCopy.ValDef(vd,
                                                            mods,
                                                            name,
                                                            tpt.duplicate,
                                                            EmptyTree) :: rest
        ...
        atPos(in.offset) {
          if (lookingAhead(isSimpleExprIntro)) {
            ...
            if (uname == nme.UNARY_- && isNumericLit)
              ...
              simpleExprRest(literal(isNegated = true, start = namePos),
                             canApply = true)
            else
      ...
      val t =
        if (isLiteral) literal()
          ...
          in.token match {
            ...
            case _ =>
              syntaxErrorOrIncompleteAnd("illegal start of simple expression",
                                         skipIt = true)(errorTermTree)
          }
    ...
    /** {{{
      ...
      */
    def guard(): Tree =
      if (in.token == IF) { in.nextToken(); stripParens(postfixExpr()) } else
        EmptyTree
    ...
    def enumerator(isFirst: Boolean,
                   allowNestedIf: Boolean = true): List[Tree] =
    ...
    def generator(eqOK: Boolean, allowNestedIf: Boolean = true): List[Tree] = {
      ...
      if (hasVal) {
        def msg(what: String, instead: String): String =
          s"`val` keyword in for comprehension is $what: $instead"
        if (hasEq) {
          val without = "instead, bind the value without `val`"
          if (currentRun.isScala214)
            syntaxError(in.offset, msg("unsupported", without))
          else
            deprecationWarning(in.offset, msg("deprecated", without), "2.10.0")
    ...
    def makeFilter(start: Offset, tree: Tree) =
      gen.Filter(tree).setPos(r2p(start, tree.pos.point, tree.pos.end))
    ...
    /** Methods which implicitly propagate whether the initial call took
      ...
      */
    final class SeqContextSensitive(val isSequenceOK: Boolean,
                                    val isXML: Boolean)
      ...
      def pattern(): Tree = {
        ...
        def loop(): List[Tree] = pattern1() :: {
          if (isRawBar) { in.nextToken(); loop() } else Nil
        }
        loop() match {
          case pat :: Nil => pat
      ...
      def pattern2(): Tree = (pattern3(), in.token) match {
        ...
        case (p @ Ident(name), AT) =>
          ...
          atPos(p.pos.start, p.pos.start, body.pos.end) {
            ...
            body match {
              case Ident(nme.WILDCARD) if settings.warnUnusedPatVars =>
                t updateAttachment NoWarnAttachment
              case _ => t
            }
          }
        case (p, _) => p
      ...
      def pattern3(): Tree = {
        ...
        def checkWildStar: Tree = top match {
          case Ident(nme.WILDCARD) if isSequenceOK && isRawStar =>
            peekingAhead(
              if (isCloseDelim)
      ...
      def badPattern3(): Tree = {
        ...
        def isSeqPatternClose =
          isUnderscore && isStar && isSequenceOK && isDelimiter
        ...
        val subtext = (isUnderscore, isStar, isSequenceOK) match {
          ...
          case (true, true, true) if isDelimiter =>
            "bad brace or paren after _*"
          case (true, true, false) if isDelimiter =>
            "bad use of _* (sequence pattern not allowed)"
          case (false, true, true) if isDelimiter =>
            "use _* to match a sequence"
          case (false, true, _) if isCommaOrDelimiter =>
            "trailing * is not a valid pattern"
          case _ => null
        }
        val msg =
          if (subtext != null) s"$preamble $subtext"
          else "illegal start of simple pattern"
        ...
        in.token match {
          case IDENTIFIER | BACKQUOTED_IDENT | THIS =>
            ...
            val typeAppliedTree = in.token match {
              case LBRACKET =>
                atPos(start, in.offset)(
                  AppliedTypeTree(convertToTypeId(t), typeArgs()))
              case _ => t
            }
            in.token match {
              case LPAREN =>
    ...
    final val seqOK =
      new SeqContextSensitive(isSequenceOK = true, isXML = false)

    /** The implementation for parsing inside of patterns at points where sequences are disallowed. */
    final val noSeq =
      new SeqContextSensitive(isSequenceOK = false, isXML = false)

    /** For use from xml pattern, where sequence is allowed and encouraged. */
    final val xmlSeqOK =
      new SeqContextSensitive(isSequenceOK = true, isXML = true)
    ...
    def xmlSeqPatterns(): List[Tree] =
      xmlSeqOK.patterns() // Called from xml parser
    ...
    private def addMod(mods: Modifiers, mod: Long, pos: Position): Modifiers = {
      if (mods hasFlag mod)
        syntaxError(in.offset, "repeated modifier", skipIt = false)
      in.nextToken()
    ...
    def modifiers(): Modifiers = normalizeModifiers {
      def loop(mods: Modifiers): Modifiers = in.token match {
        case PRIVATE | PROTECTED =>
          loop(
            accessQualifierOpt(
              addMod(mods, flagTokens(in.token), tokenRange(in))))
    ...
    /** {{{
      ...
      *  }}}
      */
    def paramClauses(owner: Name,
                     contextBounds: List[Tree],
                     ofCaseClass: Boolean): List[List[ValDef]] = {
        ...
        if (in.token == RPAREN) Nil
          ...
          val implicitmod =
            if (in.token == IMPLICIT) {
              if (implicitOffset == -1) {
                implicitOffset = in.offset; implicitSection = vds.length
      ...
      if (ofCaseClass) {
        ...
        if (vds.isEmpty)
          syntaxError(
            start,
            s"case classes must have a parameter list; try 'case class $name()' or 'case object $name'")
        else if (vds.head.nonEmpty && vds.head.head.mods.isImplicit) {
          if (currentRun.isScala213)
            syntaxError(
              start,
              s"case classes must have a non-implicit parameter list; try 'case class $name()$elliptical'")
          else {
            deprecationWarning(
              start,
              s"case classes should have a non-implicit parameter list; adapting to 'case class $name()$elliptical'",
              "2.12.2")
            vds.insert(0, List.empty[ValDef])
            vds(1) = vds(1).map(vd =>
              copyValDef(vd)(mods = vd.mods & ~Flags.CASEACCESSOR))
            if (implicitSection != -1) implicitSection += 1
          }
        }
      }
      if (implicitSection != -1 && implicitSection != vds.length - 1)
        syntaxError(implicitOffset,
                    "an implicit parameter section must be last")
      if (warnAt != -1)
        syntaxError(warnAt,
                    "multiple implicit parameter sections are not allowed")
      else if (settings.warnExtraImplicit) {
        // guard against anomalous class C(private implicit val x: Int)(implicit s: String)
        val ttl = vds.count {
          case ValDef(mods, _, _, _) :: _ => mods.isImplicit; case _ => false
        }
        if (ttl > 1)
          warning(in.offset,
                  s"$ttl parameter sections are effectively implicit")
      }
    ...
    /** {{{
      ...
      */
    def paramType(): Tree =
      paramType(repeatedParameterOK = true, useStartAsPosition = false)
    def paramType(repeatedParameterOK: Boolean,
                  useStartAsPosition: Boolean): Tree = {
      val start = in.offset
      in.token match {
        case ARROW =>
          in.nextToken()
    ...
    def param(owner: Name, implicitmod: Long, caseParam: Boolean): ValDef = {
      ...
      atPos(start, if (name == nme.ERROR) start else nameOffset) {
        ValDef((mods | implicitmod | bynamemod) withAnnotations annots,
               name.toTermName,
               tpt,
               default)
      }
    ...
    /** {{{
      ...
      *  }}}
      */
    def typeParamClauseOpt(owner: Name,
                           contextBoundBuf: ListBuffer[Tree]): List[TypeDef] = {
      def typeParam(ms: Modifiers): TypeDef = {
        ...
        if (contextBoundBuf ne null) {
          def msg(what: String) =
            s"""view bounds are $what; use an implicit parameter instead.
                                     |  example: instead of `def f[A <% Int](a: A)` use `def f[A](a: A)(implicit ev: A => Int)`""".stripMargin
          while (in.token == VIEWBOUND) {
            if (currentRun.isScala214)
              syntaxError(in.offset, msg("unsupported"))
      ...
      if (in.token == LBRACKET)
        inBrackets(
          commaSeparated(
            typeParam(NoMods withAnnotations annotations(skipNewLines = true))))
      else Nil
    }
    ...
    def bound(tok: Token): Tree =
      if (in.token == tok) { in.nextToken(); typ() } else EmptyTree
        ...
        val selectors: List[ImportSelector] = in.token match {
          ...
          case _ =>
            ...
            if (in.token == DOT) {
              ...
              val t =
                atPos(start, if (name == nme.ERROR) in.offset else nameOffset)(
                  Select(expr, name))
              in.nextToken()
              return loop(t)
            }
    ...
    def importSelectors(): List[ImportSelector] = {
      ...
      for (t <- selectors.init if t.isWildcard)
        syntaxError(t.namePos, "Wildcard import must be in last position")
      selectors
    }
    ...
    def importSelector(): ImportSelector = {
      ...
      val rename = in.token match {
        case ARROW =>
          ...
          if (name == nme.WILDCARD && !bbq)
            syntaxError(renameOffset, "Wildcard import cannot be renamed")
          wildcardOrIdent()
    ...
    private def caseAwareTokenOffset =
      if (in.token == CASECLASS || in.token == CASEOBJECT) in.prev.offset
      else in.offset
    ...
    def patDefOrDcl(pos: Int, mods: Modifiers): List[Tree] = {
      ...
      def mkDefs(p: Tree, tp: Tree, rhs: Tree): List[Tree] = {
        val trees = {
          val pat =
            if (tp.isEmpty) p else Typed(p, tp) setPos (p.pos union tp.pos)
        ...
        if (newmods.isDeferred) {
          trees match {
            ...
            case _ =>
              syntaxError(p.pos,
                          "pattern definition may not be abstract",
                          skipIt = false)
          }
        }
        trees
      }
      val trees = (lhs.toList.init flatMap (mkDefs(
        _,
        tp.duplicate,
        rhs.duplicate))) ::: mkDefs(lhs.last, tp, rhs)
      val hd = trees.head
      ...
      if (in.token == THIS) {
        atPos(start, in.skipToken()) {
          val vparamss = paramClauses(nme.CONSTRUCTOR,
                                      classContextBounds map (_.duplicate),
                                      ofCaseClass = false)
          newLineOptWhenFollowedBy(LBRACE)
    ...
    def funDefRest(start: Offset,
                   nameOffset: Offset,
                   mods: Modifiers,
                   name: Name): Tree = {
      val result =
        atPos(start, if (name.toTermName == nme.ERROR) start else nameOffset) {
          var newmods = mods
          ...
          val vparamss =
            paramClauses(name, contextBoundBuf.toList, ofCaseClass = false)
          newLineOptWhenFollowedBy(LBRACE)
          ...
          val rhs =
            if (isStatSep || in.token == RBRACE) {
              if (restype.isEmpty) {
                if (currentRun.isScala214)
                  syntaxError(in.lastOffset, msg("unsupported", ": Unit"))
                else
                  deprecationWarning(in.lastOffset,
                                     msg("deprecated", ": Unit"),
                                     "2.13.0")
              ...
              if (currentRun.isScala214)
                syntaxError(in.offset, msg("unsupported", ": Unit ="))
              else
    ...
    def typeDefOrDcl(start: Offset, mods: Modifiers): Tree = {
      ...
      atPos(start, in.offset) {
        ...
        in.token match {
          ...
          case t
              if t == SUPERTYPE || t == SUBTYPE || t == COMMA || t == RBRACE || isStatSep(
                t) =>
    ...
    def tmplDef(pos: Offset, mods: Modifiers): Tree = {
      ...
      in.token match {
        case TRAIT =>
          classDef(
            pos,
            (mods | Flags.TRAIT | Flags.ABSTRACT) withPosition (Flags.TRAIT, tokenRange(
              in)))
        case CLASS =>
          classDef(pos, mods)
        case CASECLASS =>
      ...
      atPos(start, if (name == tpnme.ERROR) start else nameOffset) {
        savingClassContextBounds {
          ...
          if (!classContextBounds.isEmpty && mods.isTrait) {
            val viewBoundsExist =
              if (currentRun.isScala214) "" else " nor view bounds `<% ...`"
            syntaxError(
              s"traits cannot have type parameters with context bounds `: ...`$viewBoundsExist",
              skipIt = false)
            classContextBounds = List()
          }
          val constrAnnots =
            if (!mods.isTrait) constructorAnnotations() else Nil
          ...
          val template = templateOpt(mods,
                                     name,
                                     constrMods withAnnotations constrAnnots,
                                     vparamss,
                                     tstart)
    ...
    /** {{{
      ...
      *  }}}
      */
    def objectDef(start: Offset,
                  mods: Modifiers,
                  isPackageObject: Boolean = false): ModuleDef = {
      in.nextToken()
      val nameOffset = in.offset
      val name = ident()
      val tstart = in.offset
      atPos(start, if (name == nme.ERROR) start else nameOffset) {
        val template = templateOpt(mods,
                                   if (isPackageObject) nme.PACKAGEkw else name,
                                   NoMods,
                                   Nil,
                                   tstart)
    ...
    def templateParents(): List[Tree] = {
      ...
      def readAppliedParent() = {
        ...
        parents += (in.token match {
          case LPAREN =>
            atPos(start)(multipleArgumentExprs().foldLeft(parent)(Apply.apply))
          case _ => parent
        })
      }
      readAppliedParent()
        ...
        if (in.token == WITH && (self eq noSelfType)) {
          val advice =
            if (currentRun.isScala214) "use trait parameters instead."
              ...
              "they will be replaced by trait parameters in 2.14, see the migration guide on avoiding var/val in traits."
          deprecationWarning(braceOffset,
                             s"early initializers are deprecated; $advice",
                             "2.13.0")
          val earlyDefs: List[Tree] =
            body.map(ensureEarlyDef).filter(_.nonEmpty)
          in.nextToken()
    ...
    def ensureEarlyDef(tree: Tree): Tree = tree match {
      ...
      case tdef @ TypeDef(mods, name, tparams, rhs) =>
        def msg(what: String): String =
          s"early type members are $what: move them to the regular body; the semantics are the same"
        if (currentRun.isScala214)
          syntaxError(tdef.pos.point, msg("unsupported"))
    ...
    /** {{{
      ...
      *  }}}
      */
    def templateOpt(mods: Modifiers,
                    name: Name,
      ...
      val tstart1 =
        if (body.isEmpty && in.lastOffset < tstart) in.lastOffset else tstart
      ...
      if (name == nme.PACKAGEkw && parents.nonEmpty)
        deprecationWarning(
          tstart,
          s"package object inheritance is deprecated (https://github.com/scala/scala-dev/issues/441);\n" +
            s"drop the `extends` clause or use a regular object instead",
          "2.13.0"
        )
    ...
    def templateBody(isPre: Boolean) =
      inBraces(templateStatSeq(isPre = isPre)) match {
        case (self, Nil) => (self, EmptyTree.asList)
        case result      => result
      }
    def templateBodyOpt(
        parenMeansSyntaxError: Boolean): (ValDef, List[Tree]) = {
      newLineOptWhenFollowedBy(LBRACE)
      if (in.token == LBRACE) {
        ...
        if (in.token == LPAREN) {
          if (parenMeansSyntaxError)
            syntaxError(s"traits or objects may not have parameters",
                        skipIt = true)
    ...
    def makePackaging(start: Offset, pkg: Tree, stats: List[Tree]): PackageDef =
      pkg match {
    ...
    def makeEmptyPackage(start: Offset, stats: List[Tree]): PackageDef = (
      makePackaging(start,
                    atPos(start, start, start)(Ident(nme.EMPTY_PACKAGE_NAME)),
                    stats)
    )
    ...
    def templateOrTopStatSeq(): List[Tree] =
      statSeq(templateStat.orElse(topStat))
