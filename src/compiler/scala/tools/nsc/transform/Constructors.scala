...
abstract class Constructors
  ...
  class ConstructorTransformer(unit: CompilationUnit)
  ...
  private trait OmittablesHelper {
    def computeOmittableAccessors(clazz: Symbol,
                                  defs: List[Tree],
  ...
  private trait GuardianOfCtorStmts extends ConstructorTransformerBase {
     ...
     */
    private def mergeConstructors(genericClazz: Symbol,
...
//        println("residual specialized constructor statements: " + specBuf)
  ...
  private class TemplateTransformer(val unit: CompilationUnit,
    ...
    def parameter(acc: Symbol): Symbol =
      parameterNamed(acc.unexpandedName.getterName)
    ...
    object intoConstructor extends Transformer {
      ...
      private def canBeSupplanted(sym: Symbol) =
        !isDelayedInitSubclass && isStationaryParamRef(sym) && !specializeTypes
    ...
    /** Triage definitions and statements in this template into the following categories.
      ...
      */
    class Triage {
      private val defBuf, auxConstructorBuf, constrPrefixBuf, constrStatBuf,
      ...
      private def triage() = {
        ...
        def memoizeValue(sym: Symbol) =
          !sym.info.resultType.isInstanceOf[FoldableConstantType]
        ...
        for (stat <- primaryConstrBody.stats) {
          ...
          stat match {
            case ValDef(mods, name, _, _)
                if mods.hasFlag(PRESUPER) => // TODO trait presupers
        ...
        for (stat <- stats) {
          ...
          def moveEffectToCtor(mods: Modifiers,
            ...
            val initializingRhs =
              if ((assignSym eq NoSymbol) || statSym.isLazy)
                ...
                intoConstructor(statSym, primaryConstrSym)(rhs)
              else rhs
          ...
          stat match {
            ...
            case vd: ValDef =>
              if (vd.rhs eq EmptyTree) { defBuf += vd } else {
            ...
            case dd: DefDef =>
              ...
              def shouldMoveRHS =
                clazz.isTrait && statSym.isAccessor && !statSym.isLazy && (statSym.isSetter || memoizeValue(
                  statSym))

              if ((dd.rhs eq EmptyTree) || !shouldMoveRHS) { defBuf += dd } else {
                if (statSym.isGetter)
                  moveEffectToCtor(
                    dd.mods,
                    dd.rhs,
                    statSym.asTerm.referenced orElse statSym.setterIn(clazz))
                defBuf += deriveDefDef(stat)(_ => EmptyTree)
              }

            // all other statements go into the constructor
            case _ =>
              constrStatBuf += intoConstructor(impl.symbol, primaryConstrSym)(
                stat)
          }
        }
      }
    ...
    def transformed = {
      ...
      val paramAccessors =
        if (clazz.isTrait)
          clazz.info.decls.toList.filter(sym =>
            sym.hasAllFlags(STABLE | PARAMACCESSOR)) // since a trait does not have constructor parameters (yet), these can only come from lambdalift -- right?
      ...
      val paramInits = paramAccessors filterNot omittableSym map { acc =>
        ...
        val conflict =
          clazz.info.decl(acc.name.localName).filter(sym => sym ne acc)
        if (conflict ne NoSymbol) {
          val orig = exitingTyper(
            clazz.info.nonPrivateMember(acc.name).filter(_ hasFlag ACCESSOR))
          reporter.error(
            acc.pos,
            s"parameter '${acc.name}' requires field but conflicts with ${(orig orElse conflict).fullLocationString}")
        }
      ...
      val (uptoSuperStats, remainingConstrStats) =
        treeInfo.splitAtSuper(constructorStats, classOnly = false)
      ...
      val fence =
        if (clazz.primaryConstructor
              .hasAttachment[ConstructorNeedsFence.type]) {
          val tree = localTyper.typedPos(clazz.primaryConstructor.pos)(
            gen.mkMethodCall(RuntimeStaticsModule, nme.releaseFence, Nil))
          tree :: Nil
        } else Nil
      ...
      val primaryConstructor = deriveDefDef(primaryConstr)(_ => {
        treeCopy.Block(
          ...
          paramInits ::: constructorPrefix ::: uptoSuperStats ::: guardSpecializedInitializer(
            remainingConstrStatsDelayedInit) ::: fence,
          primaryConstrBody.expr
        )
      })

      if ((exitingPickler(clazz.isAnonymousClass) || clazz.originalOwner.isTerm) && omittableAccessor
            .exists(_.isOuterField) && !constructorStats.exists(_.exists {
            case i: Ident if i.symbol.isOuterParam => true; case _ => false
          }))
      ...
      val statsWithInitChecks =
        if (settings.checkInit) {
          ...
          prunedStats mapConserve {
            case dd: DefDef if addChecks.needsWrapping(dd) =>
              deriveDefDef(dd)(addChecks.wrapRhsWithInitChecks(dd.symbol))
            case stat => stat
          }
        } else prunedStats
      ...
      if (classInitStats.isEmpty) deriveTemplate(impl)(_ => statsWithInitChecks)
        ...
        val staticCtor =
          staticConstructor(statsWithInitChecks, localTyper, impl.pos)(
            classInitStats)
