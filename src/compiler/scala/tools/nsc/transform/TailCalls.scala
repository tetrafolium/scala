...
abstract class TailCalls extends Transform {
  ...
  class TailCallElimination(unit: CompilationUnit) extends Transformer {
    ...
    def yesTailTransform(tree: Tree): Tree =
      transform(tree, ctx.yesTailContext())
    ...
    override def transform(tree: Tree): Tree = {
      ...
      def rewriteApply(target: Tree,
        ...
        def matchesTypeArgs =
          (ctx.tparams corresponds targs)((p, a) =>
            !isSpecialized(p) || p == a.tpe.typeSymbol)
        ...
        if (!ctx.isEligible)
          ...
          failHere(
            "it is called recursively with different specialized type arguments")
      ...
      tree match {
        ...
        case Block(stats, expr) =>
          treeCopy.Block(tree, noTailTransforms(stats), transform(expr))
        ...
        case Apply(fun, args @ (arg :: Nil))
            if fun.symbol.isLabel && ctx.tailLabels(fun.symbol) =>
