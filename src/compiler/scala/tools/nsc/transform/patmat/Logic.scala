  ...
  trait PropositionalLogic {
    ...
    def simplify(f: Prop): Prop = {
      ...
      def negationNormalFormNot(p: Prop): Prop = p match {
        case And(ops) =>
          Or(mapConserve(ops)(negationNormalFormNot)) // De Morgan
        case Or(ops) =>
          And(mapConserve(ops)(negationNormalFormNot)) // De Morgan
    ...
    object AnalysisBudget {
      ...
      abstract class Exception(val advice: String)
          extends RuntimeException("CNF budget exceeded")
    ...
    if (System.getProperty("scalac.patmat.analysisBudget") != null) {
      reportWarning(
        s"Please remove -Dscalac.patmat.analysisBudget, it is ignored.")
    }
    ...
    def removeVarEq(props: List[Prop],
      ...
      val start =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startTimer(statistics.patmatAnaVarEq)
        else null
      ...
      if (StatisticsStatics.areSomeColdStatsEnabled)
        statistics.stopTimer(statistics.patmatAnaVarEq, start)
        ...
        def implies(lower: Const, upper: Const): Boolean =
          ...
          lower == upper ||
            ...
            (lower != NullConst && !upper.isValue &&
              instanceOfTpImplies(if (lower.isValue) lower.wideTp else lower.tp,
                                  upper.tp))
...
[ HALF BAKED FANCINESS: //!equalitySyms.exists(common => implies(common.const, a) && implies(common.const, b)))
 ...
 but we can safely pretend types are mutually exclusive as long as there are no counter-examples in the match we're analyzing}
      ...
      lazy val domainSyms: Option[Set[Sym]] = domain map {
    ...
    private def widenToClass(tp: Type): Type =
      if (tp.typeSymbol.isClass) tp
        ...
        throw new IllegalArgumentException("Bad type: " + tp)
