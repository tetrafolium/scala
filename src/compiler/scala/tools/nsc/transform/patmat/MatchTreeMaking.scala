...
trait MatchTreeMaking extends MatchCodeGen with Debugging {
  ...
  final case class Suppression(suppressExhaustive: Boolean,
                               suppressUnreachable: Boolean)
  object Suppression {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  trait TreeMakers extends TypedSubstitution with CodegenCore {
    def optimizeCases(
        ...
        pt: Type,
        selectorPos: Position): (List[List[TreeMaker]], List[Tree])
    def analyzeCases(prevBinder: Symbol,
                     cases: List[List[TreeMaker]],
                     pt: Type,
                     suppression: Suppression): Unit

    def emitSwitch(scrut: Tree,
                   scrutSym: Symbol,
                   cases: List[List[TreeMaker]],
                   pt: Type,
    ...
    def emitTypeSwitch(bindersAndCases: List[(Symbol, List[TreeMaker])],
    ...
    abstract class TreeMaker {
      ...
      private[TreeMakers] def incorporateOuterSubstitution(
        ...
        if (currSub ne null) {
          debug.patmat(
            "BUG: incorporateOuterSubstitution called more than once for " + ((this,
                                                                               currSub,
                                                                               outerSubst)))
          Thread.dumpStack()
    ...
    case class TrivialTreeMaker(tree: Tree)
        extends TreeMaker
        with NoNewBinders {
      def pos = tree.pos
    ...
    case class BodyTreeMaker(body: Tree, matchPt: Type)
        extends TreeMaker
        with NoNewBinders {
      def pos = body.pos
    ...
    case class SubstOnlyTreeMaker(prevBinder: Symbol, nextBinder: Symbol)
        extends TreeMaker {
      val pos = NoPosition
    ...
    sealed trait PreserveSubPatBinders extends TreeMaker {
      ...
      private lazy val (stored, substed) =
        subPatBinders.lazyZip(subPatRefs).partition {
    ...
    /**
      ...
      */
    case class NonNullTestTreeMaker(prevBinder: Symbol,
                                    expectedTp: Type,
    ...
    /**
      ...
      */
    case class ExtractorTreeMaker(extractor: Tree,
                                  extraCond: Option[Tree],
                                  nextBinder: Symbol)(
        val subPatBinders: List[Symbol],
        val subPatRefs: List[Tree],
    ...
    /**
      ...
      */
    case class ProductExtractorTreeMaker(prevBinder: Symbol,
                                         extraCond: Option[Tree])(
        val subPatBinders: List[Symbol],
        val subPatRefs: List[Tree],
    ...
    object TypeTestTreeMaker {
      ...
      trait TypeTestCondStrategy {
        ...
        def withOuterTest(orig: Result)(testedBinder: Symbol,
                                        expectedTp: Type): Result = orig
      ...
      object pureTypeTestChecker extends TypeTestCondStrategy {
        ...
        def and(a: Result, b: Result): Result =
          false // we don't and type tests, so the conjunction must include at least one false
        def tru = true
      }

      def nonNullImpliedByTestChecker(binder: Symbol) =
        new TypeTestCondStrategy {
          type Result = Boolean

          def typeTest(testedBinder: Symbol, expectedTp: Type): Result =
            testedBinder eq binder
          def nonNullTest(testedBinder: Symbol): Result = testedBinder eq binder
          def equalsTest(pat: Tree, testedBinder: Symbol): Result =
            false // could in principle analyse pat and see if it's statically known to be non-null
          def eqTest(pat: Tree, testedBinder: Symbol): Result =
            false // could in principle analyse pat and see if it's statically known to be non-null
    ...
    **/
    case class TypeTestTreeMaker(prevBinder: Symbol,
                                 testedBinder: Symbol,
                                 expectedTp: Type,
                                 nextBinderTp: Type)(
        override val pos: Position,
        extractorArgTypeTest: Boolean = false)
        extends CondTreeMaker {
      import TypeTestTreeMaker._
      debug.patmat(
        "TTTM" + ((prevBinder,
                   extractorArgTypeTest,
                   testedBinder,
                   expectedTp,
                   nextBinderTp)))
      ...
      def renderCondition(cs: TypeTestCondStrategy): cs.Result = {
        ...
        def isExpectedPrimitiveType =
          isAsExpected && isPrimitiveValueType(expectedTp)
        ...
        def addOuterTest(res: cs.Result): cs.Result =
          withOuterTest(res)(testedBinder, expectedTp)
      ...
      def impliesBinderNonNull(binder: Symbol) =
        renderCondition(nonNullImpliedByTestChecker(binder))

      override def toString =
        "TT" + ((expectedTp, testedBinder.name, nextBinderTp))
    }

    // need to substitute to deal with existential types -- TODO: deal with existentials better, don't substitute (see RichClass during quick.comp)
    case class EqualityTestTreeMaker(prevBinder: Symbol,
                                     patTree: Tree,
                                     override val pos: Position)
        extends CondTreeMaker {
    ...
    case class AlternativesTreeMaker(prevBinder: Symbol,
                                     var altss: List[List[TreeMaker]],
                                     pos: Position)
        extends TreeMaker
        with NoNewBinders {
    ...
    case class GuardTreeMaker(guardTree: Tree)
      ...
      def chainBefore(next: Tree)(casegen: Casegen): Tree =
        casegen.flatMapGuard(substitution(guardTree), next)
    ...
    def combineCases(scrut: Tree,
                     scrutSym: Symbol,
                     casesRaw: List[List[TreeMaker]],
                     pt: Type,
    ...
    def combineCasesNoSubstOnly(
        scrut: Tree,
        scrutSym: Symbol,
        casesNoSubstOnly: List[List[TreeMaker]],
        pt: Type,
        selectorPos: Position,
        owner: Symbol,
        matchFailGenOverride: Option[Tree => Tree]): Tree =
      fixerUpper(owner, scrut.pos) {
              ...
              case Typed(tree, tpt) =>
                ...
                val suppressUnreachable = tree match {
                  case Ident(name)
                    ...
                    true // scala/bug#7183 don't warn for withFilter's that turn out to be irrefutable.
                  case _ => false
                }
                val suppression =
        ...
        emitSwitch(scrut,
          ...
          if (casesNoSubstOnly nonEmpty) {
            ...
            val (cases, toHoist) =
              optimizeCases(scrutSym, casesNoSubstOnly, pt, selectorPos)
