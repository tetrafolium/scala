...
trait MatchOptimization extends MatchTreeMaking with MatchAnalysis {
  ...
  trait CommonSubconditionElimination
    ...
    /** a flow-sensitive, generalised, common sub-expression elimination
      ...
      */
    def doCSE(prevBinder: Symbol,
              cases: List[List[TreeMaker]],
              pt: Type,
              selectorPos: Position): List[List[TreeMaker]] = {
      ...
      def storeDependencies(test: Test) = {
        ...
        if (conds(False))
          false // stop when we encounter a definite "no" or a "not sure"
        else {
            ...
            for (test <- sharedPrefix; reusedTest <- test.reuses)
              reusedTest.treeMaker match {
                case reusedCTM: CondTreeMaker =>
                  reused(reusedCTM) =
                    ReusedCondTreeMaker(reusedCTM, selectorPos)
                case _ =>
              }
    ...
    object ReusedCondTreeMaker {
      def apply(orig: CondTreeMaker, selectorPos: Position) =
...
//  }
  ...
  trait SwitchEmission extends TreeMakers with MatchMonadInterface {
    ...
    abstract class SwitchMaker {
      abstract class SwitchableTreeMakerExtractor {
      ...
      /** Collapse guarded cases that switch on the same constant (the last case may be unguarded).
        ...
        */
      private def collapseGuardedCases(
          cases: List[CaseDef]): (List[CaseDef], Boolean) = {
        ...
        def collapse(same: List[CaseDef], isDefault: Boolean): CaseDef = {
          ...
          val binders = same.collect {
            case CaseDef(x @ Bind(_, _), _, _) if x.symbol != NoSymbol =>
              x.symbol
          }
        ...
        while (remainingCases.nonEmpty) {
          ...
          val unguardedComesLastOrAbsent =
            (!isGuardedCase(currCase) && impliesCurr.isEmpty) || {
              val LastImpliesCurr = impliesCurr.length - 1
      ...
      private def patternImplies(x: Tree)(y: Tree): Boolean = (x, y) match {
        ...
        // type-switch for catch
        case (Typed(Ident(nme.WILDCARD), tpX),
              Typed(Ident(nme.WILDCARD), tpY)) =>
      ...
      def apply(cases: List[(Symbol, List[TreeMaker])],
        ...
        if (cases.isEmpty || cases.tail.isEmpty) Nil
          ...
          val caseDefs = traverseOpt(cases) {
    ...
    class RegularSwitchMaker(scrutSym: Symbol,
    ...
    override def emitSwitch(scrut: Tree,
                            scrutSym: Symbol,
                            cases: List[List[TreeMaker]],
                            pt: Type,
                            matchFailGenOverride: Option[Tree => Tree],
    ...
    override def unreachableTypeSwitchCase(
        cases: List[CaseDef]): Option[CaseDef] =
      typeSwitchMaker.unreachableCase(cases)

    // TODO: drop null checks
    override def emitTypeSwitch(
