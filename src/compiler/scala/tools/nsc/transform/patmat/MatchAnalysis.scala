...
trait MatchApproximation
  ...
  trait MatchApproximator extends TreeMakers with TreesAndTypesDomain {
    ...
    class TreeMakersToPropsIgnoreNullChecks(root: Symbol)
        extends TreeMakersToProps(root) {
    ...
    class TreeMakersToProps(val root: Symbol) {
      ...
      private[this] val uniqueEqualityProps =
        new mutable.HashMap[(Tree, Tree), Eq]
      ...
      abstract class TreeMakerToProp extends (TreeMaker => Prop) {
        ...
        def updateSubstitution(subst: Substitution): Unit = {
          ...
          pointsToBound ++= okSubst.from.lazyZip(okSubst.to).collect {
            case (f, t)
        ...
        def apply(tm: TreeMaker): Prop = {
          if (!substitutionComputed)
            updateSubstitution(tm.subPatternsAsSubstitution)
          ...
          tm match {
            ...
            case NonNullTestTreeMaker(prevBinder, _, _) =>
              uniqueNonNullProp(binderToUniqueTree(prevBinder))
      ...
      private val rewriteListPattern: PartialFunction[TreeMaker, Prop] = {
        case p @ ExtractorTreeMaker(_, _, testedBinder)
            if testedBinder.tpe.typeSymbol == ListClass && p.checkedLength == Some(
              0) =>
    ...
    def approximateMatchConservative(
        root: Symbol,
        cases: List[List[TreeMaker]]): List[List[Test]] =
...
trait MatchAnalysis extends MatchApproximation {
  ...
  trait MatchAnalyzer extends MatchApproximator {
    ...
    def warn(pos: Position, ex: AnalysisBudget.Exception, kind: String) =
      uncheckedWarning(pos, s"Cannot check match for $kind.\n${ex.advice}")
    def reportWarning(message: String) =
      global.reporter.warning(typer.context.tree.pos, message)
    ...
    def unreachableCase(prevBinder: Symbol,
                        cases: List[List[TreeMaker]],
                        pt: Type): Option[Int] = {
      val start =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startTimer(statistics.patmatAnaReach)
        else null
      ...
      try {
        ...
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.stopTimer(statistics.patmatAnaReach, start)
    ...
    def exhaustive(prevBinder: Symbol,
                   cases: List[List[TreeMaker]],
                   pt: Type): List[String] =
      if (uncheckableType(prevBinder.info)) Nil
        ...
        val start =
          if (StatisticsStatics.areSomeColdStatsEnabled)
            statistics.startTimer(statistics.patmatAnaExhaust)
          else null
        var backoff = false
        ...
        if (backoff) Nil
          ...
          try {
            ...
            if (StatisticsStatics.areSomeColdStatsEnabled)
              statistics.stopTimer(statistics.patmatAnaExhaust, start)
            pruned
          } catch {
            case ex: AnalysisBudget.Exception =>
    ...
    class CounterExample {
      ...
      def coveredBy(other: CounterExample): Boolean =
        this == other || other == WildcardExample
    }
    case class ValueExample(c: ValueConst) extends CounterExample {
      override def toString = c.toString
    }
    case class TypeExample(c: Const) extends CounterExample {
      override def toString = "(_ : " + c + ")"
    }
    case class NegativeExample(eqTo: Const, nonTrivialNonEqualTo: List[Const])
        extends CounterExample {
      // require(nonTrivialNonEqualTo.nonEmpty, nonTrivialNonEqualTo)
      override def toString = {
        val negation =
          if (nonTrivialNonEqualTo.tail.isEmpty)
            nonTrivialNonEqualTo.head.toString
          else
            nonTrivialNonEqualTo.map(_.toString).sorted.mkString("(", ", ", ")")
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case other @ ListExample(_) =>
            this == other || ((elems.sizeCompare(other.elems) == 0) && (elems zip other.elems)
    ...
    case class TupleExample(ctorArgs: List[CounterExample])
        extends CounterExample {
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case TupleExample(otherArgs) =>
            this == other || ((ctorArgs.sizeCompare(otherArgs) == 0) && (ctorArgs zip otherArgs)
              .forall { case (a, b) => a coveredBy b })
          case _ => super.coveredBy(other)
        }
    }
    ...
    /**
      ...
      */
    def expandModel(
        solution: Solution): List[Map[Var, (Seq[Const], Seq[Const])]] = {
    ...
    def modelToCounterExample(scrutVar: Var)(
      ...
      case class VariableAssignment(variable: Var,
        ...
        private val fields: mutable.Map[Symbol, VariableAssignment] =
          mutable.HashMap.empty
        // need to prune since the model now incorporates all super types of a constant (needed for reachability)
        private lazy val uniqueEqualTo = equalTo filterNot (subsumed =>
          equalTo.exists(better =>
            (better ne subsumed) && instanceOfTpImplies(better.tp,
                                                        subsumed.tp)))
        ...
        def allFieldAssignmentsLegal: Boolean =
          (fields.keySet subsetOf caseFieldAccs.toSet) && fields.values.forall(
            _.allFieldAssignmentsLegal)
