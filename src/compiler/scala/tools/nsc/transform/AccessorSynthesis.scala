...
trait AccessorSynthesis extends Transform with ast.TreeDSL {
  ...
  trait AccessorTreeSynthesis {
    ...
    class UncheckedAccessorSynth(protected val clazz: Symbol) {
      ...
      protected def addDef(tree: ValOrDefDef): Unit =
        _newDefs += typedPos(position(tree.symbol))(tree)
      ...
      def addDefDef(sym: Symbol, rhs: Tree = EmptyTree) =
        addDef(DefDef(sym, rhs))
      def addValDef(sym: Symbol, rhs: Tree = EmptyTree) =
        addDef(ValDef(sym, rhs))
      ...
      def accessorBody(sym: Symbol) =
        if (sym.isSetter) setterBody(sym, sym.getterIn(clazz))
        else getterBody(sym)
      ...
      protected def setterBody(setter: Symbol, getter: Symbol): Tree = {
        assert(
          getter.hasFlag(PARAMACCESSOR),
          s"missing implementation for non-paramaccessor $setter in $clazz")
  ...
  case class BitmapInfo(symbol: Symbol, mask: Literal) {
    ...
    def applyToMask(on: This, op: Name): Tree =
      Apply(member(select(on), op), List(mask))
    def member(bitmapRef: Tree, name: Name): Tree =
      Select(bitmapRef, getMember(storageClass, name))
    def convert(bitmapRef: Tree): Tree =
      Apply(member(bitmapRef, newTermName("to" + storageClass.name)), Nil)
  ...
  class CheckedAccessorSymbolSynth(val clazz: Symbol) {
    ...
    private[AccessorSynthesis] def bitmapFor(field: Symbol): BitmapInfo =
      _bitmapInfo(field)
    protected def bitmapOf(field: Symbol): Option[BitmapInfo] =
      _bitmapInfo.get(field)
    ...
    def computeBitmapInfos(fields: List[Symbol]): List[Symbol] = {
      def bitmapCategory(field: Symbol): Name = {
        ...
        if (field.isLazy)
          if (field hasAnnotation TransientAttr) BITMAP_TRANSIENT
          else BITMAP_NORMAL
        else if (doCheckInit && !(field hasFlag DEFAULTINIT | PRESUPER | PARAMACCESSOR))
          if (field hasAnnotation TransientAttr) BITMAP_CHECKINIT_TRANSIENT
      ...
      def allocateBitmaps(fieldsWithBitmaps: List[Symbol], category: Name) = {
        ...
        val bitmapSyms =
          (0 to maxBitmapNumber).toArray map { bitmapNumber =>
            val bitmapSym = (
              clazz.newVariable(
                nme.newBitmapName(category, bitmapNumber).toTermName,
                clazz.pos.focus)
        ...
        fieldsWithBitmaps.zipWithIndex foreach {
          case (f, idx) =>
      ...
      fields
        .groupBy(bitmapCategory)
        .flatMap {
          case (category, fields)
              if category != nme.NO_NAME && fields.nonEmpty =>
            allocateBitmaps(fields, category): Iterable[Symbol]
          case _ => Nil
        }
        .toList
    }
    ...
    def newSlowPathSymbol(lzyVal: Symbol): Symbol = {
      ...
      val sym = clazz.newMethod(
        nme.newLazyValSlowComputeName(lzyVal.name.toTermName),
        pos,
        PRIVATE) setInfo MethodType(Nil, lzyVal.tpe.resultType)
  ...
  trait CheckedAccessorTreeSynthesis extends AccessorTreeSynthesis {
    ...
    class SynthCheckedAccessorsTreesInClass(clazz: Symbol)
        extends CheckedAccessorSymbolSynth(clazz) {
      ...
      def mkTest(bm: BitmapInfo, equalToZero: Boolean = true): Tree =
        if (bm.isBoolean)
          ...
          Apply(bm.member(bm.applyToMask(thisRef, nme.AND),
                          if (equalToZero) nme.EQ else nme.NE),
                List(ZERO))

      /** Return an (untyped) tree of the form 'Clazz.this.bmp = Clazz.this.bmp | mask'. */
      def mkSetFlag(bitmap: BitmapInfo): Tree =
        Assign(
          bitmap.select(thisRef),
    ...
    class SynthLazyAccessorsIn(clazz: Symbol)
        extends SynthCheckedAccessorsTreesInClass(clazz) {
      ...
      /**
        ...
        */
      def expandLazyClassMember(lazyVar: global.Symbol,
                                lazyAccessor: global.Symbol,
                                transformedRhs: global.Tree): Tree = {
        ...
        val storeRes =
          if (isUnit) rhsAtSlowDef
          else Assign(selectVar, fields.castHack(rhsAtSlowDef, lazyVar.info))
        ...
        val slowPathRhs = Block(
          gen.mkSynchronized(thisRef)(If(needsInit, doInit, EmptyTree)) :: Nil,
          selectVar)
        ...
        val accessorRhs = fields.castHack(
          If(needsInit, Apply(Select(thisRef, slowPathSym), Nil), selectVar),
          lazyVar.info)

        afterOwnPhase { // so that we can assign to vals
          Thicket(
            List((DefDef(slowPathSym, slowPathRhs)),
                 DefDef(lazyAccessor, accessorRhs)) map typedPos(
              lazyAccessor.pos.focus))
        }
      }
    }

    class SynthInitCheckedAccessorsIn(clazz: Symbol)
        extends SynthCheckedAccessorsTreesInClass(clazz) {
        ...
        override def transformStats(stats: List[Tree], exprOwner: Symbol) = {
          val checkedStats = stats flatMap {
            ...
            case stat @ Assign(lhs @ Select(This(_), _), _) =>
              stat :: bitmapOf(lhs.symbol).toList.map(bitmap =>
                typedPos(stat.pos.focus)(mkSetFlag(bitmap)))
      ...
      def wrapRhsWithInitChecks(sym: Symbol)(rhs: Tree): Tree =
        if (sym.isConstructor) addInitBitsTransformer transform rhs
          ...
          if (field == NoSymbol) rhs
            ...
            bitmapOf(field) match {
              case Some(bitmap) =>
                if (sym.isGetter)
                  mkCheckedAccessorRhs(if (isUnitGetter(sym)) UNIT else rhs,
                                       rhs.pos,
                                       bitmap) // TODO: why not always use rhs?
                else
                  Block(List(rhs, typedPos(rhs.pos.focus)(mkSetFlag(bitmap))),
                        UNIT)
              case _ => rhs
            }
        } else rhs

      private def mkCheckedAccessorRhs(retVal: Tree,
                                       pos: Position,
                                       bitmap: BitmapInfo): Tree = {
