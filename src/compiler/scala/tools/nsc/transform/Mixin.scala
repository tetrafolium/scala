...
// --------- helper functions -----------------------------------------------
  ...
  /** Returns the symbol that is accessed by a super-accessor in a mixin composition.
    ...
    */
  private def rebindSuper(base: Symbol,
                          acc: Symbol,
                          mixinClass: Symbol): Symbol = {
    ...
    exitingSpecialize {
      ...
      val superTargetClass =
        if (mixinClass.isSpecialized) unspecializedSymbol(mixinClass)
        else mixinClass
      ...
      while (sym == NoSymbol && bcs.nonEmpty) {
        sym = acc
          .matchingSymbol(bcs.head, site)
          .suchThat(sym => !sym.hasFlag(DEFERRED | BRIDGE | likeSpecialized))
        bcs = bcs.tail
      }
      ...
      if ((sym ne acc) && sym.exists && !(sym.isErroneous || (site.memberInfo(
            sym) <:< site.memberInfo(acc))))
        reporter.error(
          base.pos,
          s"illegal trait super target found for $acc required by $mixinClass;" +
            s"\n found   : ${exitingTyper {
              sym.defStringSeenAs(site.memberInfo(sym))
            }} in ${sym.owner};" +
            s"\n expected: ${exitingTyper {
              acc.defStringSeenAs(site.memberInfo(acc))
            }} in ${acc.owner}"
        )
...
// --------- type transformation -----------------------------------------------
  ...
  @inline final def notDeferred(sym: Symbol) =
    fields.notDeferredOrSynthImpl(sym)
  ...
  def isOverriddenAccessor(member: Symbol, bcs: List[Symbol]): Boolean =
    beforeOwnPhase {
      ...
      && (hasOverridingAccessor(bcs.head) || isOverriddenAccessor(member,
                                                                  bcs.tail)))
    }
  ...
  def cloneAndAddMember(mixinClass: Symbol,
                        mixinMember: Symbol,
                        clazz: Symbol): Symbol =
    addMember(clazz, cloneBeforeErasure(mixinClass, mixinMember, clazz))

  def cloneBeforeErasure(mixinClass: Symbol,
                         mixinMember: Symbol,
                         clazz: Symbol): Symbol = {
  ...
  def addMixedinMembers(clazz: Symbol, unit: CompilationUnit): Unit = {
    def cloneAndAddMixinMember(mixinClass: Symbol,
    ...
    def mixinTraitForwarders(mixinClass: Symbol): Unit = {
      for (member <- mixinClass.info.decls;
        ...
        member overridingSymbol clazz match {
          case NoSymbol =>
            val isMemberOfClazz = clazz.info
            ...
            if (isMemberOfClazz) {
              def genForwarder(required: Boolean): Unit = {
                ...
                if (isJavaInterface && !clazz.parentSymbols.contains(owner)) {
                  if (required) {
                    val text =
                      s"Unable to implement a mixin forwarder for $member in $clazz unless interface ${owner.name} is directly extended by $clazz."
                    reporter.error(clazz.pos, text)
                  }
                } else {
                  if (isJavaInterface)
                    erasure.requiredDirectInterfaces
                      .getOrElseUpdate(clazz, mutable.Set.empty) += owner
              ...
              @tailrec
              def existsCompetingMethod(baseClasses: List[Symbol]): Boolean =
                baseClasses match {
                  case baseClass :: rest =>
                    if (baseClass ne mixinClass) {
                      ...
                      val isCompeting = m.exists && {
                        ...
                        (!m.isDeferred && !mixinClass.isNonBottomSubClass(
                          m.owner))
                      }
              ...
              def generateJUnitForwarder: Boolean = {
                ...
                JUnitAnnotations.exists(annot =>
                  annot.exists && member.hasAnnotation(annot))
              }
        ...
        if (mixinMember.hasFlag(SUPERACCESSOR)) { // mixin super accessors
          ...
          rebindSuper(clazz, mixinMember.alias, mixinClass) match {
            case NoSymbol =>
              reporter.error(
                clazz.pos,
                "Member %s of mixin %s is missing a concrete super implementation."
                  .format(mixinMember.alias, mixinClass))
            case alias1 =>
              if (alias1.owner.isJavaDefined && alias1.owner.isInterface) {
                if (!clazz.parentSymbols.contains(alias1.owner)) {
                  val suggestedParent = exitingTyper(
                    clazz.info.baseType(alias1.owner))
                  reporter.error(
                    clazz.pos,
                    s"Unable to implement a super accessor required by trait ${mixinClass.name} unless $suggestedParent is directly extended by $clazz.")
                } else
                  erasure.requiredDirectInterfaces
                    .getOrElseUpdate(clazz, mutable.Set.empty) += alias1.owner
              }
...
// --------- term transformation -----------------------------------------------
  ...
  class MixinTransformer(unit: CompilationUnit)
      extends Transformer
      with AccessorTreeSynthesis {
    ...
    private val nullables =
      mutable.AnyRefMap[Symbol, Map[Symbol, List[Symbol]]]()
    ...
    private def preTransform(tree: Tree): Tree = {
      ...
      tree match {
        ...
        case dd: DefDef if dd.symbol.name.endsWith(nme.LAZY_SLOW_SUFFIX) =>
          val fieldsToNull =
            nullables.getOrElse(sym.enclClass, Map()).getOrElse(sym, Nil)
            ...
            deriveDefDef(dd) {
              case blk @ Block(stats, expr) =>
                ...
                def nullify(sym: Symbol) =
                  Select(gen.mkAttributedThis(sym.enclClass),
                         sym.accessedOrSelf) === NULL
    ...
    def lazyValNullables(clazz: Symbol,
                         templStats: List[Tree]): Map[Symbol, List[Symbol]] = {
      ...
      if (!clazz.info.decls.exists(_.isLazy)) Map()
        ...
        val singleUseFields: Map[Symbol, List[Symbol]] = {
          val usedIn = mutable
            .HashMap[Symbol, List[Symbol]]() withDefaultValue Nil
                ...
                case tree: RefTree if tree.symbol != NoSymbol =>
                  ...
                  if ((sym.hasAccessorFlag || (sym.isTerm && !sym.isMethod)) && sym.isPrivate && !sym.isLazy && !sym.isModule // non-lazy private field or its accessor
                      && !definitions.isPrimitiveValueClass(
                        sym.tpe.resultType.typeSymbol) // primitives don't hang on to significant amounts of heap
          ...
          val singlyUsedIn = usedIn.filter {
            case (_, member :: Nil)
                if member.name.endsWith(nme.LAZY_SLOW_SUFFIX) =>
              val lazyAccessor = member.owner.info.decl(
                member.name.stripSuffix(nme.LAZY_SLOW_SUFFIX))
        ...
        for ((field, users) <- singleUseFields; lazyFld <- users)
          map(lazyFld) += field
    ...
    private def addNewDefs(clazz: Symbol, stats: List[Tree]): List[Tree] = {
      ...
      for (sym <- clazz.info.decls; if sym hasFlag MIXEDIN) {
        ...
        // implement methods mixed in from a supertrait (the symbols were created by mixinTraitMembers)
          ...
          assert(sym hasFlag (PARAMACCESSOR),
                 s"mixed in $sym from $clazz is not param?!?")
          ...
          addDefDef(sym,
                    Apply(SuperSelect(clazz, sym.alias),
      ...
      if (clazz.isTrait)
        implementedAccessors filter {
          case vd: ValDef =>
            assert(vd.symbol.hasFlag(PRESUPER | PARAMACCESSOR),
                   s"unexpected valdef $vd in trait $clazz"); false
          case _ => true
        } else {
        ...
        def completeSuperAccessor(stat: Tree) = stat match {
          case DefDef(_, _, _, vparams :: Nil, _, EmptyTree)
            ...
            debuglog(
              s"implementing super accessor in $clazz for ${stat.symbol} --> ${stat.symbol.alias.owner} . ${stat.symbol.alias}")
            ...
            copyDefDef(stat)(
              rhs = enteringMixin(transform(localTyper.typed(body, pt))))
          case _ =>
            stat
        }
    ...
    private def postTransform(tree: Tree): Tree = {
      ...
      tree match {
        ...
        case Select(qual, name) if sym.owner.isTrait && !sym.isMethod =>
          assert(
            sym.hasFlag(PARAMACCESSOR | PRESUPER),
            s"!!! Unexpected reference to field $sym in trait $currentOwner")
          ...
          if (ifaceGetter == NoSymbol)
            abort("No getter for " + sym + " in " + sym.owner)
