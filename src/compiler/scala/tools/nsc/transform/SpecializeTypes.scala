...
abstract class SpecializeTypes extends InfoTransform with TypingTransformers {
  ...
  val specializedClass =
    perRunCaches.newAnyRefMap[Symbol, mutable.AnyRefMap[TypeEnv, Symbol]]
  ...
  private val anyrefSpecCache =
    perRunCaches.newMap[Symbol, mutable.Map[Symbol, Symbol]]()
  ...
  private def specializedOn(sym: Symbol): List[Symbol] = {
    ...
    sym.getAnnotation(SpecializedClass) match {
      ...
      case Some(AnnotationInfo(_, args, _)) =>
        args.map(_.tpe).flatMap(expandGroup)
      case _ => Nil
    }
  }
  ...
  object TypeEnv {
    ...
    def fromSpecialization(sym: Symbol, args: List[Type]): TypeEnv = {
      ifDebug(
        assert(sym.info.typeParams.sizeCompare(args) == 0,
               "" + sym + " args: " + args))
  ...
  private def newOverload(method: Symbol,
                          specializedMethod: Symbol,
                          env: TypeEnv) = {
  ...
  override def newPhase(prev: scala.tools.nsc.Phase): StdPhase =
    new SpecializationPhase(prev)
  class SpecializationPhase(prev: scala.tools.nsc.Phase)
      extends super.Phase(prev) {
  ...
  case class SpecialOverload(original: Symbol, env: TypeEnv)
      extends SpecializedInfo {
    def target = original
  }
  ...
  case class SpecializedInnerClass(target: Symbol, env: TypeEnv)
      extends SpecializedInfo
  ...
  case class NormalizedMember(target: Symbol) extends SpecializedInfo {
    ...
    override lazy val degenerate = {
      val stvTypeParams = specializedTypeVars(
        target.info.typeParams map (_.info))
      val stvResult = specializedTypeVars(target.info.resultType)

      debuglog(
        "degenerate: " + target + " stv tparams: " + stvTypeParams + " stv info: " + stvResult)
  ...
  def isSpecializedIn(sym: Symbol, site: Type) =
    specializedTypeVars(sym) exists { tvar =>
      ...
      (concretes contains AnyRefTpe) || (concretes contains site.memberType(
        tvar))
    }
  ...
  val specializedType = new TypeMap {
    override def apply(tp: Type): Type = tp match {
      case TypeRef(pre, sym, args) if args.nonEmpty =>
        ...
        specializedClass
          .getOrElse(sym, Map.empty[TypeEnv, Symbol])
          .get(TypeEnv.fromSpecialization(sym, args1)) match {
  ...
  def specializedFunctionName(sym: Symbol, args: List[Type]) =
    exitingSpecialize {
  ...
  private def specializedName(name: Name,
                              tvars: immutable.Set[Symbol],
                              env: TypeEnv): TermName = {
  ...
  private def needsSpecialization(env: TypeEnv, sym: Symbol): Boolean = (
    !hasUnspecializableAnnotation(sym) && (
      specializedTypeVars(sym)
        .intersect(env.keySet)
        .diff(wasSpecializedForTypeVars(sym))
        .nonEmpty
  ...
  def isNormalizedMember(m: Symbol) =
    m.isSpecialized && (info get m exists {
  ...
  def specializedTypeVarsBuffer(sym: Symbol,
                                result: mutable.Buffer[Symbol]): Unit = (
  ...
  /** Return the set of @specialized type variables mentioned by the given type.
    ...
    *      (arrays are considered as Array[@specialized T])
    */
  def specializedTypeVarsBuffer(tpe: Type,
                                result: mutable.Buffer[Symbol]): Unit =
    tpe match {
      ...
      case PolyType(tparams, resTpe) =>
        specializedTypeVarsBuffer(resTpe, result);
        tparams.foreach(sym => specializedTypeVarsBuffer(sym.info, result))
      // since this method may be run at phase typer (before uncurry, where NMTs are eliminated)
      case NullaryMethodType(resTpe) =>
        specializedTypeVarsBuffer(resTpe, result)
      case MethodType(argSyms, resTpe) =>
        specializedTypeVarsBuffer(resTpe, result);
        argSyms.foreach(sym => specializedTypeVarsBuffer(sym.tpe, result))
      ...
      case TypeBounds(lo, hi) =>
        specializedTypeVarsBuffer(lo, result);
        specializedTypeVarsBuffer(hi, result)
      case RefinedType(parents, _) =>
        parents.foreach(p => specializedTypeVarsBuffer(p, result))
      case _ => ()
    }
  ...
  private def typeParamSubAnyRef(tparam: Symbol, sClass: Symbol): Type = {
    val sClassMap =
      anyrefSpecCache.getOrElseUpdate(sClass, mutable.Map[Symbol, Symbol]())

    sClassMap
      .getOrElseUpdate(
        tparam,
        tparam.cloneSymbol(sClass,
                           tparam.flags,
                           tparam.name append tpnme.SPECIALIZED_SUFFIX)
  ...
  /** Produces the symbols from type parameters `syms` of the original owner,
    ...
    */
  def produceTypeParameters(syms: List[Symbol],
                            nowner: Symbol,
                            env: TypeEnv) = {
  ...
  /** Maps AnyRef bindings from a raw environment (holding AnyRefs) into type parameters from
    ...
    */
  private def mapAnyRefsInSpecSym(env: TypeEnv,
                                  origsym: Symbol,
                                  specsym: Symbol): TypeEnv = env map {
    case (sym, AnyRefTpe) if sym.owner == origsym =>
      (sym, typeParamSubAnyRef(sym, specsym))
    case x => x
  }
    ...
    def specializedClass(env0: TypeEnv, normMembers: List[Symbol]): Symbol = {
      ...
      val specializedInfoType: Type = {
        ...
        if (extraSpecializedMixins.nonEmpty)
          debuglog(
            "extra specialized mixins for %s: %s"
        ...
        if (clazz.typeOfThis.typeConstructor ne clazz.typeConstructor) {
          ...
          debuglog(
            "Rewriting self-type for specialized class:\n" +
              "    " + clazz.defStringSeenAs(clazz.typeOfThis) + "\n" +
              " => " + sClass.defStringSeenAs(sClass.typeOfThis))
        }
        GenPolyType(
          newClassTParams,
          ClassInfoType(parents ::: extraSpecializedMixins, decls1, sClass))
      }
      ...
      for (m <- normMembers;
           if needsSpecialization(fullEnv, m) && satisfiable(fullEnv)) {
        if (!m.isDeferred)
        ...
        if (m.isConstructor) {
          ...
          if (nonConflicting(env ++ typeEnv(m))) {
            if (info(m).degenerate) {
              ...
              val specMember = enterMember(
                cloneInSpecializedClass(m, _ & ~DEFERRED))
        ...
        } else if (m.isMethod && !m.hasFlag(DEFERRED) && (!m.hasFlag(ACCESSOR) || m
                     .hasFlag(LAZY))) { // other concrete methods
          forwardToOverload(m)
          ...
          if (m.isLazy) {
            ...
            debuglog(
              "specialize accessor in %s: %s -> %s".format(
                sClass.name.decode,
                origGetter.name.decode,
                specGetter.name.decode))

            clazz.caseFieldAccessors.find(_.name.startsWith(m.name)) foreach {
              cfa =>
                val cfaGetter = overrideIn(sClass, cfa)
                info(cfaGetter) = SpecializedAccessor(specVal)
                enterMember(cfaGetter)
                debuglog(
                  "override case field accessor %s -> %s"
                    .format(m.name.decode, cfaGetter.name.decode))
            }
            ...
            if (specVal.isVariable && m.setterIn(clazz) != NoSymbol) {
              ...
              specSetter.setInfo(
                MethodType(
                  specSetter.newSyntheticValueParams(List(specVal.info)),
                  UnitTpe))
    ...
    val decls1 = clazz.info.decls.toList flatMap { m: Symbol =>
      if (m.isAnonymousClass) List(m)
        ...
        normalizeMember(m.owner, m, outerEnv) flatMap { normalizedMember =>
          val ms = specializeMember(m.owner,
                                    normalizedMember,
                                    outerEnv,
                                    clazz.info.typeParams)
  ...
  /** Expand member `sym` to a set of normalized members. Normalized members
    ...
    *     // etc.
    */
  private def normalizeMember(owner: Symbol,
                              sym: Symbol,
    ...
    sym :: (
      if (!sym.isMethod || enteringTyper(sym.typeParams.isEmpty)) Nil
        ...
        for (env0 <- specializations(specializingOn)
          ...
          val specMember = sym.cloneSymbol(
            owner,
            (sym.flags | SPECIALIZED) & ~DEFERRED) // <-- this needs newName = ...
          ...
          val methodType = sym.info.resultType
            .instantiateTypeParams(keys ++ tps, vals ++ tps1.map(_.tpe))
            .cloneInfo(specMember)
          specMember setInfo GenPolyType(tps1, methodType)

          debuglog(
            "%s expands to %s in %s"
              .format(sym, specMember.name.decode, pp(env)))
  ...
  /** Specialize member `m` w.r.t. to the outer environment and the type
    ...
    */
  private def specializeMember(owner: Symbol,
                               sym: Symbol,
                               outerEnv: TypeEnv,
                               tps: List[Symbol]): List[Symbol] = {
    def specializeOn(tparams: List[Symbol]): List[Symbol] =
      specializations(tparams) map { spec0 =>
        ...
        wasSpecializedForTypeVars(specMember) ++= spec collect {
          case (s, tp) if s.tpe == tp => s
        }

        val wasSpec = wasSpecializedForTypeVars(specMember)
        if (wasSpec.nonEmpty)
          debuglog(
            "specialized overload for %s in %s".format(specMember,
                                                       pp(typeEnv(specMember))))
    ...
    if (!sym.isMethod || sym.isConstructor || hasUnspecializableAnnotation(sym) || sym.isSuperAccessor) {
      ...
      if (stvars.nonEmpty)
        debuglog(
          "specialized %s on %s".format(sym.fullLocationString,
  ...
  private def specializedOverload(owner: Symbol,
                                  sym: Symbol,
                                  env: TypeEnv,
                                  nameSymbol: Symbol = NoSymbol): Symbol = {
  ...
  /** For each method m that overrides an inherited method m', add a special
    ...
    */
  private def specialOverrides(clazz: Symbol) =
    logResultIf[List[Symbol]]("specialized overrides in " + clazz, _.nonEmpty) {
      ...
      def needsSpecialOverride(overriding: Symbol): (Symbol, TypeEnv) = {
        def checkOverriddenTParams(overridden: Symbol): Unit = {
          foreach2(overridden.info.typeParams, overriding.info.typeParams) {
            (baseTvar, derivedTvar) =>
              val missing = concreteTypes(baseTvar).toSet diff concreteTypes(
                derivedTvar).toSet
              if (missing.nonEmpty) {
                reporter.error(
                  derivedTvar.pos,
        ...
        if (!overriding.isParamAccessor) {
          for (overridden <- overriding.allOverriddenSymbols) {
            ...
            if (stvars.nonEmpty) {
              debuglog(
                "specialized override of %s by %s%s".format(
                  overridden.fullLocationString,
                  overriding.fullLocationString,
                  if (stvars.isEmpty) ""
                  else stvars.map(_.name).mkString("(", ", ", ")")))

              if (currentRun compiles overriding)
                checkOverriddenTParams(overridden)

              val env =
                unify(overridden.info, overriding.info, emptyEnv, false, true)
              def atNext =
                exitingSpecialize(
        ...
        needsSpecialOverride(overriding) match {
          case (NoSymbol, _) =>
            ...
            if (overriding.isSuperAccessor) {
              ...
              debuglog(
                s"checking special overload for super accessor: ${overriding.fullName}, alias for ${alias.fullName}")
              needsSpecialOverride(alias) match {
                case nope @ (NoSymbol, _) => None
                case (overridden, env) =>
                  val om =
                    specializedOverload(clazz, overriding, env, overridden)
          ...
          case (overridden, env) =>
            ...
            debuglog(
              s"specialized overload $om for ${overriding.name.decode} in ${pp(
                env)}: ${om.info}")
            om.setFlag(overriding.flags & (ABSOVERRIDE | SYNCHRONIZED))
            om.withAnnotations(
              overriding.annotations.filter(_.symbol == ScalaStrictFPAttr))
            typeEnv(om) = env
            addConcreteSpecMethod(overriding)
            if (overriding.isDeferred) { // abstract override
              debuglog(
                "abstract override " + overriding.fullName + " with specialized " + om.fullName)
  ...
  /** Return the most general type environment that specializes tp1 to tp2.
    ...
    */
  private def unify(tp1: Type,
                    tp2: Type,
                    env: TypeEnv,
                    strict: Boolean,
                    tparams: Boolean = false): TypeEnv = (tp1, tp2) match {
    ...
    case (MethodType(params1, res1), MethodType(params2, res2)) =>
      ...
      unify(res1 :: (params1 map (_.tpe)),
            res2 :: (params2 map (_.tpe)),
            env,
            strict)
    case (PolyType(tparams1, res1), PolyType(tparams2, res2)) =>
      ...
      if (strict && tparams1.length != tparams2.length)
        ...
        unify(res1 :: tparams1.map(_.info),
              res2 :: tparams2.map(_.info),
              env,
              strict)
      else
    ...
    case (TypeBounds(lo1, hi1), TypeBounds(lo2, hi2)) =>
      unify(List(lo1, hi1), List(lo2, hi2), env, strict)
    case _ =>
      debuglog(
        s"don't know how to unify $tp1 [${tp1.getClass}] with $tp2 [${tp2.getClass}]")
      env
  }

  private def unify(tp1: List[Type],
                    tp2: List[Type],
                    env: TypeEnv,
      ...
      (tp1 zip tp2).foldLeft(env) { (env, args) =>
        if (!strict) unify(args._1, args._2, env, strict)
          ...
          if (env.keySet.intersect(nenv.keySet).isEmpty) env ++ nenv
            ...
            debuglog(
              s"could not unify: u(${args._1}, ${args._2}) yields $nenv, env: $env")
            unifyError(tp1, tp2)
          }
        }
      }
  ...
  private def subst(env: TypeEnv, tpe: Type): Type = {
    class FullTypeMap(from: List[Symbol], to: List[Type])
        extends SubstTypeMap(from, to)
        with AnnotationFilter {
      def keepAnnotation(annot: AnnotationInfo) =
        !(annot matches uncheckedVarianceClass)
        ...
        case cinfo @ ClassInfoType(parents, decls, clazz)
          ...
          if (!currentRun.compiles(sym) && isPast(ownPhase)) {
            ...
            if (parents ne parents1) {
              debuglog(
                "specialization transforms %s%s parents to %s".format(
  ...
  /** Is any type variable in `env` conflicting with any if its type bounds, when
    ...
    */
  def nonConflicting(env: TypeEnv) = env forall {
    case (tvar, tpe) =>
      (subst(env, tvar.info.lowerBound) <:< tpe) && (tpe <:< subst(
        env,
        tvar.info.upperBound))
  }
  ...
  def satisfiable(env: TypeEnv, warnings: Boolean): Boolean = {
    def matches(tpe1: Type, tpe2: Type): Boolean =
      (tpe2 == AnyTpe) || { // opt for common case of unbounded type parameter
    ...
    env forall {
      case (tvar, tpe) =>
        matches(tvar.info.lowerBound, tpe) && matches(tpe, tvar.info.upperBound) || {
          if (warnings)
            reporter.warning(tvar.pos,
                             s"Bounds prevent specialization of $tvar")
  ...
  def satisfiabilityConstraints(env: TypeEnv): Option[TypeEnv] = {
    ...
    def matches(tpe1: Type, tpe2: Type): Option[TypeEnv] = {
      ...
      else if (specializedTypeVars(t1).nonEmpty)
        Some(unify(t1, t2, env, false, false) -- env.keys)
      else if (specializedTypeVars(t2).nonEmpty)
        Some(unify(t2, t1, env, false, false) -- env.keys)
      else None
    }
  ...
  class Duplicator(casts: Map[Symbol, Type]) extends {
    ...
    class BodyDuplicator(_context: Context)
        extends super.BodyDuplicator(_context) {
    ...
    protected override def newBodyDuplicator(context: Context) =
  ...
  /** Introduced to fix scala/bug#7343: Phase ordering problem between Duplicators and Specialization.
    ...
    */
  class SpecializationDuplicator(casts: Map[Symbol, Type])
      extends Duplicator(casts) {
    override def retyped(context: Context,
                         tree: Tree,
                         oldThis: Symbol,
                         newThis: Symbol,
  ...
  class ImplementationAdapter(from: List[Symbol],
      ...
      extends TreeSymSubstituter(from, to) {
    ...
    override def transform(tree: Tree): Tree = tree match {
      case Select(qual, name) =>
        ...
        if (sym.isPrivate)
          debuglog(
            "seeing private member %s, currentClass: %s, owner: %s, isAccessible: %b, isLocalName: %b"
              .format(sym,
                      currentClass,
                      sym.owner.enclClass,
                      isAccessible(sym),
  ...
  def illegalSpecializedInheritance(clazz: Symbol): Boolean = (
    clazz.isSpecialized
      && originalClass(clazz).parentSymbols.exists(p =>
        hasSpecializedParams(p) && !p.isTrait)
  )

  class SpecializationTransformer(unit: CompilationUnit)
      extends TypingTransformer(unit) {

    override def transformUnit(unit: CompilationUnit): Unit =
      if (!settings.nospecialization) {
    ...
    def doesConform(origSymbol: Symbol,
                    treeType: Type,
                    memberType: Type,
                    env: TypeEnv) = {
      (treeType =:= memberType) || { // anyref specialization
        memberType match {
          case PolyType(_, resTpe) =>
            debuglog(
              s"Conformance for anyref - polytype with result type: $resTpe and $treeType\nOrig. sym.: $origSymbol")
            try {
    ...
    override def transform(tree: Tree): Tree = {
      ...
      def matchingSymbolInPrefix(pre: Type,
                                 member: Symbol,
                                 env: TypeEnv): Symbol = {
      ...
      /** Computes residual type parameters after rewiring, like "String" in the following example:
        ...
        *  ```
        */
      def computeResidualTypeVars(baseTree: Tree,
                                  specMember: Symbol,
                                  specTree: Tree,
                                  baseTargs: List[Tree],
                                  env: TypeEnv): Tree = {
        val residualTargs = symbol.info.typeParams zip baseTargs collect {
          case (tvar, targ)
              if !env.contains(tvar) || !isPrimitiveValueClass(
                env(tvar).typeSymbol) =>
            targ
        }
        ifDebug(
          assert(residualTargs.sizeCompare(specMember.info.typeParams) == 0,
                 "residual: %s, tparams: %s, env: %s"
                   .format(residualTargs, specMember.info.typeParams, env)))
      ...
      tree match {
        case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
          def transformNew = {
            debuglog(
              s"Attempting to specialize new $tpt(${args.mkString(", ")})")
        ...
        case TypeApply(sel @ Select(qual, name), targs)
          ...
          debuglog(
            "checking typeapp for rerouting: " + tree + " with sym.tpe: " + symbol.tpe + " tree.tpe: " + tree.tpe)
          val qual1 = transform(qual)
          log(">>> TypeApply: " + tree + ", qual1: " + qual1)
          specSym(qual1) match {
            case NoSymbol =>
              // See pos/exponential-spec.scala - can't call transform on the whole tree again.
              treeCopy.TypeApply(tree,
                                 treeCopy.Select(sel, qual1, name),
                                 transformTrees(targs))
            case specMember =>
              debuglog("found " + specMember.fullName)
              ifDebug(
                assert(symbol.info.typeParams.sizeCompare(targs) == 0,
                       "" + symbol.info.typeParams + " / " + targs))

              val env = typeEnv(specMember)
              computeResidualTypeVars(tree,
                                      specMember,
                                      gen.mkAttributedSelect(qual1, specMember),
                                      targs,
                                      env)
          }
        ...
        case TypeApply(sel @ Ident(name), targs) if name != nme.CONSTRUCTOR =>
          ...
          if (env.isEmpty) super.transform(tree)
            ...
            overloads(symbol) find (_ matchesEnv env) match {
              case Some(Overload(specMember, _)) =>
                computeResidualTypeVars(tree,
                                        specMember,
                                        Ident(specMember),
                                        targs,
        ...
        case Select(Super(_, _), _)
            if illegalSpecializedInheritance(currentClass) =>
          val pos = tree.pos
          debuglog(
            pos.source.file.name + ":" + pos.line + ": not specializing call to super inside illegal specialized inheritance class.\n" + pos.lineContent)
          tree
        ...
        case PackageDef(pid, stats) =>
          ...
          atOwner(tree, symbol) {
            ...
            treeCopy.PackageDef(
              tree,
              pid,
              transformStats(stats ::: specMembers, symbol.moduleClass))
          }
        ...
        case ddef @ DefDef(_, _, _, vparamss, _, _)
            if info.isDefinedAt(symbol) =>
          def transformDefDef = {
            if (symbol.isConstructor) {
              val t = atOwner(symbol)(
                forwardCtorCall(tree.pos,
                                gen.mkSuperInitCall,
                                vparamss,
                                symbol.owner))
              if (symbol.isPrimaryConstructor)
                localTyper.typedPos(symbol.pos)(deriveDefDef(tree)(_ =>
                  Block(List(t), Literal(Constant(())))))
              ...
              info(symbol) match {
                case Implementation(target) =>
                  assert(
                    body.isDefinedAt(target),
                    "sym: " + symbol.fullName + " target: " + target.fullName)
                ...
                case NormalizedMember(target) =>
                  logResult("constraints")(
                    satisfiabilityConstraints(typeEnv(symbol))) match {
                    ...
                    case _ =>
                      deriveDefDef(tree)(_ =>
                        localTyper typed gen.mkThrowNewRuntimeException(
                          "Fatal error in code generation: this should never be called."))
                  }
                ...
                case SpecialOverride(target) =>
                  ...
                  log(
                    "%s is param accessor? %b"
                      .format(ddef.symbol, ddef.symbol.isParamAccessor))
                  // we have an rhs, specialize it
                ...
                case fwd @ Forward(_) =>
                  ...
                  val rhs1 = forwardCall(
                    tree.pos,
                    gen.mkAttributedRef(symbol.owner.thisType, fwd.target),
                    vparamss)
                  debuglog(
                ...
                case SpecializedAccessor(target) =>
                  val rhs1 =
                    if (symbol.isGetter)
                      ...
                      Assign(gen.mkAttributedRef(target),
                             Ident(vparamss.head.head.symbol))
                ...
                case SpecialSuperAccessor(_) => // same as abstract method
                  debuglog(
                    s"special super accessor: $tree with $symbol -> ${symbol.alias} in ${symbol.alias.owner} (in $currentClass)")
                  localTyper.typed(deriveDefDef(tree)(rhs => rhs))
              }
          }
        ...
        case ValDef(_, _, _, _)
            if symbol.hasFlag(SPECIALIZED) && !symbol.isParamAccessor =>
          def transformValDef = {
            assert(body.isDefinedAt(symbol.alias), body)
            val tree1 = deriveValDef(tree)(_ => body(symbol.alias).duplicate)
            debuglog(
              "now typing: " + tree1 + " in " + tree.symbol.owner.fullName)
    ...
    /** Duplicate the body of the given method `tree` to the new symbol `source`.
      ...
      *  `castmap`.
      */
    private def duplicateBody(tree: DefDef,
                              source: Symbol,
                              castmap: TypeEnv = emptyEnv) = {
      val symbol = tree.symbol
    ...
    private def addBody(tree: DefDef, source: Symbol): DefDef = {
      ...
      val origtparams = source.typeParams.filter(tparam =>
        !boundTvars(tparam) || !isPrimitiveValueType(env(tparam)))
      ...
      val newSyms = cloneSymbolsAtOwnerAndModify(
        vparams map (_.symbol),
        symbol,
        _.substSym(oldtparams, newtparams))
          ...
          if (m.isClassConstructor) {
            ...
            val vparams = (
              map2(m.info.paramTypes, origParams)(
                (tp, sym) =>
                  m.newValue(specializedName(sym, typeEnv(sClass)),
                             sym.pos,
...
//            log("created synthetic class: " + m.fullName)
      ...
      if (hasSpecializedFields) {
        ...
        val sym = sClass.newMethod(nme.SPECIALIZED_INSTANCE, sClass.pos) setInfoAndEnter MethodType(
          Nil,
          BooleanTpe)

        mbrs += DefDef(
          sym,
          Literal(Constant(isSpecializedInstance)).setType(BooleanTpe))
          .setType(NoType)
      }
      mbrs.toList
    }
            ...
            case map =>
              ...
              map.iterator.map {
                case (env, specCls) =>
                  ...
                  ClassDef(
                    specCls,
                    atPos(impl.pos)(Template(parents, noSelfType, List()))
  ...
  private def forwardCall(pos: scala.reflect.internal.util.Position,
                          receiver: Tree,
  ...
  /** Forward to the generic class constructor. If the current class initializes
    ...
    */
  private def forwardCtorCall(pos: scala.reflect.internal.util.Position,
                              receiver: Tree,
    ...
    val argss = mmap(paramss)(
      x =>
        if (initializesSpecializedField(x.symbol))
          gen.mkAsInstanceOf(Literal(Constant(null)).updateAttachment(
