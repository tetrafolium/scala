...
abstract class ExtensionMethods extends Transform with TypingTransformers {
  ...
  def extensionMethod(imeth: Symbol): Symbol =
    enteringPhase(currentRun.refchecksPhase) {
      ...
      val candidates = extensionNames(imeth) map (companionInfo
        .decl(_)) filter (_.exists)
      ...
      tree match {
        case Template(_, _, _) =>
          if (currentOwner.isDerivedValueClass) {
            ...
            currentOwner.info.decls.foreach(
              sym =>
                if (sym.isParamAccessor && sym.isMethod)
                  sym.makeNotPrivate(currentOwner))
        ...
        case DefDef(_, _, tparams, vparamss, _, rhs)
          ...
          def makeExtensionMethodSymbol = {
            ...
            if (origMeth.isStrictFP && !extensionMeth.hasAnnotation(
                  ScalaStrictFPAttr))
          ...
          val castBody =
            if (extensionBody.tpe <:< extensionMono.finalResultType)
              ...
              gen.mkCastPreservingAnnotations(
                extensionBody,
                extensionMono.finalResultType) // scala/bug#7818 e.g. mismatched existential skolems
          ...
          val sel = Select(
            gen.mkAttributedRef(companion.owner.thisType, companion),
            extensionMeth)
    ...
    override def transform(tree: Tree): Tree = tree match {
      ...
      case treeInfo.Applied(sel @ Select(qual, _), targs, argss)
        ...
        localTyper.typedPos(tree.pos) {
          ...
          val fun =
            gen.mkAttributedTypeApply(gen.mkAttributedThis(extensionMeth.owner),
                                      extensionMeth,
                                      allTargs)
