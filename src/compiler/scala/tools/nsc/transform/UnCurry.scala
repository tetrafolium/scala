    ...
    private def mustExpandFunction(fun: Function) = {
      ...
      val canUseLambdaMetaFactory = (fun.attachments.get[SAMFunction] match {
        case Some(SAMFunction(userDefinedSamTp, sam, _)) =>
          ...
          erasure.compilesToPureInterface(
            erasure.javaErasure(userDefinedSamTp).typeSymbol) &&
        ...
        case _ =>
          true // our built-in FunctionN's are suitable for LambdaMetaFactory by construction
      })
...
// ------ Transforming anonymous functions and by-name-arguments ----------------
    ...
    def transformFunction(fun: Function): Tree =
      ...
      if (fun.vparams.isEmpty && isByNameRef(fun.body) && fun.attachments.get[SAMFunction].isEmpty) {
        ...
        val liftedMethod =
          gen.mkLiftedFunctionBodyMethod(localTyper)(fun.symbol.owner, fun)
        ...
        val typedNewFun = localTyper.typedPos(fun.pos)(
          Block(liftedMethod :: Nil, super.transform(newFun)))
        if (mustExpand) {
          val Block(stats, expr: Function) = typedNewFun
          treeCopy.Block(
            typedNewFun,
            stats,
            gen.expandFunction(localTyper)(expr, inConstructorFlag))
        } else {
          typedNewFun
        }
      }

    def transformArgs(pos: Position,
                      fun: Symbol,
                      args: List[Tree],
                      formals: List[Type]): List[Tree] = {
          ...
          exitingUncurry {
            localTyper.typedPos(pos) {
              ...
              if (copy) {
                ...
                gen.mkMethodCall(PredefModule,
                                 nme.copyArrayToImmutableIndexedSeq,
                                 List(elemtp),
                                 List(adaptedTree))
        ...
        var suffix: Tree =
          if (treeInfo isWildcardStarArgList args) {
            ...
            if (javaStyleVarArgs) mkArray
              ...
              arrayToSequence(mkArray, varargsElemType, copy = false) // fresh array, no need to copy
          }
      ...
      val args1 =
        if (isVarArgTypes(formals))
          transformVarargs(formals.last.typeArgs.head.widen)
        else args
      ...
      map2(formals, args1) { (formal, arg) =>
        if (!isByNameParamType(formal)) arg
          ...
          def canUseDirectly(qual: Tree) =
            qual.tpe.typeSymbol.isSubClass(FunctionClass(0)) && treeInfo
              .isExprSafeToInline(qual)
          arg match {
            // don't add a thunk for by-name argument if argument already is an application of
            // a Function0. We can then remove the application and use the existing Function0.
            case Apply(Select(qual, nme.apply), Nil) if canUseDirectly(qual) =>
              qual
            case body =>
              val thunkFun = localTyper
                .typedPos(body.pos)(Function(Nil, body))
                .asInstanceOf[Function]
              log(
                s"Change owner from $currentOwner to ${thunkFun.symbol} in ${thunkFun.body}")
      ...
      tree match {
        case DefDef(_, _, _, _, _, rhs) =>
          val rhs1 =
            ...
            else Block(Nil, elisionOf(rhs.tpe)) setType rhs.tpe
...
// ------ The tree transformers --------------------------------------------------------
    ...
    def mainTransform(tree: Tree): Tree = {
      ...
      def checkIsElidable(sym: Symbol): Boolean =
        (sym ne null) && sym.elisionLevel.exists { level =>
          if (sym.isMethod) level < settings.elidebelow.value
            ...
            if (currentRun.isScala213)
              reporter.error(
                sym.pos,
                s"${sym.name}: Only methods can be marked @elidable.")
            false
          }
        }
        ...
        if (checkIsElidable(sym))
          ...
          translateSynchronized(tree) match {
            ...
            case fun @ Function(_, Apply(target, _))
                if !forceExpandFunction && isLiftedLambdaMethod(
                  target.symbol) =>
              super.transform(fun)
    ...
    def postTransform(tree: Tree): Tree = exitingUncurry {
      ...
      tree match {
          ...
          val literalRhsIfConst =
            if (newParamss.head.isEmpty) { // We know newParamss.length == 1 from above
              ddSym.info.resultType match {
                case tp @ FoldableConstantType(value) =>
        ...
        case ret @ Return(expr) if isNonLocalReturn(ret) =>
          log(
            s"non-local return from ${currentOwner.enclMethod} to ${ret.symbol}")
          if (settings.warnNonlocalReturn)
            reporter.warning(
              ret.pos,
              s"return statement uses an exception to pass control to the caller of the enclosing named ${ret.symbol}")
    ...
    private object dependentParamTypeErasure {
      ...
      final case class Packed(param: ValDef, tempVal: ValDef)
          extends ParamTransform
    ...
    private def validateVarargs(dd: DefDef): Unit =
      if (dd.symbol.isConstructor)
        ...
        if (!hasRepeated)
          reporter.error(
            dd.symbol.pos,
            "A method without repeated parameters cannot be annotated with the `varargs` annotation.")
      }
    ...
    private def addJavaVarargsForwarders(dd: DefDef, flatdd: DefDef): DefDef = {
      ...
      val forwSym: Symbol = {
        ...
        flatdd.symbol.attachments.get[VarargsSymbolAttachment] match {
          ...
          case None =>
            reporter.warning(
              dd.pos,
              s"Could not generate Java varargs forwarder for ${flatdd.symbol}. Please file a bug.")
            return flatdd
        }
      }

      val newPs = forwSym.tpe.params
      val isRepeated = enteringUncurry(dd.symbol.info.paramss.flatten.map(sym =>
        definitions.isRepeatedParamType(sym.tpe)))
      ...
      val forwTree = theTyper.typedPos(dd.pos) {
          ...
          map3(newPs, oldPs, isRepeated)((param, oldParam, isRep) => {
            if (!isRep) Ident(param)
              ...
              param.attachments.get[TypeParamVarargsAttachment] match {
                case Some(TypeParamVarargsAttachment(tp)) =>
                  gen.mkCast(wrap, seqType(tp))
                case _ => wrap
              }
            }
          })
        ...
        DefDef(forwSym,
               if (forwSym.isConstructor) Block(List(forwCall), UNIT)
               else forwCall)
      }
