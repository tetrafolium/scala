...
abstract class CleanUp extends Statics with Transform with ast.TreeDSL {
  ...
  def getEntryPoints: List[String] =
    entryPoints.toList.map(_.fullName('.')).sorted
  ...
  class CleanUpTransformer(unit: CompilationUnit) extends StaticsTransformer {
          ...
          BLOCK(
            ...
            IF(REF(methodSym) OBJ_NE NULL).THEN(Return(REF(methodSym)))
              ELSE {
                ...
                BLOCK(
                  REF(methodSym) === (REF(
                    currentRun.runDefinitions.ensureAccessibleMethod) APPLY (methodSymRHS)),
                  cacheAdd,
    ...
    override def transform(tree: Tree): Tree = tree match {
      case _: ClassDef
          if genBCode.codeGen.CodeGenImpl.isJavaEntryPoint(
            tree.symbol,
            currentUnit,
            settings.mainClass.valueSetByUser.map(_.toString)) =>
      ...
      case app @ Apply(TypeApply(fun, _), args)
      ...
      case Apply(appMeth,
                 Apply(wrapRefArrayMeth,
                       (arg @ StripCast(ArrayValue(_, _))) :: Nil) :: _ :: Nil)
          if wrapRefArrayMeth.symbol == currentRun.runDefinitions.wrapVarargsRefArrayMethod && appMeth.symbol == ArrayModule_genericApply =>
        arg.transform(this)
      case Apply(appMeth,
                 elem0 :: Apply(wrapArrayMeth,
                                (rest @ ArrayValue(elemtpt, _)) :: Nil) :: Nil)
          if wrapArrayMeth.symbol == wrapVarargsArrayMethod(elemtpt.tpe) && appMeth.symbol == ArrayModule_apply(
            elemtpt.tpe) =>
        treeCopy
          .ArrayValue(rest, rest.elemtpt, elem0 :: rest.elems)
          .transform(this)

      // List(a, b, c) ~> new ::(a, new ::(b, new ::(c, Nil)))
      case Apply(appMeth @ Select(qual, _),
                 List(
                   Apply(wrapArrayMeth,
                         List(StripCast(rest @ ArrayValue(elemtpt, _))))))
          if wrapArrayMeth.symbol == currentRun.runDefinitions.wrapVarargsRefArrayMethod
            && currentRun.runDefinitions
              .isListApply(appMeth) && rest.elems.lengthIs < transformListApplyLimit =>
        val consed =
          rest.elems.reverse.foldLeft(gen.mkAttributedRef(NilModule): Tree)(
