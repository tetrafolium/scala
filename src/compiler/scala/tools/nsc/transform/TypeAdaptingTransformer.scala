  ...
  abstract class TypeAdapter {
    ...
    private def isSafelyRemovableUnbox(fn: Tree, arg: Tree): Boolean = {
      currentRun.runDefinitions.isUnbox(fn.symbol) && {
        ...
        arg.hasAttachment[
          specializeTypes.SpecializedSuperConstructorCallArgument.type] ||
    ...
    final def isPrimitiveValueMember(sym: Symbol) =
      isPrimitiveValueClass(sym.owner)
    final def isMethodTypeWithEmptyParams(tpe: Type) =
      tpe.isInstanceOf[MethodType] && tpe.params.isEmpty
    final def applyMethodWithEmptyParams(qual: Tree) =
      Apply(qual, List()) setPos qual.pos setType qual.tpe.resultType
      ...
      case _ =>
        ...
        val tree1 = pt match {
          ...
          case ErasedValueType(clazz, underlying) =>
            cast(unboxValueClass(tree, clazz, underlying), pt)
          case _ =>
            pt.typeSymbol match {
              case UnitClass =>
    ...
    final def unboxValueClass(tree: Tree,
                              clazz: Symbol,
                              underlying: Type): Tree =
      if (tree.tpe.typeSymbol == NullClass && isPrimitiveValueClass(
    ...
    @tailrec final def adaptToType(tree: Tree, pt: Type): Tree = {
      ...
      else if (tpe.isInstanceOf[ErasedValueType])
        adaptToType(box(tree), pt) // what if pt is an erased value type?
      else if (pt.isInstanceOf[ErasedValueType])
      ...
      // [H] this does not hold here, however: `assert(tree.symbol.isStable)` (when typechecking !(SomeClass.this.bitmap) for single lazy val)
      else if (isMethodTypeWithEmptyParams(tpe))
        adaptToType(applyMethodWithEmptyParams(tree), pt)
      else {
        ...
        else if (!gotPrimitiveVC && expectedPrimitiveVC)
          adaptToType(unbox(tree, pt), pt)
        else cast(tree, pt)
      }
    }
