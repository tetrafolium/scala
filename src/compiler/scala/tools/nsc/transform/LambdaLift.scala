...
abstract class LambdaLift extends InfoTransform {
  ...
  def transformInfo(sym: Symbol, tp: Type): Type =
    if (sym.isCapturedVariable)
      capturedVariableType(sym, tpe = lifted(tp), erasedTypes = true)
    else lifted(tp)

  protected def newTransformer(unit: CompilationUnit): Transformer =
    new LambdaLifter(unit)

  class LambdaLifter(unit: CompilationUnit)
      extends explicitOuter.OuterPathTransformer(unit) {
    ...
    private def logicallyEnclosingMember(sym: Symbol): Symbol = {
      if (sym.isLocalDummy) {
        ...
        if (enclClass.isSubClass(DelayedInitClass))
          delayedInitDummies.getOrElseUpdate(
            enclClass,
            enclClass.newMethod(nme.delayedInit))
        else
    ...
    private def isSameOwnerEnclosure(sym: Symbol) =
      logicallyEnclosingMember(sym.owner) == logicallyEnclosingMember(
        currentOwner)
...
//      println(s"mark free: ${sym.fullLocationString} marked free in $enclosure")
      (enclosure == logicallyEnclosingMember(sym.owner)) || {
        debuglog(
          "%s != %s".format(enclosure, logicallyEnclosingMember(sym.owner)))
        if (enclosure.isPackageClass || !markFree(
              sym,
              logicallyEnclosingMember(enclosure.skipConstructor.owner))) false
        else {
...
//       }
    ...
    private def computeFreeVars(): Unit = {
      ...
      def newName(sym: Symbol): Name = {
        ...
        if (sym.isAnonymousFunction && sym.owner.isMethod) {
          freshen(
            sym.name + join + nme.ensureNonAnon(sym.owner.name.toString) + join)
        } else {
          ...
          if (originalName.isTermName && calledFromInner(sym))
            newTermNameCached(
              nme.ensureNonAnon(sym.enclClass.fullName('$')) + nme.EXPAND_SEPARATOR_STRING + name)
          else
            name
        }
      }
      ...
      afterOwnPhase {
        for ((owner, freeValues) <- free.toList) {
          val newFlags = SYNTHETIC | (if (owner.isClass) PARAMACCESSOR
                                      else PARAM)
          ...
          proxies(owner) = for (fv <- freeValues.toList) yield {
            ...
            val proxy =
              if (owner.isTrait) {
                ...
                val setter = owner.newMethod(
                  nme.expandedSetterName(proxyName.setterName, owner),
                  fv.pos,
                  accessorFlags)
                setter setInfoAndEnter MethodType(
                  setter.newSyntheticValueParams(List(fv.info)),
                  UnitTpe)

                // the getter serves as the proxy -- entered below
                owner.newMethod(
                  proxyName.getterName,
                  fv.pos,
                  accessorFlags | STABLE) setInfo MethodType(Nil, fv.info)
              } else
                owner.newValue(proxyName.toTermName,
                               fv.pos,
                               newFlags.toLong | PrivateLocal) setInfo fv.info
      ...
      def searchIn(enclosure: Symbol): Symbol = {
        if (enclosure eq NoSymbol)
          throw new IllegalArgumentException(
            "Could not find proxy for " + sym.defString + " in " + sym.ownerChain + " (currentOwner= " + currentOwner + " )")
        debuglog(
          "searching for " + sym + "(" + sym.owner + ") in " + enclosure + " " + logicallyEnclosingMember(
            enclosure))
    ...
    private def memberRef(sym: Symbol): Tree = {
      ...
      def prematureSelfReference(): Tree = {
        ...
        val msg =
          s"Implementation restriction: access of ${sym.fullLocationString} from ${currentClass.fullLocationString}, would require illegal premature access to $what"
    ...
    def freeArgsOrNil(sym: Symbol) =
      free.getOrElse[Iterable[Symbol]](sym, Nil).toList
          ...
          val freeParamDefs =
            if (tree.symbol.isTrait) {
              freeParamSyms flatMap { getter =>
                val setter =
                  getter.setterIn(tree.symbol, hasExpandedName = true)
                List(DefDef(getter, EmptyTree) setPos tree.pos setType NoType,
                     DefDef(setter, EmptyTree) setPos tree.pos setType NoType)
              }
            } else
              freeParamSyms map (p => ValDef(p) setPos tree.pos setType NoType)
    ...
    private def liftDef(tree: Tree): Tree = {
      ...
      if (sym.isMethod && isUnderConstruction(sym.owner.owner)) { // # bug 1909
        if (sym.isModule) { // Yes, it can be a module and a method, see comments on `isModuleNotMethod`!
          ...
          def msg =
            s"scala/bug#1909 Unable to STATICally lift $sym, which is defined in the self- or super-constructor call of ${sym.owner.owner}. A VerifyError is likely."
