...
private[jvm] object GeneratedClassHandler {
  def apply(global: Global): GeneratedClassHandler = {
    ...
    val handler = settings.YaddBackendThreads.value match {
      ...
      case maxThreads =>
        ...
        val queueSize =
          if (settings.YmaxQueue.isSetByUser) settings.YmaxQueue.value
          else maxThreads * 2
        val threadPoolFactory = ThreadPoolFactory(global, currentRun.jvmPhase)
        val javaExecutor = threadPoolFactory.newBoundedQueueFixedThreadPool(
          additionalThreads,
          queueSize,
  ...
  sealed abstract class WritingClassHandler(val javaExecutor: Executor)
      extends GeneratedClassHandler {
    ...
    def process(unit: GeneratedCompilationUnit): Unit = {
      val unitInPostProcess = new CompilationUnitInPostProcess(
        unit.classes,
        CompilationUnitPaths(
          unit.sourceFile,
    ...
    final def postProcessUnit(
        unitInPostProcess: CompilationUnitInPostProcess): Unit = {
      unitInPostProcess.task = Future {
        frontendAccess.withThreadLocalReporter(
          unitInPostProcess.bufferedReporting) {
    ...
    final def complete(): Unit = {
      ...
      def stealWhileWaiting(
        ...
        while (!task.isCompleted) tryStealing match {
          case Some(r) => r.run()
      ...
      takeProcessingUnits().foreach { unitInPostProcess =>
        try {
          ...
          unitInPostProcess.bufferedReporting.relayReports(
          ...
          case NonFatal(t) =>
            ...
            frontendAccess.backendReporting.error(
              NoPosition,
              s"unable to write ${unitInPostProcess.paths.sourceFile} $t")
        }
      }
    }
  }
  ...
  private final class AsyncWritingClassHandler(
      val postProcessor: PostProcessor,
      override val javaExecutor: ThreadPoolExecutor)
      extends WritingClassHandler(javaExecutor) {

    override def toString: String =
      s"AsyncWriting[additional threads:${javaExecutor.getMaximumPoolSize}]"
...
final case class CompilationUnitPaths(sourceFile: AbstractFile,
...
final class CompilationUnitInPostProcess(
    ...
    val paths: CompilationUnitPaths) {
