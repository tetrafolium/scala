...
abstract class BCodeHelpers extends BCodeIdiomatic {
  ...
  def considerAsTopLevelImplementationArtifact(classSym: Symbol) =
    classSym.isSpecialized ||
      classSym.isSynthetic && classSym.name.containsName(
  ...
  def needsStaticImplMethod(sym: Symbol) =
  ...
  def classOriginallyNestedInClass(nestedClass: Symbol,
  ...
  final case class EnclosingMethodEntry(owner: String,
                                        name: String,
  ...
  @inline private def withoutReporting[T](fn: => T) = {
    ...
    reporter = NoReporter
    try fn
    finally reporter = currentReporter
  }
  ...
  object isJavaEntryPoint {
    ...
    def apply(sym: Symbol,
              csymCompUnit: CompilationUnit,
              mainClass: Option[String]): Boolean = sym.hasModuleFlag && {
      val warn = mainClass.fold(true)(_ == sym.fullNameString)
      def warnBadMain(msg: String, pos: Position): Unit =
        if (warn)
          reporter.warning(
            pos,
      ...
      def warnNoForwarder(msg: String, hasExact: Boolean, mainly: Type) =
        if (warn)
          reporter.warning(
            sym.pos,
            s"""|${sym.name.decoded} has a ${if (hasExact) "valid " else ""}main method${if (mainly != NoType)
                 " " + mainly
               else ""},
      ...
      def check(): Boolean = enteringErasure {
        ...
        companionAdvice.foreach(msg =>
          warnNoForwarder(msg, hasExact, exactly.fold(alternate)(_.info)))
  ...
  trait BCPickles {
     ...
     * can-multi-thread
     */
    def createJAttribute(name: String,
                         b: Array[Byte],
                         offset: Int,
                         len: Int): asm.Attribute = {
      new asm.Attribute(name) {
        override def write(classWriter: ClassWriter,
                           code: Array[Byte],
                           codeLength: Int,
                           maxStack: Int,
                           maxLocals: Int): asm.ByteVector = {
    ...
    def getAnnotPickle(jclassName: String,
      ...
      currentRun.symData get sym match {
        case Some(pickle)
            if !sym.isModuleClass => // pickles for module classes are in the companion / mirror class
          val scalaAnnot = {
            val sigBytes = new ScalaSigBytes(
              pickle.bytes.take(pickle.writeIndex))
            val (annTp, arg) = if (sigBytes.fitsInOneString) {
              ...
              (tp,
               ArrayAnnotArg(
                 sigBytes.arrEncode.map(s => LiteralAnnotArg(Constant(s)))))
            }
  ...
  trait BCInnerClassGen {
    ...
    final def internalName(sym: Symbol): String =
      classBTypeFromSymbol(sym).internalName
  } // end of trait BCInnerClassGen

  trait BCAnnotGen extends BCInnerClassGen {
    private lazy val AnnotationRetentionPolicyModule =
      AnnotationRetentionPolicyAttr.companionModule
    private lazy val AnnotationRetentionPolicySourceValue =
      AnnotationRetentionPolicyModule.tpe.member(TermName("SOURCE"))
    private lazy val AnnotationRetentionPolicyClassValue =
      AnnotationRetentionPolicyModule.tpe.member(TermName("CLASS"))
    private lazy val AnnotationRetentionPolicyRuntimeValue =
      AnnotationRetentionPolicyModule.tpe.member(TermName("RUNTIME"))
    ...
    def descriptorForErasedType(tp: Type): String =
      typeToBType(erasedType(tp)).descriptor
    ...
    def emitArgument(av: asm.AnnotationVisitor,
      ...
      (arg: @unchecked) match {
        ...
        case LiteralAnnotArg(const) =>
          if (const.isNonUnitAnyVal) { av.visit(name, const.value) } else {
            const.tag match {
              case StringTag =>
              ...
              case ClazzTag =>
                av.visit(name,
                         typeToBType(erasedType(const.typeValue)).toASMType)
              case EnumTag =>
     ...
     */
    def emitAssocs(av: asm.AnnotationVisitor,
                   assocs: List[(Name, ClassfileAnnotArg)]): Unit = {
    ...
    def emitAnnotations(cw: asm.ClassVisitor,
                        annotations: List[AnnotationInfo]): Unit = {
      for (annot <- annotations; if shouldEmitAnnotation(annot)) {
        val AnnotationInfo(typ, args, assocs) = annot
        assert(args.isEmpty, args)
        val av = cw.visitAnnotation(descriptorForErasedType(typ),
    ...
    def emitAnnotations(mw: asm.MethodVisitor,
      ...
      for (annot <- annotations; if shouldEmitAnnotation(annot)) {
        val AnnotationInfo(typ, args, assocs) = annot
        assert(args.isEmpty, args)
        val av = mw.visitAnnotation(descriptorForErasedType(typ),
                                    isRuntimeVisible(annot))
        emitAssocs(av, assocs)
      }
    }
    ...
    def emitAnnotations(fw: asm.FieldVisitor,
      ...
      for (annot <- annotations; if shouldEmitAnnotation(annot)) {
        val AnnotationInfo(typ, args, assocs) = annot
        assert(args.isEmpty, args)
        val av = fw.visitAnnotation(descriptorForErasedType(typ),
                                    isRuntimeVisible(annot))
        emitAssocs(av, assocs)
      }
    }
    ...
    def emitParamAnnotations(jmethod: asm.MethodVisitor,
                             pannotss: List[List[AnnotationInfo]]): Unit = {
  ...
  trait BCJGenSigGen {
    ...
    def getGenericSignature(sym: Symbol,
                            owner: Symbol,
      ...
      def enterReferencedClass(sym: Symbol): Unit =
        enteringJVM(classBTypeFromSymbol(sym))
      ...
      def wrap(op: => Unit) = {
        try { op; true } catch { case _: Throwable => false }
      }
  ...
  trait BCForwardersGen extends BCAnnotGen with BCJGenSigGen {
     ...
     * must-single-thread
     */
    private def addForwarder(jclass: asm.ClassVisitor,
                             moduleClass: Symbol,
                             m: Symbol): Unit = {
      ...
      mirrorMethod.visitFieldInsn(asm.Opcodes.GETSTATIC,
                                  ...
                                  classBTypeFromSymbol(moduleClass).descriptor)
     ...
     * must-single-thread
     */
    def addForwarders(jclass: asm.ClassVisitor,
                      jclassName: String,
                      moduleClass: Symbol): Unit = {
  ...
  class JMirrorBuilder extends JCommonBuilder {
     ...
     *  must-single-thread
     */
    def genMirrorClass(moduleClass: Symbol,
                       cunit: CompilationUnit): asm.tree.ClassNode = {
...
object BCodeHelpers {
  ...
  final class ScalaSigBytes(bytes: Array[Byte]) {
    ...
    /**
      ...
      */
    lazy val sevenBitsMayBeZero: Array[Byte] = mapToNextModSevenBits(
      ByteCodecs.encode8to7(bytes))
    ...
    def strEncode: String =
      new java.lang.String(ubytesToCharArray(sevenBitsMayBeZero))
