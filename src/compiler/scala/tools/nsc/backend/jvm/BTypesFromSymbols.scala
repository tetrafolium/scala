...
abstract class BTypesFromSymbols[G <: Global](val global: G) extends BTypes {
  ...
  final def classBTypeFromSymbol(sym: Symbol): ClassBType = {
    ...
    if (classSym == NothingClass) srNothingRef
      ...
      val btype = ClassBType(internalName, fromSymbol = true) {
        res: ClassBType =>
      ...
      if (currentRun.compiles(classSym))
        assert(
          btype.fromSymbol,
          s"ClassBType for class being compiled was already created from a classfile: ${classSym.fullName}")
      btype
    }
  }
  ...
  final def methodBTypeFromSymbol(methodSymbol: Symbol): MethodBType = {
    ...
    methodBTypeFromMethodType(
      methodSymbol.info,
      methodSymbol.isClassConstructor || methodSymbol.isConstructor)
  }
  ...
  def staticHandleFromSymbol(sym: Symbol): asm.Handle = {
    ...
    new asm.Handle(asm.Opcodes.H_INVOKESTATIC,
                   ownerBType.internalName,
                   sym.name.encoded,
                   descriptor, /* itf = */ ownerBType.isInterface.get)
  }
      ...
      case tp =>
        ...
        tp match {
          case ThisType(ArrayClass) =>
            ObjectRef // was introduced in 9b17332f11 to fix scala/bug#999, but this code is not reached in its test, or any other test
  ...
  def implementedInterfaces(classSym: Symbol): List[Symbol] = {
    ...
    val minimizedParents =
      if (classSym.isJavaDefined) classParents
      else erasure.minimizeParents(classSym, classParents)
  ...
  private def memberClassesForInnerClassTable(
    ...
    List.from(classSymbol.info.decls.iterator.collect({
      case sym
          if sym.isClass && !considerAsTopLevelImplementationArtifact(sym) =>
        sym
      case sym
          if sym.isModule && !considerAsTopLevelImplementationArtifact(sym) =>
        val r = exitingPickler(sym.moduleClass)
        assert(r != NoSymbol, sym.fullLocationString)
        r
    }))

  private def computeClassInfo(
      classSym: Symbol,
      classBType: ClassBType): Right[Nothing, ClassInfo] = {
    ...
    /**
      ...
      */
    def nestedClassSymbolsNoJavaModuleClasses =
      nestedClassSymbols.filter(s => {
  ...
  private def isEmptyNestedInfo(innerClassSym: Symbol): Boolean = {
    ...
    if (isTopLevel || considerAsTopLevelImplementationArtifact(innerClassSym))
      true
  ...
  private def buildNonEmptyNestedInfo(
      innerClassSym: Symbol): Option[NestedInfo] = {
    assert(innerClassSym.isClass,
           s"Cannot build NestedInfo for non-class symbol $innerClassSym")
    // See comment in BTypes, when is a class marked static in the InnerClass table.
    val isStaticNestedClass = isOriginallyStaticOwner(
      innerClassSym.originalOwner)
    ...
    val innerName: Option[String] = {
      ...
      if (exitingPickler(
        ...
        Some(s"${innerClassSym.rawname}${innerClassSym.moduleSuffix}") // moduleSuffix for module classes
    }
  ...
  private def buildInlineInfo(classSym: Symbol,
    ...
    if (exitingPickler(currentRun.compiles(classSym)))
      buildInlineInfoFromClassSymbol(classSym) // InlineInfo required for classes being compiled, we have to create the classfile attribute
    else if (!settings.optInlinerEnabled)
      BTypes.EmptyInlineInfo // For other classes, we need the InlineInfo only if the inliner is enabled
    else {
  ...
  def buildInlineInfoFromClassSymbol(classSym: Symbol): InlineInfo = {
    ...
    val sam = {
      val considerSam = !classSym.isEffectivelyFinal && {
        isFunctionSymbol(classSym) || classSym.hasAnnotation(
      ...
      if (!considerSam) None
        ...
        if (samSym == NoSymbol) None
          ...
          Some(
            samSym.javaSimpleName.toString + methodBTypeFromSymbol(samSym).descriptor)
      }
    }

    var warning = Option.empty[ClassSymbolInfoFailureSI9111]

    def keepMember(sym: Symbol) =
      sym.isMethod && !scalaPrimitives.isPrimitive(sym)
    val classMethods = classSym.info.decls.iterator.filter(keepMember)
    val methods =
      if (!classSym.isJavaDefined) classMethods
      else {
        ...
        val staticMethods = companion.info.decls.iterator.filter(m =>
          !m.isConstructor && keepMember(m))
      ...
      case methodSym =>
        if (completeSilentlyAndCheckErroneous(methodSym)) {
          ...
          if (!classSym.isJavaDefined)
            devWarning(
              "scala/bug#9111 should only be possible for Java classes")
          ...
          if (needsStaticImplMethod(methodSym)) {
            ...
            val selfParam =
              methodSym.newSyntheticValueParam(methodSym.owner.typeConstructor,
                                               nme.SELF)
            val staticMethodType = methodSym.info match {
              case mt @ MethodType(params, res) =>
                copyMethodType(mt, selfParam :: params, res)
            }
            val staticMethodSignature = staticName + methodBTypeFromMethodType(
              staticMethodType,
              isConstructor = false)
            val staticMethodInfo =
              MethodInlineInfo(effectivelyFinal = true,
  ...
  def mirrorClassClassBType(moduleClassSym: Symbol): ClassBType = {
    ...
    val internalName =
      moduleClassSym.javaBinaryNameString.stripSuffix(nme.MODULE_SUFFIX_STRING)
    ClassBType(internalName, fromSymbol = true) { c: ClassBType =>
      val shouldBeLazy = moduleClassSym.isJavaDefined || !currentRun.compiles(
        moduleClassSym)
      val nested =
        Lazy.withLockOrEager(shouldBeLazy,
                             exitingPickler(memberClassesForInnerClassTable(
  ...
  final def javaFlags(sym: Symbol): Int = {
    ...
    (if (privateFlag) ACC_PRIVATE else ACC_PUBLIC) |
      (if ((sym.isDeferred && !sym.hasFlag(symtab.Flags.JAVA_DEFAULTMETHOD)) || sym.hasAbstractFlag)
         ACC_ABSTRACT
       else 0) |
