...
abstract class BTypes {
  ...
  val classBTypeCache
    : ju.concurrent.ConcurrentHashMap[InternalName, ClassBType] =
  ...
  sealed trait BType {
    ...
    final def buildString(builder: java.lang.StringBuilder): Unit = this match {
      ...
      case ArrayBType(component) =>
        builder.append('['); component.buildString(builder)
            ...
            if (other == ObjectRef || other == jlCloneableRef || other == jiSerializableRef)
              ...
              other match {
                case ArrayBType(otherComponent) =>
                  ...
                  if (component.isPrimitive || otherComponent.isPrimitive)
  ...
  /**
    ...
    */
  sealed abstract class ClassBType protected (val internalName: InternalName)
      extends RefBType {
    ...
    private def checkInfoConsistency(): Unit = {
      ...
      def ifInit(c: ClassBType)(p: ClassBType => Boolean): Boolean =
        c._info == null || c.info.isLeft || p(c)
    ...
    def isPublic: Either[NoClassBTypeInfo, Boolean] =
      info.map(i => (i.flags & asm.Opcodes.ACC_PUBLIC) != 0)

    def isNestedClass: Either[NoClassBTypeInfo, Boolean] =
      info.map(_.nestedInfo.force.isDefined)
    ...
    def enclosingNestedClassesChain
      ...
      isNestedClass.flatMap(isNested => {
        ...
        if (isNested)
          info.get.nestedInfo.force.get.enclosingClass.enclosingNestedClassesChain
            .map(this :: _)
        else Right(Nil)
      })
    }
          ...
          case NestedInfo(_, outerName, innerName, isStaticNestedClass) =>
            InnerClassEntry(
              ...
              (i.flags & ~Opcodes.ACC_STATIC |
                (if (isStaticNestedClass) Opcodes.ACC_STATIC else 0)) & BCodeHelpers.INNER_CLASSES_FLAGS
            )
      })
      ...
      tryEither {
        val res: ClassBType =
          (this.isInterface.orThrow, other.isInterface.orThrow) match {
            ...
            case _ =>
              firstCommonSuffix(superClassesChain.orThrow,
                                other.superClassesChain.orThrow)
          }
  ...
  object ClassBType {
    ...
    def apply(internalName: InternalName, fromSymbol: Boolean)(
        init: (ClassBType) => Either[NoClassBTypeInfo, ClassInfo])
      : ClassBType = {
  ...
  private final class ClassBTypeFromSymbol(internalName: InternalName)
      extends ClassBType(internalName) {
    override def fromSymbol: Boolean = true
  }
  private final class ClassBTypeFromClassfile(internalName: InternalName)
      extends ClassBType(internalName) {
  ...
  /**
    ...
    */
  final case class ClassInfo(superClass: Option[ClassBType],
  ...
  object Lazy {
    ...
    private abstract class AbstractLazy[T <: AnyRef](private var t: () => T)
        extends Lazy[T] {
    ...
    /**
      ...
      */
    private final class LazyWithLock[T <: AnyRef](t: () => T)
        extends AbstractLazy[T](t) {
    ...
    /**
      ...
      * to be executed when it's forced.
      */
    private final class LazyWithoutLock[T <: AnyRef](t: () => T)
        extends AbstractLazy[T](t) {
...
final class ClearableJConcurrentHashMap[K, V]
    extends scala.collection.mutable.Clearable {
