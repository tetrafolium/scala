...
import scala.tools.nsc.backend.jvm.BackendReporting.{
  NoClassBTypeInfoMissingBytecode,
  NoInlineInfoAttribute
}
...
abstract class BTypesFromClassfile {
  ...
  /**
    ...
    */
  def bTypeForDescriptorOrInternalNameFromClassfile(descOrIntN: String): BType =
    (descOrIntN(0): @switch) match {
      case '[' =>
        ArrayBType(bTypeForDescriptorFromClassfile(descOrIntN.substring(1)))
      case 'L' if descOrIntN.last == ';' =>
        bTypeForDescriptorFromClassfile(descOrIntN)
      case _ => classBTypeFromParsedClassfile(descOrIntN)
    }

  def bTypeForDescriptorFromClassfile(desc: String): BType =
    (desc(0): @switch) match {
      case 'V' => UNIT
      case 'Z' => BOOL
      case 'C' => CHAR
  ...
  private def computeClassInfoFromClassNode(
    ...
    def nestedClasses: List[ClassBType] =
      classNode.innerClasses.asScala.iterator
        .collect({
    ...
    val interfaces: List[ClassBType] = classNode.interfaces.asScala.iterator
      .map(classBTypeFromParsedClassfile)
      .toList
  ...
  def inlineInfoFromClassfile(classNode: ClassNode): InlineInfo = {
    def fromClassfileAttribute: Option[InlineInfo] = {
      if (classNode.attrs == null) None
        ...
        classNode.attrs.asScala.collectFirst {
    ...
    def fromClassfileWithoutAttribute = {
      val warning = {
        val isScala = classNode.attrs != null && classNode.attrs.asScala
          .exists(a =>
      ...
      val methodInfos: Map[String, MethodInlineInfo] =
        classNode.methods.asScala.iterator
          .map(methodNode => {
          ...
          .toMap
      InlineInfo(isEffectivelyFinal = BytecodeUtils.isFinalClass(classNode),
