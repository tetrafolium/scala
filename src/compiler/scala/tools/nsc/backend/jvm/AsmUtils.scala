...
//  def main(args: Array[String]): Unit = println(textify(sortedClassRead(classBytes(args.head))))
  ...
  def zapScalaClassAttrs(node: ClassNode): node.type = {
    if (node.visibleAnnotations != null)
      node.visibleAnnotations = node.visibleAnnotations.asScala
        .filterNot(a =>
          a == null || a.desc.contains("Lscala/reflect/ScalaSignature"))
        .asJava
  ...
  def main(args: Array[String]): Unit = args /*.par*/.foreach { classFileName =>
    val node = zapScalaClassAttrs(
      sortClassMembers(classFromBytes(classBytes(classFileName))))
