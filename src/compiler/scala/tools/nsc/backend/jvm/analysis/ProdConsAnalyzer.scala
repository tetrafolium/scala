...
class ProdConsAnalyzer(methodNode: MethodNode, classInternalName: InternalName)
  ...
  def consumersOfOutputsFrom(insn: AbstractInsnNode): Set[AbstractInsnNode] =
    insn match {
      ...
      case ExceptionProducer(handlerLabel, handlerStackTop) =>
        consumersOfValueAt(handlerLabel, handlerStackTop)
      case _ =>
        _consumersOfOutputsFrom
          .get(insn)
          .map(v =>
            Set.from[AbstractInsnNode](v.indices.iterator.flatMap(v.apply)))
          .getOrElse(Set.empty)
    }
  ...
  private lazy val _consumersOfOutputsFrom
    ...
    for {
      ...
      val currentConsumers = res.getOrElse(
        producer,
...
abstract class InitialProducer extends AbstractInsnNode(-1) {
  ...
  override def clone(labels: util.Map[LabelNode, LabelNode]): AbstractInsnNode =
...
class InitialProducerSourceInterpreter
    extends SourceInterpreter(scala.tools.asm.Opcodes.ASM7) {
  ...
  override def newExceptionValue(tryCatchBlockNode: TryCatchBlockNode,
                                 handlerFrame: Frame[SourceValue],
    ...
    new SourceValue(
      1,
      ExceptionProducer(tryCatchBlockNode.handler, handlerStackTop))
  }
}
