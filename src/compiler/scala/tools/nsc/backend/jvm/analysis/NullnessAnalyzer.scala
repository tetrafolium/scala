...
sealed abstract class NullnessValue(final val isSize2: Boolean) extends Value {
  ...
  def invert: NullnessValue =
    if (this == NullValue) NotNullValue
    else if (this == NotNullValue) NullValue
    else this
}
...
object NullnessValue {
  ...
  def unknown(insn: AbstractInsnNode) =
    if (instructionResultSize(insn) == 2) UnknownValue2 else UnknownValue1
}

final class NullnessInterpreter(
    knownNonNullInvocation: MethodInsnNode => Boolean,
    modulesNonNull: Boolean,
    method: MethodNode)
...
class NullnessFrame(nLocals: Int, nStack: Int)
  ...
  private def setNullness(s: AliasSet, v: NullnessValue) = {
    val it = s.iterator
    while (it.hasNext) this.setValue(it.next(), v)
  }
  ...
  override def execute(insn: AbstractInsnNode,
    ...
    val nullCheckedAliases: AliasSet = (insn.getOpcode: @switch) match {
      ...
      case INVOKESTATIC =>
        var nullChecked = BackendUtils.argumentsNullCheckedByCallee(
          insn.asInstanceOf[MethodInsnNode])
        var i = 0
...
class NullnessAnalyzerImpl(methodNode: MethodNode,
                           knownNonNullInvocation: MethodInsnNode => Boolean,
                           modulesNonNull: Boolean)
    extends Analyzer[NullnessValue](
      new NullnessInterpreter(knownNonNullInvocation,
                              modulesNonNull,
                              methodNode)) {
...
class NullnessAnalyzer(methodNode: MethodNode,
                       classInternalName: InternalName,
                       knownNonNullInvocation: MethodInsnNode => Boolean,
                       modulesNonNull: Boolean)
    extends AsmAnalyzer(methodNode,
                        classInternalName,
                        new NullnessAnalyzerImpl(methodNode,
                                                 knownNonNullInvocation,
                                                 modulesNonNull))
