...
abstract class BackendUtils extends PerRunInit {
  ...
  /**
    ...
    * has the method.
    */
  private val indyLambdaImplMethods: ConcurrentHashMap[
    InternalName,
    mutable.Map[MethodNode, mutable.Map[InvokeDynamicInsnNode, asm.Handle]]] =
    recordPerRunJavaMapCache(new ConcurrentHashMap)

  // unused objects created by these constructors are eliminated by pushPop
  private[this] lazy val sideEffectFreeConstructors
    : LazyVar[Set[(String, String)]] = perRunLazy(this) {
  ...
  private[this] lazy val classesOfSideEffectFreeConstructors
    : LazyVar[Set[String]] =
    perRunLazy(this)(sideEffectFreeConstructors.get.map(_._1))

  lazy val classfileVersion: LazyVar[Int] =
    perRunLazy(this)(compilerSettings.target match {
      case "jvm-1.8" => asm.Opcodes.V1_8
    })

  lazy val majorVersion: LazyVar[Int] =
    perRunLazy(this)(classfileVersion.get & 0xFF)

  lazy val emitStackMapFrame: LazyVar[Boolean] =
    perRunLazy(this)(majorVersion.get >= 50)
   ...
   * methods.
   */
  def addLambdaDeserialize(classNode: ClassNode,
                           implMethods: Iterable[Handle]): Unit = {
    val cw = classNode
    ...
    def nextLabel(i: Int) =
      if (i == numGroups - 2) terminalLabel else initialLabels(i + 1)

    for ((label, i) <- initialLabels.iterator.zipWithIndex) {
      mv.visitTryCatchBlock(label,
                            nextLabel(i),
                            nextLabel(i),
  ...
  def isJavaBox(insn: MethodInsnNode): Boolean =
    calleeInMap(insn, javaBoxMethods)
  ...
  def isRefZero(insn: MethodInsnNode): Boolean =
    calleeInMap(insn, srRefZeroMethods)
  ...
  def isTraitSuperAccessor(method: MethodNode, owner: ClassBType): Boolean = {
    ...
    findSingleCall(
      method,
      mi =>
        mi.itf && mi.getOpcode == INVOKESPECIAL && mi.name + "$" == method.name).nonEmpty
  }
  ...
  def isMixinForwarder(method: MethodNode, owner: ClassBType): Boolean = {
    ...
    findSingleCall(
      method,
      mi =>
        mi.itf && mi.getOpcode == INVOKESTATIC && mi.name == method.name + "$").nonEmpty
  }

  def isTraitSuperAccessorOrMixinForwarder(method: MethodNode,
                                           owner: ClassBType): Boolean = {
  ...
  /**
    ...
    */
  def looksLikeForwarderOrFactoryOrTrivial(method: MethodNode,
                                           owner: InternalName,
                                           allowPrivateCalls: Boolean): Int = {
    val paramTypes = Type.getArgumentTypes(method.desc)
    val numPrimitives = paramTypes.count(_.getSort < Type.ARRAY) + (if (Type
                                                                          .getReturnType(
                                                                            method.desc)
                                                                          .getSort < Type.ARRAY)
                                                                      1
    ...
    if (method.instructions.iterator.asScala.count(_.getOpcode > 0) > maxSize)
      return -1
    ...
    while (it.hasNext && numCallsOrNew < 2) {
      ...
      if (t == AbstractInsnNode.METHOD_INSN) {
        ...
        if (!allowPrivateCalls && i.getOpcode == INVOKESPECIAL && mi.name != GenBCode.INSTANCE_CONSTRUCTOR_NAME) {
          ...
          if (isScalaBox(mi) || isScalaUnbox(mi) || isPredefAutoBox(mi) || isPredefAutoUnbox(
                mi) || isJavaBox(mi) || isJavaUnbox(mi))
            numBoxConv += 1
          else {
            numCallsOrNew += 1
            callMi = mi
        ...
        if (i.getOpcode == GETSTATIC) {
          if (!allowPrivateCalls && owner == i
                .asInstanceOf[FieldInsnNode]
                .owner)
   ...
   * can-multi-thread
   */
  final def addInnerClasses(jclass: asm.ClassVisitor,
                            refedInnerClasses: List[ClassBType]): Unit = {
  ...
  def onIndyLambdaImplMethodIfPresent[T](hostClass: InternalName)(
      action: mutable.Map[MethodNode,
                          mutable.Map[InvokeDynamicInsnNode, asm.Handle]] => T)
    : Option[T] =
  ...
  def onIndyLambdaImplMethod[T](hostClass: InternalName)(
      action: mutable.Map[MethodNode,
                          mutable.Map[InvokeDynamicInsnNode, asm.Handle]] => T)
    : T = {
    val methods =
      indyLambdaImplMethods.computeIfAbsent(hostClass, _ => mutable.Map.empty)
    methods.synchronized(action(methods))
  }

  def addIndyLambdaImplMethod(hostClass: InternalName,
                              method: MethodNode,
                              indy: InvokeDynamicInsnNode,
                              handle: asm.Handle): Unit = {
    onIndyLambdaImplMethod(hostClass)(
      _.getOrElseUpdate(method, mutable.Map.empty)(indy) = handle)
  }

  def removeIndyLambdaImplMethod(hostClass: InternalName,
                                 method: MethodNode,
                                 indy: InvokeDynamicInsnNode): Unit = {
    onIndyLambdaImplMethodIfPresent(hostClass)(
      _.get(method).foreach(_.remove(indy)))
  }
  ...
  /**
    ...
    */
  def indyLambdaBodyMethods(
      hostClass: InternalName): mutable.SortedSet[Handle] = {
    val res = mutable.TreeSet.empty[Handle](handleOrdering)
    onIndyLambdaImplMethodIfPresent(hostClass)(methods =>
      res addAll methods.valuesIterator.flatMap(_.valuesIterator))
    res
  }
  ...
  def indyLambdaBodyMethods(
      ...
      method: MethodNode): Map[InvokeDynamicInsnNode, Handle] = {
    onIndyLambdaImplMethodIfPresent(hostClass)(ms =>
      ms.getOrElse(method, Nil).toMap).getOrElse(Map.empty)
  }
}
...
object BackendUtils {
  ...
  def isMaxsComputed(method: MethodNode) =
    (method.access & ACC_MAXS_COMPUTED) != 0
  def setMaxsComputed(method: MethodNode) = method.access |= ACC_MAXS_COMPUTED
  def clearMaxsComputed(method: MethodNode) =
    method.access &= ~ACC_MAXS_COMPUTED
  ...
  private def isLabelFlagSet(l: LabelNode1, f: Int): Boolean =
    (l.flags & f) != 0
  ...
  def isLabelReachable(label: LabelNode) =
    isLabelFlagSet(label.asInstanceOf[LabelNode1], LABEL_REACHABLE_STATUS)
  def setLabelReachable(label: LabelNode) =
    setLabelFlag(label.asInstanceOf[LabelNode1], LABEL_REACHABLE_STATUS)
  def clearLabelReachable(label: LabelNode) =
    clearLabelFlag(label.asInstanceOf[LabelNode1], LABEL_REACHABLE_STATUS)

  final case class LambdaMetaFactoryCall(indy: InvokeDynamicInsnNode,
                                         samMethodType: Type,
                                         implMethod: Handle,
  ...
  object LambdaMetaFactoryCall {
    ...
    def unapply(insn: AbstractInsnNode)
      : Option[(InvokeDynamicInsnNode, Type, Handle, Type, Array[Type])] =
      insn match {
            ...
            case Array(samMethodType: Type,
              ...
              val isIndyLambda = (
                Type.getType(implMethod.getDesc) == expectedImplMethodType // (1)
                  && receiverType.forall(rt =>
                    implMethod.getOwner == rt.getInternalName) // (2)
  ...
  abstract class NestedClassesCollector[T] extends GenericSignatureVisitor {
    ...
    private def containsChar(s: String,
                             offset: Int,
                             length: Int,
                             char: Char): Boolean = {
    ...
    def visitInternalName(internalName: String,
                          offset: Int,
                          length: Int): Unit =
      if (internalName != null && containsChar(internalName,
                                               offset,
                                               length,
                                               '$')) {
  ...
  abstract class GenericSignatureVisitor {
    final def visitInternalName(internalName: String): Unit =
      visitInternalName(internalName,
                        0,
                        if (internalName eq null) 0 else internalName.length)
    ...
    private final class Parser(sig: String) {
      ...
      @inline def safely(f: => Unit): Unit =
        try f
        catch {
          case Aborted =>
          case NonFatal(e) =>
            raiseError(s"Exception thrown during signature parsing",
                       sig,
                       Some(e))
        }
      ...
      private def appendUntil(builder: java.lang.StringBuilder,
                              isDelimiter: CharBooleanFunction): Unit = {
        val start = index
  ...
  def classTagNewArrayArg(mi: MethodInsnNode,
                          prodCons: ProdConsAnalyzer): InternalName = {
    if (mi.name == "newArray" && mi.owner == "scala/reflect/ClassTag" && mi.desc == "(I)Ljava/lang/Object;") {
      val prods = prodCons.initialProducersForValueAt(
        mi,
        prodCons.frameAt(mi).stackTop - 1)
      if (prods.size == 1) prods.head match {
        case ctApply: MethodInsnNode =>
          if (ctApply.name == "apply" && ctApply.owner == "scala/reflect/ClassTag$" && ctApply.desc == "(Ljava/lang/Class;)Lscala/reflect/ClassTag;") {
            val clsProd = prodCons.initialProducersForValueAt(
              ctApply,
              prodCons.frameAt(ctApply).stackTop)
            if (clsProd.size == 1) clsProd.head match {
              case ldc: LdcInsnNode =>
                ldc.cst match {
                  case tp: Type
                      if tp.getSort == Type.OBJECT || tp.getSort == Type.ARRAY =>
  ...
  def isArrayGetLengthOnStaticallyKnownArray(
      mi: MethodInsnNode,
      typeAnalyzer: NonLubbingTypeFlowAnalyzer): Boolean = {
    isArrayGetLength(mi) && {
  ...
  def getClassOnStaticallyKnownPrimitiveArray(
      mi: MethodInsnNode,
      typeAnalyzer: NonLubbingTypeFlowAnalyzer): Type = {
  ...
  private val primitiveManifestApplies
    : Map[String, String] = primitiveTypes map {
  ...
  def isModuleLoad(insn: AbstractInsnNode,
                   nameMatches: InternalName => Boolean): Boolean = insn match {
    case fi: FieldInsnNode =>
