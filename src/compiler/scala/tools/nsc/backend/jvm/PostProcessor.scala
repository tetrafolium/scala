...
abstract class PostProcessor extends PerRunInit {
  ...
  import frontendAccess.{
    backendReporting,
    compilerSettings,
    recordPerRunJavaMapCache
  }

  val backendUtils: BackendUtils { val postProcessor: self.type } = new {
    val postProcessor: self.type = self
  } with BackendUtils
  val byteCodeRepository: ByteCodeRepository { val postProcessor: self.type } =
    new { val postProcessor: self.type = self } with ByteCodeRepository
  val localOpt: LocalOpt { val postProcessor: self.type } = new {
    val postProcessor: self.type = self
  } with LocalOpt
  val inliner: Inliner { val postProcessor: self.type } = new {
    val postProcessor: self.type = self
  } with Inliner
  val inlinerHeuristics: InlinerHeuristics { val postProcessor: self.type } =
    new { val postProcessor: self.type = self } with InlinerHeuristics
  val closureOptimizer: ClosureOptimizer { val postProcessor: self.type } =
    new { val postProcessor: self.type = self } with ClosureOptimizer
  val callGraph: CallGraph { val postProcessor: self.type } = new {
    val postProcessor: self.type = self
  } with CallGraph
  val bTypesFromClassfile
    : BTypesFromClassfile { val postProcessor: self.type } = new {
    val postProcessor: self.type = self
  } with BTypesFromClassfile
  val classfileWriters: ClassfileWriters { val postProcessor: self.type } =
    new { val postProcessor: self.type = self } with ClassfileWriters

  var classfileWriter: classfileWriters.ClassfileWriter = _

  // from lowercase to first-seen name and position thereof
  private val caseInsensitively = recordPerRunJavaMapCache(
    new ConcurrentHashMap[String, (String, Position)])
  ...
  def sendToDisk(clazz: GeneratedClass, paths: CompilationUnitPaths): Unit = {
    ...
    val bytes = try {
      if (!clazz.isArtifact) {
        ...
        val indyLambdaBodyMethods =
          backendUtils.indyLambdaBodyMethods(internalName)
      ...
      case ex: Throwable            =>
        ...
        backendReporting.error(
          NoPosition,
          s"Error while emitting $internalName\n${ex.getMessage}")
        null
    }

    if (bytes != null) {
      if (AsmUtils.traceSerializedClassEnabled && internalName.contains(
  ...
  private def warnCaseInsensitiveOverwrite(clazz: GeneratedClass): Unit = {
    ...
    val overwrites = caseInsensitively.putIfAbsent(lowercaseJavaClassName,
                                                   (name, clazz.position))
    if (overwrites ne null) {
      ...
      backendReporting.warning(
        clazz.position,
        sm"""Generated class ${nicify(name)} differs only in case from ${nicify(
          dupName)}$locationAddendum.
            |  Such classes will overwrite one another on case-insensitive filesystems."""
      )
    }
  }

  def runGlobalOptimizations(
      generatedUnits: Iterable[GeneratedCompilationUnit]): Unit = {
    ...
    if (compilerSettings.optAddToBytecodeRepository) {
      ...
      if (compilerSettings.optBuildCallGraph)
        for (u <- generatedUnits; c <- u.classes if !c.isArtifact) {
  ...
  def setInnerClasses(classNode: ClassNode): Unit = {
    ...
    backendUtils.addInnerClasses(classNode,
                                 backendUtils.collectNestedClasses(classNode))
  }
...
/**
  ...
  */
case class GeneratedClass(classNode: ClassNode,
                          sourceClassName: String,
                          position: Position,
                          isArtifact: Boolean)
case class GeneratedCompilationUnit(sourceFile: AbstractFile,
