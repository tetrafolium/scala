...
abstract class ClassfileWriters {
  ...
  sealed trait ClassfileWriter {
    ...
    /**
      * Write a classfile
      */
    def write(name: InternalName,
              bytes: Array[Byte],
              paths: CompilationUnitPaths): Unit
  ...
  object ClassfileWriter {
    ...
    def apply(global: Global): ClassfileWriter = {
      ...
      def jarManifestMainClass: Option[String] =
        settings.mainClass.valueSetByUser.orElse {
      ...
      def singleWriter(file: AbstractFile): UnderlyingClassfileWriter = {
        if (file hasExtension "jar") {
          new JarClassWriter(file,
                             jarManifestMainClass,
                             settings.YjarCompressionLevel.value)
      ...
      val basicClassWriter = settings.outputDirs.getSingleOutput match {
        ...
        case None =>
          val distinctOutputs: Set[AbstractFile] =
            settings.outputDirs.outputs.iterator.map(_._2).toSet
          if (distinctOutputs.size == 1) singleWriter(distinctOutputs.head)
          else
            new MultiClassWriter(distinctOutputs.iterator.map {
              output: AbstractFile =>
                output -> singleWriter(output)
            }.toMap)
      }

      val withAdditionalFormats =
        if (settings.Ygenasmp.valueSetByUser.isEmpty && settings.Ydumpclasses.valueSetByUser.isEmpty)
          basicClassWriter
        else {
          val asmp = settings.Ygenasmp.valueSetByUser map { dir: String =>
            new AsmClassWriter(getDirectory(dir))
          }
          val dump = settings.Ydumpclasses.valueSetByUser map { dir: String =>
            new DumpClassWriter(getDirectory(dir))
          }
          new AllClassWriter(basicClassWriter, asmp, dump)
        }

      val enableStats = statistics.enabled && settings.YaddBackendThreads.value == 1
      if (enableStats) new WithStatsWriter(withAdditionalFormats)
      else withAdditionalFormats
    }
    ...
    private final class JarClassWriter(file: AbstractFile,
                                       mainClass: Option[String],
                                       compressionLevel: Int)
        extends UnderlyingClassfileWriter {
      ...
      val jarWriter: JarOutputStream = {
        ...
        mainClass foreach { c =>
          manifest.getMainAttributes.put(Name.MAIN_CLASS, c)
        }
        val jar = new JarOutputStream(
          new BufferedOutputStream(new FileOutputStream(file.file), 64000),
          manifest)
      ...
      override def write(className: InternalName,
                         bytes: Array[Byte],
                         paths: CompilationUnitPaths): Unit =
        this.synchronized {
    ...
    private sealed class DirClassWriter extends UnderlyingClassfileWriter {
            ...
            case e: FileAlreadyExistsException =>
              ...
              if (!Files.isDirectory(parent))
                throw new FileConflictException(
                  s"Can't create directory $parent; there is an existing (non-directory) file in its path",
                  e)
          }
      ...
      protected def getPath(className: InternalName,
                            paths: CompilationUnitPaths) =
        paths.outputPath.resolve(className + ".class")
      ...
      private val fastOpenOptions =
        util.EnumSet.of(StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)
      private val fallbackOpenOptions = util.EnumSet.of(
        ...
        try {
          ...
          val os = if (isWindows) {
            try FileChannel.open(path, fastOpenOptions)
              ...
              case _: FileAlreadyExistsException =>
                FileChannel.open(path, fallbackOpenOptions)
            }
          ...
          case e: FileConflictException =>
            frontendAccess.backendReporting.error(
              NoPosition,
              s"error writing $className$qualifier: ${e.getMessage}")
          case e: java.nio.file.FileSystemException =>
            if (frontendAccess.compilerSettings.debug)
              e.printStackTrace()
            frontendAccess.backendReporting.error(
              NoPosition,
              s"error writing $className$qualifier: ${e.getClass.getName} ${e.getMessage}")
    ...
    private final class AsmClassWriter(asmOutputPath: Path)
        extends DirClassWriter {
      override protected def getPath(className: InternalName,
                                     paths: CompilationUnitPaths) =
        asmOutputPath.resolve(className + ".asmp")

      override protected def formatData(rawBytes: Array[Byte]) =
        AsmUtils
          .textify(AsmUtils.readClass(rawBytes))
          .getBytes(StandardCharsets.UTF_8)

      override protected def qualifier: String = " [for asmp]"
    }

    private final class DumpClassWriter(dumpOutputPath: Path)
        extends DirClassWriter {
      override protected def getPath(className: InternalName,
                                     paths: CompilationUnitPaths) =
        dumpOutputPath.resolve(className + ".class")

      override protected def qualifier: String = " [for dump]"
    }

    private final class VirtualClassWriter extends UnderlyingClassfileWriter {
      private def getFile(base: AbstractFile,
                          clsName: String,
      ...
      private def writeBytes(outFile: AbstractFile,
    ...
    private final class MultiClassWriter(
        underlying: Map[AbstractFile, UnderlyingClassfileWriter])
        extends ClassfileWriter {
      private def getUnderlying(paths: CompilationUnitPaths) =
        underlying.getOrElse(
          paths.outputDir, {
            throw new Exception(
              s"Cannot determine output directory for ${paths.sourceFile} with output ${paths.outputDir}. Configured outputs are ${underlying.keySet}")
          }
        )
    ...
    private final class AllClassWriter(basic: ClassfileWriter,
                                       asmp: Option[UnderlyingClassfileWriter],
                                       dump: Option[UnderlyingClassfileWriter])
        extends ClassfileWriter {
      override def write(className: InternalName,
                         bytes: Array[Byte],
                         paths: CompilationUnitPaths): Unit = {
  ...
  class FileConflictException(msg: String, cause: Throwable = null)
