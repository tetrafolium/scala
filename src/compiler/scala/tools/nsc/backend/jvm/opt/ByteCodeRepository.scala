...
abstract class ByteCodeRepository extends PerRunInit {
  ...
  /**
    ...
    */
  val compilingClasses: concurrent.Map[InternalName, (ClassNode, String)] =
  ...
  /**
    ...
    */
  val parsedClasses
    : mutable.Map[InternalName, Either[ClassNotFound, ClassNode]] =
    recordPerRunCache(
      LruMap[InternalName, Either[ClassNotFound, ClassNode]](maxCacheSize,
                                                             true))
  ...
  /**
    ...
    */
  private lazy val javaDefinedClasses =
    perRunLazy(this)(frontendAccess.javaDefinedClasses)

  def add(classNode: ClassNode, sourceFilePath: Option[String]) =
    sourceFilePath match {
      case Some(path) if path != "<no file>" =>
        compilingClasses(classNode.name) = (classNode, path)
      case _ => parsedClasses(classNode.name) = Right(classNode)
    }

  private def parsedClassNode(
  ...
  /**
    ...
    */
  def classNodeAndSourceFilePath(internalName: InternalName)
    : Either[ClassNotFound, (ClassNode, Option[String])] = {
  ...
  def methodNode(ownerInternalNameOrArrayDescriptor: String,
    ...
    def isSignaturePolymorphic(owner: InternalName) =
      owner == coreBTypes.jliMethodHandleRef.internalName && (name == "invoke" || name == "invokeExact")
    ...
    def findInSuperClasses(owner: ClassNode,
                           publicInstanceOnly: Boolean = false)
      : Either[ClassNotFound, Option[(MethodNode, InternalName)]] = {
      findMethod(owner) match {
        case Some(m)
            if !publicInstanceOnly || (isPublicMethod(m) && !isStaticMethod(
              m)) =>
          Right(Some((m, owner.name)))
        case _ =>
          if (isSignaturePolymorphic(owner.name))
            Right(
              Some(
                (owner.methods.asScala.find(_.name == name).get, owner.name)))
          else if (owner.superName == null) Right(None)
          else
            classNode(owner.superName).flatMap(
              findInSuperClasses(_, publicInstanceOnly = isInterface(owner)))
      }
    }

    def findInInterfaces(initialOwner: ClassNode)
      : Either[ClassNotFound, Option[(MethodNode, InternalName)]] = {
      ...
      def findIn(owner: ClassNode): Option[ClassNotFound] = {
        for (i <- owner.interfaces.asScala if !visited(i)) classNode(i) match {
          ...
          case Right(c) =>
            ...
            for (m <- findMethod(c)
                 if !isPrivateMethod(m) && !isStaticMethod(m)) found += ((m, c))
      ...
      val result =
        if (found.size <= 1) found.headOption
              ...
              isAbstractMethod(method) || {
                ...
                found exists {
                  case (other, otherOwner) =>
                    (other ne method) && {
                      val otherTp =
                        bTypesFromClassfile.classBTypeFromClassNode(otherOwner)
          ...
          else if (found.forall(p => isAbstractMethod(p._1)))
            found.headOption // (*)
          else None
        }
    ...
    if (ownerInternalNameOrArrayDescriptor.charAt(0) == '[') {
      ...
      def notFound(cnf: Option[ClassNotFound]) =
        Left(
          MethodNotFound(name,
                         descriptor,
                         ownerInternalNameOrArrayDescriptor,
  ...
  private def removeLineNumbersAndAddLMFImplMethods(
          ...
          case AbstractInsnNode.INVOKE_DYNAMIC_INSN =>
            insn match {
              case LambdaMetaFactoryCall(indy, _, implMethod, _, _) =>
                postProcessor.backendUtils.addIndyLambdaImplMethod(
                  classNode.name,
                  m,
                  indy,
                  implMethod)
  ...
  private def parseClass(
    ...
    backendClassPath.findClassFile(fullName) map { classFile =>
      ...
      case None =>
        Left(ClassNotFound(internalName, javaDefinedClasses.get(internalName)))
    }
  }
}
