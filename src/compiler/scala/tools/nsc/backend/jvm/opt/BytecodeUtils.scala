...
object BytecodeUtils {
  ...
  def isLoadOrStore(instruction: AbstractInsnNode): Boolean =
    isLoad(instruction) || isStore(instruction)
  ...
  def isSynchronizedMethod(methodNode: MethodNode): Boolean =
    (methodNode.access & ACC_SYNCHRONIZED) != 0

  def isNativeMethod(methodNode: MethodNode): Boolean =
    (methodNode.access & ACC_NATIVE) != 0

  def isSyntheticMethod(methodNode: MethodNode): Boolean =
  ...
  def isFinalMethod(methodNode: MethodNode): Boolean =
    (methodNode.access & (ACC_FINAL | ACC_PRIVATE | ACC_STATIC)) != 0

  def isStrictfpMethod(methodNode: MethodNode): Boolean =
    (methodNode.access & ACC_STRICT) != 0
  ...
  @tailrec def nextExecutableInstruction(insn: AbstractInsnNode,
  ...
  def findSingleCall(
      method: MethodNode,
      such: MethodInsnNode => Boolean): Option[MethodInsnNode] = {
    @tailrec def noMoreInvoke(insn: AbstractInsnNode): Boolean = {
      insn == null || (!insn.isInstanceOf[MethodInsnNode] && noMoreInvoke(
        insn.getNext))
    }
  ...
  def removeJumpAndAdjustStack(method: MethodNode, jump: JumpInsnNode): Unit = {
    ...
    if ((op >= IFEQ && op <= IFLE) || op == IFNULL || op == IFNONNULL) {
      ...
      assert(
        jump.getOpcode == GOTO,
        s"Cannot remove JSR instruction in ${method.name} (at ${method.instructions
          .indexOf(jump)}")
    }
  ...
  def finalJumpTarget(source: JumpInsnNode): LabelNode = {
    @tailrec def followGoto(label: LabelNode,
  ...
  def callsiteTooLargeAfterInlining(caller: MethodNode,
    ...
    def roughUpperBound(methodNode: MethodNode): Int =
      methodNode.instructions.size * 8
  ...
  def cloneLocalVariableNodes(
      ...
      localIndexMap: Int => Int): List[LocalVariableNode] = {
      ...
      if (newIdx >= 0) {
        val name =
          if (calleeMethodName.length + localVariable.name.length < BTypes.InlinedLocalVariablePrefixMaxLength) {
            ...
            val (methNames, varName) =
              (calleeMethodName +: parts.init, parts.last)
            ...
            usedMethNames.foldLeft("")((res, methName) =>
              res + methName.take(charsPerMethod) + "_") + varName
          }
        res += new LocalVariableNode(name,
                                     localVariable.desc,
  ...
  implicit class FrameExtensions[V <: Value](val frame: Frame[V])
      extends AnyVal {
