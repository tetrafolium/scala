...
abstract class BoxUnbox {
  ...
  def boxUnboxElimination(method: MethodNode, owner: InternalName): Boolean = {
    AsmAnalyzer.sizeOKForSourceValue(method) && {
      ...
      def replaceBoxOperationsSingleCreation(creation: BoxCreation,
        ...
        if (canRewrite) {
          val localSlots = Vector.from[(Int, Type)](
            boxKind.boxedTypes.iterator.map(tp => (getLocal(tp.getSize), tp)))

          // store boxed value(s) into localSlots
          val storeOps = localSlots.reverseIterator map {
            case (slot, tp) =>
          ...
          if (keepBox) {
            val loadOps = List.from[VarInsnNode](localSlots.iterator.map({
              case (slot, tp) =>
          ...
          finalCons foreach {
            case write: StaticSetterOrInstanceWrite =>
              assert(
                !keepBox,
                s"cannot eliminate box write if the box remains (and escapes): $write")
          ...
          if (boxKind.boxedTypes.lengthCompare(1) == 0) {
            ...
            for (extraction <- allConsumers) {
              ...
              val replacementOps = if (valueIndex == 0) {
                ...
                val consumeStack =
                  boxKind.boxedTypes.zipWithIndex.reverseIterator
                    .map {
                      case (tp, i) =>
                        if (i == valueIndex) {
  ...
  /**
    ...
    */
  def checkCopyOpReplacements(initialProds: Set[BoxCreation],
                              finalCons: Set[BoxConsumer],
    ...
    while (replaceOK && copyOps.hasNext) copyOps.next() match {
      case vi: VarInsnNode =>
        ...
        replacements += vi -> (if (isLoad) locs.map(typedVarOp)
                               else locs.map(typedVarOp).reverse)
  ...
  class CopyOpsIterator(initialCreations: Set[BoxCreation],
    ...
    private val queue = mutable.Queue
      .empty[AbstractInsnNode] ++= initialCreations.iterator.flatMap(
        ...
        if (prodCons.producersForInputsOf(dupOp) == Set(newOp)) {
          ...
          val initCalls = dupCons collect {
            case mi: MethodInsnNode
                if mi.name == GenBCode.INSTANCE_CONSTRUCTOR_NAME && mi.owner == newOp.desc =>
              mi
          }
          if (initCalls.size == 1) {
            ...
            val receiverProds = prodCons.producersForValueAt(
              initCall,
              prodCons.frameAt(initCall).stackTop - numArgs)
  ...
  case class PrimitiveBox(boxedType: Type, boxClass: InternalName)
    ...
    def checkBoxCreation(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxCreation] =
      checkPrimitiveBox(insn, Some(this), prodCons).map(_._1)
    def checkBoxConsumer(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxConsumer] =
      checkPrimitiveUnbox(insn, this, prodCons)
  ...
  object PrimitiveBox {
    ...
    private val primBoxSupertypes: Map[InternalName, Set[InternalName]] = {
      ...
      def transitiveSupertypes(clsbt: ClassBType): Set[ClassBType] =
        (clsbt.info.get.superClass ++ clsbt.info.get.interfaces)
          .flatMap(transitiveSupertypes)
          .toSet + clsbt
    ...
    def checkPrimitiveUnbox(insn: AbstractInsnNode,
                            kind: PrimitiveBox,
                            prodCons: ProdConsAnalyzer): Option[BoxConsumer] = {
  ...
  case class Ref(boxedType: Type, refClass: InternalName) extends BoxKind {
    ...
    def checkBoxCreation(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxCreation] =
      checkRefCreation(insn, Some(this), prodCons).map(_._1)
    def checkBoxConsumer(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxConsumer] =
      checkRefConsumer(insn, this, prodCons)
    def boxedTypes: List[Type] = List(boxedType)
    def extractedValueIndex(extraction: BoxConsumer): Int = 0
    def isMutable = true
  }
  ...
  object Ref {
    ...
    private val refSupertypes = {
      import postProcessor.bTypes._
      Set(coreBTypes.jiSerializableRef, coreBTypes.ObjectRef)
        .map(_.internalName)
    }
    ...
    def checkRefConsumer(insn: AbstractInsnNode,
                         kind: Ref,
                         prodCons: ProdConsAnalyzer): Option[BoxConsumer] =
      insn match {
        ...
        case ti: TypeInsnNode if ti.getOpcode == INSTANCEOF =>
          Some(
            BoxedPrimitiveTypeCheck(
              ti,
              ti.desc == kind.refClass || refSupertypes.contains(ti.desc)))
  ...
  case class Tuple(boxedTypes: List[Type], tupleClass: InternalName)
    ...
    def checkBoxCreation(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxCreation] =
      checkTupleCreation(insn, Some(this), prodCons).map(_._1)
    def checkBoxConsumer(insn: AbstractInsnNode,
                         prodCons: ProdConsAnalyzer): Option[BoxConsumer] =
  ...
  object Tuple {
    ...
    private def isSpecializedTupleClass(tupleClass: InternalName) =
      specializedTupleClassR.pattern.matcher(tupleClass).matches

    private val specializedTupleGetterR = "_[12]\\$mc[IJDCZ]\\$sp".r
    private def isSpecializedTupleGetter(mi: MethodInsnNode) =
      specializedTupleGetterR.pattern.matcher(mi.name).matches

    private val tupleGetterR = "_\\d\\d?".r
    private def isTupleGetter(mi: MethodInsnNode) =
      tupleGetterR.pattern.matcher(mi.name).matches
  ...
  case class StaticFactory(producer: MethodInsnNode,
      ...
      extends BoxCreation
  case class ModuleFactory(moduleLoad: AbstractInsnNode,
                           producer: MethodInsnNode)
      extends BoxCreation {
    val loadInitialValues: Option[List[AbstractInsnNode]] = None
  }
  case class InstanceCreation(newOp: TypeInsnNode,
                              dupOp: InsnNode,
                              initCall: MethodInsnNode)
      extends BoxCreation {
    def producer = newOp
    val loadInitialValues: Option[List[AbstractInsnNode]] = None
  }
  ...
  case class PrimitiveUnboxingGetter(consumer: MethodInsnNode,
                                     unboxedPrimitive: Type)
      extends BoxConsumer

  /** An extractor method in a Scala module, e.g., `Predef.Integer2int` */
  case class ModuleGetter(moduleLoad: AbstractInsnNode,
                          consumer: MethodInsnNode)
      extends BoxConsumer

  /** PUTFIELD or setter invocation */
  case class StaticSetterOrInstanceWrite(consumer: AbstractInsnNode)
      extends BoxConsumer

  /** `.$isInstanceOf[T]` (can be statically proven true or false) */
  case class BoxedPrimitiveTypeCheck(consumer: AbstractInsnNode,
                                     success: Boolean)
      extends BoxConsumer
