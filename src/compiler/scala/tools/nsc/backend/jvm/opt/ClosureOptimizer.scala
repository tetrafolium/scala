...
import scala.tools.nsc.backend.jvm.analysis.{
  AsmAnalyzer,
  BackendUtils,
  ProdConsAnalyzer
}
...
abstract class ClosureOptimizer {
  ...
  /**
    ...
    */
  def rewriteClosureApplyInvocations(
      methods: Option[Iterable[MethodNode]],
      inlinerState: mutable.Map[MethodNode, inliner.MethodInlinerState])
    : mutable.LinkedHashSet[MethodNode] = {
    ...
    val methodsToRewrite =
      methods.getOrElse(closureInstantiations.keysIterator.toList)
    ...
    for (method <- methodsToRewrite if AsmAnalyzer.sizeOKForBasicValue(method))
    ...
    for ((closureInit, invocations) <- toRewrite) {
      ...
      for ((invocation, stackHeight) <- invocations)
        rewriteClosureApplyInvocation(closureInit,
                                      invocation,
                                      stackHeight,
      ...
      if (closureInit.ownerMethod != previousMethod) {
        ...
        val state = inlinerState.getOrElseUpdate(previousMethod,
                                                 new inliner.MethodInlinerState)
        state.inlineLog.logClosureRewrite(
          closureInit,
          invocations,
          invocations.headOption.flatMap(p => state.outerCallsite(p._1)))
      }
    }
  ...
  private def localsForClosureRewrite(
    ...
    val argTypes =
      closureInit.lambdaMetaFactoryCall.samMethodType.getArgumentTypes
        ...
        case invocation: MethodInsnNode
          ...
          val bodyAccessible: Either[OptimizerWarning, Boolean] = for {
            (bodyMethodNode, declClass) <- byteCodeRepository.methodNode(
              ...
              lambdaBodyHandle.getDesc): Either[OptimizerWarning,
  ...
  private def isSamInvocation(invocation: MethodInsnNode,
    ...
    if (invocation.getOpcode == INVOKESTATIC) false
      ...
      def specializedDescMatches(specMethodDesc: String,
  ...
  private def rewriteClosureApplyInvocation(
    ...
    val invocationStackHeight = stackHeight + numCapturedValues - 1 + (if (isNew)
                                                                         2
                                                                       else
                                                                         0) // -1 because the closure is gone
    ...
    if (!isNew) {
      ...
      if (isPrimitiveType(invocationReturnType) && bodyReturnType.getDescriptor == ObjectRef.descriptor) {
        ...
        fixLoadedNothingOrNullValue(bodyReturnType,
                                    bodyInvocation,
                                    ownerMethod,
                                    bTypes)
      }
    ...
    val bodyMethod = byteCodeRepository.methodNode(lambdaBodyHandle.getOwner,
                                                   lambdaBodyHandle.getName,
                                                   lambdaBodyHandle.getDesc)
    val sourceFilePath = byteCodeRepository.compilingClasses
      .get(lambdaBodyHandle.getOwner)
      .map(_._2)
    val callee = bodyMethod.map({
      case (bodyMethodNode, bodyMethodDeclClass) =>
        ...
        Callee(
          ...
          samParamTypes =
            callGraph.samParamTypes(bodyMethodNode,
                                    Type.getArgumentTypes(bodyMethodNode.desc),
                                    bodyDeclClassType),
