...
class DefaultScriptRunner(settings: GenericRunnerSettings)
...
abstract class AbstractScriptRunner(settings: GenericRunnerSettings)
  ...
  /** Compile a script and then run the specified closure with
    ...
    */
  private def withCompiledScript(scriptFile: String)(
    ...
    def hasClassToRun(d: Directory): Boolean =
      DirectoryClassPath(d.jfile).findClass(mainClass).isDefined
  ...
  /** Run a script after it has been compiled. Prints any exceptions.
    ...
    */
  private def runCompiled(compiledLocation: String,
  ...
  final def runScript(scriptFile: String,
                      scriptArgs: List[String]): Option[Throwable] = {
    ...
    if (!f.exists) Some(new IOException(s"no such file: $scriptFile"))
      ...
      Some(new IOException(s"can't compile a directory: $scriptFile"))
    else if (!settings.nc && !f.isFile)
      Some(
        new IOException(s"compile server requires a regular file: $scriptFile"))
    else {
  ...
  final def runScriptText(command: String,
                          scriptArgs: List[String]): Option[Throwable] = {
