  ...
  def apply(settings: Settings): Profiler =
    if (!settings.YprofileEnabled) NoOpProfiler
      ...
      val reporter = settings.YprofileDestination.value match {
        case _ if !settings.YprofileDestination.isSetByUser =>
          NoOpProfileReporter
        case "-" => ConsoleProfileReporter
        case path =>
          new StreamProfileReporter(new PrintWriter(new FileWriter(path, true)))
      }
...
case class GcEventData(pool: String,
                       reportTimeNs: Long,
                       gcStartMillis: Long,
                       gcEndMillis: Long,
                       durationMillis: Long,
...
case class ProfileSnap(threadId: Long,
                       threadName: String,
                       snapTimeNanos: Long,
                       idleTimeNanos: Long,
                       cpuTimeNanos: Long,
                       ...
                       allocatedBytes: Long,
                       heapBytes: Long,
                       totalClassesLoaded: Long,
                       totalJITCompilationTime: Long) {
  def updateHeap(heapBytes: Long) = {
    copy(heapBytes = heapBytes)
  }
}
case class ProfileRange(start: ProfileSnap,
                        end: ProfileSnap,
                        phase: Phase,
                        purpose: String,
                        taskCount: Int,
...
sealed abstract class Profiler {
  ...
  def beforeCompletion(root: Global#Symbol,
...
private[profile] object RealProfiler {
  ...
  lazy val allPlugins =
    ServiceLoader.load(classOf[ProfilerPlugin]).iterator.asScala.toList
...
private[profile] class RealProfiler(reporter: ProfileReporter,
  ...
  def outDir =
    settings.outputDirs.getSingleOutput
      .getOrElse(settings.outputDirs.outputs.head._2.file)
      .toString
  ...
  override def finished(): Unit = {
    ...
    if (chromeTrace != null) {
      for (gcEvent <- gcEvents) {
        val durationNanos =
          TimeUnit.MILLISECONDS.toNanos(gcEvent.durationMillis)
        val startNanos = gcEvent.endNanos - durationNanos
        chromeTrace.traceDurationEvent(gcEvent.name,
                                       startNanos,
                                       durationNanos,
                                       GcThreadId)
      }
  ...
  override def beforePhase(phase: Phase): ProfileSnap = {
    ...
    if (chromeTrace != null)
      chromeTrace.traceDurationEventStart(Category.Phase, phase.name)
  ...
  override def afterPhase(phase: Phase, snapBefore: ProfileSnap): Unit = {
    ...
    if (chromeTrace != null)
      chromeTrace.traceDurationEventEnd(Category.Phase, phase.name)
  ...
  override def beforeUnit(phase: Phase, file: AbstractFile): Unit = {
    assert(mainThread eq Thread.currentThread())
    if (chromeTrace != null)
      chromeTrace.traceDurationEventStart(Category.File, file.name)
  }
  ...
  override def afterUnit(phase: Phase, file: AbstractFile): Unit = {
    ...
    if (chromeTrace != null) {
      ...
      if (now > nextAfterUnitSnap) {
        val initialSnap = RealProfiler.snapThread(0)
        chromeTrace.traceCounterEvent("allocBytes",
                                      "allocBytes",
                                      initialSnap.allocatedBytes,
                                      processWide = false)
        chromeTrace.traceCounterEvent("heapBytes",
                                      "heapBytes",
                                      initialSnap.heapBytes,
                                      processWide = true)
        chromeTrace.traceCounterEvent("classesLoaded",
                                      "classesLoaded",
                                      initialSnap.totalClassesLoaded,
                                      processWide = true)
        chromeTrace.traceCounterEvent("jitCompilationTime",
                                      "jitCompilationTime",
                                      initialSnap.totalJITCompilationTime,
                                      processWide = true)
        chromeTrace.traceCounterEvent("userTime",
                                      "userTime",
                                      initialSnap.userTimeNanos,
                                      processWide = false)
        chromeTrace.traceCounterEvent("cpuTime",
                                      "cpuTime",
                                      initialSnap.cpuTimeNanos,
                                      processWide = false)
        chromeTrace.traceCounterEvent("idleTime",
                                      "idleTime",
                                      initialSnap.idleTimeNanos,
                                      processWide = false)
  ...
  override def beforeTypedImplDef(sym: Global#Symbol): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventStart(Category.TypeCheck,
                                          sym.rawname.toString)
  }
  override def afterTypedImplDef(sym: Global#Symbol): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventEnd(Category.TypeCheck,
                                        sym.rawname.toString)
  }

  override def beforeImplicitSearch(pt: Global#Type): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventStart(Category.Implicit,
                                          "?[" + pt.typeSymbol.rawname + "]",
                                          colour = "yellow")
  }

  override def afterImplicitSearch(pt: Global#Type): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventEnd(Category.Implicit,
                                        "?[" + pt.typeSymbol.rawname + "]",
                                        colour = "yellow")
  }
  ...
  override def beforeMacroExpansion(macroSym: Global#Symbol): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventStart(Category.Macro,
                                          "«" + macroSym.rawname + "»",
                                          colour = "olive")
  }
  ...
  override def afterMacroExpansion(macroSym: Global#Symbol): Unit = {
    if (chromeTrace != null)
      chromeTrace.traceDurationEventEnd(Category.Macro,
                                        "«" + macroSym.rawname + "»",
                                        colour = "olive")
  }

  override def beforeCompletion(root: Global#Symbol,
                                associatedFile: AbstractFile): Unit = {
    if (chromeTrace != null) {
      chromeTrace.traceDurationEventStart(Category.Completion,
                                          "↯",
                                          colour = "thread_state_sleeping")
      chromeTrace.traceDurationEventStart(Category.File, associatedFile.name)
      chromeTrace.traceDurationEventStart(Category.Completion,
                                          completionName(root, associatedFile))
    }
  }

  override def afterCompletion(root: Global#Symbol,
                               associatedFile: AbstractFile): Unit = {
    if (chromeTrace != null) {
      chromeTrace.traceDurationEventEnd(Category.Completion,
                                        completionName(root, associatedFile))
  ...
  private def completionName(root: Global#Symbol,
                             associatedFile: AbstractFile): String = {
...
object ConsoleProfileReporter extends ProfileReporter {
  ...
  override def reportBackground(profiler: RealProfiler,
                                threadRange: ProfileRange): Unit =
    delegate.reportBackground(profiler, threadRange)
  override def reportForeground(profiler: RealProfiler,
                                threadRange: ProfileRange): Unit =
    delegate.reportForeground(profiler, threadRange)
...
class StreamProfileReporter(out: PrintWriter) extends ProfileReporter {
  ...
  private def reportCommon(tpe: EventType.value,
