...
trait StructuredTypeStrings extends DestructureTypes {
  ...
  sealed abstract class TypeNode {
    ...
    def show(indent: Int, showLabel: Boolean): String =
      maybeBlock(indent, grouping)(mkPrefix(showLabel), nodes)
  ...
  class MonoFunction(in: TypeNode, out: TypeNode)
      extends TypeProduct(List(in, out)) {
    override def typeName = "MethodType"
  }
  class PolyFunction(in: TypeNode, out: TypeNode)
      extends TypeProduct(List(in, out)) {
