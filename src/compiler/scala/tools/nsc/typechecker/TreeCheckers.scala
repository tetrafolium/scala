...
abstract class TreeCheckers extends Analyzer {
  ...
  override protected def onTreeCheckerError(pos: Position,
                                            msg: String): Unit = {
  ...
  private def clean_s(s: String) =
    s.replaceAllLiterally("scala.collection.", "s.c.")
  private def typestr(x: Type) = " (tpe = " + x + ")"
  private def treestr(t: Tree) =
    t + " [" + classString(t) + "]" + typestr(t.tpe)
  ...
  private def nonPackageOwners(s: Symbol) =
    s.ownerChain drop 1 takeWhile (!_.hasPackageFlag)
  private def nonPackageOwnersPlusOne(s: Symbol) =
    nonPackageOwners(s) ::: (s.ownerChain dropWhile (!_.hasPackageFlag) take 1)
  ...
  object SymbolTracker extends Traverser {
    ...
    def symbolTreeMap[T <: Tree]() =
      mutable.Map[Symbol, List[T]]() withDefaultValue Nil
  ...
  class TreeChecker(context0: Context) extends Typer(context0) {
    ...
    object precheck extends TreeStackTraverser {
      ...
      private def traverseInternal(tree: Tree): Unit = {
        ...
        def fail(msg: String) =
          errorFn(tree.pos, msg + tree.shortClass + " / " + tree)
      ...
      private def checkSymbolRefsRespectScope(
        ...
        def encls =
          enclosingMemberDefs.filterNot(_.symbol == treeSym).zipWithIndex map {
            case (md, i) => mk(s"encl(${i + 1})", md.symbol, signature)
          }
        ...
        def mkErrorMsg(outOfScope: Symbol): String = {
          ...
          val lines = pairs map {
            case (s: Symbol, msg) =>
              fmt.format(msg) + "  in  " + ownersString(s)
