...
trait Implicits {
  ...
  // standard usage
  def inferImplicitFor(pt: Type,
                       tree: Tree,
                       context: Context,
                       reportAmbiguous: Boolean = true): SearchResult =
    inferImplicit(tree,
                  pt,
                  reportAmbiguous,
                  isView = false,
  ...
  def inferImplicitView(from: Type,
                        to: Type,
                        tree: Tree,
                        context: Context,
  ...
  def inferImplicitByType(pt: Type,
                          context: Context,
                          pos: Position = NoPosition): SearchResult =
    inferImplicit(EmptyTree,
                  pt,
  ...
  def inferImplicitByTypeSilent(pt: Type,
                                context: Context,
                                pos: Position = NoPosition): SearchResult =
    inferImplicit(EmptyTree,
                  pt,
  ...
  def inferImplicit(tree: Tree,
                    ...
                    isView: Boolean,
                    context: Context,
                    saveAmbiguousDivergent: Boolean): SearchResult =
    inferImplicit(tree,
                  pt,
                  reportAmbiguous,
                  isView,
  ...
  private def inferImplicit1(tree: Tree,
                             pt: Type,
                             reportAmbiguous: Boolean,
                             isView: Boolean,
                             context: Context,
    ...
    val search = new ImplicitSearch(tree,
                                    dpt,
                                    isView,
    ...
    if (result.isSuccess && settings.warnSelfImplicit && result.tree.symbol != null) {
      ...
      if (context.owner.hasTransOwner(s))
        context.warning(result.tree.pos,
                        s"Implicit resolves to enclosing ${result.tree.symbol}")
    }
  ...
  private def depoly(tp: Type): Type = tp match {
    case PolyType(tparams, restpe) =>
      deriveTypeWithWildcards(tparams)(ApproximateDependentMap(restpe))
  ...
  /** The result of an implicit search
    ...
    */
  class SearchResult(val tree: Tree,
  ...
  lazy val SearchFailure =
    new SearchResult(EmptyTree, EmptyTreeTypeSubstituter, Nil) {
      override def isFailure = true
    }

  lazy val DivergentSearchFailure =
    new SearchResult(EmptyTree, EmptyTreeTypeSubstituter, Nil) {
      override def isFailure = true
      override def isDivergent = true
    }

  lazy val AmbiguousSearchFailure =
    new SearchResult(EmptyTree, EmptyTreeTypeSubstituter, Nil) {
      override def isFailure = true
  ...
  object OpenImplicit {
    def apply(info: ImplicitInfo,
              pt: Type,
              tree: Tree,
              isView: Boolean,
              isByName: Boolean): OpenImplicit = {
  ...
  def SearchedPrefixImplicitInfo(pre: Type) =
    new ImplicitInfo(null, pre, NoSymbol)
  ...
  /** A class that sets up an implicit search. For more info, see comments for `inferImplicit`.
    ...
    *                          If it's set to NoPosition, then position-based services will use `tree.pos`
    */
  class ImplicitSearch(val tree: Tree,
                       val pt: Type,
                       val isView: Boolean,
                       val context0: Context,
                       val pos0: Position = NoPosition,
                       val isByNamePt: Boolean = false)
    ...
    final def functionArity(tp: Type): Int =
      if (tp eq pt) cachedPtFunctionArity else functionArityOf(tp)
    ...
    @inline final def failure(what: Any,
                              reason: => String,
                              pos: Position = this.pos): SearchResult = {
      if (settings.XlogImplicits)
        reporter.echo(
          pos,
          what + " is not a valid implicit value for " + pt + " because:\n" + reason)
      SearchFailure
    }
      ...
      (info1 != NoImplicitInfo) && {
        if (info1.sym.isStatic && info2.sym.isStatic) {
          improvesCache get ((info1, info2)) match {
            ...
            case None =>
              val result = isStrictlyMoreSpecific(info1.tpe,
                                                  info2.tpe,
                                                  info1.sym,
                                                  info2.sym)
    ...
    def isPlausiblyCompatible(tp: Type, pt: Type) =
      checkCompatibility(fast = true, tp, pt)
    def normSubType(tp: Type, pt: Type) =
      checkCompatibility(fast = false, tp, pt)
    ...
    private def core(tp: Type): Type = tp.dealiasWiden match {
      ...
      case ExistentialType(tparams, result) =>
        core(result).subst(tparams, tparams map (t => core(t.info.upperBound)))
    ...
    private def allSymbols(tp: Type): Set[Symbol] = {
      ...
      def loop(tps: List[Type], acc: Set[Symbol]): Set[Symbol] = tps match {
        ...
        case hd :: tl =>
          ...
          hd.dealias match {
            case SingleType(pre, sym) =>
              loop(pre :: hd.dealiasWiden :: tl, hdSym(acc + sym))
            case ThisType(sym) => loop(tl, hdSym(acc + sym))
            case TypeRef(pre, sym, args) =>
              loop(pre :: args ++ tl, hdSym(acc + sym))
    ...
    /** Try to construct a typed tree from given implicit info with given
      ...
      */
    private def typedImplicit(info: ImplicitInfo,
                              ptChecked: Boolean,
            ...
            ois match {
              ...
              case (hd @ OpenImplicit(info1, tp, tree1)) :: tl =>
                (if (!info1.sym.isMacro && tree1.symbol == tree.symbol) {
                   if (belowByName && (tp =:= pt))
                     Some(false) // if there is a byname argument between tp and pt we can tie the knot
      ...
      if (existsDominatedImplicit) {
        ...
        if (ref != EmptyTree)
          ...
          @tailrec
          def loop(ois: List[OpenImplicit],
                   isByName: Boolean): Option[OpenImplicit] =
            ois match {
              case hd :: tl if (isByName || hd.isByName) && hd.pt <:< pt =>
                Some(hd)
          ...
          val recursiveImplicit: Option[OpenImplicit] =
            loop(context.openImplicits, isByNamePt)
          ...
          recursiveImplicit match {
            ...
            case None =>
              try {
                context.openImplicits = OpenImplicit(
                  info,
                  pt,
                  tree,
                  isView,
                  isByNamePt) :: context.openImplicits
    ...
    private def matchesPtView(tp: Type,
                              ptarg: Type,
                              ptres: Type,
                              undet: List[Symbol]): Boolean = tp match {
      case MethodType(p :: _, restpe) if p.isImplicit =>
        matchesPtView(restpe, ptarg, ptres, undet)
      case MethodType(p :: Nil, restpe) =>
        matchesArgRes(p.tpe, restpe, ptarg, ptres, undet)
      case ExistentialType(_, qtpe) =>
        matchesPtView(methodToExpressionTp(qtpe), ptarg, ptres, undet)
      case Function1(arg1, res1) =>
        matchesArgRes(arg1, res1, ptarg, ptres, undet)
      case _ => false
    }

    private def matchesArgRes(tparg: Type,
                              tpres: Type,
                              ptarg: Type,
                              ptres: Type,
    ...
    private def matchesPtInst(info: ImplicitInfo): Boolean = {
      if (StatisticsStatics.areSomeColdStatsEnabled)
        statistics.incCounter(matchesPtInstCalls)
      info.tpe match {
        case PolyType(tparams, restpe) =>
          try {
            ...
            if (!matchesPt(tpInstantiated, wildPt, allUndetparams)) {
              if (StatisticsStatics.areSomeColdStatsEnabled)
                statistics.incCounter(matchesPtInstMismatch1)
              false
            } else {
              ...
              val targs = solvedTypes(tvars,
                                      allUndetparams,
                                      allUndetparams map varianceInType(wildPt),
                                      upper = false,
                                      lubDepth(tpInstantiated :: wildPt :: Nil))
              val AdjustedTypeArgs(okParams, okArgs) =
                adjustTypeArgs(allUndetparams, tvars, targs)
              val remainingUndet = allUndetparams diff okParams
              val tpSubst = deriveTypeWithWildcards(remainingUndet)(
                tp.instantiateTypeParams(okParams, okArgs))
              if (!matchesPt(tpSubst, wildPt, remainingUndet)) {
                if (StatisticsStatics.areSomeColdStatsEnabled)
                  statistics.incCounter(matchesPtInstMismatch2)
                false
              } else true
            }
          } catch {
    ...
    /** Capturing the overlap between isPlausiblyCompatible and normSubType.
      ...
      */
    private def checkCompatibility(fast: Boolean,
                                   tp0: Type,
                                   pt0: Type): Boolean = {
      @tailrec def loop(tp: Type, pt: Type): Boolean = tp match {
        ...
        case ExistentialType(_, qtpe) =>
          if (fast) loop(qtpe, pt) else methodToExpressionTp(tp) <:< pt // is !fast case needed??
          ...
          val tp2Wide =
            tp2.dealiasWiden.upperBound match {
              case et: ExistentialType =>
                et.underlying // OPT meant as cheap approximation of skolemizeExistential?
              case tp => tp
            }
          tp2Wide match {
            case TypeRef(_, sym2, args2) if sym2 ne SingletonClass =>
              ...
              if ((sym1 eq sym2) && (args1 ne Nil))
                corresponds3(sym1.typeParams, args1, args2)(isSubArg)
              else
            ...
            case RefinedType(parents, decls) =>
              ...
              decls.isEmpty || {
                ...
                tr1.baseClasses.exists(
                  _.info.decls.lookupEntry(firstDeclName) != null)
              }
    ...
    private def typedImplicit1(info: ImplicitInfo,
                               isLocalToCallsite: Boolean): SearchResult = {
      if (StatisticsStatics.areSomeColdStatsEnabled)
      ...
      @inline def fail(reason: => String): SearchResult =
        failure(itree0, reason)
              ...
              case Function1(arg1, arg2) =>
                typed1(
                  atPos(itree0.pos)(
                    Apply(
                      itree1,
                      Ident(nme.argument).setType(approximate(arg1)) :: Nil)),
                  EXPRmode,
                  approximate(arg2)
                ) match {
        ...
        if (context.reporter.hasErrors)
          ...
          if (matchesPt(itree3.tpe, ptInstantiated, undetParams)) {
            if (tvars.nonEmpty)
              typingLog(
                "solve",
                ptLine("tvars" -> tvars, "tvars.constr" -> tvars.map(_.constr)))
            ...
            itree3.attachments.get[MacroExpansionAttachment] match {
              case Some(
                  MacroExpansionAttachment(exp @ TypeApply(fun, targs), _)) =>
                checkBounds(exp,
                            NoPrefix,
                            NoSymbol,
                            fun.symbol.typeParams,
                            targs.map(_.tpe),
                            "inferred ")
              case _ => ()
            }
            ...
            context.reporter.firstError match {
              case Some(err) =>
                return fail(
                  "type parameters weren't correctly instantiated outside of the implicit tree: " + err.errMsg)
              case None =>
            }
            ...
            itree3 match {
              case TypeApply(fun, args) =>
                typedTypeApply(itree3, EXPRmode, fun, args)
              case Apply(TypeApply(fun, args), _) =>
                typedTypeApply(itree3, EXPRmode, fun, args) // t2421c
              case t => t
            }
            ...
            context.reporter.firstError match {
              ...
              case None =>
                val result = new SearchResult(unsuppressMacroExpansion(itree3),
                                              subst,
                                              context.undetparams)
                if (StatisticsStatics.areSomeColdStatsEnabled)
    ...
    class ImplicitComputation(iss: Infoss, isLocalToCallsite: Boolean) {
      ...
      if (eligible.nonEmpty)
        printTyping(
          tree,
          eligible.size + s" eligible for pt=$pt at ${fullSiteString(context)}")
      ...
      lazy val wildPtNotInstantiable =
        wildPt.exists {
          case _: BoundedWildcardType | _: TypeVar => true;
          case tp if typeIsNothing(tp)             => true; case _ => false
        }
      ...
      @tailrec private def rankImplicits(
        ...
        case firstPending :: otherPending =>
          ...
          val typedFirstPending = {
            try {
              if (isView || wildPtNotInstantiable || matchesPtInst(
                    firstPending))
                typedImplicit(firstPending, ptChecked = true, isLocalToCallsite)
              else SearchFailure
            } finally {
              foreach2(undetParams, savedInfos) { (up, si) =>
                up.setInfo(si)
              }
            }
          }
          if (typedFirstPending.isFailure && currentRun.isScala213)
            undoLog
              .undoTo(mark) // Don't accumulate constraints from typechecking or type error message creation for failed candidates
          ...
          DivergentImplicitRecovery(typedFirstPending,
                                    firstPending,
                                    context.reporter.errors) match {
          ...
          case (chosenResult, chosenInfo) :: rest =>
            rest find { case (_, alt) => !improves(chosenInfo, alt) } match {
              case Some((competingResult, competingInfo)) =>
                AmbiguousImplicitError(chosenInfo,
                                       chosenResult.tree,
                                       competingInfo,
                                       competingResult.tree,
                                       "both",
                                       "and",
    ...
    /** Search list of implicit info lists for one matching prototype `pt`.
      ...
      *                           symbols of the same name in succeeding lists.
      */
    def searchImplicit(implicitInfoss: Infoss,
                       isLocalToCallsite: Boolean): SearchResult =
    ...
    private def companionImplicitMap(tp: Type): InfoMap = {
       ...
       */
      def getClassParts(tp: Type)(implicit infoMap: InfoMap,
                                  seen: mutable.HashSet[Type],
        ...
        case TypeRef(pre, sym, args) =>
          ...
          if (!infos1.exists(pre =:= _.pre.prefix)) {
              ...
              val mergedInfos =
                if (infos1.isEmpty) infos
                  ...
                  if (nonDependentInfos.nonEmpty)
                    log(s"Implicit members ${nonDependentInfos
                      .mkString("(", ", ", ")")} of $pre#$sym which are also visible via another prefix: ${infos1.head.pre.prefix}")
                  infos1.filter(_.dependsOnPrefix) ++ infos.filter(
                    _.dependsOnPrefix)
                }
            ...
            val bts =
              (if (infos1.isEmpty) tp else tp.map(_.withoutAnnotations)).baseTypeSeq
            var i = 1
            while (i < bts.length) {
              getParts(bts(i))
              i += 1
       ...
       */
      def getParts(tp: Type)(implicit infoMap: InfoMap,
                             seen: mutable.HashSet[Type],
                             pending: Set[Symbol]): Unit = {
                ...
                infoMap ++= {
                  infoMapCache get sym match {
                    ...
                    case None =>
                      ...
                      getClassParts(sym.tpeHK)(result,
                                               new mutable.HashSet(),
                                               pending + sym)
      ...
      val emptyInfos =
        infoMap.iterator.filter(_._2.exists(_.isSearchedPrefix)).map(_._1).toSeq
    ...
    /** Creates a tree will produce a tag of the requested flavor.
      ...
      */
    private def tagOfType(pre: Type,
                          tp: Type,
      ...
      def success(arg: Tree) = {
        ...
        def processMacroExpansionError(pos: Position,
    ...
    private def manifestOfType(tp: Type, flavor: Symbol): SearchResult = {
      ...
      def manifestFactoryCall(constructor: String,
                              tparg: Type,
                              args: Tree*): Tree =
      ...
      def findManifest(tp: Type,
                       manifestClass: Symbol =
                         if (full) FullManifestClass
        ...
        tp1 match {
          case ThisType(_) | SingleType(_, _) =>
            ...
            if (containsExistential(tp1)) EmptyTree
              ...
              manifestFactoryCall("singleType",
                                  tp,
                                  gen.mkAttributedQualifier(tp1))
          ...
          case TypeRef(pre, sym, args) =>
            if (isPrimitiveValueClass(sym) || isPhantomClass(sym)) {
              ...
              manifestFactoryCall("wildcardType",
                                  tp,
                                  findManifest(tp.lowerBound),
                                  findManifest(tp.upperBound))
            }
      ...
      if (full) {
        val tagInScope = resolveTypeTag(pos,
                                        NoType,
                                        tp,
                                        concrete = true,
        ...
        if (tagInScope.isEmpty) mot(tp, Nil, Nil)
          ...
          val internal = gen.mkAttributedSelect(
            ...
            UniverseInternal)
          val interop = gen.mkMethodCall(
            Select(internal, nme.typeTagToManifest),
            List(tp),
            List(cm, tagInScope))
          wrapResult(interop)
        }
    ...
    private def valueOfType(tp: Type): SearchResult = {
      def success(t: Tree) =
        wrapResult(
          Apply(Select(New(gen.scalaDot(tpnme.ValueOf)), nme.CONSTRUCTOR),
                List(t)))
      ...
      tp.dealias match {
        ...
        case TypeRef(pre, sym, Nil) if sym.isModuleClass =>
          success(gen.mkAttributedRef(pre, sym.sourceModule) setType tp)
            ...
            val fail =
              if (out.annotations.isEmpty && (outSym == ObjectClass || outSym == AnyValClass))
                maybeInvalidConversionError(
                  s"the result type of an implicit conversion must be more specific than $out")
  ...
  class ImplicitAnnotationMsg(f: Symbol => Option[String],
                              clazz: Symbol,
  ...
  object ImplicitNotFoundMsg
      extends ImplicitAnnotationMsg(_.implicitNotFoundMsg,
                                    ImplicitNotFoundClass,
                                    "implicitNotFound")

  object ImplicitAmbiguousMsg
      extends ImplicitAnnotationMsg(_.implicitAmbiguousMsg,
                                    ImplicitAmbiguousClass,
                                    "implicitAmbiguous")
  ...
  class Message(sym: Symbol, msg: String, annotationName: String) {
    ...
    def referencedTypeParams: List[String] =
      Intersobralator.findAllMatchIn(msg).map(_.group(1)).distinct.toList

    private def symTypeParamNames: List[String] =
    ...
    def formatParameterMessage(fun: Tree): String = {
      ...
      val argTypes1 =
        if (prefix == NoType) paramTypeRefs
        else paramTypeRefs.map(t => t.asSeenFrom(prefix, fun.symbol.owner))
      val argTypes2 = fun match {
        case treeInfo.Applied(_, targs, _) =>
          argTypes1.map(
            _.instantiateTypeParams(fun.symbol.info.typeParams,
                                    targs.map(_.tpe)))
        case _ => argTypes1
      }
      ...
      val argTypes = argTypes2.map {
        case PolyType(tps, tr @ TypeRef(_, _, tprefs)) =>
          if (tps.corresponds(tprefs)((p, r) => p == r.typeSymbol))
            tr.typeConstructor.toString
          else {
            val freshTpars = tps.mapConserve {
              case p if p.name == tpnme.WILDCARD =>
                p.cloneSymbol.setName(newTypeName("?T" + tps.indexOf(p)))
              case p => p
            }
            freshTpars.map(_.name).mkString("[", ", ", "] -> ") + tr
              .instantiateTypeParams(tps, freshTpars.map(_.typeConstructor))
              .toString
          }
    ...
    def validate: Option[String] = {
      ...
      val decls =
        if (isMessageOnParameter)
          referencedTypeParams.filterNot(p => lookupTypeParam(p) == NoSymbol)
      ...
      refs.diff(decls) match {
        ...
        case unboundNames =>
          ...
          val where =
            if (isMessageOnParameter) s"in scope" else s"defined by $sym"
          Some(
            s"The type parameter$ess ${unboundNames mkString ", "} referenced in the message of the @$annotationName annotation $bee not $where.")
      }
    }
  }
...
trait ImplicitsStats {
  ...
  val improvesCount =
    newSubCounter("  #implicit improves tests", implicitSearchCount)
  val improvesCachedCount =
    newSubCounter("  #implicit improves cached ", implicitSearchCount)
  val inscopeImplicitHits =
    newSubCounter("  #implicit inscope hits", implicitSearchCount)
  val oftypeImplicitHits =
    newSubCounter("  #implicit oftype hits ", implicitSearchCount)
  ...
  val implicitCacheHits =
    newSubCounter("implicit cache hits", implicitCacheAccs)
