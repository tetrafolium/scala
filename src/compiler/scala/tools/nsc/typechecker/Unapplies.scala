...
trait Unapplies extends ast.TreeDSL {
  ...
  private def constrParamss(cdef: ClassDef): List[List[ValDef]] = {
    val prunedClassDef = deriveClassDef(cdef)(tmpl =>
      deriveTemplate(tmpl)(stats =>
        treeInfo.firstConstructor(stats).duplicate :: Nil))
    val ClassDef(_, _, _, Template(_, _, firstConstructor :: Nil)) = resetAttrs(
      prunedClassDef)
  ...
  private def constrTparamsInvariant(cdef: ClassDef): List[TypeDef] = {
    val prunedClassDef =
      deriveClassDef(cdef)(tmpl => Template(Nil, noSelfType, Nil))
    val ClassDef(_, _, tparams, _) = resetAttrs(prunedClassDef.duplicate)
    val tparamsInvariant = tparams.map(tparam =>
      copyTypeDef(tparam)(mods = tparam.mods &~ (COVARIANT | CONTRAVARIANT)))
    tparamsInvariant
  }
  ...
  def caseModuleApplyMeth(cdef: ClassDef): DefDef =
    factoryMeth(caseMods, nme.apply, cdef)
  ...
  def caseModuleUnapplyMeth(cdef: ClassDef): DefDef = {
    ...
    val cparams = List(
      ValDef(Modifiers(PARAM | SYNTHETIC),
             ...
             EmptyTree))
    val resultType =
      if (!currentRun.isScala212) TypeTree()
      else { // fix for scala/bug#6541 under -Xsource:2.12
    ...
    def selectCaseFieldAccessor(constrParam: ValDef): Tree = {
      ...
      val accSel =
        cdef.impl.body collectFirst {
          case localAccessor @ ValOrDefDef(mods, constrParam.name, _, _)
              if mods.isPrivateLocal =>
            Select(unapplyParam, localAccessor.symbol)
        } getOrElse Select(unapplyParam,
                           caseAccessorName(cdef.symbol, constrParam.name))

      constrParam.tpt match {
        case AppliedTypeTree(Select(_, tpnme.REPEATED_PARAM_CLASS_NAME), tps) =>
          Typed(accSel, AppliedTypeTree(gen.rootScalaDot(tpnme.Seq), tps))
        case _ => accSel
      }
    }
    ...
    val body =
      If(
        Ident(unapplyParamName) OBJ_EQ NULL,
