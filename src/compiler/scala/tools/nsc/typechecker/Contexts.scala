...
trait Contexts { self: Analyzer =>
  ...
  def ambiguousImports(imp1: ImportInfo, imp2: ImportInfo) =
    LookupAmbiguous(
      s"it is imported twice in the same scope by\n$imp1\nand $imp2")
  ...
  val excludedRootImportsCached =
    perRunCaches.newMap[CompilationUnit, List[Symbol]]
  ...
  def registerImport(ctx: Context, imp: Import): Unit = {
    ...
    if (!sym.isPackage && ctx.enclosingNonImportContext.owner.isPackage && rootImports(
          ctx.unit).contains(sym)) {
  ...
  protected def rootImports(unit: CompilationUnit): List[Symbol] = {
    ...
    if (unit.isJava) RootImports.javaList
      ...
      rootImportsCached.get(unit).getOrElse {
  ...
  def rootContext(unit: CompilationUnit,
    ...
    val rootImportsContext =
      rootImports(unit).foldLeft(startContext)((c, sym) =>
  ...
  class Context private[typechecker] (
    ...
    def set(enable: ContextMode = NOmode,
            disable: ContextMode = NOmode): this.type = {
    ...
    final def linkByNameImplicit(tpe: Type): Tree =
      implicitRootContext.linkImpl(tpe)
    ...
    final def refByNameImplicit(tpe: Type): Tree =
      implicitRootContext.refImpl(tpe)

    private def defineImpl(tpe: Type, result: SearchResult): SearchResult = {
      @tailrec
      def patch(d: ImplicitDict,
                acc: ImplicitDict): (ImplicitDict, SearchResult) = d match {
    ...
    def defineByNameImplicit(tpe: Type, result: SearchResult): SearchResult =
      implicitRootContext.defineImpl(tpe, result)

    def emitImplicitDictionary(pos: Position,
                               result: SearchResult): SearchResult =
      if (implicitDictionary == null || implicitDictionary.isEmpty || result.tree == EmptyTree)
        result
      else {
        val typer = newTyper(this)

        @tailrec
        def prune(trees: List[Tree],
                  pending: List[(Symbol, Tree)],
                  acc: List[(Symbol, Tree)]): List[(Symbol, Tree)] =
          pending match {
            case Nil => acc
            case ps =>
              val (in, out) = ps.partition {
                case (vsym, rhs) => trees.exists(_.exists(_.symbol == vsym))
              }
              if (in.isEmpty) acc
        ...
        if (pruned.isEmpty) result
          ...
          val (dictClassSym, dictClass0) = {
            ...
            val vdefs = pruned.map {
              case (vsym, rhs) =>
                ...
                atPos(pos)(
                  ValDef(Modifiers(FINAL | SYNTHETIC),
                         vsym.name.toTermName,
                         TypeTree(rhs.tpe),
                         rhs))
            }
          ...
          val dictClass = {
            class DictionarySubstituter
                extends TreeSymSubstituter(preSyms, postSyms) {
          ...
          val dict = {
            val rhs = atPos(pos)(
              Apply(Select(New(Ident(dictClassSym)), nme.CONSTRUCTOR), List()))
          ...
          val resultTree = {
            class ReferenceSubstituter
                extends TreeSymSubstituter(preSyms, postSyms) {
              override def transform(tree: Tree): Tree = tree match {
                case i: Ident if symMap.contains(i.symbol) =>
                  super.transform(
                    atPos(i.pos)(
                      treeCopy.Select(i, gen.mkAttributedRef(dictSym), i.name)))
          ...
          val resultBlock = atPos(pos.focus)(
            Block(dictClass, dict, resultTree).setType(resultTree.tpe))
    ...
    /** Run `body` with this context with no undetermined type parameters, restore the original
      ...
      */
    def savingUndeterminedTypeParams[A](reportAmbiguous: Boolean =
                                          ambiguousErrors)(body: => A): A = {
      withMode() {
    ...
    private def setAmbiguousErrors(report: Boolean): Unit =
      this(AmbiguousErrors) = report
    ...
    @inline final def withImplicits[T](enabled: Boolean)(op: => T): T =
      if (enabled) withImplicitsEnabled(op) else withImplicitsDisabled(op)
    ...
    @inline final def withMacros[T](enabled: Boolean)(op: => T): T =
      if (enabled) withMacrosEnabled(op) else withMacrosDisabled(op)
    ...
    @inline final def withinAnnotation[T](op: => T): T =
      withMode(enabled = TypingAnnotation)(op)

    @inline final def withSuppressDeadArgWarning[T](suppress: Boolean)(
        op: => T): T =
      if (suppress) withMode(enabled = SuppressDeadArgWarning)(op)
      else withMode(disabled = SuppressDeadArgWarning)(op)
    ...
    def initRootContext(throwing: Boolean = false,
    ...
    def makeSilent(reportAmbiguousErrors: Boolean = ambiguousErrors,
                   newtree: Tree = tree): Context = {
    ...
    private[typechecker] def issue(err: AbsTypeError) =
      reporter.issue(err)(this)

    /** Issue/buffer/throw the given implicit ambiguity error according to the current mode for error reporting. */
    private[typechecker] def issueAmbiguousError(err: AbsAmbiguousTypeError) =
      reporter.issueAmbiguousError(err)(this)

    /** Issue/throw the given error message according to the current mode for error reporting. */
    def error(pos: Position, msg: String) =
      reporter.error(fixPosition(pos), msg)
    ...
    final def outermostContextAtCurrentPos: Context = {
      ...
      while (encl.outer.tree.pos == pos && encl != NoContext) encl = encl.outer
      encl
    }
    ...
    private def isSubClassOrCompanion(sub: Symbol, base: Symbol) =
      sub.isNonBottomSubClass(base) ||
        ...
        (base.isJavaDefined && base.isModuleClass && sub.isNonBottomSubClass(
          base.linkedClassOfClass))
    ...
    def isAccessible(sym: Symbol,
                     pre: Type,
                     superAccess: Boolean = false): Boolean = {
    ...
    /** A symbol `sym` qualifies as an implicit if it has the IMPLICIT flag set,
      ...
      */
    private def isQualifyingImplicit(name: Name,
                                     sym: Symbol,
                                     pre: Type,
                                     imported: Boolean) =
      sym.isImplicit &&
    ...
    /** Do something with the symbols with name `name` imported via the import in `imp`,
      ...
      */
    private def withQualifyingImplicitAlternatives(
        imp: ImportInfo,
        name: Name,
    ...
    private def collectImplicitImports(imp: ImportInfo): List[ImplicitInfo] =
      if (isExcludedRootImport(imp)) List()
      else {
        val qual = imp.qual
    ...
    /** It's possible that seemingly conflicting identifiers are
      ...
      */
    private[Contexts] def resolveAmbiguousImport(
        name: Name,
        imp1: ImportInfo,
    ...
    /** Does the import just import the defined symbol?
      ...
      */
    private[Contexts] def reconcileAmbiguousImportAndDef(
        name: Name,
        impSym: Symbol,
        defSym: Symbol): Boolean = {
    ...
    /** The symbol with name `name` imported via the import in `imp`,
      ...
      */
    private[Contexts] def importedAccessibleSymbol(imp: ImportInfo,
                                                   name: Name,
    ...
    private def isExcludedRootImport(imp: ImportInfo): Boolean =
      imp.isRootImport && excludedRootImportsCached
        .get(unit)
        .exists(_.contains(imp.qual.symbol))
    ...
    final def lookupCompanionInIncompleteOwner(original: Symbol): Symbol = {
      ...
      def isCompanion(sym: Symbol): Boolean =
        (original.isModule && sym.isClass || sym.isModule && original.isClass) && sym
          .isCoDefinedWith(original)
    ...
    final def javaFindMember(pre: Type,
                             name: Name,
                             qualifies: Symbol => Boolean): (Type, Symbol) = {
      ...
      if (sym.exists || preSym.isPackageClass || !preSym.isClass) (pre, sym)
        ...
        val toSearch =
          if (preSym.isModuleClass)
            companionSymbolOf(pre.typeSymbol.sourceModule, this).baseClasses
          else preSym.baseClasses
        toSearch.iterator
          .map { bc =>
          ...
          .find(_._2 ne NoSymbol)
          .getOrElse(NoJavaMemberFound)
      }
    }
  ...
  private[Contexts] final val symbolLookupCache =
    ReusableInstance[SymbolLookup](new SymbolLookup)
  private[Contexts] final class SymbolLookup {
    private[this] var lookupError
      : NameLookup = _ // set to non-null if a definite error is encountered
    private[this] var inaccessible
      : NameLookup = _ // records inaccessible symbol for error reporting in case none is found
    private[this] var defSym: Symbol = _ // the directly found symbol
    private[this] var pre
      : Type = _ // the prefix type of defSym, if a class member
    private[this] var cx: Context = _ // the context under consideration
    private[this] var symbolDepth
      : Int = _ // the depth of the directly found symbol

    def apply(thisContext: Context, name: Name)(
        qualifies: Symbol => Boolean): NameLookup = {
      lookupError = null
      inaccessible = null
      defSym = NoSymbol
      pre = NoPrefix
      ...
      def finishDefSym(sym: Symbol, pre0: Type): NameLookup = {
        val qual =
          if (!thisContext.unit.isJava && thisContext.requiresQualifier(sym))
            gen.mkAttributedQualifier(pre0)
          else EmptyTree
        finish(qual, sym)
      }
      ...
      if (name == nme.CONSTRUCTOR) {
        ...
        val constructorSym =
          lookupInScope(enclClassSym, cx.enclClass.prefix, scope)
      ...
      def lookupImport(imp: ImportInfo, requireExplicit: Boolean) =
        thisContext.importedAccessibleSymbol(imp,
                                             name,
                                             requireExplicit,
       ...
       */
      def foreignDefined =
        defSym.exists && thisContext.isPackageOwnedInDifferentUnit(defSym) // SI-2458
      ...
      def importCanShadowAtDepth(imp: ImportInfo) = imp.depth > symbolDepth || (
        if (thisContext.unit.isJava)
          imp.depth == symbolDepth && imp.isExplicitImport(name)
        else foreignDefined
      )

      while (!impSym.exists && importCursor.imp1Exists && importCanShadowAtDepth(
               importCursor.imp1)) {
      ...
      if (defSym.exists && impSym.exists) {
        ...
        else if (foreignDefined && thisContext.reconcileAmbiguousImportAndDef(
                   name,
                   impSym,
                   defSym))
          impSym = NoSymbol
      ...
      if (defSym.exists) finishDefSym(defSym, pre)
        ...
        while (lookupError == null && importCursor.keepLooking) {
          ...
          // actually used.
          val other =
            lookupImport(imp2, requireExplicit = !importCursor.sameDepth)

          @inline def imp1wins(): Unit = { importCursor.advanceImp2() }
          @inline def imp2wins(): Unit = {
            impSym = other; importCursor.advanceImp1Imp2()
          }
          if (!other.exists) // imp1 wins; drop imp2 and continue.
            ...
            thisContext.resolveAmbiguousImport(name, imp1, imp2) match {
  ...
  trait ImportContext extends Context {
    private val impInfo: ImportInfo = {
      val info =
        new ImportInfo(tree.asInstanceOf[Import], outerDepth, isRootImport)
    ...
    override final def toString =
      s"${super.toString} with ImportContext { $impInfo; outer.owner = ${outer.owner} }"
  }
  ...
  abstract class ContextReporter(
    ...
    def issue(err: AbsTypeError)(implicit context: Context): Unit =
      handleError(context.fixPosition(err.errPos), addDiagString(err.errMsg))
    ...
    protected def handleSuppressedAmbiguous(err: AbsAmbiguousTypeError): Unit =
      ()
    protected def handleWarning(pos: Position, msg: String): Unit =
    ...
    protected final def info0(pos: Position,
                              msg: String,
                              severity: Severity,
                              force: Boolean): Unit =
      severity match {
    ...
    final override def hasErrors =
      super.hasErrors || (_errorBuffer != null && errorBuffer.nonEmpty)
    ...
    protected def addDiagString(msg: String)(
      ...
      if (context.contextMode.inAny(ContextMode.DiagUsedDefaults) && !(msg endsWith diagUsedDefaultsMsg))
        msg + "\n" + diagUsedDefaultsMsg
      else msg
    }
    ...
    final private[typechecker] def ++=(errors: Iterable[AbsTypeError]): Unit =
      errorBuffer ++= errors
  ...
  private[typechecker] class ImmediateReporter(
      ...
      _warningBuffer: mutable.LinkedHashSet[(Position, String)] = null)
  ...
  private[typechecker] class BufferingReporter(
      _errorBuffer: mutable.LinkedHashSet[AbsTypeError] = null,
      _warningBuffer: mutable.LinkedHashSet[(Position, String)] = null)
      extends ContextReporter(_errorBuffer, _warningBuffer) {
    override def isBuffering = true

    override def issue(err: AbsTypeError)(implicit context: Context): Unit =
      errorBuffer += err
    ...
    protected def handleError(pos: Position, msg: String): Unit =
      errorBuffer += TypeErrorWrapper(new TypeError(pos, msg))
    override protected def handleSuppressedAmbiguous(
        err: AbsAmbiguousTypeError): Unit = errorBuffer += err
    override protected def handleWarning(pos: Position, msg: String): Unit =
      warningBuffer += ((pos, msg))
  ...
  private[typechecker] class CheckingReporter extends ContextReporter {
    protected def handleError(pos: Position, msg: String): Unit =
      onTreeCheckerError(pos, msg)
  }

  class ImportInfo(val tree: Import,
                   val depth: Int,
    ...
    def isExplicitImport(name: Name): Boolean =
      tree.selectors.exists(_.introduces(name.toTermName))
    ...
    def importedSymbol(name: Name,
      ...
      while ((selectors ne Nil) && result == NoSymbol) {
        if (current.introduces(name.toTermName))
          result = qual.tpe
            .nonLocalMember( // #2733: consider only non-local members for imports
...
object ContextMode {
  ...
  private implicit def liftIntBitsToContextState(bits: Int): ContextMode =
    apply(bits)
