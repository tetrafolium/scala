...
trait Adaptations {
  ...
  trait Adaptation {
    ...
    def checkValidAdaptation(t: Tree, args: List[Tree]): Boolean = {
      ...
      @inline def msg(what: String): String =
        s"adaptation of an empty argument list by inserting () $what"
      @inline def noAdaptation = {
        context.error(
          t.pos,
          adaptWarningMessage(msg("has been removed"), showAdaptation = false))
      ...
      @inline def deprecatedAdaptation = {
        ...
        context.deprecationWarning(t.pos,
                                   t.symbol,
                                   adaptWarningMessage(text),
                                   "2.11.0")
