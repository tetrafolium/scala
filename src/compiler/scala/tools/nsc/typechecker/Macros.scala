...
trait Macros extends MacroRuntimes with Traces with Helpers {
    ...
    def unpickle(pickle: Tree): MacroImplBinding = {
      ...
      def unpickle[T](field: String, clazz: Class[T]): T = {
        ...
        if (!expected.isAssignableFrom(actual))
          failField(s"has wrong type: expected $expected, actual $actual")
        raw.asInstanceOf[T]
      }

      if (!payload.contains("macroEngine"))
        MacroCantExpand210xMacrosError("macroEngine field not found")
      val macroEngine = unpickle("macroEngine", classOf[String])
      if (self.macroEngine != macroEngine)
        MacroCantExpandIncompatibleMacrosError(
          s"expected = ${self.macroEngine}, actual = $macroEngine")
  ...
  def loadMacroImplBinding(macroDef: Symbol): Option[MacroImplBinding] =
    macroDef.getAnnotation(MacroImplAnnotation) collect {
      case AnnotationInfo(_, List(pickle), _) =>
  ...
  def typedMacroBody(typer: Typer, macroDdef: DefDef): Tree =
    pluginsTypedMacroBody(typer, macroDdef)
  ...
  def standardTypedMacroBody(typer: Typer, macroDdef: DefDef): Tree = {
    ...
    if (fastTrack contains macroDef) {
      ...
      if (!typer.checkFeature(macroDdef.pos,
        ...
        if (macroImplRef.isEmpty) fail()
        else {
          def hasTypeTag = {
            val marker = NoSymbol.newErrorValue("restricted")
            val xformed =
              transformTypeTagEvidenceParams(macroImplRef, (_, _) => marker)
  ...
  def macroArgs(typer: Typer, expandee: Tree): MacroArgs =
    pluginsMacroArgs(typer, expandee)
  ...
  def standardMacroArgs(typer: Typer, expandee: Tree): MacroArgs = {
    ...
    if (paramss.sizeCompare(argss) > 0 && !isNullaryArgsEmptyParams)
    ...
    val macroImplArgs: List[Any] =
      if (fastTrack contains macroDef) {
        ...
        def calculateMacroArgs(binding: MacroImplBinding) = {
          ...
          val trees = map3(argss, paramss, signature)((args,
                                                       defParams,
                                                       implParams) => {
            val isVarargs = isVarArgsList(defParams)
            if (isVarargs) {
              if (defParams.lengthIs > (args.length + 1))
                MacroTooFewArgumentsError(expandee)
            } else {
              if (defParams.sizeCompare(args) < 0)
            ...
            val wrappedArgs = mapWithIndex(args)((arg, j) => {
              ...
              fingerprint match {
                case LiftedTyped =>
                  context.Expr[Nothing](duplicatedArg)(TypeTag.Nothing) // TODO: scala/bug#5752
          ...
          val tags = signature.flatten collect {
            ...
            val tpe = if (targ.isTypeParameterOrSkolem) {
              if (targ.owner == macroDef) {
                ...
                targ.tpe.asSeenFrom(if (prefix == EmptyTree) macroDef.owner.tpe
                                    else prefix.tpe,
                                    macroDef.owner)
            } else
              targ.tpe
  ...
  abstract class MacroExpander(val typer: Typer, val expandee: Tree) {
    ...
    protected def expand(desugared: Tree): Tree = {
      ...
      def summary() =
        s"expander = $this, expandee = ${showDetailed(expandee)}, desugared = ${if (expandee == desugared) ()
        else showDetailed(desugared)}"
      ...
      val start =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startTimer(statistics.macroExpandNanos)
        else null
      if (StatisticsStatics.areSomeColdStatsEnabled)
        statistics.incCounter(statistics.macroExpandCount)
      try {
        ...
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.stopTimer(statistics.macroExpandNanos, start)
      }
    }
  }
  ...
  /** Expands a term macro used in apply role as `M(2)(3)` in `val x = M(2)(3)`.
    ...
    */
  class DefMacroExpander(typer: Typer,
                         expandee: Tree,
                         mode: Mode,
                         outerPt: Type)
    ...
    lazy val innerPt = {
      val tp =
        if (isNullaryInvocation(expandee)) expandee.tpe.finalResultType
        else expandee.tpe
    ...
    override def onSuccess(expanded0: Tree) = {
      ...
      def typecheck(label: String, tree: Tree, pt: Type): Tree = {
        if (tree.isErrorTyped) tree
          ...
          val result =
            typer.context.withImplicitsEnabled(typer.typed(tree, mode, pt))
          if (result.isErrorTyped && macroDebugVerbose)
      ...
      if (isBlackbox(expandee)) {
        val expanded1 = atPos(enclosingMacroPosition.makeTransparent)(
          Typed(expanded0, TypeTree(innerPt)))
        ...
        val expanded1 =
          typecheck("whitebox typecheck #0", expanded0, WildcardType)
  ...
  /** Default implementation of `macroExpand`.
    ...
    */
  def standardMacroExpand(typer: Typer,
                          expandee: Tree,
                          mode: Mode,
                          pt: Type): Tree = {
  ...
  case class Fallback(fallback: Tree) extends MacroStatus(fallback) {
    currentRun.reporting.seenMacroExpansionsFallingBack = true
  }
  ...
  /** Expands a macro when a runtime (i.e. the macro implementation) can be successfully loaded
    ...
    */
  def macroExpandWithRuntime(typer: Typer,
                             expandee: Tree,
                             runtime: MacroRuntime): MacroStatus = {
    ...
    (wasDelayed, nowDelayed) match {
      ...
      case (false, true) =>
        ...
        expandee updateAttachment MacroRuntimeAttachment(
          delayed = true,
          typerContext = typer.context,
          macroContext = Some(macroArgs(typer, expandee).c))
        Delay(expandee)
      case (false, false) =>
        import typer.TyperErrorGen._
        macroLogLite(
          "performing macro expansion %s at %s".format(expandee, expandee.pos))
        ...
        try {
          ...
          val isTermMacro = expandee.symbol.isTermMacro || currentRun.runDefinitions
            .treatLikeMacro(expandee.symbol)
          expanded match {
            case expanded: Expr[_] if isTermMacro =>
              validateResultingTree(expanded.tree)
            case expanded: Tree if isTermMacro =>
  ...
  def typerShouldExpandDeferredMacros: Boolean =
    hasPendingMacroExpansions && !delayed.isEmpty
  private val forced = perRunCaches.newWeakSet[Tree]
  private val delayed =
    perRunCaches.newWeakMap[Tree, scala.collection.mutable.Set[Int]]()
  private def isDelayed(expandee: Tree) = delayed contains expandee
  def clearDelayed(): Unit = delayed.clear()
  private def calculateUndetparams(
  ...
  def notifyUndetparamsAdded(newUndets: List[Symbol]): Unit = {
    ...
    if (macroDebugVerbose)
      newUndets foreach (sym => println("undetParam added: %s".format(sym)))
  }
  def notifyUndetparamsInferred(undetNoMore: List[Symbol],
                                inferreds: List[Type]): Unit = {
    undetparams --= undetNoMore map (_.id)
    if (macroDebugVerbose) (undetNoMore zip inferreds) foreach {
      case (sym, tpe) =>
    ...
    new Transformer {
      override def transform(tree: Tree) =
        super.transform(tree match {
          ...
          case tree
              if (delayed contains tree) && calculateUndetparams(tree).isEmpty && !tree.isErroneous =>
            val context =
...
object Macros {
  final val macroClassLoadersCache =
    new scala.tools.nsc.classpath.FileBasedCache[
