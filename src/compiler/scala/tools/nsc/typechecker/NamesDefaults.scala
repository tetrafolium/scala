...
trait NamesDefaults { self: Analyzer =>
  ...
  class ConstructorDefaultsAttachment(val classWithDefault: ClassDef,
                                      var companionModuleClassNamer: Namer) {
  ...
  class CaseApplyDefaultGetters(
      val defaultGetters: mutable.Set[Symbol] = mutable.Set.empty)
  ...
  class DefaultsOfLocalMethodAttachment(
      val defaultGetters: mutable.Set[Symbol]) {
  ...
  private def nameOfNamedArg(arg: Tree) = Some(arg) collect {
    case NamedArg(Ident(name), _) => name
  }
  ...
  private def reorderArgsInv[T: ClassTag](args: List[T],
                                          pos: Int => Int): List[T] = {
  ...
  def transformNamedApplication(typer: Typer, mode: Mode, pt: Type)(
    ...
    def baseFunBlock(baseFun: Tree): Tree = {
        ...
        case Select(New(tpt @ TypeTree()), _) if isConstr =>
          val targsInSource = tpt.tpe match {
            case TypeRef(pre, sym, args)
                if (!args.forall(
                  a => context.undetparams contains a.typeSymbol)) =>
              args.map(TypeTree(_))
            case _ =>
              Nil
          }
      ...
      def blockWithQualifier(qual: Tree, selected: Name) = {
        val sym = blockTyper.context.owner.newValue(freshTermName(
                                                      nme.QUAL_PREFIX)(
                                                      typer.fresh),
        ...
        b.updateAttachment(
          NamedApplyInfo(Some(newQual), defaultTargs, Nil, blockTyper))
        b
      }
      ...
      def blockWithoutQualifier(defaultQual: Option[Tree]) = {
        ...
        b.updateAttachment(
          NamedApplyInfo(defaultQual, defaultTargs, Nil, blockTyper))
        b
      }
      ...
      baseFun1 match {
        ...
        case Select(sp @ Super(_, _), _) if isConstr =>
          ...
          blockWithoutQualifier(
            moduleQual(baseFun.pos, sp.symbol.tpe.firstParent))
        ...
        arg match {
          ...
          case _ =>
            ...
            val s = context.owner.newValue(
              freshTermName(nme.NAMEDARG_PREFIX)(typer.fresh),
              arg.pos,
    ...
    tree match {
      ...
      case Apply(fun, namelessArgs) =>
        ...
        if (transformedFun.isErroneous) setError(tree)
          ...
          val NamedApplyBlock(NamedApplyInfo(qual, targs, vargss, blockTyper)) =
            transformedFun
  ...
  def makeNamedTypes(syms: List[Symbol]) =
    syms map (sym => NamedType(sym.name, sym.tpe))
  ...
  /**
    ...
    */
  def missingParams[T](args: List[T],
                       params: List[Symbol],
                       argName: T => Option[Name]): (List[Symbol], Boolean) = {
  ...
  /**
    ...
    */
  def addDefaults(givenArgs: List[Tree],
                  qual: Option[Tree],
                  targs: List[Tree],
  ...
  /** Removes name assignments from args. Additionally, returns an array mapping
    ...
    */
  def removeNames(typer: Typer)(
      args: List[Tree],
      params: List[Symbol]): (List[Tree], Array[Int]) = {
    implicit val context0 = typer.context
    def matchesName(param: Symbol, name: Name, argIndex: Int) = {
      def warn(msg: String, since: String) =
        context0.deprecationWarning(args(argIndex).pos, param, msg, since)
      ...
      def checkName = {
        ...
        if (res && checkDeprecation(true))
          warn(s"naming parameter $name is deprecated$since.", version)
        res
      }
      def checkAltName = {
        ...
        if (res)
          warn(
            s"the parameter name $name is deprecated$since: use ${param.name} instead",
            version)
        res
      }
    ...
    val namelessArgs = {
      ...
      def stripNamedArg(arg: NamedArg, argIndex: Int): Tree = {
        ...
        params indexWhere (p => matchesName(p, name, argIndex)) match {
          case -1 =>
            val warnVariableInScope = !currentRun.isScala214 && context0
              .lookupSymbol(name, _.isVariable)
              .isSuccess
            UnknownParameterNameNamesDefaultError(arg,
                                                  name,
                                                  warnVariableInScope)
      ...
      mapWithIndex(args) {
        case (arg: NamedArg, argIndex) =>
          ...
          if (!t.isErroneous && argPos(argIndex) < 0)
