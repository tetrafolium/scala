...
abstract class SuperAccessors
  ...
  class SuperAccTransformer(unit: CompilationUnit)
      ...
      if (sym.isDeferred) {
        ...
        if (mix == tpnme.EMPTY && !owner.isTrait) {
          ...
          val intermediateClasses =
            clazz.info.baseClasses.tail.takeWhile(_ != owner)
          intermediateClasses
            .map(sym.overridingSymbol)
            .find(s =>
              s.isDeferred && !s.isAbstractOverride && !s.owner.isTrait)
            .foreach { absSym =>
              reporter.error(
                sel.pos,
        ...
        if (mix != tpnme.EMPTY && !owner.isTrait && owner != clazz.superClass && hasClassOverride(
              sym,
              clazz.superClass)) {
          reporter.error(
            sel.pos,
          ...
          val receiverInBytecode = erasure
            .accessibleOwnerOrParentDefiningMember(
              sym,
              sup.tpe.typeSymbol.parentSymbols,
              localTyper.context.asInstanceOf[erasure.Context])
            .getOrElse(sym.owner)
          if (!clazz.parentSymbols.contains(receiverInBytecode))
            reporter.error(
              sel.pos,
              s"unable to emit super call unless interface ${owner.name} (which declares $sym) is directly extended by $clazz.")
        }
      }
    ...
    override def transform(tree: Tree): Tree = {
      ...
      def mayNeedProtectedAccessor(sel: Select,
                                   args: List[Tree],
                                   goToSuper: Boolean) =
    ...
    private def makeSetter(tree: Select): Tree = {
      ...
      val protectedAccessor = clazz.info decl accName orElse {
        ...
        storeAccessorDefinition(
          clazz,
          DefDef(protAcc, Assign(Select(Ident(obj), field.name), Ident(value))))
    ...
    /** Return the innermost enclosing class C of referencingClass for which either
      ...
      */
    private def hostForAccessorOf(sym: Symbol,
