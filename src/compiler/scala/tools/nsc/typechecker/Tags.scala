...
trait Tags {
  ...
  trait Tag {
    ...
    private def resolveTag(pos: Position,
                           taggedTp: Type,
                           allowMaterialization: Boolean) = enteringTyper {
      context.withMacros(enabled = allowMaterialization) {
        inferImplicitByType(taggedTp, context, pos).tree
      }
    }
    ...
    def resolveTypeTag(pos: Position,
      ...
      if (pre == NoType && ApiUniverseClass == NoSymbol) EmptyTree
        ...
        val tagSym =
          if (concrete) currentRun.runDefinitions.TypeTagClass
          else currentRun.runDefinitions.WeakTypeTagClass
        val tagTp =
          if (pre == NoType)
            TypeRef(ApiUniverseClass.toTypeConstructor, tagSym, List(tp))
