...
trait TypeDiagnostics {
  ...
  /** For errors which are artifacts of the implementation: such messages
    ...
    */
  def restrictionWarning(pos: Position,
                         unit: CompilationUnit,
                         msg: String): Unit =
    reporter.warning(pos, "Implementation restriction: " + msg)
  def restrictionError(pos: Position,
                       unit: CompilationUnit,
                       msg: String): Unit =
  ...
  /** Does the positioned line assigned to t1 precede that of t2?
    */
  def posPrecedes(p1: Position, p2: Position) =
    p1.isDefined && p2.isDefined && p1.line < p2.line
  ...
  def treeSymTypeMsg(tree: Tree): String = {
    ...
    def patternMessage =
      "pattern " + tree.tpe.finalResultType + valueParamsString(tree.tpe)
    def exprMessage = "expression of type " + tree.tpe
    def overloadedMessage =
      s"overloaded method $sym with alternatives:\n" + alternativesString(tree)
        ...
        if (foundArgs.nonEmpty && foundArgs.length == reqArgs.length) {
          val relationships = foundArgs.lazyZip(reqArgs).lazyZip(params).map {
            case (arg, reqArg, param) =>
              ...
              if (conforms) Some("")
              else if ((arg <:< reqArg) && invariant)
                mkMsg(isSubtype = true) // covariant relationship
              else if ((reqArg <:< arg) && invariant)
                mkMsg(isSubtype = false) // contravariant relationship
  ...
  def finalOwners(tpe: Type): Boolean =
    (tpe.prefix == NoPrefix) || recursivelyFinal(tpe)
  ...
  def foundReqMsg(found: Type, req: Type): String = {
    ...
    if (easilyMistakable) {
      ...
      val paddedFoundName =
        foundWiden.nameAndArgsString.padTo(longestNameLength, ' ')
      val paddedReqName =
        reqWiden.nameAndArgsString.padTo(longestNameLength, ' ')
      ";\n found   : " + (paddedFoundName + s" (in ${found.prefix.typeSymbol.fullNameString}) ") + explainAlias(
        found) +
        "\n required: " + (paddedReqName + s" (in ${req.prefix.typeSymbol.fullNameString}) ") + explainAlias(
        req)
    } else {
      def baseMessage = {
  ...
  object checkDead {
    ...
    def apply(context: Context, tree: Tree): Tree = {
      if (settings.warnDeadCode && context.unit.exists && treeOK(tree) && !context.contextMode
            .inAny(ContextMode.SuppressDeadArgWarning))
        context.warning(tree.pos, "dead code following this construct")
      tree
    }

    // The checkDead call from typedArg is more selective.
    def inMode(context: Context, mode: Mode, tree: Tree): Tree =
      if (mode.typingMonoExprByValue) apply(context, tree) else tree
  }
  ...
  class checkUnused(typer: Typer) {
    ...
    class UnusedPrivates extends Traverser {
      ...
      override def traverse(t: Tree): Unit = if (!t.isErrorTyped) {
        ...
        t match {
          case m: MemberDef if qualifies(sym) =>
            t match {
              case ValDef(mods @ _, name @ _, tpt @ _, rhs @ _)
                  if wasPatVarDef(t) =>
              ...
              case DefDef(mods @ _,
                          ...
                          tpt @ _,
                          rhs @ _)
                  if !sym.isAbstract && !sym.isDeprecated && !sym.isMacro =>
                if (sym.isPrimaryConstructor)
                  for (cpa <- sym.owner.constrParamAccessors
                       if cpa.isPrivateLocal) params += cpa
          ...
          case CaseDef(pat, guard @ _, rhs @ _) if settings.warnUnusedPatVars =>
            pat.foreach {
              case b @ Bind(n, _) if !atBounded(b) && n != nme.DEFAULT_CASE =>
                patvars += b.symbol
              case _ =>
            }
          ...
          case Function(ps, _) if settings.warnUnusedParams =>
            params ++= ps
              .filterNot(p => atBounded(p) || p.symbol.isSynthetic)
              .map(_.symbol)
          case _ =>
        }
        ...
        if (t.tpe ne null) {
          for (tp <- t.tpe if !treeTypes(tp)) {
            ...
            if (isAlias || !currentOwner.hasTransOwner(tp.typeSymbol))
              tp match {
                ...
                case _ =>
                  log(
                    s"${if (isAlias) "alias " else ""}$tp referenced from $currentOwner")
                  treeTypes += tp
              }
          }
      ...
      def sympos(s: Symbol): Int =
        ...
        else if (s.isTerm) s.asTerm.referenced.pos.point
        else -1
      def treepos(t: Tree): Int =
        if (t.pos.isDefined) t.pos.point else sympos(t.symbol)

      def unusedTypes =
        defnTrees.toList.filter(t => isUnusedType(t.symbol)).sortBy(treepos)
      def unusedTerms = {
        ...
        def sameReference(g: Symbol, s: Symbol) =
          if (g.accessed.exists && s.accessed.exists) g.accessed == s.accessed
            ...
            g.owner == s.owner && g.setterName == s.name //sympos(g) == sympos(s)

        // filter out setters if already warning for getter.
        val clean = all.filterNot(v =>
          v.symbol.isSetter && all.exists(g =>
            g.symbol.isGetter && sameReference(g.symbol, v.symbol)))
        clean.sortBy(treepos)
      }
      // local vars which are never set, except those already returned in unused
      def unsetVars =
        localVars
          .filter(v => !isSuppressed(v) && !setVars(v) && !isUnusedTerm(v))
          .sortBy(sympos)
      def unusedParams = params.toList.filter(isUnusedParam).sortBy(sympos)
      def inDefinedAt(p: Symbol) =
        p.owner.isMethod && p.owner.name == nme.isDefinedAt && p.owner.owner.isAnonymousFunction
      def unusedPatVars =
        patvars.toList
          .filter(p => isUnusedTerm(p) && !inDefinedAt(p))
          .sortBy(sympos)
    }
    ...
    object checkMacroExpandee extends UnusedPrivates {
      override def traverse(t: Tree): Unit =
        super.traverse(
          if (hasMacroExpansionAttachment(t)) macroExpandee(t) else t)
    }
    ...
    def run(unusedPrivates: UnusedPrivates)(body: Tree): Unit = {
      ...
      if (settings.warnUnusedLocals || settings.warnUnusedPrivates) {
          ...
          val what =
            (
              if (sym.isDefaultGetter) "default argument"
                       ...
                       || sym.isGetter && (sym.accessed.isVar || (sym.owner.isTrait && !sym
                         .hasFlag(STABLE))))
                s"var ${sym.name.getterName.decoded}"
              else if (sym.isVal
                       || sym.isGetter && (sym.accessed.isVal || (sym.owner.isTrait && sym
                         .hasFlag(STABLE)))
                       || sym.isLazy) s"val ${sym.name.decoded}"
        ...
        for (v <- unusedPrivates.unsetVars) {
          typer.context
            .warning(v.pos, s"local var ${v.name} in ${v.owner} ${valAdvice}")
        }
        for (t <- unusedPrivates.unusedTypes) {
          val sym = t.symbol
          val wrn =
            if (sym.isPrivate) settings.warnUnusedPrivates
      ...
      if (settings.warnUnusedPatVars) {
        for (v <- unusedPrivates.unusedPatVars)
          typer.context.warning(
            v.pos,
            s"pattern var ${v.name} in ${v.owner} is never used; `${v.name}@_` suppresses this warning")
      }
      if (settings.warnUnusedParams) {
        def isImplementation(m: Symbol): Boolean = {
          def classOf(s: Symbol): Symbol =
            if (s.isClass || s == NoSymbol) s else classOf(s.owner)
        ...
        def isConvention(p: Symbol): Boolean = {
          ...
          (
            ...
            ||
            p.isImplicit && cond(p.tpe.typeSymbol) {
              case SameTypeClass | SubTypeClass | DummyImplicitClass => true
            }
          )
        }
        def warningIsOnFor(s: Symbol) =
          if (s.isImplicit) settings.warnUnusedImplicits
          else settings.warnUnusedExplicits
        ...
        for (s <- unusedPrivates.unusedParams if warnable(s))
          typer.context.warning(
            s.pos,
            s"parameter $s in ${if (s.owner.isAnonymousFunction) "anonymous function"
            else s.owner} is never used")
      }
    }
    def apply(unit: CompilationUnit): Unit =
      if (warningsEnabled && !unit.isJava && !typer.context.reporter.hasErrors) {
        val body = unit.body
  ...
  trait TyperDiagnostics {
    ...
    private def symWasOverloaded(sym: Symbol) =
      sym.owner.isClass && sym.owner.info.member(sym.name).isOverloaded
    private def cyclicAdjective(sym: Symbol) =
      if (symWasOverloaded(sym)) "overloaded" else "recursive"
    ...
    def cyclicReferenceMessage(sym: Symbol, tree: Tree) = whenever(tree) {
      ...
      case DefDef(_, _, _, _, TypeTree(), _) =>
        s"${cyclicAdjective(sym)} $sym needs result type"
    ...
    /** Report a type error.
      ...
      */
    def reportTypeError(context0: Context,
                        pos: Position,
                        ex: TypeError): Unit = {
