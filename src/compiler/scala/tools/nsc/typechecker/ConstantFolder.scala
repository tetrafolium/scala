...
abstract class ConstantFolder {
  ...
  object FoldableTerm {
    def unapply(tree: Tree): Option[Constant] = tree match {
      ...
      case term
          if term.symbol != null && !term.symbol.isLazy && (term.symbol.isVal || (term.symbol.isGetter && term.symbol.accessed.isVal)) =>
  ...
  def apply(tree: Tree): Tree = {
    try {
      tree match {
        case Apply(Select(FoldableTerm(x), op), List(FoldableTerm(y))) =>
          fold(tree, foldBinop(op, x, y), true)
        case Apply(Select(ConstantTerm(x), op), List(ConstantTerm(y))) =>
          fold(tree, foldBinop(op, x, y), false)
      ...
      case e: ArithmeticException =>
        if (settings.warnConstant)
          warning(
            tree.pos,
            s"Evaluation of a constant expression results in an arithmetic error: ${e.getMessage}")
        tree
    }
  }
  ...
  private def foldLongOp(op: Name, x: Constant, y: Constant): Constant =
    op match {
      ...
      case nme.LSL if x.tag <= IntTag => Constant(x.intValue << y.longValue)
      case nme.LSL                    => Constant(x.longValue << y.longValue)
      case nme.LSR if x.tag <= IntTag => Constant(x.intValue >>> y.longValue)
      case nme.LSR                    => Constant(x.longValue >>> y.longValue)
      case nme.ASR if x.tag <= IntTag => Constant(x.intValue >> y.longValue)
