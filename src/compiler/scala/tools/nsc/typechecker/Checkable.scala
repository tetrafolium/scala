...
trait Checkable {
  ...
  private object CheckabilityChecker {
    ...
    def areIrreconcilableAsParents(sym1: Symbol, sym2: Symbol): Boolean =
      areUnrelatedClasses(sym1, sym2) && (
        isEffectivelyFinal(sym1) // initialization important
          ...
          || isSealedOrFinal(sym1) && isSealedOrFinal(sym2) && allChildrenAreIrreconcilable(
            sym1,
            sym2) && !currentRun.compiles(sym1) && !currentRun.compiles(sym2)
      )
    ...
    private def isNeverSubArgs(tps1: List[Type],
                               tps2: List[Type],
  ...
  trait InferCheckable {
    ...
    /** TODO: much better error positions.
      ...
      */
    def checkCheckable(tree: Tree,
                       P0: Type,
                       X0: Type,
                       inPattern: Boolean,
                       canRemedy: Boolean = false): Unit = {
      ...
      if (P0.typeSymbol == SingletonClass)
        reporter.warning(
          tree.pos,
          s"fruitless type test: every non-null value will be a Singleton dynamically")
      else {
