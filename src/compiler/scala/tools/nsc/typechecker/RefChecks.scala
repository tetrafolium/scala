...
abstract class RefChecks extends Transform {
  ...
  val toJavaRepeatedParam = new SubstSymMap(
    RepeatedParamClass -> JavaRepeatedParamClass)
  ...
  def overridesTypeInPrefix(tp1: Type,
                            tp2: Type,
                            prefix: Type,
    ...
    (tp1.dealiasWiden, tp2.dealiasWiden) match {
      ...
      case (_, TypeRef(_, sym, _)) if sym.isModuleClass =>
        overridesTypeInPrefix(tp1,
                              NullaryMethodType(tp2),
                              prefix,
                              isModuleOverride)
      ...
      case _ =>
        ...
        (tp1 <:< tp2) || isModuleOverride && (
          ...
          tp1.typeSymbol.isModuleClass && tp2.typeSymbol.isModuleClass && {
            ...
            (cb1 <:< cb2) && {
              log(
                "Allowing %s to override %s because %s <:< %s"
                  .format(tp1, tp2, cb1, cb2))
              true
            }
          }
        )
  ...
  private val separatelyCompiledScalaSuperclass =
    perRunCaches.newAnyRefMap[Symbol, Unit]()
  final def isSeparatelyCompiledScalaSuperclass(sym: Symbol) =
    if (globalPhase.refChecked) {
    ...
    private def checkOverloadedRestrictions(clazz: Symbol,
      ...
      defaultMethodNames.toList.distinct foreach { name =>
        ...
        val haveDefaults = methods.filter(
          sym =>
            mexists(sym.info.paramss)(_.hasDefault) && !nme
      ...
      if (settings.warnPolyImplicitOverload && settings.developer) {
        clazz.info.decls.foreach(sym =>
          if (sym.isImplicit && sym.typeParams.nonEmpty) {
            // implicit classes leave both a module symbol and a method symbol as residue
            val alts = clazz.info
              .decl(sym.name)
              .alternatives filterNot (_.isModule)
            if (alts.size > 1)
              alts foreach (x =>
                reporter.warning(
                  x.pos,
    ...
    def addVarargBridges(clazz: Symbol): List[Tree] = {
      ...
      if (inheritsJavaVarArgsMethod(clazz)) {
        ...
        def varargBridge(member: Symbol, bridgetpe: Type): Tree = {
          ...
          val lastarg =
            gen.wildcardStar(gen.mkWrapVarargsArray(idents.last, elemtp))
...
//        debuglog(s"Checking validity of ${member.fullLocationString} overriding ${other.fullLocationString}")
        ...
        def isRootOrNone(sym: Symbol) =
          sym != null && sym.isRoot || sym == NoSymbol
        ...
        def overrideTypeError(): Unit = {
          if (noErrorType) {
            emitOverrideError(
              ...
              else if (!other.isDeferred && other.isAliasType)
                overrideErrorMsg(
                  s"overriding concrete type alias ${infoString(member)} is not allowed except when equivalent")
              else
                overrideErrorMsg(s"${infoString(member)} has incompatible type")
            )
          }
        }
        ...
        def overrideAccessError(): Unit = {
          ...
          overrideError(
            "has weaker access privileges; it should be " + (if (otherAccess == "")
                                                               "public"
                                                             else
                                                               "at least " + otherAccess))
        }
        ...
        def checkOverrideTerm(): Unit = {
          ...
          if (!overridesTypeInPrefix(
                ...
                highType,
                rootType,
                low.isModuleOrModuleClass && high.isModuleOrModuleClass)) { // 8
            overrideTypeError()
          ...
          if (low.isStable && !highType.isVolatile) {
            if (lowType.isVolatile)
              ...
              lowType.normalize.resultType match {
                case rt: RefinedType
                    if !(rt =:= highType) && !(checkedCombinations contains rt.parents) =>
        ...
        def checkOverrideDeprecated(): Unit = {
          if (other.hasDeprecatedOverridingAnnotation && !(member.hasDeprecatedOverridingAnnotation || member.ownerChain
                .exists(_.isDeprecated))) {
            val version = other.deprecatedOverridingVersion.getOrElse("")
            val since = if (version.isEmpty) version else s" (since $version)"
            val message = other.deprecatedOverridingMessage map (msg =>
              s": $msg") getOrElse ""
            val report =
              s"overriding ${other.fullLocationString} is deprecated$since$message"
            currentRun.reporting.deprecationWarning(member.pos,
                                                    other,
                                                    report,
                                                    version)
          }
      ...
      if (clazz.isConcreteClass && !typesOnly) {
        ...
        def javaErasedOverridingSym(sym: Symbol): Symbol =
          clazz.tpe
            .nonPrivateMemberAdmitting(sym.name, BRIDGE)
            .filter(other =>
              !other.isDeferred && other.isJavaDefined && !sym.enclClass
                .isSubClass(other.enclClass) && {
        ...
        def checkNoAbstractMembers(): Unit = {
          ...
          val (missing, rest) = memberList partition (m =>
            m.isDeferred && !ignoreDeferred(m))
          ...
          def diagnose(member: Symbol): String = {
            ...
            val groupedAccessors =
              grouped.getOrElse(member.name.getterName, Nil)
            val isMultiple = groupedAccessors.size > 1

            if (groupedAccessors.exists(_.isSetter) || (member.isGetter && !isMultiple && member
                  .setterIn(member.owner)
                  .exists)) {
              ...
              val matchingArity = matchingName.filter { m =>
                ...
                m.paramLists.map(_.length).sum == abstractParamLists
                  .map(_.length)
                  .sum &&
              ...
              matchingArity match {
                ...
                case Scope(concrete) =>
                  val sideBySide = abstractParamLists.flatten.map(_.tpe) zip concrete.paramLists.flatten
                    .map(_.tpe)
                  val mismatches = sideBySide.filterNot {
                    case (abs, konkret) =>
                      abs.asSeenFrom(clazz.tpe, underlying.owner) =:= konkret
                  }
                  mismatches match {
                    ...
                    case Nil => // other overriding gotchas
                      val missingImplicit =
                        abstractParamLists.zip(concrete.paramLists).exists {
                          case (abss, konkrete) =>
                            abss.headOption
                              .exists(_.isImplicit) && !konkrete.headOption
                              .exists(_.isImplicit)
                        }
                      val msg =
                        if (missingImplicit)
          ...
          for (member <- missing; msg = diagnose(member); if msg != null) {
            ...
            abstractClassError(false,
                               s"${infoString(member)} is not defined$addendum")
          }
          ...
          for (member <- rest;
            ...
            val explanation =
              if (other != NoSymbol)
                " and overrides incomplete superclass member " + infoString(
                  other)
              else
      ...
      for (member <- clazz.info.decls)
        if (member.isAnyOverride && !(clazz.thisType.baseClasses exists (hasMatchingSym(
          ...
          val nonMatching: List[Symbol] = clazz.info
            .member(member.name)
            .alternatives
...
//            println("validate base type "+tp)
        ...
        if (baseClass.isClass) {
          if (!baseClass.isTrait && !baseClass.isJavaDefined && !currentRun
                .compiles(baseClass) && !separatelyCompiledScalaSuperclass
                .contains(baseClass))
    ...
    object varianceValidator extends VarianceValidator {
      ...
      override def issueVarianceError(base: Symbol,
                                      sym: Symbol,
                                      required: Variance): Unit = {
        reporter.error(
          base.pos,
          s"${sym.variance} $sym occurs in $required position in ${tpString(
            base.info)} of $base")
      }
    }
...
// Forward reference checking ---------------------------------------------------
    ...
    class LevelInfo(val outer: LevelInfo) {
      val scope: Scope =
        if (outer eq null) newScope else newNestedScope(outer.scope)
...
// Comparison checking -------------------------------------------------------
    ...
    def checkImplicitViewOptionApply(pos: Position,
                                     fn: Tree,
                                     args: List[Tree]): Unit =
      if (settings.warnOptionImplicit) (fn, args) match {
        case (tap @ TypeApply(fun, targs), List(view: ApplyImplicitView))
          ...
          reporter.warning(
            pos,
            s"Suspicious application of an implicit view (${view.fun}) in the argument to Option.apply.") // scala/bug#6567
        case _ =>
      }
    ...
    /**
      ...
      */
    private def checkSensibleEquals(pos: Position,
                                    qual: Tree,
                                    name: Name,
                                    sym: Symbol,
                                    other: Tree) = {
      ...
      val couldBeAnything =
        Set[Symbol](ObjectClass, ComparableClass, SerializableClass)
      def isMaybeValue(sym: Symbol): Boolean =
        couldBeAnything(erasedClass(sym.tpe))
      ...
      def isUsingDefaultScalaOp =
        sym == Object_== || sym == Object_!= || sym == Any_== || sym == Any_!=
      def haveSubclassRelationship =
        (actual isSubClass receiver) || (receiver isSubClass actual)
      ...
      def isWarnable =
        isReferenceOp || (isUsingDefaultScalaOp && isUsingWarnableEquals)
      def isEitherNullable =
        (NullTpe <:< receiver.info) || (NullTpe <:< actual.info)
      def isEitherValueClass =
        actual.isDerivedValueClass || receiver.isDerivedValueClass
      def isBoolean(s: Symbol) = unboxedValueClass(s) == BooleanClass
      def isUnit(s: Symbol) = unboxedValueClass(s) == UnitClass
      def isNumeric(s: Symbol) =
        isNumericValueClass(unboxedValueClass(s)) || isAnyNumber(s)
      ...
      def isMaybeAnyValue(s: Symbol) =
        isPrimitiveValueClass(unboxedValueClass(s)) || isMaybeValue(s)
      ...
      def isEffectivelyFinalDeep(sym: Symbol): Boolean = (
        ...
        // OPT: this condition is not included in the standard isEffectivelyFinal check, as it's expensive
          || sym.isRefinementClass && sym.info.parents.exists {
            _.typeSymbol.isEffectivelyFinal
          }
      )
      ...
      def nonSensibleWarning(what: String, alwaysEqual: Boolean) = {
        ...
        reporter.warning(
          pos,
          s"comparing $what using `${name.decode}` will always yield $msg")
        isNonSensible = true
      }
      ...
      if (nullCount == 2) // null == null
        ...
        if (isNew(qual)) // new X == y
          ...
          nonSensiblyNew()
        else if (isEffectivelyFinalDeep(actual) && isEffectivelyFinalDeep(
                   receiver) && !haveSubclassRelationship) { // object X, Y; X == Y
          if (isEitherNullable)
      ...
      // possibleNumericCount is insufficient or this will warn on e.g. Boolean == j.l.Boolean
        ...
        def warnIfLubless(): Unit = {
          if (isMaybeValue(actual) || isMaybeValue(receiver) || haveSubclassRelationship) {} // ignore trivial or related types
            ...
            val commonRuntimeClass = erasedClass(
              global.lub(List(actual.tpe, receiver.tpe)))
        ...
        if (isCaseEquals) {
          ...
          actual.info.baseClasses.find(_.isCase) match {
            ...
            case None                     =>
              ...
              if (!actual.isTrait && !(receiver isSubClass actual))
                nonSensiblyNeq()
              else warnIfLubless()
            case _ =>
          }
    ...
    def checkSensible(pos: Position, fn: Tree, args: List[Tree]) = fn match {
      case Select(qual, name @ (nme.EQ | nme.NE | nme.eq | nme.ne))
          if args.length == 1 && isObjectOrAnyComparisonMethod(fn.symbol) && (!currentOwner.isSynthetic || currentOwner.isAnonymousFunction) =>
      ...
      if (!sym.isValueParameter && sym.paramss.isEmpty) {
        rhs match {
          case t @ (Ident(_) | Select(This(_), _))
              if t hasSymbolWhich (_.accessedOrSelf == sym) =>
            reporter.warning(
              rhs.pos,
              s"${sym.fullLocationString} does nothing other than call itself recursively")
          case _ =>
        }
      }
...
// Transformation ------------------------------------------------------------
      ...
      case ValDef(_, _, _, _) =>
        ...
        if (tree1.symbol.isLazy) tree1 :: Nil
          ...
          if (sym.isLocalToBlock && index <= currentLevel.maxindex) {
            ...
            reporter.error(
              currentLevel.refpos,
              "forward reference extends over definition of " + sym)
          }
          tree1 :: Nil
        }
      case Import(_, _) => Nil
    ...
    private def checkBounds(tree0: Tree,
                            pre: Type,
                            owner: Symbol,
                            tparams: List[Symbol],
    ...
    private def checkUndesiredProperties(sym: Symbol, pos: Position): Unit = {
      ...
      if (sym.isDeprecated && !currentOwner.ownerChain.exists(x =>
            x.isDeprecated))
      ...
      if (sym.hasMigrationAnnotation && settings.Xmigration.value != NoScalaVersion) {
        val changed =
          try settings.Xmigration.value < ScalaVersion(sym.migrationVersion.get)
          catch {
      ...
      if (sym.isCompileTimeOnly && !inAnnotation && !currentOwner.ownerChain
    ...
    private def checkDelayedInitSelect(qual: Tree,
                                       sym: Symbol,
                                       pos: Position) = {
    ...
    private def lessAccessibleSymsInType(other: Type,
    ...
    private def warnLessAccessible(otherSym: Symbol,
                                   memberSym: Symbol): Unit = {
    ...
    private def checkAccessibilityOfReferencedTypes(tree: Tree): Unit = {
      ...
      def checkAccessibilityOfType(tpe: Type): Unit = {
        ...
        else if ((tpe ne tpe.normalize) && lessAccessibleSymsInType(
                   tpe.dealiasWiden,
                   member).isEmpty) ()
    ...
    private def checkTypeRefBounds(tp: Type, tree: Tree) = {
      ...
      tp match {
        case AnnotatedType(ann :: Nil, underlying)
            if ann.symbol == UncheckedBoundsClass =>
          skipBounds = true
          underlying
        case TypeRef(pre, sym, args) =>
          if (!tp.isHigherKinded && !skipBounds)
    ...
    private def applyRefchecksToAnnotations(tree: Tree): Unit = {
      def applyChecks(annots: List[AnnotationInfo]): List[AnnotationInfo] = {
        ...
        val annotsBySymbol =
          new mutable.LinkedHashMap[Symbol, ListBuffer[AnnotationInfo]]()
        val transformedAnnots = {
          val saved = inAnnotation
          inAnnotation = true
          try annots.map(_.transformArgs(transformTrees))
          finally inAnnotation = saved
        }
        for (transformedAnnot <- transformedAnnots) {
          val buffer = annotsBySymbol.getOrElseUpdate(transformedAnnot.symbol,
                                                      new ListBuffer)
          buffer += transformedAnnot
        }
        annotsBySymbol.iterator
          .flatMap(x => groupRepeatableAnnotations(x._1, x._2.toList))
          .toList
      }

      // assumes non-empty `anns`
      def groupRepeatableAnnotations(
          sym: Symbol,
          anns: List[AnnotationInfo]): List[AnnotationInfo] =
              ...
              sym.getAnnotation(AnnotationRepeatableAttr) match {
                case Some(repeatable) =>
                  repeatable.assocs.collectFirst {
                    ...
                    case Some(container) =>
                      val assocs = List(
                        nme.value -> ArrayAnnotArg(
                          multiple.map(NestedAnnotArg(_)).toArray)
                      )
                ...
                case None =>
                  reporter.error(
                    tree.pos,
                    s"$sym may not appear multiple times on ${tree.symbol}")
                  multiple
              }
          }

      def checkIsElidable(sym: Symbol): Unit =
        if (sym ne null) sym.elisionLevel.foreach { level =>
          if (!sym.isMethod || sym.isAccessor || sym.isLazy || sym.isDeferred) {
            val rest =
              if (sym.isDeferred)
                " The annotation affects only the annotated method, not overriding methods in subclasses."
              else ""
            reporter.error(
              sym.pos,
              s"${sym.name}: Only concrete methods can be marked @elidable.$rest")
          }
        }
        ...
        case m: MemberDef =>
          ...
          if (settings.lintImplicitNotFound) {
            def messageWarning(name: String)(warn: String) =
              reporter.warning(
                tree.pos,
                s"Invalid $name message for ${sym}${sym.locationString}:\n$warn")
            analyzer.ImplicitNotFoundMsg.check(sym) foreach messageWarning(
              "implicitNotFound")
            analyzer.ImplicitAmbiguousMsg.check(sym) foreach messageWarning(
              "implicitAmbiguous")
          }
          ...
          if (settings.warnSerialization && sym.isClass && sym.hasAnnotation(
            ...
            def warn(what: String) =
              reporter.warning(tree.pos,
                               s"@SerialVersionUID has no effect on $what")
    ...
    private def isSimpleCaseApply(tree: Tree): Boolean = {
      ...
      !tree.tpe.finalResultType.typeSymbol.primaryConstructor
        .isLessAccessibleThan(tree.symbol)
    }
    ...
    private def transformApply(tree: Apply): Tree = tree match {
      case Apply(Select(qual, nme.withFilter),
                 List(
    ...
    private def transformIf(tree: If): Tree = {
      ...
      def unitIfEmpty(t: Tree): Tree =
        if (t == EmptyTree)
          Literal(Constant(())).setPos(tree.pos).setType(UnitTpe)
        else t
            ...
            tree match {
              case dd: DefDef if sym.hasAnnotation(NativeAttr) =>
                if (sym.owner.isTrait) {
                  ...
                  deriveDefDef(dd)(_ =>
                    typed(gen.mkThrowNewRuntimeException("native method stub")))
                } else
                  tree
              case _ => tree
            }
          ...
          case tpt @ TypeTree() =>
            ...
            if (!inPattern) tree.tpe foreach {
              ...
              case ann: AnnotatedType
                  if ann.hasAnnotation(UncheckedBoundsClass) =>
