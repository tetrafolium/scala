...
trait Infer extends Checkable {
  ...
  /** The formal parameter types corresponding to `formals`.
    ...
    */
  def formalTypes(formals: List[Type],
                  numArgs: Int,
                  removeByName: Boolean = true,
                  removeRepeated: Boolean = true): List[Type] = {
  ...
  def instantiateSamFromFunction(funTp: Type, samTp: Type, sam: Symbol) = {
    ...
    if (tparams.isEmpty) samTp
      ...
      funTp <:< functionType(samInfoWithTVars.paramTypes,
                             samInfoWithTVars.finalResultType)
      ...
      debuglog(
        s"sam infer: $samTp --> ${appliedType(samTyCon, targs)} by ${funTp} <:< $samInfoWithTVars --> $targs for $tparams")
  ...
  private def bestAlternatives(alternatives: List[Symbol])(
    ...
    alternatives sortWith improves match {
      case best :: rest if rest.nonEmpty =>
        best :: rest.filterNot(alt => improves(best, alt))
      case bests => bests
    }
  }
  ...
  private class DeferredNoInstance(getmsg: () => String)
      extends NoInstance("") {
  ...
  /** Solve constraint collected in types `tvars`.
    ...
    *  @throws NoInstance
    */
  def solvedTypes(tvars: List[TypeVar],
                  tparams: List[Symbol],
    ...
    if (tvars.isEmpty) Nil
      ...
      printTyping(
        "solving for " + parentheses(
          tparams.lazyZip(tvars).map((p, tv) => s"${p.name}: $tv")))
  ...
  abstract class Inferencer
      extends InferencerContextErrors
      with InferCheckable {
    def context: Context
    ...
    private def checkAccessibleError(tree: Tree,
                                     sym: Symbol,
                                     pre: Type,
                                     site: Tree): Tree = {
      if (settings.debug) {
        ...
        Console.println(
          "" + pre + " " + sym.owner + " " + context.owner + " " + context.outer.enclClass.owner + " " + sym.owner.thisType + (pre =:= sym.owner.thisType))
      }
    ...
    private def isCompatible(tp: Type, pt: Type): Boolean = {
      ...
      def isCompatibleSam(tp: Type, pt: Type): Boolean =
        (definitions.isFunctionType(tp) || tp.isInstanceOf[MethodType] || tp
...
//      println(s"isCompatibleArgs $res : $tps <:< $pts")
    ...
    /** Return inferred type arguments of polymorphic expression, given
      ...
      */
    private def exprTypeArgs(tvars: List[TypeVar],
                             tparams: List[Symbol],
                             restpe: Type,
                             pt: Type,
      ...
      if (if (useWeaklyCompatible) isWeaklyCompatible(resTpVars, pt)
          else isCompatible(resTpVars, pt)) {
    ...
    /** Overload which allocates fresh type vars.
      ...
      */
    private def exprTypeArgs(tparams: List[Symbol],
                             restpe: Type,
                             pt: Type,
                             useWeaklyCompatible: Boolean): List[Type] =
      exprTypeArgs(tparams map freshVar,
                   tparams,
                   restpe,
                   pt,
                   useWeaklyCompatible)
    ...
    /** Return inferred proto-type arguments of function, given
      ...
      */
    def protoTypeArgs(tparams: List[Symbol],
                      formals: List[Type],
                      restpe: Type,
                      pt: Type): List[Type] = {
    ...
    /** Retract arguments that were inferred to Nothing because inference failed. Correct types for repeated params.
      ...
      */
    def adjustTypeArgs(tparams: List[Symbol],
                       tvars: List[TypeVar],
                       targs: List[Type],
                       restpe: Type = WildcardType): AdjustedTypeArgs.Result = {
    ...
    /** Return inferred type arguments, given type parameters, formal parameters,
      ...
      *  @throws                  NoInstance
      */
    def methTypeArgs(fn: Tree,
                     tparams: List[Symbol],
                     formals: List[Type],
                     restpe: Type,
                     argtpes: List[Type],
                     pt: Type): AdjustedTypeArgs.Result = {
      val tvars = tparams map freshVar
      ...
      val targs = solvedTypes(tvars,
                              tparams,
                              tparams map varianceInTypes(formals),
                              upper = false,
                              lubDepth(formals) max lubDepth(argtpes))
      ...
      def canWarnAboutAny = {
        ...
        def containsAny(t: Type) =
          (t contains AnyClass) || (t contains AnyValClass) || (t contains ObjectClass)
    ...
    /** One must step carefully when assessing applicability due to
      ...
      */
    private[typechecker] def isApplicableBasedOnArity(
        tpe: Type,
        argsCount: Int,
        varargsStar: Boolean,
      ...
      case _ =>
        ...
        def tuplingMatch =
          tuplingAllowed && eligibleForTupleConversion(paramsCount,
                                                       argsCount,
                                                       varargsTarget)
    ...
    private[typechecker] def followApply(tp: Type): Type = tp match {
      case _ if tp.isError =>
        tp // scala/bug#8228, `ErrorType nonPrivateMember nme.apply` returns an member with an erroneous type!
    ...
    private def checkNames(
        argtpes: List[Type],
        params: List[Symbol]): (List[Type], Array[Int], Boolean) = {
    ...
    /** True if the given parameter list can accept a tupled argument list,
      ...
      */
    def eligibleForTupleConversion(paramsCount: Int,
                                   argsCount: Int,
    ...
    def eligibleForTupleConversion(formals: List[Type],
                                   argsCount: Int): Boolean = formals match {
      case p :: Nil =>
        eligibleForTupleConversion(1,
                                   argsCount,
    ...
    /** If the argument list needs to be tupled for the parameter list,
      ...
      */
    def tupleIfNecessary(formals: List[Type],
                         argtpes: List[Type]): List[Type] = {
      if (!argtpes.exists(isRepeatedParamType) && eligibleForTupleConversion(
            formals,
            argtpes.size))
    ...
    // type history.
    def enhanceBounds(okparams: List[Symbol],
                      okargs: List[Type],
                      undets: List[Symbol]): Unit =
    ...
    private def isApplicableToMethod(undetparams: List[Symbol],
                                     mt: MethodType,
                                     argtpes0: List[Type],
                                     pt: Type): Boolean = {
      val formals =
      ...
      def tryInstantiating(args: List[Type]) = falseIfNoInstance {
        ...
        val AdjustedTypeArgs.Undets(okparams, okargs, leftUndet) =
          methTypeArgs(EmptyTree, undetparams, formals, restpe, args, pt)
    ...
    /** Is there an instantiation of free type variables `undetparams` such that
      ...
      */
    private def isApplicable(undetparams: List[Symbol],
                             ftpe: Type,
                             argtpes0: List[Type],
                             pt: Type): Boolean = {
      val res =
...
//      println(s"isApplicable $res : $ftpe to $argtpes0 for $pt under $undetparams")
    ...
    private[typechecker] def isApplicableSafe(undetparams: List[Symbol],
                                              ftpe: Type,
                                              argtpes0: List[Type],
                                              pt: Type): Boolean = {
    ...
    def isAsSpecific(ftpe1: Type, ftpe2: Type): Boolean = {
      def checkIsApplicable(mt: MethodType) = {
        ...
        val aligned =
          if (isRepeatedParamType(paramTypes.last) && isVarArgsList(
                ftpe2.params))
            paramTypes.init :+ repeatedToSingle(paramTypes.last)
          else paramTypes
    ...
    private def isAsSpecificValueType(tpe1: Type,
                                      tpe2: Type,
                                      undef1: List[Symbol],
      ...
      tpe1 match {
        ...
        case _ =>
          tpe2 match {
            ...
            case _ if !currentRun.isScala300 =>
              ...
              val flip = new VariancedTypeMap {
                def apply(tp: Type): Type = tp match {
                  case TypeRef(pre, sym, args)
                      if variance > 0 && sym.typeParams.exists(
                        _.isContravariant) =>
    ...
    def isStrictlyMoreSpecific(ftpe1: Type,
                               ftpe2: Type,
                               sym1: Symbol,
                               sym2: Symbol): Boolean = {
...
//                                 (!ftpe2.isInstanceOf[OverloadedType] || ftpe1.isInstanceOf[OverloadedType]) &&
        ...
        val subClassCount = (if (isInProperSubClassOrObject(sym1, sym2)) 1
                             else 0) -
    ...
    def checkKindBounds(tparams: List[Symbol],
                        targs: List[Type],
                        pre: Type,
    ...
    /** Substitute free type variables `undetparams` of polymorphic argument
      ...
      */
    def inferArgumentInstance(tree: Tree,
                              undetparams: List[Symbol],
                              strictPt: Type,
                              lenientPt: Type): Unit = {
      printTyping(
        tree,
        s"inferring arg instance based on pt0=$strictPt, pt1=$lenientPt")
      var targs = exprTypeArgs(undetparams,
                               tree.tpe,
                               strictPt,
                               useWeaklyCompatible = false)
      if ((targs eq null) || !(tree.tpe.subst(undetparams, targs) <:< strictPt))
        targs = exprTypeArgs(undetparams,
                             tree.tpe,
                             lenientPt,
                             useWeaklyCompatible = false)
      ...
      printTyping(
        tree,
        s"infer arg instance from pt0=$strictPt, pt1=$lenientPt; targs=$targs")
    }
    ...
    def inferExprInstance(
      ...
      def targsStrict =
        if (targs eq null) null else targs mapConserve dropByName
    ...
    /** Substitute free type variables `undetparams` of polymorphic argument
      ...
      */
    private def substExpr(tree: Tree,
                          undetparams: List[Symbol],
                          targs: List[Type],
                          pt: Type): Unit = {
      if (targs eq null) {
    ...
    /** Substitute free type variables `undetparams` of application
      ...
      */
    def inferMethodInstance(fn: Tree,
                            undetparams: List[Symbol],
                            args: List[Tree],
    ...
    /** Substitute free type variables `undetparams` of type constructor
      ...
      */
    def inferConstructorInstance(tree: Tree,
                                 undetparams: List[Symbol],
                                 pt0: Type): Unit = {
      ...
      def inferFor(pt: Type): Option[List[Type]] = {
        ...
        if (resTpV <:< pt) {
          try {
            ...
            val variances =
              if (ctorTp.paramTypes.isEmpty)
                undetparams map varianceInType(ctorTp)
        ...
        if (isFullyDefined(pt)) {
          inferFor(
            ...
            if (isPopulated(resTpInst, ptV)) {
              ...
              debuglog(
                "isPopulated " + resTpInst + ", " + ptV + " vars= " + ptvars)
              Some(targs)
            } else None
          }
        } else None
    ...
    def instantiateTypeVar(tvar: TypeVar): Unit = {
      ...
      val instType =
        if (!tvar.instValid) Nil
        else {
          val inst = toOrigin(
            genPolyType(tparams, appliedType(tvar.inst, targs)))
      ...
      if (enclCase.savedTypeBounds.nonEmpty)
        log(sm"""|instantiateTypeVar with nonEmpty saved type bounds {
      ...
      if (lo1 <:< hi1) {
        if (lo1 <:< lo0 && hi0 <:< hi1) // bounds unimproved
          ...
          val tb =
            genPolyType(tparams, appliedType(TypeBounds(lo1, hi1), targs))
    ...
    def inferTypedPattern(tree0: Tree,
                          pattp: Type,
                          pt0: Type,
                          canRemedy: Boolean): Type = {
      val pt = abstractTypesToBounds(pt0)
      val ptparams = freeTypeParamsOfTerms(pt)
          ...
          bests match {
            ...
            case best :: competing :: _ if alts0.nonEmpty =>
              ...
              if (pt.isErroneous) setError(tree)
                ...
                AmbiguousExprAlternativeError(tree,
                                              pre,
                                              best,
                                              competing,
                                              pt,
    ...
    private def containsNamedType(argtpes: List[Type]): Boolean =
      argtpes match {
        case Nil                  => false
    ...
    /** Given a list of argument types and eligible method overloads, whittle the
      ...
      */
    private def overloadsToConsiderBySpecificity(
        eligible: List[Symbol],
        argtpes: List[Type],
        varargsStar: Boolean): List[Symbol] = {
      ...
      if (eligible.isEmpty || eligible.tail.isEmpty) eligible
        ...
        namesMatch match {
          case namesMatch if namesMatch.nonEmpty =>
            namesMatch // TODO: this has no basis in the spec, remove!
          case _ =>
    ...
    /** Assign `tree` the type of an alternative which is applicable
      ...
      */
    def inferMethodAlternative(tree: Tree,
                               undetparams: List[Symbol],
                               argtpes0: List[Type],
                               pt0: Type): Unit = {
      ...
      class InferMethodAlternativeTwice extends c.TryTwice {
        ...
        private def bestForExpectedType(pt: Type, isLastTry: Boolean): Unit = {
          val applicable = overloadsToConsiderBySpecificity(
            alts filter isAltApplicable(pt),
            argtpes,
            varargsStar)
        ...
        private[this] val pt =
          if (pt0.typeSymbol == UnitClass) WildcardType else pt0
  ...
  object AdjustedTypeArgs {
    ...
    private def toLists[A1, A2](pxs: (Iterable[A1], Iterable[A2])) =
      (pxs._1.toList, pxs._2.toList)
    private def toLists[A1, A2, A3](
        pxs: (Iterable[A1], Iterable[A2], Iterable[A3])) =
      (pxs._1.toList, pxs._2.toList, pxs._3.toList)
    private def toLists[A1, A2, A3, A4](
        pxs: (Iterable[A1], Iterable[A2], Iterable[A3], Iterable[A4])) =
      (pxs._1.toList, pxs._2.toList, pxs._3.toList, pxs._4.toList)
  }
