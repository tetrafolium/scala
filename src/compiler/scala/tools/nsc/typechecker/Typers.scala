...
trait Typers
  ...
  // this class.
  private val superConstructorCalls
    : mutable.AnyRefMap[Symbol, collection.Map[Symbol, Symbol]] =
  ...
  class SilentTypeError private (val errors: List[AbsTypeError],
                                 val warnings: List[(Position, String)])
      extends SilentResult[Nothing] {
  ...
  object SilentTypeError {
    def apply(errors: AbsTypeError*): SilentTypeError =
      apply(errors.toList, Nil)
    def apply(errors: List[AbsTypeError],
              warnings: List[(Position, String)]): SilentTypeError =
      new SilentTypeError(errors, warnings)
    // todo: this extracts only one error, should be a separate extractor.
    def unapply(error: SilentTypeError): Option[AbsTypeError] =
      error.errors.headOption
  }
  ...
  object checkNoEscaping extends TypeMap {
    ...
    def privates[T <: Tree](typer: Typer, owner: Symbol, tree: T): T =
      ...
      else check(typer, owner, EmptyScope, WildcardType, tree)

    private def check[T <: Tree](typer: Typer,
                                 owner: Symbol,
                                 scope: Scope,
                                 pt: Type,
                                 tree: T): T = {
      ...
      def checkNoEscape(sym: Symbol): Unit = {
        if (sym.isPrivate && !sym.hasFlag(SYNTHETIC_PRIVATE)) {
          ...
          while (o != NoSymbol && o != sym.owner && o != sym.owner.linkedClassOfClass &&
                 ...
                 !o.privateWithin.hasTransOwner(sym.owner)) o = o.owner
  ...
  private final val typerFreshNameCreators =
    perRunCaches.newAnyRefMap[Symbol, FreshNameCreator]()
  def freshNameCreatorFor(context: Context) =
    typerFreshNameCreators.getOrElseUpdate(
      context.outermostContextAtCurrentPos.enclClassOrMethod.owner,
      new FreshNameCreator)

  abstract class Typer(context0: Context)
      extends TyperDiagnostics
      with Adaptation
      with Tag
      with PatternTyper
      with TyperContextErrors {
    ...
    private def isSyntheticAccessor(sym: Symbol) =
      sym.isAccessor && (!sym.isLazy || isPastTyper)

    private def explicitlyUnit(tree: Tree): Boolean =
      tree.hasAttachment[TypedExpectingUnitAttachment.type]
        ...
        for (param <- params) {
          ...
          val res =
            if (paramFailed || (paramTp.isErroneous && {
              ...
              inferImplicitFor(paramTp,
                               fun,
                               context,
                               reportAmbiguous = context.reportErrors)
          argResultsBuff += res
    ...
    /** Infer an implicit conversion (`view`) between two types.
      ...
      */
    def inferView(tree: Tree,
                  from: Type,
                  to: Type,
                  reportAmbiguous: Boolean = true,
                  saveErrors: Boolean = true): Tree =
      if (isPastTyper || from.isInstanceOf[MethodType] || from
            .isInstanceOf[OverloadedType] || from.isInstanceOf[PolyType])
        EmptyTree
      else {
        debuglog(
          s"Inferring view from $from to $to for $tree (reportAmbiguous= $reportAmbiguous, saveErrors=$saveErrors)")

        val fromNoAnnot = from.withoutAnnotations
        val toNorm = normalizeProtoForView(to)
        val result = inferImplicitView(fromNoAnnot,
                                       toNorm,
                                       tree,
                                       context,
                                       reportAmbiguous,
                                       saveErrors) match {
          case fail if fail.isFailure =>
            inferImplicitView(byNameType(fromNoAnnot),
                              toNorm,
                              tree,
                              context,
                              reportAmbiguous,
    ...
    def dropExistential(tp: Type): Type = tp match {
      ...
      case TypeRef(_, sym, _) if sym.isAliasType =>
        ...
        if (tp eq tp0) {
          devWarning(
            s"dropExistential did not progress dealiasing $tp, see scala/bug#7126")
          tp
        } else {
    ...
    def checkNonCyclic(pos: Position, tp: Type): Boolean = {
      ...
      tp match {
        case TypeRef(pre, sym, args) =>
          checkNotLocked(sym) &&
            ((!sym.isNonClassType) || checkNonCyclic(
              pos,
              appliedType(pre.memberInfo(sym), args),
              sym))
    ...
    def qualifyingClass(tree: Tree,
      ...
      context.enclClass.owner.ownerChain.find(o =>
        ...
        case _ =>
          ...
          if (immediate) setError(tree)
          else unit.toCheck += (() => setError(tree))
          NoSymbol
      }
    ...
    @inline
    final def typerWithCondLocalContext[T](c: => Context)(cond: Boolean)(
        f: Typer => T): T =
    ...
    /** Make symbol accessible. This means:
      ...
      */
    private def makeAccessible(tree: Tree,
                               sym: Symbol,
                               pre: Type,
                               site: Tree): (Tree, Type) =
        ...
        val qual = typedQualifier {
          atPos(tree.pos.makeTransparent) {
            tree match {
              case Ident(_) =>
                val packageObject =
                  if (!sym.isOverloaded && sym.owner.isModuleClass)
                    sym.owner.sourceModule // historical optimization, perhaps no longer needed
    ...
    /** Post-process an identifier or selection node, performing the following:
      ...
      */
    protected def stabilize(tree: Tree,
                            pre: Type,
                            mode: Mode,
                            pt: Type): Tree = {
    ...
    private def isNarrowable(tpe: Type): Boolean =
      unwrapWrapperTypes(tpe) match {
    ...
    def member(qual: Type, name: Name): Symbol = {
      def callSiteWithinClass(clazz: Symbol) =
        context.enclClass.owner hasTransOwner clazz
    ...
    def silent[T](op: Typer => T,
      ...
      val rawTypeStart =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startCounter(rawTypeFailed)
        else null
      val findMemberStart =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startCounter(findMemberFailed)
        else null
      val subtypeStart =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startCounter(subtypeFailed)
        else null
      val failedSilentStart =
        if (StatisticsStatics.areSomeColdStatsEnabled)
          statistics.startTimer(failedSilentNanos)
        else null
      def stopStats() = {
        if (StatisticsStatics.areSomeColdStatsEnabled)
      ...
      try {
        if (context.reportErrors ||
          ...
          assert(context.bufferErrors || isPastTyper,
                 "silent mode is not available past typer")
    ...
    /** Check whether feature given by `featureTrait` is enabled.
      ...
      */
    def checkFeature(pos: Position,
                     featureTrait: Symbol,
                     construct: => String = "",
                     immediate: Boolean = false): Boolean =
      isPastTyper || {
        val nestedOwners =
          featureTrait.owner.ownerChain
            .takeWhile(_ != languageFeatureModule.moduleClass)
            .reverse
        val featureName = (nestedOwners map (_.name + ".")).mkString + featureTrait.name
        def action(): Boolean = {
          def hasImport =
            inferImplicitByType(featureTrait.tpe, context).isSuccess
    ...
    def checkExistentialsFeature(pos: Position, tpe: Type, prefix: String) =
      tpe match {
        case extp: ExistentialType
            if !extp.isRepresentableWithWildcards && !tpe.isError =>
          checkFeature(pos,
                       currentRun.runDefinitions.ExistentialsFeature,
                       prefix + " " + tpe)
        case _ =>
      }
    ...
    protected def adapt(tree: Tree,
            ...
            if (original != EmptyTree && !pt.isWildcard) {
              typer1 silent { tpr =>
                ...
                _ =>
                  ...
                  typerWithLocalContext(resetContext) {
                    typer1 =>
                      val tree1 = typer1.typed(resetTree, mode)
                      // Q: `typed` already calls `pluginsTyped` and `adapt`. the only difference here is that
                      // we pass `EmptyTree` as the `original`. intended? added in 2009 (53d98e7d42) by martin.
                      tree1 setType pluginsTyped(tree1.tpe,
                                                 typer1,
                                                 tree1,
                                                 mode,
                                                 pt)
                      if (tree1.isEmpty) tree1
                      else typer1.adapt(tree1, mode, pt)
                  }
              }
            } else
      ...
      def adaptMethodTypeToExpr(mt: MethodType): Tree = {
        ...
        def warnEtaSam() = {
          ...
          if (sam.exists && (!samClazz.hasFlag(JAVA) || samClazz.hasFlag(
                INTERFACE)) && !samClazz.hasAnnotation(
        ...
        def checkCanEtaExpand(): Boolean = {
          ...
          val expectingFunctionOfArity = {
            ...
            (ptSym eq FunctionClass(arity)) || (arity > 0 && (ptSym eq FunctionClass(
              1))) // allowing for tupling conversion
          }
          ...
          val doIt =
            if (arity == 0) {
              ...
              if (doEtaZero && settings.warnEtaZero) {
                val ptHelp =
                  if (expectingFunctionOfArity) pt
                    ...
                    s"$pt, which is SAM-equivalent to ${samToFunctionType(pt)}"
              ...
              sourceLevel2_14 || expectingFunctionOfArity || expectingSamOfArity

          if (doIt && !expectingFunctionOfArity && settings.warnEtaSam)
            warnEtaSam()
          ...
          if (sourceLevel2_14 && !meth.isJavaDefined)
            context.deprecationWarning(
              ...
              s"Auto-application to `()` is deprecated. Supply the empty argument list `()` explicitly to invoke method ${meth.decodedName},\n" +
                ...
                s"In Scala 3, an unapplied method like this will be eta-expanded into a function.",
              "2.14.0"
            )
          true
        }

        if (!meth.isConstructor && checkCanEtaExpand())
          ...
          MissingArgsForMethodTpeError(tree, meth) // `context.implicitsEnabled` implies we are not in a pattern
        else UnstableTreeError(tree)
      }
      ...
      def adaptConstant(value: Constant): Tree = {
        ...
        tree match {
          ...
          case _                =>
            ...
            treeCopy
              .Literal(tree, value)
              .updateAttachment(OriginalTreeAttachment(tree))
        }
      }
      ...
      def adaptExprNotFunMode(): Tree = {
        ...
        if (mode.typingExprNotFun && !tree.tpe.isErroneous) {
          @inline def tpdPos(transformed: Tree) =
            typedPos(tree.pos, mode, pt)(transformed)
          ...
          @inline def warnValueDiscard(): Unit =
            if (!isPastTyper && settings.warnValueDiscard) {
              def isThisTypeResult = (tree, tree.tpe) match {
                case (Apply(Select(receiver, _), _), SingleType(_, sym)) =>
                  sym == receiver.symbol
                case _ => false
              }
          ...
          pt.dealias match {
            ...
            case TypeRef(_, numValueCls, _)
                if anyTyped && isNumericValueClass(numValueCls) && isNumericSubType(
                  tree.tpe,
                  pt) => // (10) (11)
              warnNumericWiden(); tpdPos(Select(tree, s"to${numValueCls.name}"))
            case dealiased
                if dealiased.annotations.nonEmpty && canAdaptAnnotations(
                  tree,
                  this,
                  mode,
                  pt) => // (13)
              ...
              if (hasUndets) instantiate(tree, mode, pt)
                ...
                if (didInferSamType) tree
                  ...
                  if (coercion ne EmptyTree) {
                    def msg =
                      s"inferred view from ${tree.tpe} to $pt via $coercion: ${coercion.tpe}"
                    if (settings.logImplicitConv) context.echo(tree.pos, msg)
                    else debuglog(msg)

                    val viewApplied = new ApplyImplicitView(
                      coercion,
                      List(tree)) setPos tree.pos
                    val silentContext =
                      context.makeImplicit(context.ambiguousErrors)
                    val typedView =
                      newTyper(silentContext).typed(viewApplied, mode, pt)
      ...
      def vanillaAdapt(tree: Tree) = {
        def applyPossible = {
          ...
          def hasPolymorphicApply =
            applyMeth.alternatives exists (_.tpe.typeParams.nonEmpty)
          def hasMonomorphicApply =
            applyMeth.alternatives exists (_.tpe.paramSectionCount > 0)

          def badDynamicApply() = {
            tree match {
              case Apply(fun, _) =>
                DynamicRewriteError(tree, ApplyWithoutArgsError(tree, fun))
              case _ => ()
            }
            false
          }
        ...
        if (tree.isType)
          adaptType()
                 ...
                 !tree.isDef && tree.symbol != null && currentRun.runDefinitions
                   .treatLikeMacro(tree.symbol) // TODO: remove once we bootstrap and StringContext.s/raw are defined as macro ???
        ...
        else if (isScalaRepeatedParamType(tree.tpe) && !isScalaRepeatedParamType(
                   pt)) adapt(tree setType (repeatedToSeq(tree.tpe)), mode, pt)
        else if (tree.tpe <:< pt)
          tree
      ...
      if (isMacroImplRef(tree)) {
        if (treeInfo.isMacroApplication(tree))
          adapt(unmarkMacroImplRef(tree), mode, pt, original)
        else tree
      } else
        tree.tpe match {
          case atp @ AnnotatedType(_, _)
              if canAdaptAnnotations(tree, this, mode, pt) => // (-1)
            adaptAnnotations(tree, this, mode, pt)
          case ct @ FoldableConstantType(value)
          ...
          case NullaryMethodType(restpe) => // (2)
            val resTpDeconst =
              if (isPastTyper || (tree.symbol.isAccessor && tree.symbol.hasFlag(
                    STABLE) && treeInfo.isExprSafeToInline(tree))) restpe
              else restpe.deconst
            adapt(tree setType resTpDeconst, mode, pt, original)
          case TypeRef(_, ByNameParamClass, arg :: Nil)
              if mode.inExprMode => // (2)
            adapt(tree setType arg, mode, pt, original)
          case tp
              if mode.typingExprNotLhs && isExistentialType(tp) && !isSyntheticAccessor(
                context.owner) =>
            adapt(
              tree setType tp.dealias.skolemizeExistential(context.owner, tree),
              mode,
              pt,
              original)
          ...
          case mt: MethodType
              if mode.typingExprNotFunNotLhs && mt.isImplicit => // (4.1)
            adaptToImplicitMethod(mt)
          case mt: MethodType
              if mode.typingExprNotFunNotLhs && !hasUndetsInMonoMode && !treeInfo
                .isMacroApplicationOrBlock(tree) => // (4.2) - (4.3)
    ...
    def adaptAfterOverloadResolution(tree: Tree,
                                     mode: Mode,
    ...
    def instantiateExpectingUnit(tree: Tree, mode: Mode): Tree = {
      ...
      silent(_.instantiate(tree, mode, UnitTpe)) orElse { _ =>
        ...
        val valueDiscard = atPos(tree.pos)(
          gen.mkUnitBlock(instantiate(tree, mode, WildcardType)))
        typed(valueDiscard, mode, UnitTpe)
      }
    }

    def instantiatePossiblyExpectingUnit(tree: Tree,
                                         mode: Mode,
                                         pt: Type): Tree = {
    ...
    /** Try to apply an implicit conversion to `qual` to that it contains
      ...
      *
      */
    def adaptToArguments(qual: Tree,
                         name: Name,
                         args: List[Tree],
                         pt: Type,
                         reportAmbiguous: Boolean = true,
      ...
      def doAdapt(restpe: Type) =
        ...
        adaptToMember(qual,
                      HasMethodMatching(name,
                                        args map (_.tpe),
                                        normalizeProtoForView(restpe)),
                      reportAmbiguous,
                      saveErrors)
      ...
      if (pt.isWildcard)
        ...
        silent(_ => doAdapt(pt)) filter (_ != qual) orElse (_ =>
          logResult(s"fallback on implicits in adaptToArguments: $qual.$name")(
    ...
    /** Try to apply an implicit conversion to `qual` so that it contains
      ...
      */
    def adaptToMemberWithArgs(tree: Tree,
                              qual: Tree,
                              name: Name,
                              mode: Mode,
                              reportAmbiguous: Boolean = true,
    ...
    private def checkEphemeral(clazz: Symbol, body: List[Tree]) = {
      ...
      def where =
        if (isValueClass) "value class"
        else "universal trait extending from class Any"
      def implRestriction(tree: Tree, what: String) =
        context.error(
          tree.pos,
          s"implementation restriction: $what is not allowed in $where" +
      ...
      object checkEphemeralDeep extends Traverser {
        override def traverse(tree: Tree): Unit = if (isValueClass) {
          tree match {
            ...
            case cd: ClassDef
                if !cd.symbol.isAnonymousClass => // Don't warn about partial functions, etc. scala/bug#7571
              implRestriction(tree, "nested class") // avoiding Type Tests that might check the $outer pointer.
            case Select(sup @ Super(qual, mix), selector)
                if selector != nme.CONSTRUCTOR && qual.symbol == clazz && mix != tpnme.EMPTY =>
      ...
      for (stat <- body) {
        def notAllowed(what: String) =
          context.error(stat.pos, s"$what is not allowed in $where")
        stat match {
          // see https://github.com/scala/bug/issues/6444
          // see https://github.com/scala/bug/issues/6463
          case ClassDef(mods, _, _, _) if isValueClass =>
            implRestriction(stat,
                            s"nested ${if (mods.isTrait) "trait" else "class"}")
          ...
          case DefDef(_, name, _, _, _, rhs) =>
            if (stat.symbol.isAuxiliaryConstructor)
              ...
              notAllowed(
                s"redefinition of $name method. See SIP-15, criterion 4.")
    ...
    private def validateDerivedValueClass(clazz: Symbol, body: List[Tree]) = {
      if (clazz.isTrait)
        context.error(clazz.pos,
                      "only classes (not traits) are allowed to extend AnyVal")
      if (!clazz.isStatic)
        context.error(clazz.pos,
        ...
        clazz.primaryConstructor.paramss match {
          case List(List(param)) =>
            ...
            if (paramAccessor.isMutable)
              context.error(paramAccessor.pos,
                            "value class parameter must not be a var")
            val accessor = decls.toList.find(x =>
              x.isMethod && x.accessedOrSelf == paramAccessor)
            accessor match {
              case None =>
                context.error(
                  paramAccessor.pos,
              ...
              case Some(acc) if acc.isProtectedLocal =>
                context.error(
                  paramAccessor.pos,
                  "value class parameter must not be protected[this]")
              case Some(acc) =>
                ...
                checkEphemeral(
                  clazz,
                  body filterNot (stat =>
                    stat.symbol != null && stat.symbol.accessedOrSelf == paramAccessor))
            }
          case _ =>
            context.error(clazz.pos,
      ...
      for (tparam <- clazz.typeParams)
        if (tparam hasAnnotation definitions.SpecializedClass)
          context.error(tparam.pos,
                        "type parameter of value class may not be specialized")
    }
    ...
    /** Typechecks a parent type reference.
      ...
      */
    private def typedParentType(encodedtpt: Tree,
                                templ: Template,
                                inMixinPosition: Boolean): Tree = {
      ...
      if (probe.isTrait || inMixinPosition) {
        ...
        val supertparams =
          if (supertpt.hasSymbolField) supertpt.symbol.typeParams else Nil
        def inferParentTypeArgs: Tree = {
          typedPrimaryConstrBody(templ) {
            val supertpe =
              PolyType(supertparams,
                       appliedType(supertpt.tpe, supertparams map (_.tpeHK)))
            val supercall = New(supertpe, mmap(argss)(_.duplicate))
            val treeInfo.Applied(Select(ctor, nme.CONSTRUCTOR), _, _) =
              supercall
            ...
            case tpt =>
              TypeTree(tpt.tpe) setPos supertpt.pos // scala/bug#7224: don't .focus positions of the TypeTree of a parent that exists in source
          }
        }

        val supertptWithTargs =
          if (supertparams.isEmpty || context.unit.isJava) supertpt
          else inferParentTypeArgs

        // this is the place where we tell the typer what argss should be used for the super call
        // if argss are nullary or empty, then (see the docs for `typedPrimaryConstrBody`)
        // the super call dummy is already good enough, so we don't need to do anything
        if (argssAreTrivial) supertptWithTargs
        else supertptWithTargs updateAttachment SuperArgsAttachment(argss)
      }
    }
    ...
    /** Typechecks the mishmash of trees that happen to be stuffed into the primary constructor of a given template.
      ...
      */
    private def typedPrimaryConstrBody(templ: Template)(
        actualSuperCall: => Tree): Tree =
      treeInfo.firstConstructor(templ.body) match {
        case ctor @ DefDef(_,
                           ...
                           vparamss,
                           _,
                           cbody @ Block(cstats, cunit)) =>
          val (preSuperStats, superCall) = {
            val (stats, rest) = cstats span (x =>
              !treeInfo.isSuperConstrCall(x))
            (stats map (_.duplicate),
             if (rest.isEmpty) EmptyTree else rest.head.duplicate)
          }
          ...
          if (dummy.isTopLevel)
            currentRun.symSource(dummy) = currentUnit.source.file
          val cbody2 = { // called both during completion AND typing.
            val typer1 = newTyper(cscope)
            // XXX: see about using the class's symbol....
            clazz.unsafeTypeParams foreach (sym =>
              typer1.context.scope.enter(sym))
          ...
          if (preSuperVals.isEmpty && preSuperStats.nonEmpty)
            devWarning(
              "Wanted to zip empty presuper val list with " + preSuperStats)
          else
            map2(preSuperStats, preSuperVals)((ldef, gdef) =>
              gdef.tpt setType ldef.symbol.tpe)
    ...
    private def normalizeFirstParent(parents: List[Tree]): List[Tree] = {
      ...
      def explode0(parents: List[Tree]): List[Tree] = {
        ...
        if (supertpt.tpe.typeSymbol == AnyClass) {
          ...
          def supersuper =
            TypeTree(supertpt1.tpe.firstParent) setPos supertpt.pos.focus
    ...
    /** Certain parents are added in the parser before it is known whether
      ...
      */
    private def fixDuplicateSyntheticParents(parents: List[Tree]): List[Tree] =
      parents match {
        case Nil => Nil
        case x :: xs =>
          val sym = x.symbol
    ...
    def validateParentClasses(parents: List[Tree], selfType: Type): Unit = {
      ...
      def validateDynamicParent(parent: Symbol, parentPos: Position) =
        if (parent == DynamicClass)
          checkFeature(parentPos, currentRun.runDefinitions.DynamicsFeature)
      ...
      def validateParentClass(parent: Tree, superclazz: Symbol) =
        if (!parent.isErrorTyped) {
          ...
          if (psym != superclazz) {
            if (context.unit.isJava && psym.isJavaAnnotation) {
              ...
              if (!ps.isEmpty && !superclazz.isSubClass(ps.head.typeSymbol))
                pending += ParentSuperSubclassError(parent,
                                                    superclazz,
                                                    ps.head.typeSymbol,
                                                    psym)
            } else {
          ...
          if (!isPastTyper && psym.hasDeprecatedInheritanceAnnotation &&
              !sameSourceFile && !context.owner.ownerChain.exists(
                _.isDeprecated)) {
            val version = psym.deprecatedInheritanceVersion.getOrElse("")
            val since = if (version.isEmpty) version else s" (since $version)"
            val message = psym.deprecatedInheritanceMessage
              .map(msg => s": $msg")
              .getOrElse("")
            val report =
              s"inheritance from ${psym.fullLocationString} is deprecated$since$message"
      ...
      for (tparam <- clazz.typeParams) {
        if (classinfo.expansiveRefs(tparam) contains tparam) {
          val newinfo = ClassInfoType(
            classinfo.parents map (_.instantiateTypeParams(List(tparam),
                                                           List(AnyRefTpe))),
            classinfo.decls,
            clazz)
    ...
    private def updatePolyClassInfo(clazz: Symbol,
                                    newinfo: ClassInfoType): clazz.type = {
      clazz.setInfo {
        clazz.info match {
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      try {
        ...
        val impl1 = newTyper(context.make(cdef.impl, clazz, newScope))
          .typedTemplate(cdef.impl, typedParentTypes(cdef.impl))
        ...
        treeCopy
          .ClassDef(cdef, typedMods, cdef.name, tparams1, impl2)
          .setType(NoType)
      } finally {
    ...
    def typedModuleDef(mdef: ModuleDef): Tree = {
      ...
      if (linkedClass != NoSymbol)
        linkedClass.info
          ...
          .alternatives foreach (_.initialize)
    ...
    private def ensurePredefParentsAreInSameSourceFile(template: Template) = {
      ...
      if (parentSyms exists (_.associatedFile != PredefModuleFile))
        context.error(
          template.pos,
          s"All parents of Predef must be defined in ${PredefModuleFile}.")
    }
    ...
    /** In order to override this in the TreeCheckers Typer so synthetics aren't re-added
      ...
      */
    protected def finishMethodSynthesis(templ: Template,
                                        clazz: Symbol,
...
//        println(s"updating parents of $clazz from ${clazz.info.parents} to $parentTypes")
        updatePolyClassInfo(clazz,
                            ClassInfoType(parentTypes, clazz.info.decls, clazz))
      }
      ...
      val body1 =
        pluginsEnterStats(this, namer.expandMacroAnnotations(templ.body))
      ...
      val bodyWithPrimaryCtor = {
        ...
        val primaryCtor1 = primaryCtor match {
          case DefDef(_,
            ...
            val pos = wrappingPos(parents1.head.pos,
                                  primaryCtor :: argss.flatten).makeTransparent
            val superCall = atPos(pos)(PrimarySuperCall(argss))
            deriveDefDef(primaryCtor)(block =>
              Block(earlyVals :+ superCall, unit) setPos pos) setPos pos
          case _ => primaryCtor
        }
        body1 mapConserve {
      ...
      if (!clazz.isTrait && clazz.isNonBottomSubClass(ConstantAnnotationClass)) {
        val (pConstrOpt, auxConstrs) = body3
          .filter(s => s.isInstanceOf[DefDef] && s.symbol.isConstructor)
          .splitAt(1)
    ...
    def typedValDef(vdef: ValDef): ValDef = {
      ...
      try {
        val valDefTyper = {
          val maybeConstrCtx =
            if ((sym.isParameter || sym.isEarlyInitialized) && sym.owner.isConstructor)
    ...
    private def typedValDefImpl(vdef: ValDef) = {
      ...
      val typedMods =
        if (nme.isLocalName(sym.name) && sym.isPrivateThis && !vdef.mods.isPrivateLocal) {
          ...
          typedModifiers(
            vdef.mods.copy(flags = sym.flags, privateWithin = tpnme.EMPTY))
      ...
      val vdef1 = treeCopy.ValDef(vdef,
                                  typedMods,
                                  sym.name,
                                  tpt1,
                                  checkDead(context, rhs1)) setType NoType
    ...
    def analyzeSuperConsructor(meth: Symbol,
      ...
      def decompose(call: Tree): (Tree, List[Tree]) = call match {
        ...
        case Apply(fn, args) =>
          ...
          val applyArgs =
            if (args.sizeCompare(params) < 0) args :+ EmptyTree
            else args take params.length

          assert(
            sameLength(applyArgs, params) || call.isErrorTyped,
          ...
          if (sameLength(superParamAccessors, superArgs)) {
            ...
            for ((superAcc, superArg @ Ident(name)) <- superParamAccessors zip superArgs) {
              if (mexists(vparamss)(_.symbol == superArg.symbol)) {
                val ownAcc =
                  clazz.info decl name suchThat (_.isParamAccessor) match {
                ...
                ownAcc match {
                  case acc: TermSymbol
                      if !acc.isVariable && !isByNameParamType(acc.info) =>
    ...
    /**
      ...
      */
    private def foreachSubTreeBoundTo[A](trees: List[Tree], clazz: Symbol)(
        f: Tree => Unit): Unit =
      for {
        tree <- trees
        subTree <- tree
    ...
    def checkMethodStructuralCompatible(ddef: DefDef): Unit = {
      ...
      def parentString =
        meth.owner.parentSymbols filterNot (_ == ObjectClass) match {
          case Nil => ""
      ...
      def failStruct(pos: Position,
                     what: String,
                     where: String = "Parameter type") =
          ...
          def checkAbstract(tp0: Type, what: String): Boolean = {
            def check(sym: Symbol): Boolean = !sym.isAbstractType || {
              ...
              ((!sym.hasTransOwner(meth.owner) && failStruct(
                ...
                what))
              || (!sym.hasTransOwner(meth) && failStruct(
                paramPos,
                "a type member of that refinement",
                what))
    ...
    def typedDefDef(ddef: DefDef): DefDef = {
      ...
      try {
        ...
        if (!isPastTyper && meth.isPrimaryConstructor) {
          for (vparams <- ddef.vparamss; vd <- vparams) {
            if (vd.mods.isParamAccessor) {
              vd.symbol setAnnotations (vd.symbol.annotations filter AnnotationInfo
                .mkFilter(ParamTargetClass, defaultRetention = true))
            }
          }
        }
        ...
        if (meth.isClassConstructor && !isPastTyper && !meth.owner.isSubClass(
              AnyValClass) && !meth.isJava) {
        ...
        if (!isPastTyper) {
          ...
          if (meth.isImplicit && !meth.isSynthetic) meth.info.paramss match {
            case List(param) :: _ if !param.isImplicit =>
              checkFeature(ddef.pos,
                           currentRun.runDefinitions.ImplicitConversionsFeature,
                           meth.toString)
            case _ =>
          }
        }
    ...
    def typedTypeDef(tdef: TypeDef): TypeDef =
      typerWithCondLocalContext(context.makeNewScope(tdef, tdef.symbol))(
        tdef.tparams.nonEmpty) {
    ...
    private def typedTypeDefImpl(tdef: TypeDef): TypeDef = {
      ...
      val rhs1 =
        checkNoEscaping.privates(this, tdef.symbol, typedType(tdef.rhs))
    ...
    def typedBlock(block0: Block, mode: Mode, pt: Type): Block = {
      ...
      try {
        ...
        val block = treeCopy.Block(
          block0,
          pluginsEnterStats(this, namer.expandMacroAnnotations(block0.stats)),
          block0.expr)
        ...
        if (phaseId(currentPeriod) <= currentRun.typerPhase.id) {
          ...
          // other.
          block match {
            case Block(List(classDef @ ClassDef(_, _, _, _)),
              ...
              def matchesVisibleMember(member: Symbol) = visibleMembers exists {
                vis =>
        ...
        val statsTyped =
          typedStats(block.stats, context.owner, warnPure = false)
        ...
        if (!isPastTyper) {
          ...
          def checkPure(t: Tree, supple: Boolean): Unit =
            if (!explicitlyUnit(t) && treeInfo.isPureExprForWarningPurposes(t)) {
              ...
              val parens =
                if (statsTyped.length + count > 1)
                  "multiline expressions might require enclosing parentheses"
                else ""
              val discard =
                if (adapted)
                  "; a value can be silently discarded when Unit is expected"
                else ""
              val text =
                if (supple) s"${parens}${discard}"
                else if (!parens.isEmpty) s"${msg}; ${parens}"
                else msg
        ...
        val statsTyped2 = statsTyped match {
          case (vd: ValDef) :: Nil
              if inlinedRightAssocValDefs.remove(vd.symbol) =>
            Nil
          case _ => statsTyped
        }

        treeCopy
          .Block(block, statsTyped2, expr1)
          .setType(
            if (treeInfo.isExprSafeToInline(block)) expr1.tpe
    ...
    def typedCase(cdef: CaseDef, pattpe: Type, pt: Type): CaseDef = {
      ...
      val guard1: Tree =
        if (cdef.guard == EmptyTree) EmptyTree
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def adaptCase(cdef: CaseDef, mode: Mode, tpe: Type): CaseDef =
      deriveCaseDef(cdef)(adapt(_, mode, tpe))

    def packedTypes(trees: List[Tree]): List[Type] =
      trees map (c => packedType(c, context.owner).deconst)

    // takes untyped sub-trees of a match and type checks them
    def typedMatch(selector: Tree,
                   cases: List[CaseDef],
                   mode: Mode,
                   pt: Type,
                   tree: Tree = EmptyTree): Match = {
      val selector1 = checkDead(context, typedByValueExpr(selector))
      val selectorTp = packCaptured(selector1.tpe.widen)
        .skolemizeExistential(context.owner, selector)
    ...
    /** synthesize and type check a PartialFunction implementation based on the match in `tree`
      ...
      */
    def synthesizePartialFunction(paramName: TermName,
                                  paramPos: Position,
                                  paramSynthetic: Boolean,
                                  tree: Tree,
                                  mode: Mode,
                                  pt: Type): Tree = {
      assert(
        pt.typeSymbol == PartialFunctionClass,
        s"PartialFunction synthesis for match in $tree requires PartialFunction expected type, but got $pt.")
      ...
      def applyOrElseMethodDef = {
        val methodSym =
          anonClass.newMethod(nme.applyOrElse, tree.pos, FINAL | OVERRIDE)

        // create the parameter that corresponds to the function's parameter
        val A1 = methodSym newTypeParameter (newTypeName("A1")) setInfo TypeBounds
          .upper(argTp)
        val x = mkParam(methodSym, A1.tpe)

        // applyOrElse's default parameter:
        val B1 = methodSym newTypeParameter (newTypeName("B1")) setInfo TypeBounds.empty
        val default = methodSym newValueParameter (newTermName("default"), tree.pos.focus, SYNTHETIC) setInfo functionType(
          List(A1.tpe),
          B1.tpe)

        val paramSyms = List(x, default)
        methodSym setInfo genPolyType(List(A1, B1),
        ...
        val match_ = {
          val cdef = mkDefaultCase(
            methodBodyTyper
              .typed1(REF(default) APPLY (REF(x)), mode, B1.tpe)
              .setType(B1.tpe))
          val List(defaultCase) =
            methodBodyTyper.typedCases(List(cdef), argTp, B1.tpe)
        ...
        val originals: Map[Symbol, Tree] = {
          def typedIdent(sym: Symbol) =
            methodBodyTyper.typedType(Ident(sym), mode)
        ...
        val defdef = newDefDef(methodSym, match_)(
          vparamss = mapParamss(methodSym)(newParam),
          tpt = TypeTree(B1.tpe))
      ...
      def isDefinedAtMethod = {
        val methodSym =
          anonClass.newMethod(nme.isDefinedAt, tree.pos.makeTransparent, FINAL)
        ...
        val match_ = methodBodyTyper.typedMatch(selector(paramSym),
                                                casesTrue :+ defaultCase,
                                                mode,
                                                BooleanTpe)
      ...
      def applyMethod = {
        val methodSym =
          anonClass.newMethod(nme.apply, tree.pos, FINAL | OVERRIDE)
        val paramSym = mkParam(methodSym)
        ...
        val match_ =
          methodBodyTyper.typedMatch(selector(paramSym), cases, mode, resTp)
      ...
      def parents(resTp: Type) =
        addSerializable(
          appliedType(AbstractPartialFunctionClass.typeConstructor,
                      List(argTp, resTp)))
      ...
      val members = {
        ...
        anonClass setInfo ClassInfoType(parents(matchResTp),
                                        newScope,
                                        anonClass)
      ...
      if (typedBlock.isErrorTyped) typedBlock
        ...
        typedPos(tree.pos, mode, pt) {
          Typed(typedBlock,
                TypeTree(typedBlock.tpe baseType PartialFunctionClass))
        }
    }
    ...
    def inferSamType(fun: Tree, pt: Type, mode: Mode): Boolean = fun match {
      case fun @ Function(vparams, _) if !isFunctionType(pt) =>
        ...
        if (!samMatchesFunctionBasedOnArity(sam, vparams)) false
          ...
          def fullyDefinedMeetsExpectedFunTp(pt: Type): Boolean =
            isFullyDefined(pt) && {
              val samMethType = pt memberInfo sam
              fun.tpe <:< functionType(samMethType.paramTypes,
                                       samMethType.resultType)
            }
          ...
          val samTp =
            if (!sam.exists) NoType
              ...
              try {
                val ptFullyDefined =
                  instantiateSamFromFunction(fun.tpe, pt, sam)
                if (ptFullyDefined <:< pt && fullyDefinedMeetsExpectedFunTp(
                      ptFullyDefined)) {
                  debuglog(
                    s"sam fully defined expected type: $ptFullyDefined from $pt for ${fun.tpe}")
                  ptFullyDefined
                } else {
                  debuglog(
                    s"Could not define type $pt using ${fun.tpe} <:< ${pt memberInfo sam} (for $sam)")
                  NoType
                }
              } catch {
                case e @ (_: NoInstance | _: TypeError) =>
                  debuglog(
                    s"Error during SAM synthesis: could not define type $pt using ${fun.tpe} <:< ${pt memberInfo sam} (for $sam)\n$e")
                  NoType
              }
    ...
    /**
      ...
      */
    private def argsResProtosFromFun(pt: Type,
                                     numVparams: Int): (List[Type], Type) =
      pt match {
        case pt: OverloadedArgProto
            if pt.hofParamTypes.lengthCompare(numVparams) == 0 =>
          (pt.hofParamTypes, WildcardType)
        case _ =>
          ...
          unwrapWrapperTypes(ptNorm baseType FunctionSymbol) match {
            case TypeRef(_, _, args :+ res) =>
              (args, res) // if it's a TypeRef, we know its symbol will be FunctionSymbol
            case _ => {
          ...
          if (paramsMissingType.nonEmpty && pt != ErrorType) {
            ...
            fun.body match {
              ...
              case Match(sel, cases)
                  if (sel ne EmptyTree) && (pt.typeSymbol == PartialFunctionClass) =>
                ...
                val p = vparams.head
                if (p.tpt.tpe == null)
                  p.tpt setType outerTyper.typedType(p.tpt).tpe

                outerTyper.synthesizePartialFunction(p.name,
                                                     p.pos,
                                                     paramSynthetic = false,
                                                     fun.body,
                                                     mode,
                                                     pt)
    ...
    /** Retry typedFunction when parameter types are missing, and they might be recovered from
      ...
      */
    private def typedFunctionUndoingEtaExpansion(fun: Function,
                                                 mode: Mode,
                                                 pt: Type,
                                                 argProtos: List[Type],
                                                 resProto: Type) = {
      ...
      fun.body match {
        ...
        case Apply(meth, args) =>
          ...
          val formalsFromApply =
            vparams.map { vd =>
              if (!vd.tpt.isEmpty) Right(vd.tpt.tpe)
                ...
                Left(args.indexWhere {
                  case Ident(name) => name == vd.name
                  case _ =>
                    false // TODO: this does not catch eta-expansion of an overloaded method that involves numeric widening scala/bug#9738 (and maybe similar patterns?)
                })
            }
          ...
          if (formalsFromApply.contains(Left(-1))) EmptyTree
            ...
            val methArgs = NoSymbol.newSyntheticValueParams(args map {
              case _ => WildcardType
            })

            silent(
              _.typed(meth, mode.forFunMode, MethodType(methArgs, resProto)))
              .fold(EmptyTree: Tree) { methTyped =>
                    ...
                    case TypeRef(_, _, argProtos :+ _) =>
                      val argProtosRecovered =
                        formalsFromApply.map {
                          case Left(idx) =>
                            val argPt =
                              if (argProtos.isDefinedAt(idx)) argProtos(idx)
                              else
                                NoType // bounds check should not be needed due to expected type `MethodType(methArgs, resProto)` above
                            if (isFullyDefined(argPt)) argPt else NoType
                          case Right(tp) => tp
                        }

                      if (argProtosRecovered contains NoType)
                        EmptyTree // cannot safely recurse
                      else {
                        ...
                        typedFunction(
                          treeCopy.Function(
                            fun,
                            vparams,
                            treeCopy.Apply(fun.body, methTyped, args)),
                          mode,
                          funPt)
                      }
                    case _ => EmptyTree
          ...
          val resTp =
            packedType(bodyTyped, fun.symbol).resultType.deconst match {
              case ct: ConstantType
                  if (bodyPt eq WildcardType) || (ct.widen <:< bodyPt) =>
                ct.widen
              case tp => tp
            }
    ...
    def typedEtaExpansion(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      if (tree.tpe.isDependentMethodType)
        DependentMethodTpeConversionToFunctionError(tree, tree.tpe) // TODO: support this
      else {
    ...
    def typedRefinement(templ: Template): Unit = {
      ...
      unit.toCheck += { () =>
        ...
        val att = templ.attachments
          .get[CompoundTypeTreeOriginalAttachment]
    ...
    def typedStats(stats: List[Tree],
                   exprOwner: Symbol,
                   warnPure: Boolean = true): List[Tree] = {
      ...
      def typedStat(stat: Tree): Tree = stat match {
        case s
            if context.owner.isRefinementClass && !treeInfo
              .isDeclarationOrTypeDef(s) =>
          OnlyDeclarationsError(s)
        case imp @ Import(_, _) =>
        ...
        case _ =>
          val localTyper =
            if (inBlock || (stat.isDef && !stat.isInstanceOf[LabelDef])) this
          ...
          if (treeInfo.isSelfOrSuperConstrCall(result)) {
            ...
            if (treeInfo.isSelfConstrCall(result)) {
              ...
              else if (result.symbol.pos
                         .pointOrElse(0) >= exprOwner.enclMethod.pos
                         .pointOrElse(0))
                ConstructorsOrderError(stat)
            }
          }
          if (warnPure && !isPastTyper && treeInfo.isPureExprForWarningPurposes(
                result)) {
            val msg = "a pure expression does nothing in statement position"
            val clause =
              if (stats.lengthCompare(1) > 0)
      ...
      def addSynthetics(stats: List[Tree], scope: Scope): List[Tree] = {
        ...
        while (moreToAdd) {
          ...
          def shouldAdd(sym: Symbol): Boolean = {
            ...
            (!sym.isModule || shouldAddAsModule) && (inBlock || !context
              .isInPackageObject(sym, context.owner))
          }
          for (sym <- scope)
        ...
        if (newStats.isEmpty) stats
          ...
          def matches(stat: Tree, synt: Tree) = (stat, synt) match {
            ...
            case (DefDef(_, statName, _, _, _, _),
    ...
    /** Does function need to be instantiated, because a missing parameter
      ...
      */
    def needsInstantiation(tparams: List[Symbol],
                           formals: List[Type],
                           args: List[Tree]) = {
      def isLowerBounded(tparam: Symbol) =
        !tparam.info.lowerBound.typeSymbol.isBottomClass
    ...
    def doTypedApply(tree: Tree,
      ...
      def preSelectOverloaded(fun: Tree): Tree = {
        if (fun.hasSymbolField && fun.symbol.isOverloaded) {
          ...
          var sym = fun.symbol filter { alt =>
            ...
            isApplicableSafe(context.undetparams,
                             followApply(pre memberType alt),
                             argtypes,
                             pt)
          }
          if (sym.isOverloaded) {
          ...
          if (sym == NoSymbol) fun
            ...
            adaptAfterOverloadResolution(
              fun setSymbol sym setType pre.memberType(sym),
              mode.forFunMode)
        } else fun
      }
      ...
      fun.tpe match {
          ...
          def handleOverloaded = {
            ...
            val (args1, argTpes) = context.savingUndeterminedTypeParams() {
              ...
              mapWithIndex(args) { (arg, argIdx) =>
                def typedArg0(
                    tree: Tree,
                    argIdxOrName: Either[Int, Name] = Left(argIdx)) = {
                  val argTyped =
                    typedArg(tree,
                             amode,
                             BYVALmode,
                             OverloadedArgProto(argIdxOrName, pre, alts))
            ...
            if (context.reporter.hasErrors)
              ...
              doTypedApply(tree,
                           adaptAfterOverloadResolution(fun,
                                                        mode.forFunMode,
                                                        WildcardType),
                           args1,
                           mode,
        ...
        case _
            if currentRun.runDefinitions.isPolymorphicSignature(fun.symbol) =>
          ...
          val cloneParams = pts map (pt =>
            clone.newValueParameter(freshTermName()).setInfo(pt))
        ...
        case mt @ MethodType(params, _) =>
          ...
          def tryTupleApply: Tree =
            if (phase.erasedTypes || !eligibleForTupleConversion(paramTypes,
                                                                 argslen))
              EmptyTree
            else {
              val tupleArgs = List(
                atPos(tree.pos.makeTransparent)(gen.mkTuple(args)))
              ...
              def validate(t: Tree): Tree = {
                ...
                val invalidAdaptation = t.symbol != null && !checkValidAdaptation(
                  t,
                  args)
              ...
              silent(_.doTypedApply(tree, fun, tupleArgs, mode, pt))
                .map(validate)
                .orElse(reset)
            }
          ...
          def tryNamesDefaults: Tree = {
            ...
            def checkNotMacro() = {
              if (treeInfo.isMacroApplication(fun))
                tryTupleApply orElse duplErrorTree(
                  NamedAndDefaultArgumentsNotSupportedForMacros(tree, fun))
            }
            ...
            if (mt.isErroneous) duplErrTree
              ...
              tryTupleApply orElse duplErrorTree {
                val (namelessArgs, argPos) =
                  removeNames(Typer.this)(args, params)
                TooManyArgsNamesDefaultsError(tree,
                                              fun,
                                              formals,
                                              args,
                                              namelessArgs,
              ...
              if (namelessArgs exists (_.isErroneous)) {
                ...
                transformNamedApplication(Typer.this, mode, pt)(
                  treeCopy.Apply(tree, fun, namelessArgs),
                  argPos)
              }
            } else {
              ...
              def ownerOf(sym: Symbol) =
                if (sym == null || sym == NoSymbol) NoSymbol else sym.owner
              val symsOwnedByContextOwner = tree.collect {
                case t @ (_: DefTree | _: Function)
                    if ownerOf(t.symbol) == context.owner =>
                  t.symbol
              }
              ...
              val fun1 =
                transformNamedApplication(Typer.this, mode, pt)(fun, x => x)
              if (fun1.isErroneous) duplErrTree
              else {
                val NamedApplyBlock(
                  NamedApplyInfo(qual, targs, previousArgss, _)) = fun1
                ...
                val (allArgs, missing) = addDefaults(args,
                                                     qual,
                                                     targs,
                                                     previousArgss,
                                                     params,
                ...
                if (!sameLength(allArgs, args) && callToCompanionConstr(
                  ...
                  tryTupleApply orElse duplErrorTree(
                    NotEnoughArgsError(tree, fun, missing))
                }
              }
            }
          }
          ...
          if (!sameLength(formals, args) || // wrong nb of arguments
              ...
              NamedApplyBlock
                .unapply(fun)
                .isDefined) { // fun was transformed to a named apply block =>
            // integrate this application into the block
            if (isApplyDynamicNamed(fun) && isDynamicRewrite(fun))
              typedNamedApply(tree, fun, args, mode, pt)
            ...
            if (tparams.isEmpty) { // all type params are defined
              def handleMonomorphicCall: Tree = {
                ...
                val restpe = mt.resultType(
                  mapList(args1)(arg => gen stableTypeFor arg orElse arg.tpe))
                ...
                args1 match {
                  case List(lit: Literal) =>
                    lit.attachments.get[OriginalTreeAttachment] match {
                      case Some(OriginalTreeAttachment(id: Ident))
                          if rightAssocValDefs.contains(id.symbol) =>
                ...
                if (args.isEmpty && canTranslateEmptyListToNil && fun.symbol.isInitialized && currentRun.runDefinitions
                      .isListApply(fun))
                  atPos(tree.pos)(gen.mkNil setType restpe)
                else {
                  val resTp = ifPatternSkipFormals(if (isPastTyper) restpe
                  else restpe.deconst) // annoying issue with classOf that shouldn't be deconsted after typers (during fields phase)
                  constfold(
                    treeCopy.Apply(tree, fun, args2) setType resTp setPos pos2)
                }
              }
              if (settings.warnDeadCode) {
                val sym = fun.symbol
                if (sym != null && sym.isMethod && !sym.isConstructor) {
                  val suppress = sym == Object_synchronized || (sym.isLabel && treeInfo
                    .isSynthCaseSymbol(sym))
              ...
              def handlePolymorphicCall = {
                ...
                val lenientTargs =
                  protoTypeArgs(tparams, formals, mt.resultApprox, pt)
                ...
                def typedArgToPoly(arg: Tree, formal: Type): Tree = { //TR TODO: cleanup
                  ...
                  if (remainingParams.tail.nonEmpty)
                    remainingParams = remainingParams.tail
                  val arg1 =
                    typedArg(arg, forArgMode(fun, mode), newmode, lenientPt)
                  val argtparams = context.extractUndetparams()
                  if (!argtparams.isEmpty) {
                    val strictPt =
                      formal.instantiateTypeParams(tparams, strictTargs)
        ...
        case _ =>
          if (treeInfo.isMacroApplication(tree))
            duplErrorTree(MacroTooManyArgumentListsError(tree, fun.symbol))
    ...
    def typedAnnotation(ann: Tree, mode: Mode = EXPRmode): AnnotationInfo =
      context.withinAnnotation {
         ...
         */
        def tree2ConstArg(tree: Tree, pt: Type): Option[ClassfileAnnotArg] =
          tree match {
            ...
            case ann @ Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
              ...
              if (!annType.typeSymbol.isSubClass(pt.typeSymbol))
                reportAnnotationError(
                  AnnotationTypeMismatchError(tpt, annType, annType))
        ...
        def trees2ConstArg(trees: List[Tree],
                           pt: Type): Option[ArrayAnnotArg] = {
        ...
        val typedFunPart =
          (
            ...
            if (targs.isEmpty && typedFun0.exists(
                  t => t.tpe != null && isDummyAppliedType(t.tpe)))
        ...
        finish(
          if (typedFun.isErroneous || annType == null)
            ErroneousAnnotation
          else if (isJava || annTypeSym.isNonBottomSubClass(
                     ConstantAnnotationClass)) {
            ...
            if (argss.lengthIs > 1) {
              ...
              val annScopeJava =
                if (isJava)
                  annType.decls.filter(sym =>
                    sym.isMethod && !sym.isConstructor && sym.isJavaDefined)
              ...
              val namedArgs = argss match {
                case List(List(arg)) if !isNamedArg(arg) && hasValue =>
              ...
              val nvPairs = namedArgs map {
                ...
                case arg =>
                  reportAnnotationError(
                    ClassfileAnnotationsAsNamedArgsError(arg))
                  (nme.ERROR, None)
              }
              for (sym <- names) {
                ...
                sym.initialize
                if (!sym
                      .hasAnnotation(AnnotationDefaultAttr) && !sym.hasDefault)
                  reportAnnotationError(
                    AnnotationMissingArgError(ann, annType, sym))
              }
            ...
            val typedAnn: Tree = {
              ...
              val localTyper = newTyper(
                context.make(ann, context.owner.newLocalDummy(ann.pos)))
              localTyper.typed(
                ann,
                mode,
                deriveTypeWithWildcards(context.undetparams)(annType))
            }
            def annInfo(t: Tree): AnnotationInfo = t match {
              case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
                // `tpt.tpe` is more precise than `annType`, since it incorporates the types of `args`
                AnnotationInfo(tpt.tpe, args, List())
                  .setOriginal(typedAnn)
                  .setPos(t.pos)
      ...
      if (!isPastTyper) {
        if (clazz != null && (clazz isNonBottomSubClass AnnotationClass)) {
          ...
          if (macroTransform != NoSymbol) {
            clazz.setFlag(MACRO)
            if (clazz.getAnnotation(CompileTimeOnlyAttr).isEmpty)
              clazz.addAnnotation(
                AnnotationInfo(CompileTimeOnlyAttr.tpe,
                               List(Literal(Constant(
                                 MacroAnnotationNotExpandedMessage)) setType StringClass.tpe),
                               Nil))
            ...
            if (clazz.getAnnotation(InheritedAttr).nonEmpty)
              MacroAnnotationCannotBeInheritedError(clazz)
    ...
    def packSymbols(hidden: List[Symbol], tp: Type): Type =
      global.packSymbols(hidden, tp, context.owner)
    ...
    def packedType(tree: Tree, owner: Symbol): Type = {
      ...
      def isLocal(sym: Symbol): Boolean =
        if (sym == NoSymbol || sym.isRefinementClass || sym.isLocalDummy) false
          ...
          containsDef(owner, sym) || isRawParameter(sym) || isCapturedExistential(
            sym)
      ...
      val dealiasLocals = new TypeMap {
        def apply(tp: Type): Type = tp match {
          case TypeRef(pre, sym, args) =>
            if (sym.isAliasType && containsLocal(tp) && (tp.dealias ne tp))
              apply(tp.dealias)
            else {
              if (pre.isVolatile) pre match {
                case SingleType(_, sym) if sym.isSynthetic && isPastTyper =>
                  debuglog(
                    s"ignoring volatility of prefix in pattern matcher generated inferred type: $tp") // See pos/t7459c.scala
                case _ =>
    ...
    protected def typedExistentialTypeTree(tree: ExistentialTypeTree,
                                           mode: Mode): Tree = {
      ...
      existentialTransform(whereClauses1 map (_.symbol), tpt1.tpe)(
        (tparams, tp) => {
    ...
    protected def typedTypeApply(tree: Tree,
                                 mode: Mode,
                                 fun: Tree,
                                 args: List[Tree]): Tree = fun.tpe match {
      case OverloadedType(pre, alts) =>
      ...
      case PolyType(tparams, restpe) if tparams.nonEmpty =>
        if (sameLength(tparams, args)) {
          ...
          if (fun.symbol.rawname == nme.classOf && currentRun.runDefinitions
                .isPredefClassOf(fun.symbol))
    ...
    private def acceptsApplyDynamic(tp: Type) =
      tp.typeSymbol isNonBottomSubClass DynamicClass
    ...
    /** Returns `Some(t)` if `name` can be selected dynamically on `qual`, `None` if not.
      ...
      */
    private def acceptsApplyDynamicWithType(qual: Tree,
    ...
    private def isApplyDynamicNamed(fun: Tree): Boolean = fun match {
      case DynamicApplicationNamed(qual, _) =>
        acceptsApplyDynamic(qual.tpe.widen)
      case _ => false
      // look deeper?
    ...
    private def typedNamedApply(orig: Tree,
                                fun: Tree,
                                args: List[Tree],
                                mode: Mode,
                                pt: Type): Tree = {
      ...
      val t =
        treeCopy.Apply(orig, unmarkDynamicRewrite(fun), args map argToBinding)
      wrapErrors(t, _.typed(t, mode, pt))
    }
    ...
    /** Translate selection that does not typecheck according to the normal rules into a selectDynamic/applyDynamic.
      ...
      *
      */
    private def mkInvoke(context: Context,
                         tree: Tree,
                         qual: Tree,
                         name: Name): Option[Tree] = {
      ...
      acceptsApplyDynamicWithType(qual, name) map { tp =>
        ...
        def matches(t: Tree) =
          isDesugaredApply || treeInfo.dissectApplied(t).core == treeSelection
        ...
        def findSelection(t: Tree): Option[(TermName, Tree)] = t match {
          case Apply(fn, args) if matches(fn) =>
            val op =
              if (args.exists(_.isInstanceOf[NamedArg])) nme.applyDynamicNamed
              else nme.applyDynamic
            // not supported: foo.bar(a1,..., an: _*)
            val fn1 =
              if (treeInfo.isWildcardStarArgList(args))
                DynamicVarArgUnsupported(fn, op)
              else fn
            Some((op, fn1))
        ...
        findSelection(cxTree) map {
          case (opName, treeInfo.Applied(_, targs, _)) =>
            ...
            val nameStringLit = atPos(
              treeSelection.pos
                .withStart(treeSelection.pos.point)
                .makeTransparent) {
          ...
          devWarning(
            s"Tree $tree not found in the context $cxTree while trying to do a dynamic application")
          setError(tree)
        }
      }
    }
    private def wrapErrors(tree: Tree, typeTree: Typer => Tree): Tree =
      silent(typeTree) orElse (err => DynamicRewriteError(tree, err.head))
    //
    ...
    def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def lookupInRoot(name: Name): Symbol =
        lookupInOwner(rootMirror.RootClass, name)
      def lookupInEmpty(name: Name): Symbol =
        rootMirror.EmptyPackageClass.info member name
      ...
      def typedAnnotated(atd: Annotated): Tree = {
        ...
        if (arg1.isType) {
          ...
          def setInlineAttachment(t: Tree,
                                  att: InlineAnnotatedAttachment): Unit =
            t match {
              case TypeApply(fun, _) => setInlineAttachment(fun, att)
              case _                 => t.updateAttachment(att)
            }
          if (atype.hasAnnotation(definitions.ScalaNoInlineClass))
            setInlineAttachment(arg1, NoInlineCallsiteAttachment)
          else if (atype.hasAnnotation(definitions.ScalaInlineClass))
            setInlineAttachment(arg1, InlineCallsiteAttachment)
      ...
      def typedBind(tree: Bind) =
        tree match {
          case Bind(name: TypeName, body) =>
            assert(
              body == EmptyTree,
              s"${context.unit} typedBind: ${name.debugString} ${body} ${body.getClass}")
            val sym =
      ...
      def typedAssign(lhs: Tree, rhs: Tree): Tree = {
        ...
        val lhs1 = unsuppressMacroExpansion(
          typedLhs(suppressMacroExpansion(lhs)))
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def typedIf(tree: If): If = {
        ...
        if (tree.elsep.isEmpty)
          return treeCopy.If(tree,
                             cond1,
                             typed(tree.thenp, UnitTpe),
                             tree.elsep) setType UnitTpe
        ...
        def samePackedTypes = (
          !isPastTyper
            ...
            && packedType(thenp1, context.owner) =:= packedType(elsep1,
                                                                context.owner)
        )
        def finish(ownType: Type) =
        ...
        // Important to deconst, otherwise `if (???) 0 else 0` evaluates to 0 (scala/bug#6331)
          ...
          thenp1.tpe.deconst :: elsep1.tpe.deconst :: Nil match {
            ...
            case tpes =>
              ...
              treeCopy.If(tree,
                          cond1,
                          adapt(thenp1, mode, lub),
                          adapt(elsep1, mode, lub)) setType lub
          }
      }
      ...
      def typedVirtualizedMatch(tree: Match): Tree = {
        ...
        if (selector == EmptyTree) {
          if (pt.typeSymbol == PartialFunctionClass)
            synthesizePartialFunction(newTermName(fresh.newName("x")),
                                      tree.pos,
                                      paramSynthetic = true,
                                      tree,
                                      mode,
            ...
            val arity = functionArityFromType(pt) match {
              case -1    => 1
              case arity => arity
            } // scala/bug#8429: consider sam and function type equally in determining function arity
            ...
            val selector1 = atPos(tree.pos.focusStart) {
              if (arity == 1) ids.head else gen.mkTuple(ids)
            }
      ...
      def typedReturn(tree: Return) = {
        ...
        if (enclMethod == NoContext ||
          ...
          if (restpt.tpe eq null) {
            ...
            val expr1 = context withinReturnExpr typedByValueExpr(expr,
                                                                  restpt.tpe)
            ...
            if (restpt.tpe.typeSymbol == UnitClass) {
              ...
              if (typed(expr).tpe.typeSymbol != UnitClass)
                context.warning(
                  tree.pos,
                  "enclosing method " + name + " has result type Unit: return value discarded")
            }
            val res = treeCopy
              .Return(tree, checkDead(context, expr1))
              .setSymbol(enclMethod.owner)
      ...
      def typedNew(tree: New) = {
        ...
        def narrowRhs(tp: Type) = {
          ...
          context.tree match {
            case ValDef(mods, _, _, Apply(Select(`tree`, _), _))
                if !mods.isMutable && sym != null && sym != NoSymbol =>
              val sym1 =
                if (sym.owner.isClass && sym.getterIn(sym.owner) != NoSymbol)
                  sym.getterIn(sym.owner)
                else sym
              val pre =
                if (sym1.owner.isClass) sym1.owner.thisType else NoPrefix
      ...
      def typedEta(methodValue: Tree): Tree = methodValue.tpe match {
        case tp @ (MethodType(_, _) | PolyType(_, MethodType(_, _))) => // (1)
          if (tp.params.lengthCompare(definitions.MaxFunctionArity) > 0)
            MaxFunctionArityError(
              methodValue,
              s"; method ${methodValue.symbol.name} cannot be eta-expanded because it takes ${tp.params.length} arguments")
          else {
            val etaPt =
              pt match {
                case pt: ProtoType =>
                  pt.asFunctionType orElse functionType(
                    tp.params.map(_ => WildcardType),
                    WildcardType) orElse WildcardType // arity overflow --> NoType
                case _ => pt
              }
        ...
        case TypeRef(_, ByNameParamClass, _) | NullaryMethodType(_) => // (2)
          ...
          typed(Function(List(), methodValue) setSymbol funSym setPos pos,
                mode,
                pt)
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        val start =
          if (StatisticsStatics.areSomeColdStatsEnabled)
            statistics.startTimer(failedApplyNanos)
          else null

        def onError(typeErrors: Seq[AbsTypeError],
                    warnings: Seq[(Position, String)]): Tree = {
          if (StatisticsStatics.areSomeColdStatsEnabled)
          ...
          def errorInResult(tree: Tree) =
            treesInResult(tree).exists(err =>
              typeErrors.exists(_.errPos samePointAs err.pos))
          ...
          if (retry) {
            ...
            tryTypedArgs(args, forArgMode(fun, mode)) match {
              case Some(args1)
                  if !args1.exists(arg => arg.exists(_.isErroneous)) =>
                val qual1 =
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        val opeqStart =
          if (StatisticsStatics.areSomeColdStatsEnabled)
            statistics.startTimer(failedOpEqNanos)
          else null

        def isConversionCandidate(qual: Tree, name: Name): Boolean =
          !mode.inPatternMode && nme.isOpAssignmentName(TermName(name.decode)) && !qual
            .exists(_.isErroneous)
        ...
        def advice1(convo: Tree,
                    errors: List[AbsTypeError],
                    err: SilentTypeError): List[AbsTypeError] =
          errors.map { e =>
            if (e.errPos samePointAs tree.pos) {
              val header =
                f"${e.errMsg}%n  Expression does not convert to assignment because:%n    "
              val expansion = f"%n    expansion: ${show(convo)}"
              NormalTypeError(tree,
                              err.errors
                                .flatMap(_.errMsg.linesIterator.toList)
                                .mkString(header, f"%n    ", expansion))
            } else e
          }
        def advice2(errors: List[AbsTypeError]): List[AbsTypeError] =
          errors.map { e =>
            if (e.errPos samePointAs tree.pos) {
              val msg =
                f"${e.errMsg}%n  Expression does not convert to assignment because receiver is not assignable."
        ...
        def onError(error: SilentTypeError): Tree = fun match {
          case Select(qual, name) if isConversionCandidate(qual, name) =>
            ...
            if (treeInfo.isVariableOrGetter(qual1)) {
              ...
              val erred = qual1.exists(_.isErroneous) || args.exists(
                _.isErroneous)
              if (erred) reportError(error)
              else {
                ...
                silent(op = _.typed1(convo, mode, pt)) match {
                  ...
                  case err: SilentTypeError =>
                    reportError(
                      SilentTypeError(advice1(convo, error.errors, err),
                                      error.warnings)
                    )
                }
              }
              ...
              val erred = qual2.exists(_.isErroneous) || args2.exists(
                _.isErroneous)
              reportError {
                if (erred) error
                else SilentTypeError(advice2(error.errors), error.warnings)
              }
            }
          case _ =>
            if (StatisticsStatics.areSomeColdStatsEnabled)
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            val fun2 =
              if (stableApplication) stabilizeFun(fun1, mode, pt) else fun1
            if (StatisticsStatics.areSomeColdStatsEnabled)
        ...
        case Apply(fun, args) =>
          normalTypedApply(tree, fun, args) match {
            case treeInfo.ArrayInstantiation(level, componentType, arg) =>
              ...
              val tagType = (1 until level).foldLeft(componentType)((res, _) =>
                arrayType(res))
              ...
              val tree1: Tree = resolveClassTag(tree.pos, tagType) match {
                ...
                case tag =>
                  atPos(tree.pos)(
                    new ApplyToImplicitArgs(Select(tag, nme.newArray),
                                            arg :: Nil))
              }
              if (tree1.isErrorTyped) tree1 else typed(tree1, mode, pt)
            case Apply(Select(fun, nme.apply), _)
                if treeInfo.isSuperConstrCall(fun) =>
              TooManyArgumentListsForConstructor(tree) //scala/bug#5696
            case tree1
                if mode.inPatternMode && tree1.tpe.paramSectionCount > 0 =>
      ...
      def convertToAssignment(fun: Tree,
                              qual: Tree,
                              name: Name,
                              args: List[Tree]): Tree = {
        ...
        def mkUpdate(table: Tree,
                     indices: List[Tree],
                     args_? : Option[List[Tree]]) =
          gen.evalOnceAll(table :: indices, context.owner, fresh) {
            case tab :: is =>
              ...
              mkCall(
                nme.update,
                Apply(
                  Select(mkApplies(mkCall(nme.apply)), prefix) setPos fun.pos,
        ...
        val assignment = qual match {
          ...
          case Apply(fn, extra) if qual.isInstanceOf[ApplyToImplicitArgs] =>
            fn match {
              case treeInfo.Applied(Select(table, nme.apply),
                                    _,
                                    indices :: Nil) =>
      ...
      def typedSuper(tree: Super) = {
        ...
        def findMixinSuper(site: Type): Type = {
          ...
          if (ps.isEmpty) {
            debuglog(
              "Fatal: couldn't find site " + site + " in " + site.parents.map(
                _.typeSymbol.name))
            if (phase.erasedTypes && context.enclClass.owner.isTrait) {
              ...
              restrictionError(
                tree.pos,
                unit,
                "traits may not select fields or methods from super[C] where C is a class")
              ErrorType
            } else {
      ...
      def typedThis(tree: This) =
        tree.symbol orElse qualifyingClass(tree,
                                           tree.qual,
                                           packageOK = false,
                                           immediate = true) match {
          case NoSymbol => tree
          case clazz =>
      ...
      def typedSelect(tree: Tree, qual: Tree, name: Name): Tree = {
        ...
        if ((qualTp eq null) || qualTp.isError) setError(tree)
          ...
          if (tree.isInstanceOf[SelectFromTypeTree])
          ...
          val sym = tree.symbol orElse member(qual.tpe, name) orElse inCompanionForJavaStatic(
            qual.symbol,
            name)
          if ((sym eq NoSymbol) && name != nme.CONSTRUCTOR && mode.inAny(
                EXPRmode | PATTERNmode)) {
          ...
          if (phase.erasedTypes && qual.isInstanceOf[Super]) {
            ...
            val qualSym =
              tree.getAndRemoveAttachment[QualTypeSymAttachment] match {
                case Some(a) => a.sym
                case None    => sym.owner
              }
          ...
          if (!reallyExists(sym)) {
            def handleMissing: Tree = {
              ...
              asTypeSelection orElse asDynamicCall getOrElse (lookupInQualifier(
                qual,
                name) match {
            ...
            if ((sym ne NoSymbol) && !qual.tpe.isStable && argsDependOnPrefix(
                  sym)) {
              ...
              if (insertionContext != NoContext) {
                val vsym = insertionContext.owner.newValue(
                  freshTermName(nme.STABILIZER_PREFIX),
                  qual.pos.focus,
                  SYNTHETIC | ARTIFACT | STABLE)
                vsym.setInfo(uncheckedBounds(qual.tpe))
                insertionContext.scope enter vsym
                val vdef = atPos(vsym.pos)(
                  ValDef(vsym, focusInPlace(qual)) setType NoType)
            ...
            val (result, accessibleError) = silent(
              _.makeAccessible(tree1, sym, qual.tpe, qual)) match {
            ...
            result match {
              ...
              case SelectFromTypeTree(qual @ TypeTree(), name)
              ...
              case _ if accessibleError.isDefined =>
                ...
                val qual1 =
                  if (name == nme.CONSTRUCTOR) qual
                  else
                    adaptToMemberWithArgs(tree,
                                          qual,
                                          name,
                                          mode,
      ...
      def typedSelectOrSuperCall(tree: Select) = tree match {
        ...
        case Select(qual, name) =>
          if (name.isTypeName) {
            val qualTyped =
              typedTypeSelectionQualifier(tree.qualifier, WildcardType)
            val qualStableOrError =
              if (qualTyped.isErrorTyped || unit.isJava || treeInfo
                    .admitsTypeSelection(qualTyped)) qualTyped
            ...
            if (tree.isInstanceOf[PostfixSelect])
              checkFeature(tree.pos,
                           currentRun.runDefinitions.PostfixOpsFeature,
                           name.decode)
            val sym = tree1.symbol
            if (sym != null && sym.isOnlyRefinementMember && !sym.isMacro)
              checkFeature(tree1.pos,
                           currentRun.runDefinitions.ReflectiveCallsFeature,
                           sym.toString)
      ...
      def typedIdent(tree: Tree, name: Name): Tree = {
        ...
        def inEmptyPackage =
          if (settings.exposeEmptyPackage) lookupInEmpty(name) else NoSymbol
        ...
        // ignore current variable scope in patterns to enforce linearity
        val startContext =
          if (mode.typingPatternOrTypePat) context.outer else context

        def asTypeName =
          if (mode.inAll(MonoQualifierModes) && unit.isJava && name.isTermName) {
        ...
        nameLookup match {
          ...
          case LookupNotFound =>
            asTypeName orElse inEmptyPackage orElse lookupInRoot(name) match {
              case NoSymbol =>
                issue(
                  SymbolNotFoundError(tree, name, context.owner, startContext))
          ...
          case LookupSucceeded(qual, sym) =>
            ...
            else if (sym.rawname == nme.classOf && currentRun.runDefinitions.isPredefClassOf(
                       sym) && pt.typeSymbol == ClassClass && pt.typeArgs.nonEmpty) {
              // Inferring classOf type parameter from expected type.  Otherwise an
              // actual call to the stubbed classOf method is generated, returning null.
              typedClassOf(tree,
                           TypeTree(pt.typeArgs.head).setPos(tree.pos.focus))
            } else {
              val pre1 =
                if (sym.isTopLevel) sym.owner.thisType
                else if (qual == EmptyTree) NoPrefix
                else qual.tpe
              val tree1 =
      ...
      def typedCompoundTypeTree(tree: CompoundTypeTree) = {
        ...
        if ((parents1 ++ templ.body) exists (_.isErrorTyped))
          ...
          val self = refinedType(parents1 map (_.tpe),
                                 context.enclClass.owner,
                                 decls,
                                 templ.pos)
          newTyper(context.make(templ, self.typeSymbol, decls))
            .typedRefinement(templ)
          templ updateAttachment CompoundTypeTreeOriginalAttachment(
            parents1,
            Nil) // stats are set elsewhere
      ...
      def typedAppliedTypeTree(tree: AppliedTypeTree) = {
        ...
        if (tpt1.isErrorTyped) {
          ...
          if (sameLength(tparams, args)) {
            ...
            if (settings.debug)
              Console.println(tpt1 + ":" + tpt1.symbol + ":" + tpt1.symbol.info) //debug
      ...
      def typedPackageDef(pdef0: PackageDef) = {
        val pdef = treeCopy.PackageDef(
          pdef0,
          pdef0.pid,
          pluginsEnterStats(this, namer.expandMacroAnnotations(pdef0.stats)))
        ...
        if (pid1.symbol.ne(NoSymbol) && !(pid1.symbol.hasPackageFlag || pid1.symbol.isModule))
          reporter.error(
            pdef.pos,
            s"There is name conflict between the ${pid1.symbol.fullName} and the package ${sym.fullName}.")
        val stats1 =
          newTyper(context.make(tree, sym.moduleClass, sym.info.decls))
      ...
      def defDefTyper(ddef: DefDef) = {
        ...
        newTyper(context.makeNewScope(ddef, sym))
          .constrTyperIf(isConstrDefaultGetter)
      }

      def typedAlternative(alt: Alternative) = {
        context withinPatAlternative (
          treeCopy.Alternative(
            tree,
            alt.trees mapConserve (alt => typed(alt, mode, pt))) setType pt
        )
      }
      ...
      def issueTryWarnings(tree: Try): Try = {
        def checkForCatchAll(cdef: CaseDef): Unit = {
          ...
          def warn(name: Name) = {
            val msg =
              s"This catches all Throwables. If this is really intended, use `case ${name.decoded} : Throwable` to clear this warning."
      ...
      def typedTry(tree: Try) = {
        ...
        def finish(ownType: Type) =
          treeCopy.Try(tree, block1, catches1, fin1) setType ownType

        issueTryWarnings(
          if (isFullyDefined(pt))
            finish(pt)
          else
            block1 :: catches1 map (_.tpe.deconst) match {
              case tpes if sameWeakLubAsLub(tpes) => finish(lub(tpes))
              case tpes =>
                val lub = weakLub(tpes)
        ...
        if (treeInfo isWildcardStarType tree.tpt)
          ...
          tree match {
            ...
            case MethodValue(expr) =>
              ...
              // See scala/bug#8299.
              val funTyped = typed1(suppressMacroExpansion(expr),
                                    mode | FUNmode,
                                    WildcardType)
              if (funTyped.tpe.isInstanceOf[OverloadedType])
                inferExprAlternative(funTyped, pt)
              funTyped match {
            ...
            case Typed(expr, tpt) =>
              ...
              val exprWithAttachment =
                if (definitions.isUnitType(tpt1.tpe))
                  expr.updateAttachment(TypedExpectingUnitAttachment)
                else expr
              val expr1 = typed(
                exprWithAttachment,
                mode.onlySticky,
                tpt1.tpe.deconst) // then type the expression with tpt1 as the expected type
      ...
      def warnMissingInterpolator(lit: Literal): Unit = if (!isPastTyper) {
        ...
        def isRecognizablyNotForInterpolation =
          context.enclosingApply.tree match {
            case Apply(Select(Apply(RefTree(_, nme.StringContextName), _), _),
                       _) =>
              true
        ...
        def requiresNoArgs(tp: Type): Boolean = tp match {
          ...
          case MethodType(Nil, restpe) =>
            requiresNoArgs(restpe) // may be a curried method - can't tell yet
          case MethodType(p :: _, _) =>
            p.isImplicit // implicit method requires no args
          case _ => true // catches all others including NullaryMethodType
        }
        def isPlausible(m: Symbol) =
          !m.isPackage && m.alternatives.exists(x => requiresNoArgs(x.info))

        def maybeWarn(s: String): Unit = {
          def warn(message: String) =
            context.warning(lit.pos, s"possible missing interpolator: $message")
          ...
          def suspiciousIdents =
            InterpolatorIdentRegex findAllIn s map (s =>
              suspiciousSym(TermName(s drop 1)))
          def isCheapIdent(expr: String) =
            (Character.isJavaIdentifierStart(expr.charAt(0)) &&
              expr.tail.forall(Character.isJavaIdentifierPart))
          def warnableExpr(expr: String) =
            !expr.isEmpty && (!isCheapIdent(expr) || isPlausible(
              suspiciousSym(TermName(expr))))
          ...
          if (suspiciousExprs.nonEmpty) {
            ...
            suspiciousIdents find isPlausible foreach (sym =>
      ...
      def typedLiteral(tree: Literal) = {
        ...
        tree setType (if (tree.value.tag == UnitTag) UnitTpe
      ...
      def typedTypeBoundsTree(tree: TypeBoundsTree) = {
        val lo1 =
          if (tree.lo.isEmpty) TypeTree(NothingTpe)
          else typedType(tree.lo, mode)
        val hi1 =
          if (tree.hi.isEmpty) TypeTree(AnyTpe) else typedType(tree.hi, mode)
        treeCopy.TypeBoundsTree(tree, lo1, hi1) setType TypeBounds(lo1.tpe,
                                                                   hi1.tpe)
      }
      ...
      def typedFunction(fun: Function) = {
        ...
        typerWithLocalContext(context.makeNewScope(fun, fun.symbol))(
          _.typedFunction(fun, mode, pt))
      }
      ...
      @inline def typedTypTree(tree: TypTree): Tree = tree match {
        ...
        case tree: SelectFromTypeTree =>
          typedSelect(tree, typedType(tree.qualifier, mode), tree.name)
        ...
        case tree: TypeTreeWithDeferredRefCheck =>
          tree // TODO: retype the wrapped tree? TTWDRC would have to change to hold the wrapped tree (not a closure)
        case _ =>
      ...
      @inline def typedMemberDef(tree: MemberDef): Tree = tree match {
        ...
        case tree: ModuleDef =>
          newTyper(context.makeNewScope(tree, sym.moduleClass))
            .typedModuleDef(tree)
      ...
      @inline def typedInAnyMode(tree: Tree): Tree = tree match {
        ...
        case tree: Select =>
          insertStabilizer(typedSelectOrSuperCall(tree), tree)
        case tree: Literal => typedLiteral(tree)
        case tree: Typed   => typedTyped(tree)
        case tree: This    => typedThis(tree) // scala/bug#6104
        case tree: UnApply =>
          abort(s"unexpected UnApply $tree") // turns out UnApply never reaches here
        case _ =>
    ...
    def typed(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      val statsEnabled = StatisticsStatics
        .areSomeHotStatsEnabled() && statistics.areHotStatsLocallyEnabled
      val startByType =
        if (statsEnabled)
      ...
      val shouldPrintTyping = printTypings && !phase.erasedTypes && !noPrintTyping(
        tree)
      val shouldPopTypingStack = shouldPrintTyping && typingStack
        .beforeNextTyped(tree, mode, pt, context)
      try {
        ...
        case ex: TypeError =>
          ...
          typingStack.printTyping(
            tree,
            "caught %s: while typing %s".format(ex, tree)) //DEBUG
    ...
    @inline final def typedByValueExpr(tree: Tree,
                                       pt: Type = WildcardType): Tree =
      typed(tree, EXPRmode | BYVALmode, pt)

    def typedPos(pos: Position, mode: Mode, pt: Type)(tree: Tree) =
      typed(atPos(pos)(tree), mode, pt)
    ...
    @inline final def typedQualifier(tree: Tree, mode: Mode, pt: Type): Tree =
      typed(
        checkRootOfQualifier(tree, mode),
        PolyQualifierModes | mode.onlyTypePat,
        pt) // TR: don't set BYVALmode, since qualifier might end up as by-name param to an implicit
    ...
    @inline final def typedQualifier(tree: Tree): Tree =
      typedQualifier(tree, NOmode, WildcardType)

    // if a package id is a selection from _root_ in scope, warn about semantics and set symbol for typedQualifier
    @inline final def typedPackageQualifier(tree: Tree): Tree =
      typedQualifier(packageQualifierTraverser(tree))
    ...
    object packageQualifierTraverser extends Traverser {
      ...
      override def traverse(tree: Tree): Unit =
        tree match {
          case Select(id @ Ident(nme.ROOTPKG), _) if !id.hasExistingSymbol =>
            checkRootSymbol(id)
    ...
    @inline def checkRootOfQualifier(q: Tree, mode: Mode): Tree =
      if (mode.typingPatternOrTypePat) patternQualifierTraverser(q)
      else nonpatternQualifierTraverser(q)
    ...
    @inline final def typedOperator(tree: Tree): Tree =
      typed(tree, OperatorModes)
    ...
    def typedTypeConstructor(tree: Tree, mode: Mode): Tree = {
      ...
      result.tpe.dealias match {
        case restpe @ TypeRef(pre, _, _)
            if !phase.erasedTypes && !pre.isStable && !context.unit.isJava =>
    ...
    def typedTypeConstructor(tree: Tree): Tree =
      typedTypeConstructor(tree, NOmode)
    ...
    def computeMacroDefType(ddef: DefDef, pt: Type): Type = {
      ...
      if (isMacroBodyOkay && shouldInheritMacroImplReturnType) {
        val commonMessage =
          "macro defs must have explicitly specified return types"
        def reportFailure() = {
    ...
    @inline final def transformedOr(tree: Tree, op: => Tree): Tree =
    ...
    final def lookupTransformed(tree: Tree): Option[Tree] =
      if (phase.erasedTypes)
        None // OPT save the hashmap lookup in erasure type and beyond
  ...
  final def finishComputeParamAlias(): Unit = {
    ...
    scala.util.Sorting.quickSort(classes)(Ordering.fromLessThan((a, b) =>
      b.isLess(a)))
      ...
      for ((ownAcc, superAcc) <- superConstructorCalls.getOrElse(sym, Nil)) {
        ...
        val alias = (
          superAcc.initialize.alias // Is the param accessor is an alias for a field further up  the class hierarchy?
            ...
            filter (alias =>
              superClazz.info.nonPrivateMember(alias.name) == alias) // the accessor must be public
        )
        if (alias.exists && !alias.accessed.isVariable && !isRepeatedParamType(
              alias.accessed.info)) {
          ownAcc match {
            case acc: TermSymbol
                if !acc.isVariable && !isByNameParamType(acc.info) =>
...
trait TypersStats {
  ...
  val byTypeNanos = newByClass("time spent by tree node", "typer")(
    newStackableTimer("", typerNanos))
