...
trait ContextErrors {
  ...
  case class TypeErrorWrapper(ex: TypeError) extends AbsTypeError {
    def errMsg = ex.msg
    def errPos = ex.pos
  }
  ...
  case class DivergentImplicitTypeError(underlyingTree: Tree,
                                        pt0: Type,
                                        sym: Symbol)
      extends TreeTypeError {
  ...
  object ErrorUtils {
    ...
    def issueTypeError(err: AbsTypeError)(implicit context: Context): Unit = {
      context.issue(err)
    }
  ...
  private def MacroIncompatibleEngineError(friendlyMessage: String,
  ...
  def NoImplicitFoundError(tree: Tree, param: Symbol)(
    ...
    def errMsg = {
      ...
      ImplicitNotFoundMsg
        .unapply(param)
        .map(_.formatParameterMessage(tree))
        .orElse(
          ImplicitNotFoundMsg
            .unapply(paramTp.typeSymbolDirect)
            .map(_.formatDefSiteMessage(paramTp)))
    ...
    object TyperErrorGen {
      ...
      def AdaptTypeError(tree: Tree, found: Type, req: Type): Tree = {
        ...
        def issueError(foundType: Type): Tree = {
          assert(!foundType.isErroneous,
                 s"AdaptTypeError - foundType is Erroneous: $foundType")
          assert(!req.isErroneous, s"AdaptTypeError - req is Erroneous: $req")
          issueNormalTypeError(
            callee,
            withAddendum(callee.pos)(typeErrorMsg(context, foundType, req)))
        ...
        req.dealiasWiden match {
          case RefinedType(parents, decls)
            ...
            if (foundDecls.isEmpty || (found.typeSymbol eq NoSymbol))
              ...
              val refined = refinedType(found.parents,
              ...
              if (found.typeSymbol.isAnonymousClass && refined.isErroneous) tree
              else issueError(refined)
            }
          case _ =>
            issueError(found)
        }
      ...
      def AuxConstrInConstantAnnotation(constr: Tree, clazz: Symbol) =
        issueNormalTypeError(
          constr,
          s"$clazz cannot have auxiliary constructors because it extends ConstantAnnotation")

      def ConstantAnnotationNeedsSingleArgumentList(constr: Tree,
                                                    clazz: Symbol) =
        issueNormalTypeError(
          constr,
          s"$clazz needs to have exactly one argument list because it extends ConstantAnnotation")

      // additional parentTypes errors
      def ConstrArgsInParentWhichIsTraitError(arg: Tree, parent: Symbol) =
        issueNormalTypeError(
          arg,
          parent + " is a trait; does not take constructor arguments")
      ...
      def AppliedTypeWrongNumberOfArgsError(tree: Tree,
                                            tpt: Tree,
                                            tparams: List[Symbol]) = {
      ...
      def SymbolEscapesScopeError[T <: Tree](tree: T, badSymbol: Symbol): T = {
        ...
        issueNormalTypeError(
          tree,
          modifierString + badSymbol + " escapes its defining scope as part of type " + tree.tpe)
        setError(tree)
      }
      ...
      private def ConstrArgsThisReferenceError(prefix: String, tree: Tree) =
        NormalTypeError(
          tree,
          s"$prefix constructor arguments cannot reference unconstructed `this`")

      def TooManyArgumentListsForConstructor(tree: Tree) = {
        issueNormalTypeError(
          tree,
          "too many argument lists for constructor invocation")
        setError(tree)
      }
      ...
      def NotAMemberError(sel: Tree, qual: Tree, name: Name, cx: Context) = {
        ...
        def errMsg: String = {
          ...
          def targetKindString =
            if (owner.isTypeParameterOrSkolem) "type parameter " else ""
          ...
          def addendum = {
            ...
            lazy val alternatives: List[String] = {
              val editThreshold = 2
              val x = name.decode
              if (context.openImplicits.nonEmpty || (x.size < 2) || x.endsWith(
                    "=")) Nil
              else {
                target.members.iterator
                  .filter(
                    sym =>
                      (sym.isTerm == name.isTermName) &&
                        !sym.isConstructor &&
            ...
            val altStr: String = {
              ...
              if (companionSymbol != NoSymbol) ""
                ...
                alternatives match {
                  case Nil => ""
                  case xs =>
                    "\ndid you mean " + StringUtil.oxford(
                      xs.sorted.take(maxSuggestions),
                      "or") + "?"
                }
            }
            val semicolon =
      ...
      def ReturnWithoutTypeError(tree: Tree, owner: Symbol) = {
        issueNormalTypeError(tree,
                             owner + " has return statement; needs result type")
        setError(tree)
      }
      ...
      def MaxFunctionArityError(fun: Tree, why: String) = {
        issueNormalTypeError(
          fun,
          s"functions may not have more than ${definitions.MaxFunctionArity} parameters$why")
        setError(fun)
      }

      def MissingParameterTypeError(fun: Tree,
                                    vparam: ValDef,
                                    pt: Type,
      ...
      def AnnotationNotAConstantError(tree: Tree) =
        NormalTypeError(
          tree,
          "annotation argument needs to be a constant; found: " + tree)
      ...
      def ArrayConstantsTypeMismatchError(tree: Tree, pt: Type) =
        NormalTypeError(tree,
                        "found array constant, expected argument of type " + pt)
      ...
      def MultipleArgumentListForAnnotationError(tree: Tree) =
        NormalTypeError(
          tree,
          "multiple argument lists on Java annotation or subclass of ConstantAnnotation")
      ...
      def ClassfileAnnotationsAsNamedArgsError(tree: Tree) =
        NormalTypeError(
          tree,
          "arguments to Java annotations or subclasses of ConstantAnnotation have to be supplied as named arguments")

      def AnnotationMissingArgError(tree: Tree, annType: Type, sym: Symbol) =
        NormalTypeError(
          tree,
          "annotation " + annType.typeSymbol.fullName + " is missing argument " + sym.name)
      ...
      def UnexpectedTreeAnnotationError(tree: Tree, unexpected: Tree) =
        NormalTypeError(
          tree,
          "unexpected tree after typing annotation: " + unexpected)

      //typedExistentialTypeTree
      def AbstractionFromVolatileTypeError(vd: ValDef) =
        issueNormalTypeError(
          vd,
          "illegal abstraction from value with volatile type " + vd.symbol.tpe)

      private[scala] def TypedApplyWrongNumberOfTpeParametersErrorMessage(
          fun: Tree) =
      ...
      def TypedApplyWrongNumberOfTpeParametersError(tree: Tree, fun: Tree) = {
        issueNormalTypeError(
          tree,
          TypedApplyWrongNumberOfTpeParametersErrorMessage(fun))
        setError(tree)
      }
      ...
      def TypedApplyDoesNotTakeTpeParametersError(tree: Tree, fun: Tree) = {
        issueNormalTypeError(
          tree,
          treeSymTypeMsg(fun) + " does not take type parameters.")
        setError(tree)
      }
      ...
      def TooManyArgsNamesDefaultsError(tree: Tree,
                                        fun: Tree,
                                        formals: List[Type],
                                        args: List[Tree],
                                        namelessArgs: List[Tree],
                                        argPos: Array[Int]) = {
        ...
        val msg = {
          val badappl = {
            ...
            else if (excess < 3 && expected <= 5)
              s"too many arguments ($supplied) for $target"
            else if (expected > 10)
              s"$supplied arguments but expected $expected for $target"
            else {
              val more =
            ...
            unknowns.size match {
              ...
              case _ =>
                unknowns.mkString(
                  "\nNote that '",
                  "', '",
                  "' are not parameter names of the invoked method.")
            }
          s"${badappl}${suppl}"
        }
      ...
      def TooManyArgsPatternError(fun: Tree) =
        issueNormalTypeError(
          fun,
          "too many arguments for unapply pattern, maximum = " + definitions.MaxTupleArity)
      ...
      def ParentSuperSubclassError(parent: Tree,
                                   superclazz: Symbol,
                                   parentSym: Symbol,
                                   mixin: Symbol) =
      ...
      def MissingArgsForMethodTpeError(tree: Tree, meth: Symbol) = {
        ...
        val message =
          if (meth.isMacro) MacroTooFewArgumentListsMessage
            ...
            s"""missing argument list for ${meth.fullLocationString}$advice"""
      ...
      def ConstructorPrefixError(tree: Tree, restpe: Type) = {
        issueNormalTypeError(
          tree,
          restpe.prefix + " is not a legal prefix for a constructor")
        setError(tree)
      }
      ...
      def InferTypeWithVolatileTypeSelectionError(tree: Tree, pre: Type) =
        issueNormalTypeError(
          tree,
          "Inferred type " + tree.tpe + " contains type selection from volatile type " + pre)
      ...
      def DependentMethodTpeConversionToFunctionError(tree: Tree,
                                                      tp: Type): Tree = {
        issueNormalTypeError(
          tree,
      ...
      def StarPatternWithVarargParametersError(tree: Tree) =
        issueNormalTypeError(
          tree,
          "star patterns must correspond with varargs parameters")

      def FinitaryError(tparam: Symbol) =
        issueSymbolTypeError(
          tparam,
          "class graph is not finitary because type parameter " + tparam.name + " is expansively recursive")

      def QualifyingClassError(tree: Tree, qual: Name) =
        issueNormalTypeError(
          tree,
          if (qual.isEmpty)
            s"$tree can be used only in a class, object, or template"
      ...
      def NotAValueError(tree: Tree, sym: Symbol) = {
        issueNormalTypeError(
          tree,
          sym.kindString + " " + sym.fullName + " is not a value")
        setError(tree)
      }
      ...
      def DefDefinedTwiceError(sym0: Symbol, sym1: Symbol) = {
        ...
        val addendum = (if (sym0.pos.source eq sym1.pos.source)
                          s"$addPref at line ${sym1.pos.line}:${sym1.pos.column}"
                        else if (sym1.pos.source ne NoSourceFile)
                          s"$addPref at line ${sym1.pos.line}:${sym1.pos.column} of '${sym1.pos.source.path}'"
                        else if (sym1.associatedFile ne NoAbstractFile)
                          s"$addPref in '${sym1.associatedFile.canonicalPath}'"
      ...
      protected def macroExpansionError(expandee: Tree,
                                        msg: String,
      ...
      private def MacroTooFewArgumentListsMessage =
        "too few argument lists for macro invocation"
      def MacroTooFewArgumentListsError(expandee: Tree) =
        macroExpansionError2(expandee, MacroTooFewArgumentListsMessage)

      private def MacroTooManyArgumentListsMessage =
        "too many argument lists for macro invocation"
      def MacroTooManyArgumentListsError(expandee: Tree) =
      ...
      def MacroExpansionHasInvalidTypeError(expandee: Tree, expanded: Any) = {
        def isUnaffiliatedExpr =
          expanded.isInstanceOf[scala.reflect.api.Exprs#Expr[_]]
        def isUnaffiliatedTree =
          expanded.isInstanceOf[scala.reflect.api.Trees#TreeApi]
      ...
      def MacroImplementationNotFoundError(expandee: Tree) =
        macroExpansionError(
          expandee,
          macroImplementationNotFoundMessage(expandee.symbol.name))
        ...
        if (sym.isOverloaded) actualSignature += "(...) = ..."
          ...
          if (sym.paramss.nonEmpty) {
            def showParam(param: Symbol) =
              s"${param.name}: ${param.typeSignature}"
            def showParams(params: List[Symbol]) = {
              val s_mods =
                if (params.nonEmpty && params(0).hasFlag(
                      scala.reflect.internal.Flags.IMPLICIT)) "implicit "
                else ""
              val s_params = params map showParam mkString ", "
              "(" + s_mods + s_params + ")"
            }
            def showParamss(paramss: List[List[Symbol]]) =
              paramss map showParams mkString ""
            actualSignature += showParamss(sym.paramss)
          }
          if (sym.isTermMacro)
            actualSignature = actualSignature.replace("macro method", "def") + " = macro ..."
          else
            actualSignature = actualSignature.replace("method", "def") + " = ..."
        }
        issueSymbolTypeError(
          clazz,
          s"""
      ...
      def MacroAnnotationCannotBeMemberError(clazz: Symbol) =
        issueSymbolTypeError(
          clazz,
          s"macro annotation cannot be a member of another class")

      def MacroAnnotationNotExpandedMessage: String = {
        "macro annotation could not be expanded " + (if (!settings.YmacroAnnotations)
                                                       "(since these are experimental, you must enable them with -Ymacro-annotations)"
                                                     else
                                                       "(you cannot use a macro annotation in the same compilation run that defines it)")
      }

      def MacroAnnotationOnlyDefinitionError(ann: Tree) =
        issueNormalTypeError(ann,
                             "macro annotations can only be put on definitions")
  ...
  trait InferencerContextErrors {
    ...
    private def applyErrorMsg(tree: Tree,
                              msg: String,
                              argtpes: List[Type],
                              pt: Type) = {
    ...
    object InferErrorGen {
      ...
      private def issueAmbiguousTypeErrorUnlessErroneous(pos: Position,
                                                         pre: Type,
                                                         sym1: Symbol,
                                                         sym2: Symbol,
                                                         rest: String): Unit = {
      ...
      def AccessError(tree: Tree,
        ...
        def errMsg = {
          val location =
            if (sym.isClassConstructor) owner0 else pre.widen.directObjectString
      ...
      private def setErrorOnLastTry(lastTry: Boolean, tree: Tree) =
        if (lastTry) setError(tree)

      def widenArgs(argtpes: List[Type],
                    params0: List[Symbol],
                    params1: List[Symbol]): List[Type] =
        argtpes.zipWithIndex map {
          case (nt @ NamedType(name, tp), _) => // a named argument
            (tp,
             params0.find(_.name == name).map(_.tpe),
             params1.find(_.name == name).map(_.tpe)) match {
      ...
      def NoBestMethodAlternativeError(tree: Tree,
                                       argtpes: List[Type],
                                       pt: Type,
                                       lastTry: Boolean) = {
        val alts = alternatives(tree)
        val widenedArgtpes =
          widenArgs(argtpes, alts.head.params, alts.tail.head.params)
      ...
      def AmbiguousMethodAlternativeError(tree: Tree,
                                          pre: Type,
                                          best: Symbol,
        ...
        if (!(argtpes exists (_.isErroneous)) && !pt.isErroneous) {
          val widenedArgtpes = widenArgs(argtpes,
                                         best.asMethod.tpe.params,
                                         firstCompeting.asMethod.tpe.params)
          val msg0 =
      ...
      def NoBestExprAlternativeError(tree: Tree, pt: Type, lastTry: Boolean) = {
        issueNormalTypeError(
          tree,
          withAddendum(tree.pos)(typeErrorMsg(context, tree.symbol.tpe, pt)))
        setErrorOnLastTry(lastTry, tree)
      }

      def AmbiguousExprAlternativeError(tree: Tree,
                                        pre: Type,
                                        best: Symbol,
                                        firstCompeting: Symbol,
                                        pt: Type,
      ...
      private[scala] def NotWithinBoundsErrorMessage(prefix: String,
        ...
        if (explaintypes) {
          ...
          targs
            .lazyZip(bounds)
            .foreach((targ, bound) => explainTypes(bound.lo, targ))
          targs
            .lazyZip(bounds)
            .foreach((targ, bound) => explainTypes(targ, bound.hi))
        }
      ...
      def PatternTypeIncompatibleWithPtError1(tree: Tree,
                                              pattp: Type,
                                              pt: Type) =
        issueNormalTypeError(
          tree,
          "pattern type is incompatible with expected type" + foundReqMsg(pattp,
                                                                          pt))

      def IncompatibleScrutineeTypeError(tree: Tree, pattp: Type, pt: Type) =
        issueNormalTypeError(
          tree,
          "scrutinee is incompatible with pattern type" + foundReqMsg(pattp,
                                                                      pt))
  ...
  trait NamerContextErrors {
    ...
    object NamerErrorGen {
      ...
      def BeanPropertyAnnotationFieldWithoutLetterError(tree: Tree) =
        issueNormalTypeError(
          tree,
          "`BeanProperty` annotation can be applied only to fields that start with a letter")

      def BeanPropertyAnnotationPrivateFieldError(tree: Tree) =
        issueNormalTypeError(
          tree,
          "`BeanProperty` annotation can be applied only to non-private fields")
      ...
      def DoubleDefError(currentSym: Symbol, prevSym: Symbol) = {
        ...
        issueSymbolTypeError(
          currentSym,
          prevSym.name + " is already defined as " + s2 + s3 + where)
      }
      ...
      def SymbolValidationError(sym: Symbol,
                                errKind: SymValidateErrors.Value): Unit = {
        val msg = errKind match {
          ...
          case AbstractVar =>
            "only traits and abstract classes can have declared but undefined members" + abstractVarMessage(
              sym)
      ...
      def AbstractMemberWithModiferError(sym: Symbol, flag: Long) =
        issueSymbolTypeError(sym,
                             "abstract member may not have " + Flags
                               .flagsToString(flag) + " modifier")

      def IllegalModifierCombination(sym: Symbol, flag1: Long, flag2: Long) =
        issueSymbolTypeError(
          sym,
          "illegal combination of modifiers: %s and %s for: %s".format(
            Flags.flagsToString(flag1),
            Flags.flagsToString(flag2),
            sym))
      ...
      def IllegalDependentMethTpeError(sym: Symbol)(context: Context) = {
        ...
        issueSymbolTypeError(
          sym,
          "illegal dependent method type" + errorAddendum)(context)
      }

      def DuplicatesError(tree: Tree,
                          name: Name,
  ...
  object NamesDefaultsErrorsGen {
    ...
    def UnknownParameterNameNamesDefaultError(
        arg: Tree,
        name: Name,
        warnVariableInScope: Boolean)(implicit context: Context) = {
      val suffix =
    ...
    def DoubleParamNamesDefaultError(
        arg: Tree,
        name: Name,
        pos: Int,
