...
trait Reporting extends scala.reflect.internal.Reporting {
  ...
  class PerRunReporting extends PerRunReportingBase {
    ...
    private class ConditionalWarning(what: String,
      ...
      def summarize() =
        if (warnings.nonEmpty && (setting.isDefault || doReport)) {
          ...
          warnings.valuesIterator.foreach {
            case (_, since) =>
          ...
          sinceAndAmount.foreach {
            case (since, numWarnings) =>
              ...
              val rerun =
                ...
                else reporter.rerunWithDetails(setting, setting.name)
              reporter.warning(
                NoPosition,
                s"there ${warningVerb} ${warningCount}${warningsSince}${rerun}")
          }
          if (deprecationSummary) {
            ...
            reporter.warning(
              NoPosition,
              s"there ${warningVerb} ${warningCount} in total${rerun}")
          }
        }
    }
    ...
    private val _inlinerWarnings = new ConditionalWarning(
      "inliner",
      !settings.optWarningsSummaryOnly,
      settings.optWarnings)
    ...
    def deprecationWarning(pos: Position, msg: String, since: String): Unit =
      _deprecationWarnings.warn(pos, msg, since)
    def uncheckedWarning(pos: Position, msg: String): Unit =
      _uncheckedWarnings.warn(pos, msg)
    def featureWarning(pos: Position, msg: String): Unit =
      _featureWarnings.warn(pos, msg)
    def inlinerWarning(pos: Position, msg: String): Unit =
      _inlinerWarnings.warn(pos, msg)
    ...
    def deprecationWarning(pos: Position, sym: Symbol): Unit = {
      ...
      val message = sym.deprecationMessage match {
        case Some(msg) => s": $msg"
        case _         => ""
      }
      deprecationWarning(
        pos,
        sym,
        s"$sym${sym.locationString} is deprecated$since$message",
        version)
    }
    ...
    def featureWarning(pos: Position,
      ...
      def isSbtCompat =
        (featureName == "postfixOps" && pos.source.path
          .endsWith("/xsbt/Compat.scala") && Thread.currentThread.getStackTrace
          .exists(_.getClassName.startsWith("sbt.")))
