...
class FscSettings(error: String => Unit) extends Settings(error) {
  ...
  val currentDir = StringSetting(
    ...
    "").internalOnly() withAbbreviation "--current-directory"
