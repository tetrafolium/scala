...
trait Parsers { self: Quasiquotes =>
  ...
  abstract class Parser extends {
    ...
    class QuasiquoteParser(source0: SourceFile)
      ...
      override def makeSafeTupleTerm(trees: List[Tree]): Tree =
        treeBuilder.makeTupleTerm(trees)
      override def makeSafeTupleType(trees: List[Tree]): Tree =
  ...
  object TermParser extends Parser {
    def entryPoint =
      parser =>
        Q(implodePatDefs(gen.mkTreeOrBlock(parser.templateOrTopStatSeq())))
  }
