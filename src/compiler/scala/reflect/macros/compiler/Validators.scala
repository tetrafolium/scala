...
trait Validators {
  ...
  trait Validator {
    ...
    private def sanityCheck() = {
      ...
      if (macroImpl.typeParams.sizeCompare(targs) != 0)
    ...
    private def checkMacroDefMacroImplCorrespondence() = {
      ...
      map2(aparamss, rparamss)((aparams, rparams) => {
        if (aparams.sizeCompare(rparams) < 0)
          MacroImplMissingParamsError(aparams, rparams)
        if (rparams.sizeCompare(aparams) < 0)
      ...
      try {
        ...
        val maxLubDepth = lubDepth(aparamss.flatten map (_.tpe)) max lubDepth(
        ...
        boundsOk match {
          ...
          case SilentResultValue(false) | SilentTypeError(_) =>
            MacroImplTargMismatchError(atargs, atparams)
        }
      } catch {
    ...
    private lazy val referenceMacroImplSig: MacroImplSig = {
      ...
      object SigGenerator {
        ...
        val ctxTpe =
          if (isBlackbox) BlackboxContextClass.tpe else WhiteboxContextClass.tpe
        val ctxPrefix =
          if (isImplMethod)
            singleType(
              NoPrefix,
