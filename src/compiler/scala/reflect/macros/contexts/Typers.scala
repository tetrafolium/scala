...
trait Typers {
  ...
  def openImplicits: List[ImplicitCandidate] =
    callsiteTyper.context.openImplicits.map(_.toImplicitCandidate)
  ...
  def typecheck(tree: Tree,
    ...
    macroLogVerbose(
      s"typechecking $tree with expected type $pt, implicit views = ${!withImplicitViewsDisabled}, macros = ${!withMacrosDisabled}")
      ...
      context.withImplicits(enabled = !withImplicitViewsDisabled) {
        context.withMacros(enabled = !withMacrosDisabled) {
          callsiteTyper.silent(
            _.typed(universe.duplicateAndKeepPositions(wrapped), mode, pt),
  ...
  def inferImplicitValue(pt: Type,
    ...
    macroLogVerbose(
      s"inferring implicit value of type $pt, macros = ${!withMacrosDisabled}")
  ...
  def inferImplicitView(tree: Tree,
                        ...
                        pos: Position = enclosingPosition): Tree = {
    macroLogVerbose(
      s"inferring implicit view from $from to $to for $tree, macros = ${!withMacrosDisabled}")
