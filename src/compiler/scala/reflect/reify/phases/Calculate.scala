...
trait Calculate {
  ...
  implicit class RichCalculateSymbol(sym: Symbol) {
    def metalevel: Int = {
      assert(sym != null && sym != NoSymbol); localSymbols.getOrElse(sym, 0)
    }
    def isLocalToReifee =
      (localSymbols contains sym) // todo. how do I account for local skolems?
  }
    ...
    if (sym != null && sym != NoSymbol) {
      if (localSymbols contains sym)
        assert(
          ...
          "metalevel mismatch: expected %s, actual %s".format(localSymbols(sym),
                                                              metalevel))
      else
        localSymbols += (sym -> metalevel)
    }
