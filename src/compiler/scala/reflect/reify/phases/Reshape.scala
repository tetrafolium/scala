  ...
  val reshape = new Transformer {
      ...
      tree.attachments.get[analyzer.MacroExpansionAttachment] match {
        case Some(analyzer.MacroExpansionAttachment(original, _)) =>
          ...
          original match {
            ...
            case Apply(TypeApply(_, List(tt)), _)
                if sym == materializeClassTag =>
              mkImplicitly(appliedType(ClassTagClass, tt.tpe))
            case Apply(TypeApply(_, List(tt)), List(pre))
                if sym == materializeWeakTypeTag =>
              mkImplicitly(typeRef(pre.tpe, WeakTypeTagClass, List(tt.tpe)))
            case Apply(TypeApply(_, List(tt)), List(pre))
    ...
    private def toPreTyperModifiers(mods: Modifiers, sym: Symbol) = {
      if (!sym.annotations.isEmpty) {
        ...
        if (reifyDebug && !postTyper.isEmpty)
          println("reify symbol annotations for: " + sym)
        if (reifyDebug && !postTyper.isEmpty)
    ...
    private def toPreTyperCompoundTypeTree(ctt: CompoundTypeTree): Tree = {
      ...
      val CompoundTypeTreeOriginalAttachment(parents1, stats1) =
        att.getOrElse(CompoundTypeTreeOriginalAttachment(parents, stats))
    ...
    private def toPreTyperAnnotation(ann: AnnotationInfo): Tree = {
      val args = if (ann.assocs.isEmpty) {
        ...
        ann.assocs map {
          case (nme, arg) => NamedArg(Ident(nme), toScalaAnnotation(arg))
        }
      }
      ...
      assert(extractOriginal.isDefinedAt(ann.original),
             s"$ann has unexpected original ${showRaw(ann.original)}")
          ...
          val mods1 = if (accessors.contains(vdef)) {
            ...
            val annotations1 =
              accessors(vdef).foldLeft(annotations) { (curr, acc) =>
                curr ++ (acc.symbol.annotations
                  .filterNot(_ == UnmappableAnnotation)
                  .map(toPreTyperAnnotation))
              }
