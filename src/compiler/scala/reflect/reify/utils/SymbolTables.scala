...
trait SymbolTables {
  ...
  class SymbolTable private[SymbolTable] (
    ...
    def symRef(sym: Symbol): Tree =
      symtab.get(sym) match {
        case Some(FreeDef(_, name, binding, _, _)) =>
    ...
    def +(sym: Symbol, name: TermName, reification: Tree): SymbolTable =
      add(sym, name, reification)
    def +(symDef: Tree): SymbolTable = add(symDef)
    def ++(symDefs: IterableOnce[Tree]): SymbolTable =
      symDefs.iterator.foldLeft(this)((symtab, symDef) => symtab.add(symDef))
    def ++(symtab: SymbolTable): SymbolTable = {
      val updated = this ++ symtab.symtab.values;
      new SymbolTable(updated.symtab, updated.aliases ++ symtab.aliases)
    }
    ...
    def --(syms: IterableOnce[Symbol]): SymbolTable =
      syms.iterator.foldLeft(this)((symtab, sym) => symtab.remove(sym))
    def --(names: List[TermName]): SymbolTable =
      names.foldLeft(this)((symtab, name) => symtab.remove(name))
    def --(symDefs: Iterable[Tree]): SymbolTable =
      this -- (symDefs map (reifyBinding(_)))
    def --(symtab: SymbolTable): SymbolTable = {
      val updated = this -- symtab.symtab.values;
      new SymbolTable(updated.symtab, updated.aliases diff symtab.aliases)
    }
    def filterSyms(p: Symbol => Boolean): SymbolTable =
      this -- (syms filterNot p)
    def filterAliases(p: (Symbol, TermName) => Boolean): SymbolTable =
      this -- (aliases filterNot (tuple => p(tuple._1, tuple._2)) map (_._2))
    ...
    private def add(sym: Symbol,
                    name0: TermName,
                    reification: Tree): SymbolTable = {
    ...
    def encode: List[Tree] =
      cache.getOrElseUpdate(this, SymbolTable.encode(this)) map (_.duplicate)
  ...
  object SymbolTable {
    ...
    private[SymbolTable] def encode(symtab0: SymbolTable): List[Tree] = {
      ...
      try {
        val cumulativeSymtab =
          mutable.ArrayBuffer[Tree](symtab0.symtab.values.toList: _*)
        val cumulativeAliases =
          mutable.ArrayBuffer[(Symbol, TermName)](symtab0.aliases: _*)

        def fillInSymbol(sym: Symbol): Tree = {
          if (reifyDebug)
            println("Filling in: %s (%s)".format(sym, sym.accurateKindString))
          ...
          if (sym.annotations.isEmpty) rset
            ...
            reifier.mirrorBuildCall(
              nme.setAnnotations,
              rset,
              reifier.mkList(sym.annotations map reifier.reifyAnnotationInfo))
        }
