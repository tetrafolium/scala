...
object ScalaSigParser {
  ...
  def scalaSigFromAnnotation(classFile: ClassFile): Option[ScalaSig] = {
    ...
    def mergedLongSignatureBytes(
      ...
      signatureParts.iterator.flatMap {
        case ConstValueIndex(index) => bytesForIndex(index)
      }.toArray
...
object ScalaSigEntryParsers extends RulesWithState with MemoisableRules {
  ...
  lazy val entry
    : EntryParser[Any] = symbol | typeEntry | literal | name | attributeInfo | annotInfo | children | get
  ...
  lazy val symbol: EntryParser[Symbol] = oneOf(noSymbol,
                                               typeSymbol,
                                               aliasSymbol,
                                               classSymbol,
                                               objectSymbol,
