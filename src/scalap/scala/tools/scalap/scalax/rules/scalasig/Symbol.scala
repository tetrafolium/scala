...
abstract class ScalaSigSymbol extends Symbol {
  ...
  lazy val children
    : Seq[Symbol] = applyScalaSigRule(ScalaSigParsers.symbols) filter (sym =>
    sym.parent == Some(this) && !sym.isParam)
  lazy val attributes: Seq[AttributeInfo] = applyScalaSigRule(
    ScalaSigParsers.attributes) filter (_.symbol == this)
}
