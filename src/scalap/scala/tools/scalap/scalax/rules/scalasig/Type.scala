...
case class TypeRefType(prefix: Type, symbol: Symbol, typeArgs: Seq[Type])
    extends Type
...
case class ClassInfoTypeWithCons(symbol: Symbol,
                                 typeRefs: Seq[Type],
                                 cons: String)
    extends Type
...
case class PolyTypeWithCons(typeRef: Type,
                            symbols: Seq[TypeSymbol],
                            cons: String)
    extends Type
case class AnnotatedType(typeRef: Type, attribTreeRefs: List[Int]) extends Type
case class AnnotatedWithSelfType(typeRef: Type,
                                 symbol: Symbol,
                                 attribTreeRefs: List[Int])
    extends Type
