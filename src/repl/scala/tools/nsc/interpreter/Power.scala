...
//        repldbg("Not a package class! " + packageClass)
  ...
  trait LowPriorityPrettifier {
    implicit object AnyPrettifier extends Prettifier[Any] {
      ...
      def prettify(x: Any): IterableOnce[String] = x match {
        ...
        case Tuple2(k, v) =>
          List(
            prettify(k).iterator ++ Iterator("->") ++ prettify(v) mkString " ")
  ...
  trait Prettifier[T] {
    ...
    def prettify(xs: IterableOnce[T]): IterableOnce[String] =
      xs.iterator flatMap (x => prettify(x))
  }
  ...
  abstract class PrettifierClass[T: Prettifier]() {
    ...
    def freq[U](p: T => U) =
      value.groupMapReduce(p)(_ => 1)(_ + _).toList sortBy (-_._2) map (_.swap)
  ...
  trait Implicits1 {
    // fallback
    implicit def replPrinting[T](x: T)(implicit pretty: Prettifier[T] =
                                         Prettifier.default[T]) =
  ...
  trait Implicits2 extends Implicits1 {
    ...
    implicit def replMultiPrinting[T: Prettifier](
        xs: IterableOnce[T]): MultiPrettifierClass[T] =
