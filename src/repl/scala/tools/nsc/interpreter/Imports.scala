...
trait Imports {
  ...
  protected def importsCode(wanted: Set[Name],
    ...
    def reqsToUse: List[ReqAndHandler] = {
        ...
        reqs match {
          ...
          case rh :: rest =>
            ...
            val augment = rh match {
              case ReqAndHandler(_, _: ImportHandler) =>
                referencedNames // for "import a.b", add "a" to names to be resolved
              case _ => Nil
            }
      ...
      select(allReqAndHandlers.reverseIterator.map {
        case (r, h) => ReqAndHandler(r, h)
      }.toList, wanted).reverse
    }
